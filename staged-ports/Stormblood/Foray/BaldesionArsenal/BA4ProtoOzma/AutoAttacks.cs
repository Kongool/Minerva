...
sealed class AutoAttacksCube(ModuleBase module) : Components.GenericBaitAway(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        switch (spell.Action.ID)
        {
            case (uint)AID.AutoAttackSphere:
                if (targets.Count == 3)
                    targets.Clear();
                targets.Add(World.Actors.Find(spell.MainTargetID)!);
                break;
            case (uint)AID.FlareStarVisual:
                if (caster == Module.PrimaryActor)
                {
                    var activation = World.FutureTime(4.3d);
            ...
            case (uint)AID.AutoAttackCube:
                ...
                if (NumCasts % 3 == 0)
                {
                    ...
                    if (NumCasts <= 42)
                    {
                        var activation = World.FutureTime(2.6d); // time varies wildly depending on current mechanic, taking lowest
                        AddBaits(ref activation);
                    }
                }
                break;
...
sealed class AutoAttacksPyramid(ModuleBase module) : Components.GenericBaitAway(module, centerAtTarget: true)
{
...
class AutoAttacksStar(ModuleBase module) : Components.GenericStackSpread(module)
{
    private bool active;
    private readonly List<Actor> players = [];
