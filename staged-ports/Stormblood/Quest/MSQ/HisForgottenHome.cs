...
namespace Minerva.Stormblood.Quest.MSQ.HisForgottenHome;
...
sealed class Kasaya(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Kasaya, new AOEShapeCone(7.6f, 60f.Degrees()));
sealed class WaterIII(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WaterIII, 8f);

sealed class BlizzardIIIIcon(ModuleBase module) : Components.BaitAwayIcon(module, 5f, 26u)
{
...
sealed class BlizzardIIICast(ModuleBase module) : Components.VoidzoneAtCastTarget(module, 6f, (uint)AID.BlizzardIII, m => m.Enemies(0x1E8D9Cu).Where(x => x.EventState != 7), 0);

sealed class SlickshellCaptainStates : StateMachineBuilder
{
    public SlickshellCaptainStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
            ...
            .Raw.Update = () => module.World.CurrentCFCID != 469u;
    }
}

[ModuleInfo(Group = ModuleGroup.Quest, CFCID = 68563u, NameID = 6891u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")]
public sealed class SlickshellCaptain(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
