...
namespace Minerva.Stormblood.Raid.O8NKefka;
...
class Hyperdrive(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.Hyperdrive);
class BlizzardBlitzDonut(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.BlizzardBlitzDonut1, (uint)AID.BlizzardBlitzDonut2], new AOEShapeDonut(10f, 40f));
class BlizzardBlitzCircle(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.BlizzardBlitzCircle1, (uint)AID.BlizzardBlitzCircle2], 10f);

class FlagrantFireSpread(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.FlagrantFireSpread, 5f);
class FlagrantFireStack(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.FlagrantFireStack, 6f, 8, 8);

class ThrummingThunder(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.ThrummingThunder1, (uint)AID.ThrummingThunder2], new AOEShapeRect(40, 5f));

class UltimaUpsurge(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.UltimaUpsurge);

class AeroAssault(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.AeroAssault, 10f)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (Casters.Count != 0)
        {
            ...
            {
                hints.AddForbiddenZone(new SDKnockbackInCircleAwayFromOrigin(Center, c.Origin, 10f, 19f), c.Activation);
            }
        }
    }
}

class Shockwave(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.Shockwave, 15f, kind: Kind.DirForward)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (Casters.Count != 0)
        {
            ...
            if (!IsImmune(slot, act))
            {
                hints.AddForbiddenZone(new SDKnockbackInCircleFixedDirection(Center, 15f * c.Direction.ToDirection(), 19f), act);
            }
        }
    }
}

class WaveCannon(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WaveCannon, new AOEShapeRect(70f, 3f));

class GravitationalWaveIntemperateWill(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.GravitationalWave, (uint)AID.IntemperateWill], new AOEShapeCone(100f, 90f.Degrees()));
class AveMaria(ModuleBase module) : Components.CastGaze(module, (uint)AID.AveMaria, true);
class IndolentWill(ModuleBase module) : Components.CastGaze(module, (uint)AID.IndolentWill);

class RevoltingRuin(ModuleBase module) : Components.GenericAOEs(module)
{
...
{
    public O8NKefkaStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 289u, NameID = 7131u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (LTS, Malediktus) (ported from BMR)")]
public class O8NKefka(WorldState ws, Actor primary) : ModuleBase(ws, primary, default, new ArenaBoundsCircle(20f));
