...
sealed class MesmerizingMelody(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.MesmerizingMelody, 11f, kind: Kind.TowardsOrigin, stopAfterWall: true)
{
...
sealed class RuthlessRefrain(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.RuthlessRefrain, 11f, stopAfterWall: true)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (Casters.Count != 0)
...
abstract class PayThePiper : Components.GenericForcedMarch
{
    ...
    protected PayThePiper(ModuleBase module, float offset) : base(module, stopAfterWall: true)
    {
        _offset = offset;
    ...
    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        ...
        if (rot is Angle direction)
            AddForcedMovement(target, direction, 4f, World.FutureTime(10d));
    }
...
sealed class PayThePiperRegular(ModuleBase module) : PayThePiper(module, 25f);
sealed class PayThePiperHotspotCombo(ModuleBase module) : PayThePiper(module, 30f);
