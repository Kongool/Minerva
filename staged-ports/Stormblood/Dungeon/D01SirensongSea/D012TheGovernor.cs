...
namespace Minerva.Stormblood.Dungeon.D01SirensongSea.D012TheGovernor;
...
[SkipLocalsInit]
sealed class EnterNightPull(ModuleBase module) : Components.GenericKnockback(module)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        if (iconID == ((uint)IconID.EnterNight))
        {
            ...
            _kb = [new(Module.PrimaryActor.Position, 40f, World.FutureTime(3d), default, default, Kind.TowardsOrigin)];
        }
    }
...
[SkipLocalsInit]
sealed class EnterNight(ModuleBase module) : Components.StretchTetherSingle(module, (uint)TetherID.EnterNight, 16f, activationDelay: 4.3d);

[SkipLocalsInit]
sealed class ShadowFlow(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void Update()
    {
        if (_aoes.Count != 0)
        {
            ...
            if (aoe.Activation < World.CurrentTime)
            {
                _aoes.Clear();
            }
        }
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        {
            var activation = World.FutureTime(8d);
...
{
    public DO12TheGovernorStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 238u, CFCID = 238u, NameID = 6072u, PrimaryActorOID = (uint)OID.TheGovernor, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus), erdelf (ported from BMR)")]
[SkipLocalsInit]
public sealed class DO12TheGovernor : ModuleBase
{
