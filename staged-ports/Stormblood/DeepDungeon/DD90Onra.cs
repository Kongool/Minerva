...
namespace Minerva.Stormblood.DeepDungeon.HeavenOnHigh.DD90Onra;
...
class BurningRave(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BurningRave, 8f);
class KnucklePress(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.KnucklePress, 9f);
class AuraCannon(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AuraCannon, new AOEShapeRect(63f, 5f));
class AncientQuaga(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.AncientQuaga);
class MeteorImpact(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MeteorImpact, 20f);

class Subduction(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        if (actor.OID == (uint)OID.SandSphere)
            _aoes.Add(new(circle, actor.Position.Quantized(), default, World.FutureTime(3.2d)));
    }
...
{
    public DD90OnraStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 548u, NameID = 7493u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class DD90Onra(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
