...
class EpicStormsplitter(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        {
            _caster = caster;
            _stormsplitterCastStart = World.CurrentTime;
        ...
        {
            _seasplitterCastEnd = World.CurrentTime.AddSeconds(spell.TotalTime);
    ...
    public override void Update()
    {
        ...
        if (World.CurrentTime >= _stormsplitterCastStart.AddSeconds(5d))
        {
            _aoes.RemoveAll(aoe => aoe.Shape == _centerRect);
            UpdateSideAOEColors();
        }

        if (_sideAOEsDisplayed && World.CurrentTime >= _seasplitterCastEnd)
        {
