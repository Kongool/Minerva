...
namespace Minerva.Stormblood.Trial.T02Lakshmi;
...
sealed class EmanationTrash(ModuleBase module) : Components.Adds(module, (uint)OID.DreamingKshatriya);

// Half angle is an estimation.
sealed class TailSlap(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TailSlap, new AOEShapeCone(7f, 60f.Degrees()));

sealed class InnerDemonsGaze(ModuleBase module) : Components.CastGaze(module, (uint)AID.InnerDemons);
 ...
 */
sealed class VrilOrbs(ModuleBase module) : ModuleComponent(module)
{
    ...
    {
        var vrilCharge = World.Client.DutyActions[0].CurCharges > 0;
...
sealed class DivineDenial(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.DivineDenial, 6.5f)
{
...
// Regular raidwide
sealed class Stotram1(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Stotram1);
...
sealed class StotramShielding(ModuleBase module) : Components.RaidwideCasts(module, [(uint)AID.Stotram2, (uint)AID.Jagadishwari])
{
    private bool _casting;
...
sealed class ThePallOfLightStack(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.ThePallOfLightStack, 7, 8, 8);

// These should probably be converted to bait away tankbusters
sealed class ThePullOfLightTB1(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.ThePullOfLightTB1);
sealed class ThePullOfLightTB2(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.ThePullOfLightTB2);

sealed class PathOfLightCleave(ModuleBase module) : Components.GenericBaitAway(module, (uint)AID.ThePathOfLightCleave, tankbuster: true)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        ...
        _nextActivation = World.FutureTime(4.7f);
 ...
 */
sealed class BlissfulBaits(ModuleBase module) : Components.GenericBaitAway(module, (uint)AID.BlissfulSpear2)
{
    private Actor? _nextTarget;
    private DateTime _nextActivation;
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        _nextActivation = World.FutureTime(4.7d);
        // next target is the pc with the icon.
        _nextTarget = World.Actors.Find(targetID);
...
sealed class CircleZone(ModuleBase module) : Components.Voidzone(module, 10, m => m.Enemies((uint)OID.VoidZone));

sealed class T02LakshmiStates : StateMachineBuilder
{
    public T02LakshmiStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
            // Dreaming Kshatriya
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 263u, CFCID = 263u, NameID = 6385u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "wen, Combat Reborn Team (ported from BMR)")]

public sealed class T02Lakshmi(WorldState ws, Actor primary) : ModuleBase(ws, primary, default, new ArenaBoundsCircle(20f))
{
