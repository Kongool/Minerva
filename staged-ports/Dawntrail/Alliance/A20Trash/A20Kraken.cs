...
namespace Minerva.Dawntrail.Alliance.A20Kraken;
...
sealed class MightyStrikes(ModuleBase module) : Components.CastInterruptHint(module, (uint)AID.MightyStrikes, showNameInHint: true);
sealed class PowerAttack(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PowerAttack, new AOEShapeCone(20f, 60f.Degrees()));
sealed class CatapultParalyzeIIIBanishga(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.Catapult1, (uint)AID.Catapult2,
(uint)AID.Banishga, (uint)AID.ParalyzeIII], 6f);
sealed class ImpactRoar(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.ImpactRoar);

sealed class Sucker(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.Sucker, 15f, kind: Kind.TowardsOrigin)
{
...
sealed class Flood(ModuleBase module) : Components.GenericAOEs(module)
{
...
{
    public A20KrakenStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 1058u, CFCID = 1058u, NameID = 14071u, PrimaryActorOID = (uint)OID.Kraken, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class A20Kraken(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
