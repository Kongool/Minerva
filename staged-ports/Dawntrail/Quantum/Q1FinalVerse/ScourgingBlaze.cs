...
[SkipLocalsInit]
sealed class ScourgingBlaze(ModuleBase module) : Components.Exaflare(module, 5f)
{
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor) => Show ? Aoes : [];
    ...
    public override void OnActorCreated(Actor actor)
    {
        if (actor.OID == (uint)OID.Crystal)
        {
            ...
            if (crystals.Count == 12)
            {
                ...
                shape = new(Center, [new Rectangle(Q1FinalVerse.ArenaCenter, 20f, 15f)], rects);
            }
        }
    }
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        switch (spell.Action.ID)
        {
            ...
            case (uint)AID.ScourgingBlazeFirst:
                ...
                {
                    var line = Lines[i];
                    if (line.Next.AlmostEqual(pos, 0.1f) && (World.CurrentTime - line.NextExplosion).TotalSeconds < 1d)
                    {
