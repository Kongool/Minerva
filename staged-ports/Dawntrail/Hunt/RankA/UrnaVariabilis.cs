...
namespace Minerva.Dawntrail.Hunt.RankA.UrnaVariabilis;
...
sealed class MagnetismCircleDonut(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class ThunderousShower(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.ThunderousShower, 6f, 8);
sealed class ElectrowaveMagnetron(ModuleBase module) : Components.RaidwideCasts(module, [(uint)AID.Electrowave, (uint)AID.Magnetron]);

sealed class UrnaVariabilisStates : StateMachineBuilder
{
    public UrnaVariabilisStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.Hunt, CFCID = 0u, NameID = 13158u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
