...
sealed class Snowboulder(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance>[] _aoesPerPlayer = new List<AOEInstance>[PartyState.MaxPartySize];
    private readonly List<RectangleSE> rectangles = [];
    private readonly List<DateTime> activations = [];
...
sealed class SnowBoulderKnockback(ModuleBase module) : Components.GenericKnockback(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (!_charge.Vulnerable[slot] && _kbs.Count != 0)
        {
            ...
            {
                hints.AddForbiddenZone(new SDInvertedCircle(Center, 20f), kb.Activation);
            }
        }
    }
}

sealed class AvalaunchTether(ModuleBase module) : Components.StretchTetherDuo(module, 58f, 8f, (uint)TetherID.AvalaunchBad);

sealed class IceboundBuffoonery(ModuleBase module) : ModuleComponent(module)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
