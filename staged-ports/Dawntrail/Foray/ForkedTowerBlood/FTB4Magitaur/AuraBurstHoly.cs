...
sealed class AuraBurstHoly(ModuleBase module) : ModuleComponent(module)
{
    private bool? isHoly;
...
sealed class ArcaneReaction(ModuleBase module) : Components.GenericBaitAway(module)
{
...
sealed class ArcaneRecoil(ModuleBase module) : ModuleComponent(module)
{
    private readonly List<Actor> targets = [];
    private List<Actor> conduits = [];
    private bool? isHoly;
    public int NumCasts;
    ...
    public override void Update()
    {
        ...
        for (var i = 0; i < countConduits; ++i)
        {
            var c = conduits[i];
            var pos = c.Position;
            List<(Actor actor, float distSq)> distances = [];
            // note: this is problematic because player culling messes this up and leads to incorrect results (eg not finding the actual bait target)
            // in any frame players can be removed or added to the object table and will likely never contain all 48 players at the same time
            // caching removed players is also pointless since the player position no longer gets updated when this happens
            foreach (var a in Module.World.Actors.Actors.Values)
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (count != 0)
        {
            ...
            hints.AddPredictedDamage(mask, default);
        }
    }
}

sealed class AuraBurstHolyRaidwide(ModuleBase module) : Components.RaidwideCastsDelay(module, [(uint)AID.AuraBurstVisual, (uint)AID.HolyVisual], [(uint)AID.AuraBurst, (uint)AID.Holy], 1d)
{
    public override void AddGlobalHints(GlobalHints hints)
    {
        ...
        if ((Activation - World.CurrentTime).TotalSeconds < 6d)
        {
            hints.Add(Hint);
        }
    }
