...
namespace Minerva.Dawntrail.Foray.CriticalEngagement.CE101BlackRegiment;
...
sealed class ChocoSlaughter(ModuleBase module) : Components.SimpleExaflare(module, 5f, (uint)AID.ChocoSlaughterFirst, (uint)AID.ChocoSlaughterRest, 5f, 1.1d, 5, 3, true);

sealed class ChocoBeak(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorPlayActionTimelineEvent(Actor actor, ushort id)
    {
        ...
        {
            _aoes.Add(new(rect, actor.Position.Quantized(), actor.Rotation, World.FutureTime(9.1d), actorID: actor.InstanceID));
        }
    }
...
sealed class ChocoAeroII(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ChocoAeroII, 4f);
sealed class ChocoMaelfeather(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ChocoMaelfeather, 8f);
sealed class ChocoWindstorm(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ChocoWindstorm, 16f, riskyWithSecondsLeft: 4d);
sealed class ChocoCyclone(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ChocoCyclone, new AOEShapeDonut(8f, 30f));
sealed class ChocoDoublades(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.ChocoBlades1, (uint)AID.ChocoBlades2], new AOEShapeCone(40f, 22.5f.Degrees()), 4, 8);
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, CFCID = 1018u, NameID = 34u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class CE101BlackRegiment(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(450f, 357f), new ArenaBoundsSquare(20f))
{
    ...
    protected override bool CheckPull() => base.CheckPull() && Raid.Player()!.Position.InSquare(Module.Center, 20f); // not targetable at start
}
