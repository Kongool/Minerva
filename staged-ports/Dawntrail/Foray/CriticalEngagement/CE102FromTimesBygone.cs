...
namespace Minerva.Dawntrail.Foray.CriticalEngagement.CE102FromTimesBygone;
...
sealed class MythicMirror(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class MysticHeat(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MysticHeat, new AOEShapeCone(40f, 30f.Degrees()));
sealed class LotsCast(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.LotsCastVisual);
sealed class ArcaneLight(ModuleBase module) : Components.RaidwideCastDelay(module, (uint)AID.ArcaneLightVisual, (uint)AID.ArcaneLight, 0.9d);
sealed class LotsCastSpread(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.LotsCastSpread, 6f)
{
    public override void Update()
    {
        ...
        if (count != 0) // due to culling players can get removed and added to the object table in any frame, after that we need to update their reference to get the current location again...
        {
            ...
            for (var i = 0; i < count; ++i)
            {
                ...
                if (World.Actors.Find(s.Target.InstanceID) is Actor t)
                {
                    s.Target = t;
                }
            }
...
sealed class LotsCastTB(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.LotsCastTB, 6f, true, tankbuster: true)
{
    public override void Update()
    {
...
sealed class ArcaneOrb(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        {
            _aoes.Add(new(circle, spell.LocXZ, default, World.FutureTime(8.2d)));
        }
    }
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        hints.AddForbiddenZone(new SDCircle(Module.Center, 6.6f), World.FutureTime(8d));
    }
}

sealed class CE102FromTimesBygoneStates : StateMachineBuilder
{
    public CE102FromTimesBygoneStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, CFCID = 1018u, NameID = 39u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class CE102FromTimesBygone(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
