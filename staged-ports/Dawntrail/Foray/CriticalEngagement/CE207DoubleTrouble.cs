...
namespace Minerva.Dawntrail.Foray.CriticalEngagement.CE207DoubleTrouble;
...
[SkipLocalsInit]
sealed class AuraBurst(ModuleBase module) : Components.RaidwideCastDelay(module, (uint)AID.AuraBurstVisual, (uint)AID.AuraBurst, 0.8d);
[SkipLocalsInit]
sealed class Graft(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.Graft, (uint)AID.MaliciousWeave, (uint)AID.MaliciousWeave1], 6f);
[SkipLocalsInit]
sealed class DashingCut(ModuleBase module) : Components.SimpleChargeAOEGroups(module, [(uint)AID.DashingCut1, (uint)AID.DashingCut2], 5f);
[SkipLocalsInit]
sealed class HairShearsCross(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HairShearsCross, new AOEShapeCross(60f, 2f));
[SkipLocalsInit]
sealed class HairShearsCircle(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HairShearsCircle, 10f);

[SkipLocalsInit]
sealed class DualCut(ModuleBase module) : Components.GenericAOEs(module)
{
...
{
    public CE207DoubleTroubleStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, CFCID = 1093u, NameID = 50u, PrimaryActorOID = (uint)OID.ConjuredCalofisteri, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Equilius (ported from BMR)")]
[SkipLocalsInit]
public sealed class CE207DoubleTrouble : ModuleBase
{
    ...
    protected override bool CheckPull() => base.CheckPull() && Raid.Player()!.Position.InCircle(Module.Center, 22f);
}
