...
namespace Minerva.Dawntrail.Foray.CriticalEngagement.CE103WithExtremePrejudice;
...
sealed class AethericBurst(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.AethericBurst);
...
sealed class RockSlideStoneSwell(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        ...
        if (shape != null)
        {
            ...
            if (World.Actors.Find(tid) is Actor t)
            {
        ...
        void AddAOE(AOEShape shape, WPos position, Angle rotation, ulong cid) => _aoes.Add(new(shape, position.Quantized(), rotation, World.FutureTime(6.1d), actorID: cid));
    }
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, CFCID = 1018u, NameID = 43u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class CE103WithExtremePrejudice(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
