...
namespace Minerva.Dawntrail.Foray.CriticalEngagement.CE109CompanyOfStone;
...
sealed class LineOfFireSpiritSlingCageOfFire(ModuleBase module) : Components.SimpleAOEGroupsByTimewindow(module, [(uint)AID.LineOfFire1, (uint)AID.LineOfFire2,
...
sealed class KnuckleCrusher : Components.SimpleAOEs
{
    ...
    public KnuckleCrusher(ModuleBase module) : base(module, (uint)AID.KnuckleCrusher, 15f, 3)
    {
        MaxDangerColor = 1;
    }
...
sealed class KnuckleDown(ModuleBase module) : Components.RaidwideCastDelay(module, (uint)AID.KnuckleDownVisual, (uint)AID.KnuckleDown, 0.9d, "Raidwide x4");
sealed class Moatmaker(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Moatmaker, 9f);

sealed class SpinningSiege(ModuleBase module) : Components.GenericRotatingAOE(module)
{
    ...
    public override void OnCastFinished(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID is (uint)AID.SpinningSiegeCCW or (uint)AID.SpinningSiegeCW or (uint)AID.SpinningSiegeRest)
        {
            ...
            if ((NumCasts & 3) == 0)
            {
                midpoint -= 3f * (midpoint - Center).Normalized();
            }
        }
    }
...
sealed class BlastKnuckles(ModuleBase module) : Components.GenericKnockback(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (_kb.Length != 0)
        {
            ...
            if (!IsImmune(slot, act))
            {
                ...
                hints.AddForbiddenZone(new SDKnockbackInCircleAwayFromOriginPlusAOERects(Center, kb.Origin, 15f, 20f, rects, 60f, 4f, max), act);
            }
        }
    }
...
sealed class DualfistFlurry(ModuleBase module) : Components.Exaflare(module, 6f)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
...
sealed class BarefistedDeath(ModuleBase module) : Components.CastHint(module, (uint)AID.BarefistedDeath, "Enrage!", true);
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, GroupID = 1018u, CFCID = 1018u, NameID = 40u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class CE109CompanyOfStone(WorldState ws, Actor primary) : ModuleBase(ws, primary, new WPos(680f, -280f).Quantized(), new ArenaBoundsCircle(20f))
{
