...
namespace Minerva.Dawntrail.Foray.CriticalEngagement.CE211LostontheWind;
...
[SkipLocalsInit]
sealed class WindBlade(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WindBlade, new AOEShapeCone(60f, 90f.Degrees()));
[SkipLocalsInit]
sealed class CyclonicRing(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.CyclonicRing, new AOEShapeDonut(5f, 60f));
[SkipLocalsInit]
sealed class Splinter(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Splinter, 13f);
[SkipLocalsInit]
sealed class Skydive(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Skydive, 15f);
[SkipLocalsInit]
sealed class Hurricane(ModuleBase module) : Components.RaidwideCastDelay(module, (uint)AID.HurricaneVisual, (uint)AID.Hurricane, 0.9d);
[SkipLocalsInit]
...
sealed class Buffet(ModuleBase module) : Components.GenericKnockback(module)
{
    ...
    public override void OnActorEAnim(Actor actor, uint state)
    {
        if (actor.OID == (uint)OID.BuffetWind)
        {
            if (state == 0x00010002u)
            {
                ...
                knockbacks.Add(new(actor.Position, 24f, World.FutureTime(delay), direction: actor.Rotation, kind: Kind.DirForward));
            }
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (!IsImmune(slot, activation))
        {
            var aoes = _aoe.AOEs;
            var len = aoes.Length;
            var circleArcs = new AOEInstance[len];
            ...
            hints.AddForbiddenZone(new SDKnockbackInCircleAwayFromOriginMixedAOEs(Center, k.Origin, 24f, 23f, circleArcs, circleArcsCount), activation);
        }
    }
...
{
    public BitingWind(ModuleBase module) : base(module)
    {
...
[SkipLocalsInit]
sealed class TendronRipper(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        if (iconID == (uint)IconID.BitingWindAOE)
        {
            ...
            var activation = World.FutureTime(5.1d);
...
{
    public CE211LostontheWindStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
