...
[SkipLocalsInit]
sealed class HissingResonance(ModuleBase module) : Components.GenericKnockback(module)
{
    ...
    public override ReadOnlySpan<Knockback> ActiveKnockbacks(int slot, Actor actor)
    {
        ...
        WPos origin = new(posx, Module.Center.Z);
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (!IsImmune(slot, aoe1.Activation))
        {
            ...
            {
                case AOEShapeDonut:
                    hints.AddForbiddenZone(new SDKnockbackInAABBSquareFixedDirectionIntoCircle(Module.Center, direction, 19f, aoe2.Origin, 5f), aoe1.Activation);
                    break;
                case AOEShapeCross:
                    hints.AddForbiddenZone(new SDKnockbackInAABBSquareFixedDirectionPlusMixedAOEs(Module.Center, direction, 19f, [aoe2], 1), aoe1.Activation);
                    break;
            }
        }
    }
    ...
    private (Kind Kind, AOEInstance[] AOEs) GetHissingInfo(int slot)
    {
        ...
        var caster = World.Actors.Find(casterId);
