...
sealed class Steelsbreath(ModuleBase module) : Components.GenericKnockback(module)
{
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        if (status.ID == (uint)SID.LeapingLift && status.Extra is 0x47B or 0x495)
        {
            ...
            var act = World.FutureTime(_startDelay + delay + 1.1d * count);
        ...
        if (count != 0)
        {
            ...
            if (!isImmune)
            {
                if (count == 1)
                {
                    ...
                    hints.AddForbiddenZone(new SDKnockbackInCircleAwayFromOrigin(Module.Center, kb.Origin, 28f, 24f), act);
                }
                else
                {
                    ...
                    {
                        hints.AddForbiddenZone(new SDKnockbackInCircleAwayFromOriginIntoDonut(Module.Center, kb.Origin, 26f, 24f, kb1.Origin, 15f, 18f), act);
                    }
                    else
                    {
                        hints.AddForbiddenZone(new SDKnockbackInCircleAwayFromOriginIntoCircle(Module.Center, kb.Origin, 28f, 24f, kb1.Origin, 7f), act);
                    }
                }
            }
        }
    ...
    */
    private sealed class SDKnockbackInCircleAwayFromOriginIntoDonut(WPos Center, WPos Origin, float Distance, float Radius, WPos DonutOrigin, float InnerRadius, float OuterRadius) : ShapeDistance
    {
        ...
        // Minerva's ShapeDistance has ONE abstract member, Distance(WPos), and derives Contains from its
        // sign; BossmodReborn splits them and adds RowIntersectsShape. Ported to Minerva's shape: negative
        // is forbidden, so the BMR "Contains == forbidden" result maps to -1f, not to BMR's 0f -- 0 would
        // read as forbidden EVERYWHERE and quietly rule out the whole arena.
        public override float Distance(WPos p)
        {
            var landing = p + (distance * (p - origin).Normalized());
            if (!landing.InCircle(center, radius))
                return -1f;                                                    // knocked out of the arena
            return landing.InDonut(donutOrigin, innerRadius, outerRadius) ? 1f : -1f;  // safe only inside the donut
        }
    }
}

[SkipLocalsInit]
sealed class Steelsforge(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        if (status.ID == (uint)SID.LeapingLift && status.Extra is 0x47B or 0x495)
        {
            ...
            {
                var act = World.FutureTime(_startDelay + 1.1d * _counter);
