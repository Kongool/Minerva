...
sealed class GrimEmbraceBait(ModuleBase module) : Components.GenericBaitAway(module)
{
    ...
    public override void AddHints(int slot, Actor actor, TextHints hints)
    {
        ...
        if (s.Shape != null && s.Activation != default)
            hints.Add($"Dodge {(s.Shape == _shapeForward ? "backward" : "forward")} in {Math.Max(0d, (s.Activation - World.CurrentTime).TotalSeconds):f1}s", false);
...
sealed class GrimEmbraceAOE(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GrimEmbraceAOE, new AOEShapeRect(8f, 4f));
