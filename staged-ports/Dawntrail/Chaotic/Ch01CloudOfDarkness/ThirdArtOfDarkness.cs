...
sealed class ThirdArtOfDarknessCleave(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void AddHints(int slot, Actor actor, TextHints hints)
    {
        ...
        {
            var playerSide = actor.PosRot.X - Center.X;
            ...
            {
                var actorSide = kv.Key.PosRot.X - Center.X;
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        if (actor.OID == (uint)OID.StygianShadow)
        {
            ...
            if (mechanic != Mechanic.None)
                Mechanics.GetOrAdd(actor).Add((mechanic, World.FutureTime(9.5d)));
        }
    }
...
sealed class ThirdArtOfDarknessHyperFocusedParticleBeam(ModuleBase module) : Components.GenericBaitAway(module)
{
...
sealed class ThirdArtOfDarknessMultiProngedParticleBeam(ModuleBase module) : Components.UniformStackSpread(module, 3f, default, 2)
{
    private readonly ThirdArtOfDarknessCleave? _main = module.FindComponent<ThirdArtOfDarknessCleave>();
