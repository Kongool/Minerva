...
{
    public GetDownCone(ModuleBase module) : base(module, (uint)AID.GetDownCone, GetDownBait.Cone)
    {
        Color = Colors.Danger;
    }
}

sealed class GetDownOutIn(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class GetDownBait(ModuleBase module) : Components.GenericBaitAway(module)
{
            ...
            if (!First)
            {
                ...
                else
                {
                    ...
                    for (var i = 0; i < len; ++i)
                    {
                        ...
                        var actor = World.Actors.Find(targ.ID);
                        if (actor == null)
                            continue;
                        var angleToActor = Angle.FromDirection(actor.Position - Center);
