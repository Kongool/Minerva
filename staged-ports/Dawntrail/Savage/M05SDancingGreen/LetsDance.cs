...
sealed class LetsDance(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorModelStateChange(Actor actor, byte modelState, byte animState1, byte animState2)
    {
        ...
        {
            var count = _aoes.Count;
            var act = count != 0 ? _aoes.Ref(0).Activation.AddSeconds(count * 2.4d) : World.FutureTime(26.1d);
...
sealed class LetsDanceRemix(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorModelStateChange(Actor actor, byte modelState, byte animState1, byte animState2)
    {
        ...
        {
            var count = _aoes.Count;
            var act = count != 0 ? _aoes.Ref(0).Activation.AddSeconds(count * 1.5d) : World.FutureTime(26d);
        ...
        if (spell.Action.ID == (uint)AID.LetsDanceRemix)
        {
            ...
            if (count != 0)
            {
                ...
                if (count > 1)
                {
                    ...
                    ref var aoe1 = ref aoes[0];
                    var rot1 = aoe1.Rotation;
                    if (aoe1.Origin != Center.Quantized())
                    {
                        aoe1.Origin -= 5f * rot1.ToDirection();
