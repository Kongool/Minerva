...
sealed class DeadlyDoornail(ModuleBase module) : Components.Adds(module, (uint)OID.DeadlyDoornail, 3);
// prioritize for melees once doornail outside melee range
sealed class FatalFlail(ModuleBase module) : Components.Adds(module, (uint)OID.FatalFlail, 2);
...
sealed class ElectrocutionVoidzone(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        ...
        {
            var elapsed = (World.CurrentTime - _start).TotalMilliseconds;
...
sealed class GravegrazerSmall(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class GravegrazerBig(ModuleBase module) : Components.GenericAOEs(module)
{
