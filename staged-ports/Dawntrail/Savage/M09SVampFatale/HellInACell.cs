...
sealed class BloodyBondage(ModuleBase module) : Components.CastTowers(module, (uint)AID.BloodyBondage, 4f, 1, 1)
{
    ...
    // (towers[i].Position - Center).ToAngle().Deg, Util/Angle says S=0, W=-90, E=90, N=-/+180
        ...
        if (spell.Action.ID == WatchedAction)
        {
            ...
            if (Towers.Count == 4)
            {
                Towers.Sort(delegate (Tower x, Tower y)
                {
                    ...
                    var xAngle = (x.Position - Center).ToAngle();
                    var yAngle = (y.Position - Center).ToAngle();
...
sealed class CharnelCells(ModuleBase module) : Components.AddsMulti(module, [(uint)OID.CharnelCell, (uint)OID.CharnelCell1, (uint)OID.CharnelCell2]);
sealed class BloodyBondageUndeadDeathmatch(ModuleBase module) : Components.CastTowers(module, (uint)AID.BloodyBondageUndeadDeathmatch, 6f, 4, 4)
{
    private static readonly M09SVampFataleConfig _config = Service.Config.Get<M09SVampFataleConfig>();
    private static readonly PartyRolesConfig _partyConfig = Service.Config.Get<PartyRolesConfig>();
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        for (var i = 0; i < len; i++)
        {
            var angle = (towers[i].Position - Center).ToAngle();
...
sealed class UltrasonicSpreadTank(ModuleBase module) : Components.GenericBaitAway(module, (uint)AID.UltrasonicSpreadTank)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID == (uint)AID.UltrasonicSpreadCast)
        {
            ...
            for (var i = 0; i < len; i++)
            {
                if (party[i].Role == Role.Tank)
                {
                    ...
                    if (status == null || status.Value.ExpireAt.AddSeconds(-1) < World.CurrentTime)
                    {
...
sealed class UltrasonicSpreadRest(ModuleBase module) : Components.GenericBaitAway(module, (uint)AID.UltrasonicSpreadRest)
{
...
sealed class UltrasonicAmp(ModuleBase module) : Components.GenericBaitAway(module, (uint)AID.UltrasonicAmp)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
