...
sealed class AetherlettingCone : Components.SimpleAOEs
{
    ...
    public AetherlettingCone(ModuleBase module) : base(module, (uint)AID.AetherlettingCone, new AOEShapeCone(40f, 22.5f.Degrees()), 4)
    {
        MaxDangerColor = 2;
    }
...
sealed class AetherlettingPuddle(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.AetherlettingPuddle, 6f)
{
    private static readonly M09SVampFataleConfig _config = Service.Config.Get<M09SVampFataleConfig>();
    ...
    [
        module.Center + 157.5f.Degrees().ToDirection() * 18f,
        module.Center + 112.5f.Degrees().ToDirection() * 18f,
        module.Center + 67.5f.Degrees().ToDirection() * 18f,
        module.Center + 22.5f.Degrees().ToDirection() * 18f,
        module.Center - 22.5f.Degrees().ToDirection() * 18f,
        module.Center - 67.5f.Degrees().ToDirection() * 18f,
        module.Center - 112.5f.Degrees().ToDirection() * 18f,
        module.Center - 157.5f.Degrees().ToDirection() * 18f
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (baitPos == default)
        {
            hints.AddForbiddenZone(new AOEShapeCircle(18.5f), Center);
            return;
        }
...
sealed class AetherlettingCross(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AetherlettingCross, new AOEShapeCross(40f, 5f), 2)
{
