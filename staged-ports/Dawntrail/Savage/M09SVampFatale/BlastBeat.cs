...
sealed class BlastBeat(ModuleBase module) : Components.GenericAOEs(module)
{
    class Vampette
    {
        public Actor Actor;
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        ...
        for (var i = 0; i < len; i++)
        {
            ...
            if (vamps[i].Activation.AddSeconds(-1 * _blastDelay) <= World.CurrentTime)
            {
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        if (status.ID == (uint)SID.VampetteDistance)
        {
            ...
            for (var i = 0; i < len; i++)
            {
                ...
                {
                    var vamp = vamps[i];
                    var direction = actor.Rotation.ToDirection().OrthoL().Dot(actor.DirectionTo(Center)) > 0 ? 1 : -1;
                    var rotation = direction * 90.Degrees();
                    var vampdiff = actor.Position - Center;
                    var final = vampdiff.Rotate(rotation);

                    vamp.EndPos = Center + final;
                    vamp.Activation = status.Extra == 0x25 ? World.FutureTime(4.6d) : status.Extra == 0x33 ? World.FutureTime(8.1d) : World.FutureTime(11.6d);
                }
            }
...
sealed class CurseOfTheBombpyre(ModuleBase module) : Components.GenericStackSpread(module)
{
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
...
sealed class BombpyreRing(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void Update()
    {
        ...
        _radius = (float)((World.CurrentTime - _start).TotalMilliseconds / 500);
        if (_radius > 56.6f)
        {
            _start = default;
        }
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        ...
        Arena.ZoneCircleOutline(Center, _radius, Colors.Danger);
    }
}
