...
sealed class MustardBomb(ModuleBase module) : Components.UniformStackSpread(module, default, 6f)
{
    ...
    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        if (tether.ID == (uint)TetherID.MustardBomb)
        {
            ...
            if (CurMechanic == Mechanic.None)
            {
                ...
                _tetherTarget = World.Actors.Find(tether.Target);
                AddSpreads(Raid.WithoutSlot(true, true, true), World.FutureTime(8.8d));
            }
        }
    }
