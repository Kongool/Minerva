...
sealed class M04SWickedThunderStates : StateMachineBuilder
{
    ...
    private void PhaseTransition(uint id, float delay)
    {
        ComponentCondition<Soulshock>(id, delay, comp => comp.NumCasts > 0, "Raidwide 1")
            ...
            .OnEnter(() => Module.Center = M04SWickedThunder.TransitionBounds.Center)
            .OnEnter(() => Module.Bounds = M04SWickedThunder.TransitionBounds)
        ...
        ActorTargetable(id + 0x20, _module.BossP2, true, 11.9f, "Boss reappears")
            ...
            .OnEnter(() => Module.Center = M04SWickedThunder.P2Center)
            .OnEnter(() => Module.Bounds = M04SWickedThunder.P2DefaultBounds)
    ...
    private void AzureThunder(uint id, float delay)
    {
        ActorCast(id, _module.BossP2, (uint)AID.AzureThunder, delay, 5, true, "Raidwide")
            .OnExit(() => Module.Bounds = M04SWickedThunder.P2CircleBounds) // at the end of the cast arena changes to circle
            .SetHint(StateMachine.StateHint.Raidwide);
    }
    ...
    private void WickedThunder(uint id, float delay)
    {
        ActorCast(id, _module.BossP2, (uint)AID.WickedThunder, delay, 5, true, "Raidwide")
            .OnEnter(() => Module.Bounds = M04SWickedThunder.P2DefaultBounds) // at the beginning of the cast arena changes back to square
            .SetHint(StateMachine.StateHint.Raidwide);
    }
    ...
    private void FlameSlash(uint id, float delay)
    {
        ...
        ComponentCondition<FlameSlash>(id + 0x12, 1, comp => comp.NumCasts > 0, "Destroy center")
            .OnExit(() => Module.Bounds = M04SWickedThunder.P2TowersBounds);
        ...
        ComponentCondition<FlameSlash>(id + 0x50, 1, comp => !comp.SmallArena, "Restore center")
            .OnExit(() => Module.Bounds = M04SWickedThunder.P2DefaultBounds)
    ...
    private void SunriseSabbath(uint id, float delay)
    {
        ...
        ComponentCondition<SunriseSabbathElectronStream>(id + 0x81, 0.5f, comp => comp.NumCasts > 4, "Baits 2")
            ...
            .OnExit(() => Module.Bounds = M04SWickedThunder.P2DefaultBounds); // restore bounds (set to circle by azure thunder)
