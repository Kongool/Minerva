...
sealed class MidnightSabbath(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorPlayActionTimelineEvent(Actor actor, ushort id)
    {
        ...
        if (shape != default)
        {
            AOEs.Add(new(shape, actor.Position.Quantized(), actor.Rotation, World.FutureTime(delay)));
...
class ConcentratedScatteredBurst(ModuleBase module) : Components.UniformStackSpread(module, 5f, 5f)
{
        ...
        switch (spell.Action.ID)
        {
            case (uint)AID.WickedSpark:
                ...
                if (NumFinishedStacks == 0)
                    ShowStacks(World.FutureTime(3.1d));
                break;
            case (uint)AID.WickedFlare:
                ++NumFinishedStacks;
                Stacks.Clear();
                if (NumFinishedSpreads == 0)
                    ShowSpreads(World.FutureTime(3.1d));
                break;
        }
    }
