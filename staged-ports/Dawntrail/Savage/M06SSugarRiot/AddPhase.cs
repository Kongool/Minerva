...
sealed class Adds(ModuleBase module) : Components.AddsMulti(module, [(uint)OID.Mu, (uint)OID.Yan, (uint)OID.FeatherRay, (uint)OID.GimmeCat, (uint)OID.Jabberwock])
{
    public int CountMu;
    public int CountYan;
...
class ICraveViolence(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ICraveViolence, 6f);
class OreRigato(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.OreRigato);
class WaterIIIBait(ModuleBase module) : Components.BaitAwayTethers(module, 8f, (uint)TetherID.WaterIII, (uint)AID.WaterIII, (uint)OID.FeatherRay, 7.3f);
class WaterIIIVoidzone(ModuleBase module) : Components.VoidzoneAtCastTarget(module, 9f, (uint)AID.WaterIII, GetVoidzones, 1.6f) // apparently the voidzone is bigger than the AOE?
{
...
class ManxomeWindersnatch(ModuleBase module) : Components.SingleTargetInstant(module, (uint)AID.ManxomeWindersnatch, 5f)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        if (iconID == (uint)IconID.ManxomeWindersnatch)
        {
            ...
            for (var i = 0; i < count; ++i)
            {
                ...
                {
                    Targets.Add((Raid.FindSlot(targetID), World.FutureTime(5d), targetID, enemy, actor));
                    break;
                }
            }
        }
