    ...
    public override void OnMapEffect(byte index, uint state)
    {
        if (index is >= 0x1F and <= 0x23)
        {
            if (state == 0x00020001u)
            {
                ...
                if (pos != default)
                    AOE = [new(circle, pos, default, World.FutureTime(6d))];
            }
...
sealed class QuicksandDoubleStylePaintBomb(ModuleBase module) : ModuleComponent(module)
{
...
sealed class QuicksandDoubleStyleHeavenBomb(ModuleBase module) : Components.GenericKnockback(module, default, stopAfterWall: true)
{
    public BitMask Targets;
...
sealed class PaintBombHeavenBomb(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.Burst1, (uint)AID.Burst2], 10f);
sealed class PuddingGraf(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.PuddingGraf, 6f);
