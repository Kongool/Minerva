...
sealed class WolvesReignConeCircle(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        {
            var pos = spell.LocXZ;
            _aoe = [new(shape, pos, Angle.FromDirection(Center - pos), Module.CastFinishAt(spell, 3.6d), Colors.Danger)];
        }
        else if (id is (uint)AID.EminentReign or (uint)AID.RevolutionaryReign)
        {
            var pos = spell.LocXZ;
            jumpLoc = pos + 17.5f * (Center - pos).Normalized();
    ...
    private List<WPos> SafeSpots(int slot, Actor actor)
    {
        ...
        var assignment = _party[World.Party.Members[slot].ContentId];
        ...
        var bossDir = (Center - jumpLoc).Normalized();
...
sealed class WolvesReignRect(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class SovereignScar(ModuleBase module) : Components.GenericBaitStack(module)
{
...
sealed class ReignsEnd(ModuleBase module) : Components.GenericBaitAway(module)
{
...
sealed class RoaringWind(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorPlayActionTimelineEvent(Actor actor, ushort id)
    {
        ...
        {
            _aoes.Add(new(rect, actor.Position.Quantized(), actor.Rotation, World.FutureTime(5.6d)));
        }
    }
...
sealed class WealOfStone(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
