...
sealed class Adds(ModuleBase module) : ModuleComponent(module)
{
...
sealed class EarthWindborneEnd(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void AddHints(int slot, Actor actor, TextHints hints)
    {
        ...
        if (player != default)
        {
            hints.Add($"Order: {player.Order} - {(player.wind ? "wind" : "earth")}", Math.Max(0d, (player.Expiration - World.CurrentTime).TotalSeconds) < 9d);
        }
    }
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        ...
        if (player != default)
        {
            var remaining = Math.Max(0d, (player.Expiration - World.CurrentTime).TotalSeconds);
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        if (status.ID is (uint)SID.WindborneEnd or (uint)SID.EarthborneEnd)
        {
            ...
            if (slot < 0)
                return;
            var order = (status.ExpireAt - World.CurrentTime).TotalSeconds switch
            {
                < 22d => 1,
                < 38d => 2,
                _ => 3
...
sealed class StalkingStoneWind(ModuleBase module) : Components.GenericBaitStack(module)
{
...
sealed class AlphaWindStone(ModuleBase module) : Components.GenericBaitAway(module)
{
