...
namespace Minerva.Endwalker.DeepDungeon.PilgrimsTraverse.DD20ForgivenEmulation;
...
[SkipLocalsInit]
sealed class DenseBareRootPlanting(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.DensePlanting, (uint)AID.BareRootPlanting], 6f);

[SkipLocalsInit]
sealed class Touchdown(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.Touchdown, 10f)
{
...
sealed class Burst(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        ...
        {
            var count = AOEs.Count;
            AOEs.Add(new(circle, pos.Quantized(), default, count == 0 ? World.FutureTime(14.7d) : AOEs.Ref(count - 1).Activation.AddSeconds(1.3d), shapeDistance: circle.Distance(pos, default)));
            if (count == 1)
            {
...
[SkipLocalsInit]
sealed class BareRootPlantingBait(ModuleBase module) : Components.GenericBaitAway(module, centerAtTarget: true)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        ...
        {
            var activation = World.FutureTime(5.2d);
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        {
            hints.AddForbiddenZone(new SDCircle(Module.Center, 13.5f), CurrentBaits.Ref(0).Activation);
        }
    }
...
[SkipLocalsInit]
sealed class WoodsEmbrace(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
    private readonly List<SDCross> sdfs = [];
    private readonly AOEShapeCross cross = new(30f, 3f);  // 3 base range + 9 * 3 from status effect
...
{
    public DD20ForgivenEmulationStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 1033u, NameID = 13973u, PrimaryActorOID = (uint)OID.ForgivenEmulation, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
[SkipLocalsInit]
public sealed class DD20ForgivenEmulation(WorldState ws, Actor primary) : ModuleBase(ws, primary, arenaCenter, new ArenaBoundsCustom([new Polygon(arenaCenter, 14.709f, 72)]))
{
