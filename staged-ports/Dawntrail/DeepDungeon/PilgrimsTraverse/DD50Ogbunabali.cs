...
namespace Minerva.Endwalker.DeepDungeon.PilgrimsTraverse.DD50Ogbunabali;
...
[SkipLocalsInit]
sealed class Quicksand(ModuleBase module) : Components.GenericAOEs(module, warningText: "GTFO from quicksand!")
{
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        ...
        if (kbImminent && World.CurrentTime > kbActivation)
        {
            return _aoeInv;
        }
    ...
    {
        var isImminent = kbImminent && World.CurrentTime > kbActivation;
...
[SkipLocalsInit]
sealed class WindraiserArena(ModuleBase module) : Components.GenericAOEs(module)
{
    private AOEInstance[] _aoe = [];
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        if (index == 0x01)
        {
            switch (state)
            {
                case 0x00100004u:
                    Center = center;
                    Bounds = new ArenaBoundsCustom(GetBaseCircle());
                    break;
                case 0x00020001u:
                    Center = center;
                    Bounds = new ArenaBoundsCustom(GetBaseCircle(), [new Polygon(center.Quantized(), 3f, 64)]);
                    break;
            }
        }
    }
...
[SkipLocalsInit]
sealed class BitingWindKB(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.BitingWind, 20f)
{
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
...
[SkipLocalsInit]
sealed class FallingRock(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FallingRock, 5f);

[SkipLocalsInit]
sealed class PitAmbush(ModuleBase module) : Components.StandardChasingAOEs(module, 5f, (uint)AID.PitAmbushFirst, default, 5f, 2.1d, 4, true, (uint)IconID.PitAmbush)
{
...
{
    public DD50OgbunabaliStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 1036u, CFCID = 1036u, NameID = 14263u, PrimaryActorOID = (uint)OID.Ogbunabali, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
[SkipLocalsInit]
public sealed class DD50Ogbunabali : ModuleBase
{
