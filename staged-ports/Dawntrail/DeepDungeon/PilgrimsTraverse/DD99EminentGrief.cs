...
namespace Minerva.Endwalker.DeepDungeon.PilgrimsTraverse.DD99EminentGrief;
...
[SkipLocalsInit]
sealed class LightAndDark(ModuleBase module) : LightAndDarkBase(module)
{
...
[SkipLocalsInit]
sealed class TerrorEyeBallOfFire(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.TerrorEye, (uint)AID.BallOfFire], 6f);

[SkipLocalsInit]
sealed class ChainsOfCondemnation(ModuleBase module) : Components.StayMove(module, 2d)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
...
[SkipLocalsInit]
sealed class BladeOfFirstLight(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.BladeOfFirstLight1, (uint)AID.BladeOfFirstLight2], new AOEShapeRect(30f, 7.5f));

[SkipLocalsInit]
sealed class BoundsOfSinSmallAOE : Components.SimpleAOEs
{
    public BoundsOfSinSmallAOE(ModuleBase module) : base(module, (uint)AID.BoundsOfSin, 3f)
    {
        MaxDangerColor = 3;
    }
}
...
[SkipLocalsInit]
sealed class BoundsOfSinEnd(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnCastFinished(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID == (uint)AID.BoundsOfSinPull)
        {
            ...
            _aoe = [new(circle, loc, default, World.FutureTime(6d), shapeDistance: circle.Distance(loc, default))]; // true activation is 11.7, but slightly lower values seem to improve the pathfinding here
        }
    }
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        if (index is >= 0x00 and <= 0x0B)
        {
            if (state == 0x00020001u)
            {
                ...
                Bounds = new ArenaBoundsCustom([new Rectangle(new(-600f, -300f), 20f, 15f)], [.. pillars]);
            }
            else if (index == 0x00 && state == 0x00080004u)
            {
                Bounds = DD99EminentGrief.BuildArena().arena;
                pillars.Clear();
            }
        }
    }
...
[SkipLocalsInit]
sealed class SpinelashBait(ModuleBase module) : Components.GenericBaitAway(module)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        ...
        {
            CurrentBaits.Add(new(Module.PrimaryActor, actor, rect, World.FutureTime(6.3d), customRotation: Angle.AnglesCardinals[1]));
        }
    }
...
[SkipLocalsInit]
sealed class SpinelashBaitHint(ModuleBase module) : Components.GenericAOEs(module)
{
    private AOEInstance[] _aoe = [];
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        if (iconID == (uint)IconID.Spinelash)
        {
            ...
            _aoe = [new(shape, pos, default, World.FutureTime(6.3d), Colors.SafeFromAOE, shapeDistance: shape.InvertedDistance(pos, default))];
        }
    }
...
[SkipLocalsInit]
sealed class Spinelash(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Spinelash, new AOEShapeRect(60f, 2f));

[SkipLocalsInit]
sealed class BallOfFire(ModuleBase module) : Components.GenericBaitAway(module, centerAtTarget: true)
{
...
[SkipLocalsInit]
sealed class AbyssalBlaze(ModuleBase module) : Components.Exaflare(module, 5f)
{
    private readonly List<(WDir, WPos)> crystals = [];
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        ...
        shape.Outline(Arena, Center, default, Colors.Safe, 2f);
    }
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        switch (spell.Action.ID)
        {
            ...
            case (uint)AID.AbyssalBlazeFirst:
                ...
                {
                    var line = Lines[i];
                    if (line.Next.AlmostEqual(pos, 0.1f) && (World.CurrentTime - line.NextExplosion).TotalSeconds < 0.5d)
                    {
...
{
    public DD99EminentGriefStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 1041u, CFCID = 1041u, NameID = 14037u, PrimaryActorOID = (uint)OID.EminentGrief, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
[SkipLocalsInit]
public sealed class DD99EminentGrief : ModuleBase // module also works in Final Verse normal, everything but the zone ID seem to be identical
{
