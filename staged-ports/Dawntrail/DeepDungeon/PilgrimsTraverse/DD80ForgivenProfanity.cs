...
namespace Minerva.Endwalker.DeepDungeon.PilgrimsTraverse.DD80ForgivenProfanity;
...
[SkipLocalsInit]
sealed class BarefistedDeath(ModuleBase module) : Components.CastHint(module, (uint)AID.ProwlingDeath, "Watch your incoming debuff!");

[SkipLocalsInit]
sealed class RoaringRing(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.RoaringRing, new AOEShapeDonut(8f, 40f));
[SkipLocalsInit]
sealed class PerilousLair(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PerilousLair, 12f);

[SkipLocalsInit]
sealed class StalkingStaticShock(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        if (actor.OID == (uint)OID.BallOfLevin && levins.Count == 9)
        {
            ...
            var activation = World.CurrentTime;
...
[SkipLocalsInit]
sealed class ProfaneWaulShadowOfDeath(ModuleBase module) : Components.GenericAOEs(module)
{
...
[SkipLocalsInit]
sealed class NowhereToRun(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        {
            // BossmodReborn writes ForcedMovement = null here, which its own controller reads as "no forced
            // movement" rather than "do not move" -- the comment states the intent, the assignment does not
            // carry it. Minerva has the mechanism: NoMovement is what MustNotMove reads, and the dodge parks
            // itself while it holds. Open-ended (no finish) because the stacks decide when it lifts, and it
            // is re-stated every frame the condition holds.
            hints.AddSpecialMode(AIHints.SpecialMode.NoMovement, World.CurrentTime);
        }
    }
}
...
{
    public DD80ForgivenProfanityStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 1039u, CFCID = 1039u, NameID = 13968u, PrimaryActorOID = (uint)OID.ForgivenProfanity, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
[SkipLocalsInit]
public sealed class DD80ForgivenProfanity(WorldState ws, Actor primary) : ModuleBase(ws, primary, arenaCenter, new ArenaBoundsCustom([new Polygon(arenaCenter, 19.5f, 40)]))
{
