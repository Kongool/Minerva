...
namespace Minerva.Dawntrail.Quest.MSQ.TheWarmthOfTheFamily.AlphaAlligator;
...
sealed class FeedingTime(ModuleBase module) : Components.InterceptTether(module, (uint)AID.ToxicSpit, excludedAllies: [(uint)OID.Boss])
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (Active)
        {
            ...
            if (source == null)
                return;
            var target = World.Actors.Find(source.Tether.Target);
            if (target != null)
...
sealed class CriticalBite(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.CriticalBite, new AOEShapeCone(10, 60.Degrees()));
...
public sealed class AlphaAlligator(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
