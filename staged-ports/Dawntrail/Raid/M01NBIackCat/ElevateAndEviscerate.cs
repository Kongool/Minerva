...
sealed class ElevateAndEviscerate(ModuleBase module) : Components.GenericKnockback(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (kb.Length != 0)
        {
            ...
            hints.AddForbiddenZone(new SDKnockbackInComplexPolygonAwayFromOriginPlusAOEAABBSquares(Center, Tether.source.Position, 10f, poly, temp, 5f, total), kb[0].Activation);
        }
    }
}
...
sealed class ElevateAndEviscerateImpact(ModuleBase module) : Components.GenericAOEs(module, default, "GTFO from impact!")
{
    private readonly ElevateAndEviscerate _kb = module.FindComponent<ElevateAndEviscerate>()!;
    ...
    {
        var aoes = new List<AOEInstance>();
        if (_kb.Tether != default && _kb.Tether.target != actor && InBounds(_kb.Cache))
        {
    ...
    public override void OnCastFinished(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID == (uint)AID.ElevateAndEviscerate && InBounds(_kb.Cache))
        {
