...
sealed class DeepImpact(ModuleBase module) : Components.BaitAwayCast(
    module,
    (uint)AID.DeepImpact,
    6f,
    centerAtTarget: true,
...
sealed class DiversDare(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.DiversDare);
sealed class DiversDareBlue(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.DiversDare1);

sealed class DeepVarialCone(ModuleBase module) : Components.SimpleAOEs(
    module,
    (uint)AID.DeepVarial1,
    new AOEShapeCone(60f, 60f.Degrees()));

sealed class SickestTakeOffLine(ModuleBase module) : Components.SimpleAOEs(
    module,
    (uint)AID.SickestTakeOff1,
    new AOEShapeRect(50f, 7.5f));

sealed class SickSwellKB(ModuleBase module) : Components.SimpleKnockbacks(
    module,
    ...
    stopAtWall: false) // outside wall is deadly
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        var square = (ArenaBoundsSquare)Bounds;
...
sealed class PyrotationStack(ModuleBase module) : Components.StackWithIcon(module, (uint)IconID.FireStack, (uint)AID.Pyrotation1, 6f, 5d, 8, 8);

// Persistent puddles created when Pyrotation resolves; cleared by Divers' Dare.
sealed class PyrotationPuddles(ModuleBase module) : Components.GenericAOEs(module)
{
            ...
            case (uint)AID.Pyrotation1:
                {
                    var t = _stackTarget;
                    if (t != null)
                        _puddles.Add(new(Shape, t.Position, default, World.CurrentTime, Colors.AOE, true));
...
sealed class XtremeSpectacularEdge(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.XtremeSpectacular2, new AOEShapeRect(50f, 18f));
sealed class XtremeSpectacularRaidwide(ModuleBase module) : Components.RaidwideCastsDelay(module, [(uint)AID.XtremeSpectacular2], [(uint)AID.XtremeSpectacular3, (uint)AID.XtremeSpectacular4], 0d);

// =========================
// Module
// =========================

[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 1070u, CFCID = 1070u, NameID = 14370u, PrimaryActorOID = (uint)OID.RedHot, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "JoeSparkx (ported from BMR)")]
public sealed class M10NDaringDevils(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(100f, 100f), new ArenaBoundsSquare(20f))
{
