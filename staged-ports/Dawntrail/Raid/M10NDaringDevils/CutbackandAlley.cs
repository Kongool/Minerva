...
sealed class CutbackBlazeBait(ModuleBase module) : Components.BaitAwayIcon(
    module,
...
sealed class CutbackBlazePersistent(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.CutbackBlaze1)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        switch (spell.Action.ID)
        {
            case (uint)AID.CutbackBlaze1:
                _aoes.Add(new(CutbackBlazeBait.Cone, caster.Position, caster.Rotation, World.CurrentTime, Colors.AOE, true, caster.InstanceID));
                break;
...
sealed class AlleyOopInfernoSpread(ModuleBase module)
    : Components.SpreadFromCastTargets(module, (uint)AID.AlleyOopInferno1, 5f);

sealed class AlleyOopInfernoPuddles(ModuleBase module) : Components.GenericAOEs(module)
{
            ...
            case (uint)AID.AlleyOopInferno1:
                {
                    ...
                    {
                        var target = World.Actors.Find(t.ID);
                        if (target != null)
                            _puddles.Add(new(Shape, target.Position, default, World.CurrentTime, Colors.AOE, true));
                    }
                    break;
                }
...
sealed class AlleyOopMaelstromSequential(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        var activation = World.FutureTime(spell.NPCRemainingTime);
