...
sealed class RideTheWaves(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        if (index == 0x04)
        {
            ...
            if (indices.Length != 0)
            {
                for (var i = 0; i < 7; ++i)
                {
                    _aoes.Add(new(rect, new(82.5f + indices[i] * 5f, 70f), default, World.FutureTime(3d)));
                }
            }
        }
    }
