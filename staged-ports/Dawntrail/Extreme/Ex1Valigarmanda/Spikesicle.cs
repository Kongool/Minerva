...
sealed class Spikesicle(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        if (state == 0x00020004u && index is >= 4 and <= 13)
        {
            ...
            _aoes.Add(new(shape, pos, default, World.FutureTime(activationDelay), _aoes.Count == 0 ? Colors.Danger : default, shapeDistance: shape.Distance(pos, default)));
        }
    }
...
sealed class SphereShatter(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.SphereShatter)
{
    private readonly List<AOEInstance> _aoes = [];
    ...
    public override void OnActorCreated(Actor actor)
    {
        if (actor.OID == (uint)OID.IceBoulder)
        {
            ...
            _aoes.Add(new(circle, pos, default, World.FutureTime(6.5d), _aoes.Count == 0 ? Colors.Danger : default, shapeDistance: circle.Distance(pos, default)));
        }
    }
