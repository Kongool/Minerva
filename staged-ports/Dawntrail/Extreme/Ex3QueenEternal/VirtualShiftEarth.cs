...
sealed class VirtualShiftEarth(ModuleBase module) : ModuleComponent(module)
{
...
abstract class LawsOfEarthBurst(ModuleBase module) : Components.GenericTowers(module, (uint)AID.LawsOfEarthBurst)
{
...
{
    public LawsOfEarthBurst1(ModuleBase module) : base(module)
    {
        AddTowers(World.FutureTime(5d), VirtualShiftEarth.Midpoint, VirtualShiftEarth.CenterOffset, new(default, 6f), new(2f, default));
    }
}

sealed class LawsOfEarthBurst2 : LawsOfEarthBurst
{
    public LawsOfEarthBurst2(ModuleBase module) : base(module)
    {
        AddTowers(World.FutureTime(8.8d), VirtualShiftEarth.Midpoint, VirtualShiftEarth.CenterOffset, new(default, 5f));
    }
...
sealed class GravityPillar(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.GravityPillar, 10f);

// note: the tethers appear before target is created; the target is at the same location as the boss
sealed class GravityRay(ModuleBase module) : Components.BaitAwayTethers(module, new AOEShapeCone(50f, 30f.Degrees()), (uint)TetherID.GravityRay, (uint)AID.GravityRay) // TODO: verify angle
{
    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        if (tether.ID == TID)
...
sealed class MeteorImpact(ModuleBase module) : Components.CastCounter(module, default)
{
...
sealed class WeightyBlow(ModuleBase module) : Components.CastCounter(module, (uint)AID.WeightyBlowAOE)
{
    private readonly VirtualShiftEarth? _virtualShift = module.FindComponent<VirtualShiftEarth>();
