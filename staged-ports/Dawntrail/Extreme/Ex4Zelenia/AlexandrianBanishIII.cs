...
sealed class AlexandrianBanishIII(ModuleBase module) : Components.GenericBaitStack(module, (uint)AID.AlexandrianBanishIII)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        ...
        {
            CurrentBaits.Add(new(Center, actor, circle, World.FutureTime(4.1d)));
        }
    }
        ...
        for (var i = 0; i < 8; ++i)
        {
            for (var ring = 0; ring <= 1; ++ring)
            {
                ...
                while (stackPtr > 0)
                {
                    ...
                    var shape = r == 0
                        ? new DonutSegmentV(Center, 2f, 8f, FloorTiles.TileAngles[j], angle, 8)
                        : new DonutSegmentV(Center, 8f, 16f, FloorTiles.TileAngles[j], angle, 8);
...
sealed class AlexandrianBanishIIITargetHint(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        if (iconID == (uint)IconID.AlexandrianBanishIII)
        {
            ...
            _aoe = [new(new AOEShapeCustom(center, shapes, invertForbiddenZone: true), center, default, World.FutureTime(4.1d), Colors.SafeFromAOE)];
        }
    }
}
