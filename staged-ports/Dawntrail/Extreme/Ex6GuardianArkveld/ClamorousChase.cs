...
sealed class ClamorousChaseBait(ModuleBase module) : Components.GenericBaitAway(module, centerAtTarget: true)
{
    ...
    public override void Update()
    {
        ...
        void AddBaits(int amount)
        {
            ...
            for (var i = 0; i < len && CurrentBaits.Count < amount; ++i)
            {
                ...
                {
                    CurrentBaits.Add(new(Module.PrimaryActor, p.Item2, circle, count == 0 ? World.FutureTime(8.3d) : CurrentBaits.Ref(count - 1).Activation.AddSeconds(3d)));
                }
            }
        }
    }
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (spot is WPos s)
        {
            hints.AddForbiddenZone(new SDInvertedCircle(s, 2f), World.FutureTime(3d));
        }
    }
...
sealed class ClamorousChaseAOE(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        if (offset is Angle o)
        {
            ...
            _aoe = [new(cone, loc, rot, World.FutureTime(2d), shapeDistance: cone.Distance(loc, rot))];
        }
    }
}
