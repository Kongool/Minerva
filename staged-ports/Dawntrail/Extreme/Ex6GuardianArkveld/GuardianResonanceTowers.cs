...
sealed class GuardianResonanceTowers(ModuleBase module) : Components.GenericTowers(module)
{
    private BitMask forbidden = GetTanks(module);
    private bool aoesStarted, aoesEnded;

    private static BitMask GetTanks(ModuleBase module)
    {
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        else if (id == (uint)AID.GuardianResonanceAOE)
        {
            ...
            else if (!aoesEnded && (Towers.Ref(0).Activation - World.CurrentTime).TotalSeconds < 4d)
            {
                var towers = CollectionsMarshal.AsSpan(Towers);
                for (var i = 0; i < 6; ++i)
                {
