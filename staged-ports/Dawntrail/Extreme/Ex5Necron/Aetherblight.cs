...
sealed class Aetherblight(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        ...
        if (shape != null)
        {
            ...
            if (!relentlessReaping)
            {
                ...
                void AddAOE(WPos pos) => _aoes.Add(new(shape, pos.Quantized(), actor.Rotation, World.FutureTime(12.4d)));
            }
        }
    }
    ...
    public void UpdateAOEs(double delay)
    {
        var aoes = CollectionsMarshal.AsSpan(_aoes);
        var len = aoes.Length;
        var act = World.FutureTime(delay);
        for (var i = 0; i < len; ++i)
        {
    ...
    public override void OnActorModelStateChange(Actor actor, byte modelState, byte animState1, byte animState2)
    {
        if (!rotated && relentlessReaping && Hints.Count == 4) // if hints < 4 the player was probably in prison and data could not be collected
        {
            ...
            if (index != -1)
            {
                ...
                rotated = true;
                void AddAOE(AOEShape shape, WPos pos, int i) => _aoes.Add(new(shape, pos.Quantized(), rot, World.FutureTime(12.4d + i * 2.8d)));
            }
        }
    }
