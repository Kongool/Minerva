...
sealed class WyvernsVengeance(ModuleBase module) : Components.Exaflare(module, 6f)
{
...
sealed class WyvernsWealAOE(ModuleBase module)
...
sealed class WyvernsWealPulses(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        if (spell.Action.ID == (uint)AID.WyvernsWeal2)
            _aoes.Add(new AOEInstance(_shape, caster.Position, caster.Rotation, World.FutureTime(0.8f)));
    }
...
sealed class WyvernsWealIrregularCastLane(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        if (_shape == null || World.CurrentTime >= _until)
            return default;
    ...
    {
        if (_forbid == null || World.CurrentTime >= _until)
            return;
