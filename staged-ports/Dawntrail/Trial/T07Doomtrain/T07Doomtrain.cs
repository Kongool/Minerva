...
namespace Minerva.Dawntrail.Trial.T07Doomtrain;
...
sealed class ElectrayShort(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ElectrayShort, new AOEShapeRect(10f, 2.5f));
sealed class ElectrayMedium(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ElectrayMedium, new AOEShapeRect(15f, 2.5f));
sealed class ElectrayLong(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ElectrayLong, new AOEShapeRect(25f, 2.5f));
sealed class LightningBurstTankBuster(ModuleBase module) : Components.BaitAwayIcon(module, new AOEShapeCircle(5f), (uint)IconID.LightningBurstIcon, (uint)AID.LightningBurst, 5f, centerAtTarget: true, tankbuster: true);

// For first pass the knockback distance is estimated.
sealed class LightningExpress(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.LightningExpress, 16f, kind: Kind.DirForward);
 ...
 */
class LevinSignal(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        if (actor.OID == (uint)OID.LevinSignal)
        {
            ...
            _casters.Add((actor, ground, World.FutureTime(7), maxLen, offset));
        }
    }
...
sealed class WindpipeDrawIn(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.WindpipeDrawIn, 20f)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        {
            var kb = _kb[slot];
            if (!IsImmune(slot, World.CurrentTime))
            {
                ...
                if (len > 0)
                {
                    ...
                    hints.AddForbiddenZone(new SDKnockbackFixedDirectionAgainstSafewalls(dir, swalls, 20f, len), World.CurrentTime);
                }
            }
        }
    }
...
sealed class Blastpipe(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Blastpipe, new AOEShapeRect(10f, 10f), riskyWithSecondsLeft: 3);
 ...
 */
sealed class HeadOnEmission(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly CarGeometry _car = module.FindComponent<CarGeometry>()!;
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        {
            _aoes.Add(new(_car.GroundShape, Module.Center));
        }
        else if ((AID)spell.Action.ID is AID.HeadlightUpperDeck)
        {
            _aoes.Add(new(_car.AirShape, Module.Center));
        }
    }
...
sealed class AetherSurge(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AetherSurge, new AOEShapeCone(15f, 22.5f.Degrees()), riskyWithSecondsLeft: 6d);
 ...
 */
class AetherialRay(ModuleBase module) : Components.GenericBaitAway(module, (uint)AID.AetherialRay)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        ...
        {
            _nextTarget = World.Actors.Find(targetID);
            _nextActivation = World.FutureTime(9.7f);
        }
...
//RunawayTrainRaidwide = 45645, // 233C->self, no cast, range 20 circle
sealed class RunawayTrainRaidwide(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.RunawayTrainRaidwide);
...
sealed class ArcaneRevelation(ModuleBase module) : Components.GenericBaitAway(module, centerAtTarget: true)
{
...
sealed class HailOfThunder(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HailOfThunder, new AOEShapeCircle(16f), riskyWithSecondsLeft: 3.2d);

sealed class Electray3(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Electray3, new AOEShapeRect(20f, 2.5f), riskyWithSecondsLeft: 5d);

sealed class ElectrayUpper(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
...
sealed class RunawayTrain(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.RunawayTrain);
...
sealed class Derail(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class DerailmentSiegeCircle(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.DerailmentSiegeCircle, 5, 8);
...
sealed class ArenaChangeCar4(ModuleBase module) : ModuleComponent(module)
{
    private readonly CarGeometry _car = module.FindComponent<CarGeometry>()!;
...
{
    public DoomtrainStates(ModuleBase module) : base(module)
    {
        SimplePhase(0, P1, "P1")
    ...
    void Intermission(uint id, float delay)
    {
        Timeout(id, delay, "Boss reappears")
            ...
            .OnEnter(() =>
            {
                ...
                Module.Center = new(-400, -400);
                Shape[] intermissionCircles = [new Circle(Module.Center, 14.5f), new Donut(Module.Center, 22.4f, 27.5f)];
                ArenaBoundsCustom intermissionArena = new(intermissionCircles, AdjustForHitboxInwards: true);
                Module.Bounds = intermissionArena;
            })
...
[ModuleInfo(CFCID = 1076u, NameID = 14284u, PrimaryActorOID = (uint)OID.Doomtrain, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "wen, Xan (ported from BMR)")]
public class Doomtrain(WorldState ws, Actor primary)
    : ModuleBase(ws, primary, new(100, 100), new ArenaBoundsRect(10f, 15f))
{
    protected override void DrawEnemies(int pcSlot, Actor pc)
    {
        if (PrimaryActor.IsTargetable)
        {
            var height = Bounds.Radius;
            if (PrimaryActor.Position.InRect(Center, default(Angle), height + 12, height + 12, 10))
                Arena.ActorInsideBounds(Center - new WDir(0, height), PrimaryActor.Rotation, Colors.Enemy);
            else
                Arena.ActorOutsideBounds(Center - new WDir(0, height), PrimaryActor.Rotation, Colors.Enemy);
        }
