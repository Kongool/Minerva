...
namespace Minerva.Shadowbringers.Dungeon.D09GrandCosmos.D093Lugus;
...
class MortalFlame(ModuleBase module) : ModuleComponent(module)
{
    private BitMask burning;
    private DateTime _activation;

    public static List<Actor> Furniture(ModuleBase module)
    {
...
class BlackFlame(ModuleBase module) : Components.GenericBaitAway(module, centerAtTarget: true)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        ...
        {
            var activation = World.FutureTime(4d);
...
class OtherworldlyHeat(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.OtherworldlyHeat, new AOEShapeCross(10f, 2f));

sealed class Scorching(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.ScorchingLeft, (uint)AID.ScorchingRight], new AOEShapeCone(40f, 90f.Degrees()));

class CullingBlade(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.CullingBlade);
class CaptiveBolt(ModuleBase module) : Components.SingleTargetDelayableCast(module, (uint)AID.CaptiveBolt);
class FiresIre(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FiresIre, new AOEShapeCone(20f, 45f.Degrees()));
class Plummet(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Plummet, 3f);
class FiresDomainTether(ModuleBase module) : Components.StretchTetherDuo(module, default, default)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (IsBaitTarget(actor))
        {
            hints.AddForbiddenZone(new SDRect(Module.Center + offset, Module.Center - offset, 23.5f));
        }
    }
}

class FiresDomain(ModuleBase module) : Components.GenericBaitAway(module)
{
...
class FiresIreBait(ModuleBase module) : Components.GenericBaitAway(module)
{
...
{
    public D093LugusStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 692u, NameID = 9046u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D093Lugus(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(default, -340f), new ArenaBoundsSquare(24.5f))
{
