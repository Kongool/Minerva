...
namespace Minerva.Shadowbringers.Dungeon.D12MatoyasRelict.D122Nixie;
...
class Gurgle(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        if (state == 0x00020001u && index is > 0x12 and < 0x1B)
        {
            ...
            _aoes.Add(new(rect, new WPos(posX, posZ).Quantized(), rot, World.FutureTime(9d)));
        }
    }
...
class Crack(ModuleBase module) : Components.GenericBaitAway(module, tankbuster: true)
{
    ...
    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        if (tether.ID == (uint)TetherID.Crack)
        {
            ...
            if (target is Actor t)
                CurrentBaits.Add(new(source, t, rect, World.FutureTime(5.4d)));
        }
    }
...
sealed class GeysersCloudPlatform(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        ...
        if (count != 0 && Bounds.Radius == 19.5f)
        {
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        {
            _aoes.Add(new(circle, actor.Position.Quantized(), default, World.FutureTime(3.9d), actorID: actor.InstanceID));
        }
    }
...
class Sputter(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.Sputter, 6f);

class D122NixieStates : StateMachineBuilder
{
    public D122NixieStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 746u, NameID = 9738u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D122Nixie(WorldState ws, Actor primary) : ModuleBase(ws, primary, ArenaCenter, new ArenaBoundsSquare(19.5f))
{
