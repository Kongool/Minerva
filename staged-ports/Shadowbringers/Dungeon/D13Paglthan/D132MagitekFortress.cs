...
namespace Minerva.Shadowbringers.Dungeon.D13Paglthan.D132MagitekFortress;
...
sealed class TwoTonzeMagitekMissile(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TwoTonzeMagitekMissile, 12f);
sealed class Aethershot(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Aethershot, 6f);
sealed class DefensiveReaction(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.DefensiveReaction);
sealed class Exhaust(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Exhaust, new AOEShapeRect(40f, 3.5f));

sealed class GroundToGroundBallistic(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.GroundToGroundBallistic, 10f)
{
...
sealed class StableCannon(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        {
            _aoes.Add(new(rect, new WPos(-185f + 10f * (index - 0x08), 28.3f).Quantized(), Angle.AnglesCardinals[1], World.FutureTime(12.1d)));
        }
...
sealed class MagitekMissile(ModuleBase module) : Components.GenericAOEs(module)
{
    private const float Radius = 1f, Length = 10f;
    private static readonly AOEShapeCapsule capsule = new(Radius, Length);
    private readonly List<Actor> _missiles = [];
...
sealed class CorePlatform(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        if (_aoe.Length != 0 && Bounds.Radius == 14.5f)
        {
...
{
    public D132MagitekFortressStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 777u, CFCID = 777u, NameID = 10067u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D132MagitekFortress(WorldState ws, Actor primary) : ModuleBase(ws, primary, DefaultCenter, new ArenaBoundsSquare(14.5f))
{
