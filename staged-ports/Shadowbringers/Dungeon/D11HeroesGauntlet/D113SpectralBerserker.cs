...
namespace Minerva.Shadowbringers.Dungeon.D11HeroesGauntlet.D113SpectralBerserker;
...
sealed class BeastlyFuryArenaChange(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        if (spell.Action.ID == (uint)AID.BeastlyFury && Bounds.Radius > 20f)
        {
            var center = Center;
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        {
            Bounds = new ArenaBoundsCustom([new Cross(Center, 20f, 10f)]);
            _aoe = [];
        }
    }
}

sealed class FallingRock(ModuleBase module) : Components.SpreadFromIcon(module, (uint)IconID.Spreadmarker, (uint)AID.WildAnguish2, 8.5f, 6d) // 8.5 instead of 6 to prevent aoe from intersecting additional rubble hitboxes
{
...
sealed class WildAnguish1(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.WildAnguish1, 6f, 4, 4)
{
    public static bool IsQuadrupleStack(ModuleBase module)
    {
        var rubble = module.Enemies((uint)OID.Rubble);
        var count = rubble.Count;
        for (var i = 0; i < count; ++i)
...
sealed class WildAnguish2(ModuleBase module) : Components.GenericTowers(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        if (actor.OID == (uint)OID.Rubble)
            Towers.Add(new(actor.Position, actor.HitboxRadius, activation: World.FutureTime(6.1d)));
    }
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        {
            var t = World.Actors.Find(spell.MainTargetID);
...
sealed class WildRageKnockback(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.WildRageKnockback, 15f)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (Casters.Count != 0)
        {
            ...
            if (!IsImmune(slot, act))
            {
                ...
                hints.AddForbiddenZone(new SDKnockbackInComplexPolygonAwayFromOriginPlusAOECircles(Center, c.Origin, 15f, polygon, [new(738f, 482f), new(762f, 482f)], 7.5f, 2), c.Activation);
            }
        }
    }
}

sealed class WildRageRaidwide(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.WildRageKnockback);
sealed class WildRage(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WildRage, 8f);
sealed class BeastlyFury(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.BeastlyFury);

sealed class CratersWildRampage(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        if (_aoe is AOEShapeCustom aoe)
        {
            ...
            return new AOEInstance[1] { new(aoe, Center, default, activation, invert ? Colors.SafeFromAOE : default) };
        }
        return [];
    }
    ...
    public override void OnActorEAnim(Actor actor, uint state)
    {
        if (state == 0x00010002u && actor.OID == (uint)OID.Crater)
        {
            ...
            _aoe = new AOEShapeCustom(Center, [.. circles]);
        }
    }
...
sealed class RagingSlice(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.RagingSliceFirst, (uint)AID.RagingSliceRest], new AOEShapeRect(50f, 3f));

sealed class D113SpectralBerserkerStates : StateMachineBuilder
{
    public D113SpectralBerserkerStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 737u, CFCID = 737u, NameID = 9511u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D113SpectralBerserker(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(750f, 482f), new ArenaBoundsSquare(22.5f));
