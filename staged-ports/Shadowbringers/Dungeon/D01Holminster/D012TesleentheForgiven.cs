...
namespace Minerva.Shadowbringers.Dungeon.D01Holminster.D012TesleentheForgiven;
...
sealed class TheTickler(ModuleBase module) : Components.SingleTargetDelayableCast(module, (uint)AID.TheTickler);
sealed class ScoldsBridle(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.ScoldsBridle);

sealed class FeveredFlagellation(ModuleBase module) : Components.GenericBaitAway(module)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        if (iconID is >= (uint)IconID.Icon1 and <= (uint)IconID.Icon4)
        {
            ...
            CurrentBaits.Add(new(primary, actor, new AOEShapeRect((actor.Position - primary.Position).Length(), 2f), World.FutureTime(8.3d + CurrentBaits.Count * 0.3d)));
        }
    }
...
sealed class Exorcise(ModuleBase module) : Components.StackWithIcon(module, (uint)IconID.Exorcise, (uint)AID.Exorcise, 6f, 5.1d, 4, 4);
sealed class HolyWater(ModuleBase module) : Components.VoidzoneAtCastTarget(module, 6f, (uint)AID.HolyWater, GetVoidzones, 0.8d)
{
...
{
    public D012TesleentheForgivenStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 676u, NameID = 8300u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "legendoficeman, Malediktus (ported from BMR)")]
public sealed class D012TesleentheForgiven(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
