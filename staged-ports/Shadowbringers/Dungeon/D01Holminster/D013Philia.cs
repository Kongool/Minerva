...
namespace Minerva.Shadowbringers.Dungeon.D01Holmintser.D013Philia;
...
sealed class SludgeVoidzone(ModuleBase module) : Components.Voidzone(module, 9f, GetVoidzones)
{
...
sealed class ScavengersDaughter(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.ScavengersDaughter);
sealed class HeadCrusher(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.HeadCrusher);
...
sealed class Aethersup(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class PendulumFlare(ModuleBase module) : Components.BaitAwayIcon(module, 20f, (uint)IconID.SpreadFlare, (uint)AID.PendulumAOE1, 5.1d)
{
...
sealed class PendulumAOE(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PendulumAOE3, 15f);

sealed class Knout(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.LeftKnout, (uint)AID.RightKnout], new AOEShapeCone(24f, 105f.Degrees()));

sealed class Taphephobia(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.Taphephobia, 6f);

sealed class IntoTheLight(ModuleBase module) : Components.LineStack(module, aidMarker: (uint)AID.IntoTheLightMarker, (uint)AID.IntoTheLight, 5.3d);

sealed class CatONineTails(ModuleBase module) : Components.GenericRotatingAOE(module)
{
...
sealed class FierceBeating(ModuleBase module) : Components.Exaflare(module, 4f)
{
    ...
    public override void Update()
    {
        ...
        if (lastCount != linesCount || currentVersion != lastVersion)
        {
            ...
            Aoes = new AOEInstance[total];
            for (var i = 0; i < futureLen; ++i)
            {
                ref var aoe = ref futureAOEs[i];
                var origin = aoe.Item1;
                var rotation = aoe.Item3;
                Aoes[i] = new(Shape, origin, rotation, aoe.Item2, FutureColor, shapeDistance: Shape.Distance(origin, rotation));
            }
            ...
            for (var i = 0; i < imminentLen; ++i)
            {
                ...
                var origin = aoe.Item1;
                var rotation = aoe.Item3;
                Aoes[futureLen + i] = new(Shape, origin, rotation, aoe.Item2, ImminentColor, shapeDistance: Shape.Distance(origin, rotation));
            }
            var predictions = CollectionsMarshal.AsSpan(_predictions);
            for (var i = 0; i < predictionsCount; ++i)
            {
                ref var aoe = ref predictions[i];
                Aoes[imminentLen + futureLen + i] = aoe;
            }
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        {
            AddLine(ref caster, World.FutureTime(1d));
        }
        if (Lines.Count != 0)
        {
    ...
    public void AddLine(ref Actor caster, DateTime activation)
    {
        ...
        if (NumCasts <= 14)
        {
            ...
            _predictions.Add(new(circle, origin, default, World.FutureTime(3.7d), shapeDistance: circle.Distance(origin, default)));
            ++currentVersion;
        }
        if (NumCasts == 16)
        {
...
{
    public D013PhiliaStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 676u, CFCID = 676u, NameID = 8301u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
public sealed class D013Philia(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
