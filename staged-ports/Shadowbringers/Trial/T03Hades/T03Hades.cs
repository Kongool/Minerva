...
namespace Minerva.Shadowbringers.Trial.T03Hades;
...
sealed class RavenousAssault(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.RavenousAssault, new AOEShapeCircle(5f), centerAtTarget: true, endsOnCastEvent: true, tankbuster: true);

sealed class BadFaith(ModuleBase module)
...
sealed class DarkEruptionSpread(ModuleBase module) : Components.BaitAwayIcon(module, 6f, (uint)IconID.SpreadIcon,
...
sealed class DarkEruptionCircle(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DarkEruptionAOE, new AOEShapeCircle(6f));

sealed class ShadowSpreadCones(ModuleBase module)
...
sealed class BrokenFaith(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BrokenFaith1, new AOEShapeCircle(10f))
{
    public override void OnActorEAnim(Actor actor, uint state)
    {
        if (actor.OID is (uint)OID.BrokenFaithObject && state == 0x00010002u && actor.OID != (uint)OID.Actor1ea1a1)
            Casters.Add(new(Shape, actor.Position, actor.Rotation, World.FutureTime(8.5f), shapeDistance: Shape.Distance(actor.Position, actor.Rotation)));
    }
...
sealed class AncientWaterIII(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.AncientWaterIII, 6f, 2, 8);

sealed class AncientDarkness(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.AncientDarkness, new AOEShapeCircle(5f), centerAtTarget: true);

sealed class AncientAero(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AncientAero, new AOEShapeRect(40f, 4f));

sealed class AncientDarkIV(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.AncientDarkIV);

//Phase 3 Giant Hades
sealed class Titanomachy(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Titanomachy);

//ShadowStream shoots ball down center of arena. Polydegmon is inverse of that.
sealed class AOERectangles(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.ShadowStream, (uint)AID.PolydegmonsPurgation, (uint)AID.PolydegmonsPurgation1], new AOEShapeRect(100f, 8f));

sealed class DualStrike(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.DualStrike, new AOEShapeCircle(5f), centerAtTarget: true, endsOnCastEvent: true);

//EchoOfTheLost range 100 ?-degree cone. Big Hades raises arm and swipes across arena. Angle is an estimate
sealed class EchoOfTheLost(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.EchoOfTheLost, new AOEShapeCone(100f, 45f.Degrees()));

sealed class EchoOfTheLostCone(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.EchoOfTheLost1, new AOEShapeCone(100f, 45f.Degrees()));
...
sealed class HellbornYawp(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.HellbornYawp1, new AOEShapeCone(100f, 30f.Degrees()), centerAtTarget: true, tankbuster: true);
...
sealed class CaptivityBait(ModuleBase module) : Components.BaitAwayIcon(module, 8f, (uint)IconID.CaptivityBait, (uint)AID.CaptivityCast, 4.7d);

//TODO this might need some logic for if you are the one in gaol.
sealed class AetherialGaol(ModuleBase module) : ModuleComponent(module)
{
...
*/
sealed class DoomTowers(ModuleBase module) : Components.GenericTowers(module)
{
...
sealed class NetherBlast(ModuleBase module) : Components.BaitAwayTethers(module, new AOEShapeCircle(6f),
...
sealed class LastPhaseRaidwides(ModuleBase module) : Components.RaidwideCasts(module, [(uint)AID.LifeInCaptivity, (uint)AID.BlackCauldron, (uint)AID.TheDarkDevours, (uint)AID.ChorusOfTheLost]);

[SkipLocalsInit]
sealed class HadesStates : StateMachineBuilder
{
    public HadesStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
            // Phase 1
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 687u, CFCID = 687u, NameID = 8352u, PrimaryActorOID = (uint)OID.HadesBoss, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "wen (ported from BMR)")]
[SkipLocalsInit]
public sealed class Hades(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(100f, 100f), new ArenaBoundsCircle(20f))
{
