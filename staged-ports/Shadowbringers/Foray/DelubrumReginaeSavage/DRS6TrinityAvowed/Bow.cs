...
class FlamesOfBozja(ModuleBase module, bool risky) : Components.GenericAOEs(module, (uint)AID.FlamesOfBozjaAOE)
{
...
class ShimmeringShot(ModuleBase module, double spawnToActivation) : TemperatureAOE(module)
{
    ...
    private void InitArrow(uint oid, int temp)
    {
        ...
        if ((arrow.PosRot.X < Center.X) != _pattern is Pattern.WENormal or Pattern.WEInverted)
        ...
        _activation = World.FutureTime(_spawnToActivation);
    }
}

sealed class FlamesOfBozja1(ModuleBase module) : FlamesOfBozja(module, false);

sealed class QuickMarchBow1(ModuleBase module) : QuickMarch(module)
{
...
sealed class ShimmeringShot1(ModuleBase module) : ShimmeringShot(module, 12.8d)
{
...
sealed class FlamesOfBozja2(ModuleBase module) : FlamesOfBozja(module, true);

sealed class ShimmeringShot2(ModuleBase module) : ShimmeringShot(module, 14.0d)
{
    public override void AddHints(int slot, Actor actor, TextHints hints) { } // no need for hints, quick march handles that
}

sealed class QuickMarchBow2(ModuleBase module) : QuickMarch(module)
{
    private readonly ShimmeringShot2? _shimmering = module.FindComponent<ShimmeringShot2>();

    public override bool DestinationUnsafe(int slot, Actor actor, WPos pos) => !InBounds(pos) || (_shimmering?.ActorUnsafeAt(actor, pos) ?? false);
}
