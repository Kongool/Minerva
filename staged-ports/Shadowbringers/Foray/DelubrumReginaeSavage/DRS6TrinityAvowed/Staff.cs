...
class FreedomOfBozja : TemperatureAOE
{
    ...
    public FreedomOfBozja(ModuleBase module, bool risky) : base(module)
    {
        _risky = risky;
...
sealed class FreedomOfBozja1(ModuleBase module) : FreedomOfBozja(module, false);

sealed class QuickMarchStaff1(ModuleBase module) : QuickMarch(module)
{
    private readonly FreedomOfBozja1? _freedom = module.FindComponent<FreedomOfBozja1>();

    public override bool DestinationUnsafe(int slot, Actor actor, WPos pos) => !InBounds(pos) || (_freedom?.ActorUnsafeAt(actor, pos) ?? false);
}

sealed class FreedomOfBozja2(ModuleBase module) : FreedomOfBozja(module, true);
