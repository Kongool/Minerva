...
sealed class QueensShotUnseen(ModuleBase module) : Components.CastWeakpoint(module, (uint)AID.QueensShotUnseen, 60f, default, (uint)SID.BackUnseen, (uint)SID.LeftUnseen, (uint)SID.RightUnseen);
...
sealed class FieryPortent(ModuleBase module) : Components.CastHint(module, (uint)AID.FieryPortent, "Stand still!");
sealed class IcyPortent(ModuleBase module) : Components.CastHint(module, (uint)AID.IcyPortent, "Move!");
sealed class PawnOff(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PawnOffReal, 20);
sealed class Fracture(ModuleBase module) : Components.CastCounter(module, (uint)AID.Fracture); // TODO: consider showing reflect hints

[ModuleInfo(CFCID = 761u, NameID = 9838u, PrimaryActorOID = (uint)OID.Knight, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
