...
namespace Minerva.Shadowbringers.Foray.CriticalEngagement.CE32RiseOfTheRobots;
...
sealed class ArenaChange(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnCastFinished(Actor caster, ActorCastInfo spell)
    {
        ...
        {
            _aoe = [new(donut, Center, default, Module.CastFinishAt(spell, 4.3d))];
        }
    }
...
sealed class Train(ModuleBase module) : Components.GenericRotatingAOE(module)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        ...
        _activation = World.FutureTime(8d);
        InitIfReady();
    }
...
sealed class OrderTowers(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        if (divisor < 6u)
        {
            ...
            for (var i = 0; i < len; ++i)
            {
                ...
                AOEs[p.Item1] = [new(new AOEShapeCustom(Center, shapes, invertForbiddenZone: !outsideSafe), Center, default, Module.CastFinishAt(spell), !outsideSafe ? Colors.SafeFromAOE : default)];
            }
        }
    }
...
sealed class OrderForcedMarch(ModuleBase module) : Components.StatusDrivenForcedMarch(module, 3f, (uint)SID.ForwardMarch, (uint)SID.AboutFace, (uint)SID.LeftFace, (uint)SID.RightFace)
{
...
sealed class Incinerate(ModuleBase module) : Components.RaidwideCasts(module, [(uint)AID.Incinerate1, (uint)AID.Incinerate2]);

sealed class CE32RiseOfTheRobotsStates : StateMachineBuilder
{
    public CE32RiseOfTheRobotsStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, GroupID = 735u, CFCID = 735u, NameID = 14u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class CE32RiseOfTheRobots(WorldState ws, Actor primary) : ModuleBase(ws, primary, ArenaCenter, startingArena)
{
