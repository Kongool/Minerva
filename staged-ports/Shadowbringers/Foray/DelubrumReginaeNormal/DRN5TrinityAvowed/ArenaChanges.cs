...
sealed class ArenaChanges(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        if (spell.Action.ID is var id && id == (uint)AID.GloryOfBozja && Bounds.Radius > 25f)
        {
        ...
        else if (id == (uint)AID.FlamesOfBozjaAOE)
        {
            var loc = spell.LocXZ;
            var rot = spell.Rotation;
            _aoe = [new(rect, loc, rot, Module.CastFinishAt(spell), shapeDistance: rect.Distance(loc, rot))];
        }
    }
