...
sealed class EnergyCompression(ModuleBase module) : Components.GenericTowers(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        {
            Towers.Add(new(actor.Position.Quantized(), 5f, activation: World.FutureTime(9.9d)));
        }
    }
        ...
        for (var i = 0; i < len; ++i)
        {
            ...
            for (var j = countT; j >= 0; --j)
            {
                ...
                if (t.target.Position.AlmostEqual(tower.Position, 1f))
                {
                    ...
                    tower.Activation = World.FutureTime(10.2d);
                    _tethers.RemoveAt(j);
