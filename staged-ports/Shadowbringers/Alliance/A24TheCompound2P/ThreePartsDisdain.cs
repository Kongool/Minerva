...
sealed class ThreePartsDisdainStack(ModuleBase module) : Components.GenericStackSpread(module)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID == (uint)AID.ThreePartsDisdain1 && World.Actors.Find(spell.TargetID) is Actor t)
        {
...
sealed class ThreePartsDisdainKnockback(ModuleBase module) : Components.GenericKnockback(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (knockback.Length != 0)
        {
            ...
            {
                hints.AddForbiddenZone(new SDKnockbackInAABBSquareAwayFromOrigin(Center, loc, distTarget, 29f), act);
            }
        }
    }
}
