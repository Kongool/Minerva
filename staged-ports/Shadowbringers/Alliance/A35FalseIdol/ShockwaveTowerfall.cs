...
sealed class Towerfall(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        {
            _aoes.Add(new(rect, actor.Position.Quantized(), actor.Rotation, World.FutureTime(30.2d), risky: false));
        }
    }
...
sealed class ShockwaveKB(ModuleBase module) : Components.GenericKnockback(module, (uint)AID.ShockwaveKB)
{
    private readonly List<Knockback> _kbs = [];
    private readonly ArenaChanges _arena = module.FindComponent<ArenaChanges>()!;
    private readonly List<SafeWall> walls = [];
