...
sealed class DiffuseEnergy(ModuleBase module) : Components.GenericRotatingAOE(module)
{
    private static readonly AOEShapeCone cone = new(12f, 60f.Degrees());
    private readonly List<(Actor source, Actor target)> tethers = [];
    private readonly List<(Actor source, WDir direction)> sources = [];

    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        if (source.OID != default && tether.ID == (uint)TetherID.ChildsPlay && World.Actors.Find(tether.Target) is Actor t)
        {
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        ...
        if (increment != default)
        {
            ...
            Sequences.Add(new(cone, actor.Position.Quantized(), actor.Rotation, increment, World.FutureTime(5d), 2.7d, isTethered ? 2 : 6, actorID: actor.InstanceID));
            ...
            for (var i = 0; i < countS; ++i)
            {
                ...
                {
                    Sequences.Add(new(cone, (actor.Position + s.direction).Quantized(), actor.Rotation + 2f * increment, increment, World.FutureTime(13.3d), 2.7d, 3, actorID: actor.InstanceID));
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        {
            AdvanceSequence(caster.Position, spell.Rotation, caster.InstanceID, World.CurrentTime);
        }
    }
}
