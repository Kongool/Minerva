...
sealed class ArenaChanges(ModuleBase module) : Components.GenericAOEs(module)
{
        ...
        if (index is >= 0x1C and <= 0x38)
        {
            switch (index)
            {
                ...
                default:
                    ...
                    _wipe?.UpdateAOE();
                    var center = Center;
                    Bounds = new ArenaBoundsCustom(isDefaultArena ? [new Square(center, 20f)] : [new Square(center, 24.5f)], [.. differenceShapes]);
                    break;
            }
        }
    }
    ...
    {
        if (_aoe.Length != 0 && activation < World.CurrentTime)
        {
            _aoe = [];
            var center = Center;
            Bounds = new ArenaBoundsCustom([new Square(center, 20f)], [innerSquare]);
            isDefaultArena = true;
        }
    }
}
