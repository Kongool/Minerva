...
using Minerva.Shadowbringers.Quest.SorrowOfWerlyt.SleepNowInSapphire.P1GuidanceSystem;

namespace Minerva.Shadowbringers.Quest.SorrowOfWerlyt.SleepNowInSapphire.P2SapphireWeapon;
...
class MagitekRay(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MagitekRay, new AOEShapeRect(100f, 3f));
class ServantRoar(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ServantRoar, new AOEShapeRect(100f, 4f));
class MagitekSpread(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MagitekSpread, new AOEShapeCone(43f, 120f.Degrees()));

class TailSwing(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TailSwing, 46f)
{
...
class OptimizedJudgment(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.OptimizedJudgment, new AOEShapeDonut(22f, 60f));
class SapphireRay(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SapphireRay, new AOEShapeRect(120f, 20f));

abstract class Siderays(ModuleBase module, uint aid) : Components.SimpleAOEs(module, aid, new AOEShapeCone(128f, 45f.Degrees()));
class SideraysLeft(ModuleBase module) : Siderays(module, (uint)AID.SideraysLeft);
class SideraysRight(ModuleBase module) : Siderays(module, (uint)AID.SideraysRight);

class FloodRay(ModuleBase module) : Components.CastHint(module, (uint)AID.FloodRay, "Enrage", true);
class SelfDestruct(ModuleBase module) : Components.CastHint(module, (uint)AID.SelfDestructVisual, "Turrets are enraging!", true);
class OptimizedUltima(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.OptimizedUltima);
class Swiftbreach(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Swiftbreach);
class PlasmaShot(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.PlasmaShot);
class PlasmaCannon(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PlasmaCannon, 40f);

class TheSapphireWeaponStates : StateMachineBuilder
{
    public TheSapphireWeaponStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 69431u, NameID = 9458u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")]
