...
namespace Minerva.Global.MaskedCarnivale.Stage22.Act2;
...
sealed class Sap(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.Sap1, (uint)AID.Sap2], 8f);
sealed class ScaldingScolding(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ScaldingScolding, new AOEShapeCone(11.75f, 60f.Degrees()));
sealed class Flashthoom(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Flashthoom, 7.2f);
sealed class Ignition(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Ignition, "Wipe if Grenade is not killed yet, otherwise Raidwide");
...
{
    public Stage22Act2States(ModuleBase module) : base(module)
    {
        TrivialPhase()
            .ActivateOnEnter<Sap>()
...
[ModuleInfo(Group = ModuleGroup.MaskedCarnivale, CFCID = 632u, NameID = 8123u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
public sealed class Stage22Act2 : ModuleBase
{
