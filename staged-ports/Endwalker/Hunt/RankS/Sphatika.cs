...
namespace Minerva.Endwalker.Hunt.RankS.Sphatika;
...
class Gnaw(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.Gnaw);
class Caterwaul(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Caterwaul);

class Stance(ModuleBase module) : Components.GenericAOEs(module)
{
...
{
    public SphatikaStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 0u, NameID = 10618u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")]
