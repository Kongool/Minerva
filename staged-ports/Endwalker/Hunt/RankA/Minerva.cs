...
namespace Minerva.Endwalker.Hunt.RankA.Minerva;
...
class BallisticMissile(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        switch (spell.Action.ID)
        {
            case (uint)AID.BallisticMissileCircleWarning:
                ...
                _activeTarget = World.Actors.Find(spell.TargetID);
                break;
...
class Hyperflame(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Hyperflame, new AOEShapeCone(60f, 30f.Degrees()));
class SonicAmplifier(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.SonicAmplifier);
class HammerKnuckles(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.HammerKnuckles);

class MinervaStates : StateMachineBuilder
{
    public MinervaStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 0u, NameID = 10627u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")]
