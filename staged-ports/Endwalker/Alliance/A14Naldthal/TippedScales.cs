...
class TippedScales(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.TippedScalesAOE)
{
    private readonly List<(Actor, int weight)> weightsByActor = [];
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        ...
        for (var i = 0; i < count; ++i)
        {
            ...
            {
                if (a.Item1.Position.InRect(Center, eastBoundary, 50f))
                    eastWeight += a.weight;
                else
                    westWeight += a.weight;
            }
        ...
        if (Math.Abs(westWeight - eastWeight) < 3)
            ...
            return new AOEInstance[1] { new(rect, Center, westWeight > eastWeight ? a90 : -a90, activation, Colors.SafeFromAOE, false) };
    }
    ...
    public override void AddGlobalHints(GlobalHints hints)
    {
        ...
        for (var i = 0; i < count; ++i)
        {
            ...
            {
                if (actor.Item1.Position.InRect(Center, eastBoundary, 50f))
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        hints.AddForbiddenZone(new SDInvertedRect(Center + (aoe.Rotation == a90 ? dir : -dir), aoe.Rotation == a90 ? dir : -dir, 30f, default, 30f), aoe.Activation);
    }
}
