...
namespace Minerva.Endwalker.Alliance.A12Rhalgr;

class DestructiveBolt(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.DestructiveBoltAOE, 3f, tankbuster: true);
class StrikingMeteor(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.StrikingMeteor, 6f);
class BronzeLightning(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BronzeLightning, new AOEShapeCone(50f, 22.5f.Degrees()), 4);

[ModuleInfo(CFCID = 866u, NameID = 11273u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
public class A12Rhalgr(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
