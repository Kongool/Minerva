...
sealed class ClimbingShot(ModuleBase module) : Components.GenericKnockback(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (_kb.Length != 0)
        {
            ...
            if (!IsImmune(slot, act))
            {
                ...
                hints.AddForbiddenZone(new SDKnockbackInCircleAwayFromOriginPlusAOECircles(Center, kb.Origin, 20f, 29f, origins, 6f, len), act);
            }
        }
    }
}
