...
class PortalsAOE(ModuleBase module, uint aid, uint movedOID, double activationDelay, AOEShape shape) : Components.GenericAOEs(module, aid)
{
    ...
    public override void OnActorEAnim(Actor actor, uint state)
    {
        ...
        if (movedActor != null)
            _aoes.Add(new(_shape, dest.Value, movedActor.Rotation, World.FutureTime(_activationDelay)));
    }
}

abstract class PortalsBurn(ModuleBase module, uint aid, uint oid) : PortalsAOE(module, aid, oid, 11.6d, new AOEShapeCircle(12f));
sealed class NPortalsBurn(ModuleBase module) : PortalsBurn(module, (uint)AID.NBurn, (uint)OID.NBallOfFire);
sealed class SPortalsBurn(ModuleBase module) : PortalsBurn(module, (uint)AID.SBurn, (uint)OID.SBallOfFire);

abstract class PortalsMirror(ModuleBase module, uint aid, uint oid) : PortalsAOE(module, aid, oid, 11.7d, new AOEShapeRect(60f, 5f, 60f));
sealed class NPortalsMirror(ModuleBase module) : PortalsMirror(module, (uint)AID.NBlazingBenifice, (uint)OID.NArcaneFont);
sealed class SPortalsMirror(ModuleBase module) : PortalsMirror(module, (uint)AID.SBlazingBenifice, (uint)OID.SArcaneFont);

sealed class PortalsWave(ModuleBase module) : ModuleComponent(module)
{
    public bool Done;
    ...
    public override void OnActorEAnim(Actor actor, uint state)
    {
        ...
        var dest = Portals.DestinationForEAnim(actor, state);
        if (dest == null || !InBounds(dest.Value))
            return;
