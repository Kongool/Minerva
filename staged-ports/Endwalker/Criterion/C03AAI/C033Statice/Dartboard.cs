...
class Dartboard(ModuleBase module) : ModuleComponent(module)
{
    ...
    private void DrawSegmentsOfColor(Color color, uint zoneColor)
    {
        ...
        for (var i = 0; i < 4; ++i)
        {
            Arena.ZoneCone(Center, 0, 12, dirOut + 30.Degrees(), 15.Degrees(), zoneColor);
            Arena.ZoneCone(Center, 12, Bounds.Radius, dirOut, 15.Degrees(), zoneColor);
            dirOut += 90.Degrees();
        }
    }
}
