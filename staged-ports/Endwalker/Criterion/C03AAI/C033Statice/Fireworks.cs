...
class Fireworks(ModuleBase module) : Components.UniformStackSpread(module, 3f, 20f, 2, 2)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        switch (iconID)
        {
            case (uint)IconID.FireworksSpread:
                AddSpread(actor, World.FutureTime(9.1d));
                break;
            case (uint)IconID.FireworksEnumeration:
                AddStack(actor, World.FutureTime(8.2d));
                break;
        }
    }
...
class BurningChains(ModuleBase module) : ModuleComponent(module)
{
...
class FireSpread(ModuleBase module) : Components.GenericAOEs(module)
{
        ...
        if (spell.Action.ID is (uint)AID.NFireSpreadFirst or (uint)AID.NFireSpreadRest or (uint)AID.SFireSpreadFirst or (uint)AID.SFireSpreadRest)
        {
            ...
            if (index >= 0)
            {
                ...
                if (--s.RemainingExplosions > 0)
                {
                    ...
                    s.NextActivation = World.FutureTime(1.1d);
                }
                else
                {
...
class Fireworks1Hints(ModuleBase module) : ModuleComponent(module)
{
    ...
    private void AddSafeSpot(List<WPos> list, Angle angle) => list.Add(Center + 19f * angle.ToDirection());
}

class Fireworks2Hints(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        if (_fireworks?.Spreads.Count > 0)
        {
            foreach (var dir in SafeSpots(pcSlot, pc))
                Arena.ZoneCircleOutline(Center + 19f * dir.ToDirection(), 1f, Colors.Safe);
        }
        else if (_relNorth != null)
        {
            // show rel north before assignments are done
            Arena.ZoneCircleOutline(Center + 19f * _relNorth.Value.ToDirection(), 1f, Colors.Enemy);
        }
    }
