...
class BlackHole(ModuleBase module) : ModuleComponent(module)
{
    public Actor? Baiter;
    ...
    {
        if (Voidzone != null)
            Arena.ZoneCircle(Voidzone.Position, _growthStart == default ? _startingRadius : Math.Min(_maxRadius, _startingRadius + _growthPerSecond * (float)(World.CurrentTime - _growthStart).TotalSeconds), Colors.AOE);
    }
    ...
    public override void OnActorEAnim(Actor actor, uint state)
    {
        if (actor == Voidzone)
        {
            switch (state)
            {
                // 00010002 - appear
                case 0x00100008:
                    _growthStart = World.CurrentTime;
                    break;
                case 0x00040020:
                    Voidzone = null;
                    break;
...
class FracturedEventide(ModuleBase module) : Components.GenericAOEs(module)
{
