...
sealed class MeteorImpactProximity(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MeteorImpactProximity, 10f); // TODO: verify falloff

sealed class MeteorImpactCharge(ModuleBase module) : Components.GenericAOEs(module)
{
    struct PlayerState
    {
    ...
    private readonly List<PolygonCustom> polygons = [];
    ...
    public override void DrawArenaBackground(int pcSlot, Actor pc)
    {
        if (SourceIfActive(pcSlot) is var source && source != null)
        {
            if (_aoe.Length == 0)
            {
                ...
                var center = Center;
                var error = Bounds.MaxApproxError;
...
sealed class MeteorImpactExplosion(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MeteorImpactExplosion, 10f);
