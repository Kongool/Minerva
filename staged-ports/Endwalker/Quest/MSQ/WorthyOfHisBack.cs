...
namespace Minerva.Endwalker.Quest.MSQ.WorthyOfHisBack;
...
class ArenaChange(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        {
            _aoe = [new(donut, Center, default, Module.CastFinishAt(spell, 1.3d))];
        }
    }
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        {
            _aoe = [];
            Bounds = WorthyOfHisBack.DefaultBounds;
        }
    }
}

class Kleos(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Kleos);
class TrueHolyRaidwide(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.TrueHoly);
class TrueAeroIV(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.TrueAeroIV);

class ParhelionCone(ModuleBase module) : Components.GenericRotatingAOE(module)
{
    ...
    private void InitIfReady()
    {
        if (_rotation.Count == 3 && _increment != default)
        {
            for (var i = 0; i < 3; ++i)
            {
                Sequences.Add(new(_shape, Center.Quantized(), _rotation[i], _increment, _activation, 2.6d, 9));
            }
            _rotation.Clear();
            _increment = default;
        }
...
class ParhelionDonut(ModuleBase module) : Components.ConcentricAOEs(module, [new AOEShapeCircle(10f), new AOEShapeDonut(10f, 15f), new AOEShapeDonut(15f, 20f)])
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
    ...
    public override void OnCastFinished(Actor caster, ActorCastInfo spell)
    {
        ...
        AdvanceSequence(order, spell.LocXZ, World.FutureTime(3d));
    }
}

class EpeaPteroenta(ModuleBase module) : Components.GenericAOEs(module)
{
...
class CrepuscularRay(ModuleBase module) : Components.ChargeAOEs(module, (uint)AID.CrepuscularRay, 4f);

sealed class CircumzenithalArc(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.CircumzenithalArcFirst, (uint)AID.CircumzenithalArcSecond], new AOEShapeCone(40f, 90f.Degrees()))
{
...
class CircleOfBrilliance(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.CircleOfBrilliance, 5f);

class Enomotos(ModuleBase module) : Components.Exaflare(module, 6f, (uint)AID.EnomotosFirst)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
...
class Windage(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Windage, 5f);
class AfflatusAzem(ModuleBase module) : Components.StandardChasingAOEs(module, 5f, (uint)AID.AfflatusAzemFirst, (uint)AID.AfflatusAzemRest, 5f, 2.1f, 5, true, (uint)IconID.AfflatusAzem);
class WindageSlow(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WindageSlow, 5f);
class TrueHoly(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.TrueHoly, 20f)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (Casters.Count != 0)
...
class TrueStoneIV(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TrueStoneIV, 10f, maxCasts: 7);
class EnomotosSmall(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.EnomotosSmall, 4f);

class Adds(ModuleBase module) : Components.AddsMulti(module, [(uint)OID.Thelema, (uint)OID.ThelemaAgape], 1);

public class WorthyOfHisBackStates : StateMachineBuilder
{
    public WorthyOfHisBackStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 69968u, NameID = 10586u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
public class WorthyOfHisBack(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
