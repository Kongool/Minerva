...
namespace Minerva.Endwalker.Dungeon.D09AlzadaalsLegacy.D093Kapikulu;
...
sealed class BillowingBoltsArenaChange(ModuleBase module) : ModuleComponent(module)
{
    public override void OnCastFinished(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID == (uint)AID.BillowingBolts && Bounds.Radius > 24f)
        {
...
sealed class ManaExplosion(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        ...
        {
            _target = World.Actors.Find(tether.Target)!;
            _activation = World.FutureTime(11.5d); // some variation here, have seen upto almost 12.3s
        }
    }
...
sealed class BastingBlade(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BastingBlade, new AOEShapeRect(60f, 7.5f));

sealed class SpikeTraps(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class BorderChange(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BorderChange, new AOEShapeRect(5f, 20f));
sealed class MagnitudeOpus(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.MagnitudeOpus, 6f, 4, 4);
sealed class RotaryGale(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.RotaryGale, 5f);
sealed class CrewelSlice(ModuleBase module) : Components.SingleTargetDelayableCast(module, (uint)AID.CrewelSlice);
sealed class BillowingBolts(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.BillowingBolts);

sealed class SpinningHints(ModuleBase module) : Components.Spinning(module, (uint)AID.SpinOut)
{
...
{
    public D093KapikuluStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 844u, CFCID = 844u, NameID = 11238u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS) (ported from BMR)")]
public sealed class D093Kapikulu(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(110f, -68f), new ArenaBoundsRect(19.5f, 24.5f));
