...
namespace Minerva.Endwalker.Dungeon.D09AlzadaalsLegacy.D092ArmoredChariot;
...
sealed class Voidzone(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        {
            _aoe = [new(circle, Module.Center, default, Module.CastFinishAt(spell, 0.8d))];
        }
    }
}

sealed class AssaultCannon(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        var activationFirst = World.FutureTime(7.1d);
        var activationSecond = World.FutureTime(15d);
    ...
    {
        var activation = World.FutureTime(6.9d);
    ...
    {
        for (var i = 0; i < 2; ++i)
            _aoesCones.Add(new(cone, Module.Center.Quantized(), angles[i], activation));
    }
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        if (status.ID == (uint)SID.CannonOrder)
        {
            ...
            _aoesRects.Add(new(rectShort, actor.Position.Quantized(), actor.Rotation, World.FutureTime(activation)));
        }
    }
...
sealed class DiffusionRay(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.DiffusionRay);
sealed class RailCannon(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.RailCannon);
sealed class GravitonCannon(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.GravitonCannon, 6f);

sealed class D092ArmoredChariotStates : StateMachineBuilder
{
    public D092ArmoredChariotStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 844u, NameID = 11239u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS) (ported from BMR)")]
public sealed class D092ArmoredChariot(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(default, -182f), new ArenaBoundsSquare(19.5f));
