...
namespace Minerva.Endwalker.Dungeon.D10FellCourtOfTroia.D103Scarmiglione;
...
{
    public ArenaChanges(ModuleBase module) : base(module)
    {
    ...
    public readonly List<Rectangle> PendingSafeWalls = [];
    ...
    {
        if (spell.Action.ID == (uint)AID.RottenRampageSpread && Bounds.Radius > 21f)
        {
            var center = Center;
    ...
    public override void Update()
    {
        ...
        if (_aoe.Length == 0 && defaultArena && !InBounds(player))
        {
            _aoe = [new(donut, Center)];
            ArenaBoundsCustom arena = new(D103Scarmiglione.GetStartingCircle(), [.. SafeWalls]);
            Bounds = arena;
            outOfBounds = true;
        }
        else if (outOfBounds && (player - Center).LengthSq() < 399f)
        {
            _aoe = [];
            ArenaBoundsCustom arena = new(GetDefaultCircle(), [.. SafeWalls]);
            Bounds = arena;
...
sealed class VacuumWave(ModuleBase module) : Components.GenericKnockback(module)
{
...
sealed class VacuumWaveHint(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class VoidVortex1(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.VoidVortex1, 6f, 4, 4);
sealed class VoidVortex2(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.VoidVortex2, 6f, 4, 4);
sealed class BlightedBedevilment(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BlightedBedevilment, 9f);
sealed class BlightedBladework(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BlightedBladework, 25f, riskyWithSecondsLeft: 8d);
sealed class Nox(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Nox, 10f);
sealed class RottenRampageAOE(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.RottenRampage, 6f);
sealed class RottenRampageSpread(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.RottenRampageSpread, 6f)
{
...
sealed class BlightedSweep(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BlightedSweep, new AOEShapeCone(40f, 90f.Degrees()));
sealed class CursedEcho(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.CursedEcho);
sealed class VoidGravity(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.VoidGravity, 6f);
sealed class Firedamp(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.Firedamp, 5f, tankbuster: true);

sealed class CorruptorsPitch(ModuleBase module) : Components.RaidwideCastDelay(module, (uint)AID.CorruptorsPitchVisual, (uint)AID.CorruptorsPitch3, 8.1d)
{
...
{
    public D103ScarmiglioneStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 869u, CFCID = 869u, NameID = 11372u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS) (ported from BMR)")]
public sealed class D103Scarmiglione : ModuleBase
{
