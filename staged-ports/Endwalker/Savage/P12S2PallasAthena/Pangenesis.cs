...
class Pangenesis(ModuleBase module) : Components.GenericTowers(module)
{
        ...
        switch (status.ID)
        {
            ...
            case (uint)SID.AstralTilt:
                if (Raid.FindSlot(actor.InstanceID) is var slotColor && slotColor >= 0)
                {
                    ...
                    var isLeft = actor.PosRot.X < Center.X;
                    for (var i = 0; i < Towers.Count; ++i)
                    {
                        ref var tower = ref Towers.Ref(i);
                        if ((tower.Position.X < Center.X) == isLeft)  // don't care about towers on other side, keep forbidden
                        {
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID is (uint)AID.UmbralAdvent or (uint)AID.AstralAdvent)
        {
            ...
            var isLeft = caster.PosRot.X < Center.X;
        ...
        if (spell.Action.ID is (uint)AID.UmbralAdvent or (uint)AID.AstralAdvent)
        {
            ...
            foreach (var t in spell.Targets)
            {
                ...
                if (slot >= 0)
                {
                    ...
                    _states[slot].AssignedSide = caster.PosRot.X < Center.X ? -1 : 1; // ensure correct side is assigned
...
class FactorIn(ModuleBase module) : Components.GenericBaitAway(module, (uint)AID.FactorIn, centerAtTarget: true)
{
    ...
    {
        if (tether.ID == (uint)TetherID.FactorIn && World.Actors.Find(tether.Target) is var target && target != null)
        {
