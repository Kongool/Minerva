...
class WreathOfThorns4(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void AddGlobalHints(GlobalHints hints)
    {
        if (_darkOrder != null && _activeTethers > 0)
        {
            ...
            hints.Add($"Dark order: {string.Join(" -> ", _darkOrder.Skip(skip).Select(src => World.Actors.Find(src.Tether.Target)?.Name ?? "???"))}");
        }
    }
    ...
    {
        var dir = Angle.FromDirection(pos - Center) - angle;
        return Center + radius * dir.ToDirection();
    }
