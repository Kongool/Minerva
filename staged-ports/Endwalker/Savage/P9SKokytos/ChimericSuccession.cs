...
class ChimericSuccession(ModuleBase module) : Components.UniformStackSpread(module, 6f, 20f, 4)
{
    public int NumCasts;
    ...
    private void InitBaits()
    {
        ...
        if (target != null)
            AddSpread(target, World.FutureTime(NumCasts == 0 ? 10.1f : 3));
    }
}

// TODO: think of a way to show baits before cast start to help aiming outside...
abstract class SwingingKick(ModuleBase module, uint aid) : Components.SimpleAOEs(module, aid, new AOEShapeCone(40f, 90f.Degrees()));
class SwingingKickFront(ModuleBase module) : SwingingKick(module, (uint)AID.SwingingKickFront);
class SwingingKickRear(ModuleBase module) : SwingingKick(module, (uint)AID.SwingingKickRear);
