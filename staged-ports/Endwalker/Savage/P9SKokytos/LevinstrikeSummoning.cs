...
class LevinstrikeSummoningIcemeld(ModuleBase module) : Components.GenericBaitAway(module, centerAtTarget: true)
{
...
// TODO: or is it a spread?.. one thing i like about bait-away better here is that it better distinguishes bait vs avoid
class LevinstrikeSummoningFiremeld(ModuleBase module) : Components.GenericBaitAway(module, (uint)AID.Firemeld, centerAtTarget: true)
{
...
class LevinstrikeSummoningShock(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        ...
        if (isBall)
        {
            var dir = (actor.Position - Center).Normalized();
            _explodeOrder[order] = (Center - 16f * dir).Quantized();
            _firstExplosion = World.FutureTime(12.7d);
        }
        else
        {
