...
class DualspellFire(ModuleBase module) : Components.GenericStackSpread(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        if (_active && radius != 0)
        {
            ...
            foreach (var p in Raid.WithoutSlot(true, true, true).Where(p => p.Class.IsSupport()))
                Stacks.Add(new(p, radius, 2, 2, World.FutureTime(4.5f)));
        }
    }
}

class DualspellLightning(ModuleBase module) : Components.GenericBaitAway(module)
{
    private bool _active;
...
class DualspellIce(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    private void SetMechanic(Mechanic mechanic)
    {
        ...
        _aoe = [new(new AOEShapeDonut(mechanic == Mechanic.In ? 8f : 14, 40f), Module.PrimaryActor.Position, default, World.FutureTime(4.5d))];
    }
}
