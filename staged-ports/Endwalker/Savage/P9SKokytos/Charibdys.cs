...
class Charibdys(ModuleBase module) : Components.VoidzoneAtCastTarget(module, 6, (uint)AID.CharybdisAOE, m => m.Enemies((uint)OID.Charybdis).Where(v => v.EventState != 7), 0.6f);

class Comet(ModuleBase module) : Components.Adds(module, (uint)OID.Comet)
{
...
class CometImpact(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.CometImpact, 10); // TODO: verify falloff
class CometBurst(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.CometBurstLong, 10);

class BeastlyBile(ModuleBase module) : Components.UniformStackSpread(module, 6, 0, 4)
{
    public int NumCasts;
    private readonly Comet? _comet = module.FindComponent<Comet>();
    private DateTime _activation = module.World.FutureTime(15); // assuming component is activated after proximity
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        {
            ++NumCasts;
            _activation = World.FutureTime(6);
...
class Thunderbolt(ModuleBase module) : Components.GenericBaitAway(module, (uint)AID.ThunderboltAOE)
{
    private readonly Comet? _comet = module.FindComponent<Comet>();
...
class EclipticMeteor(ModuleBase module) : Components.GenericLineOfSightAOE(module, (uint)AID.EclipticMeteorAOE, 60, safeInsideHitbox: false)
{
