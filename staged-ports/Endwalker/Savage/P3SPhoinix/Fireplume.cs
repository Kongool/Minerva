...
class Fireplume(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        switch (spell.Action.ID)
        {
            ...
            case (uint)AID.ExperimentalGloryplumeMultiAOE:
                if (_aoes.Count == 0)
                {
                    var startingDirection = Angle.FromDirection(caster.Position - Center);
                    for (var i = 0; i < 4; ++i)
                    {
                        var activation = Module.CastFinishAt(spell, i * 1);
                        var dir = 15 * (startingDirection - i * 45.Degrees()).ToDirection();
                        _aoes.Add(new(circleSmall, Center + dir, default, activation));
                        _aoes.Add(new(circleSmall, Center - dir, default, activation));
                    }
                    _aoes.Add(new(circleSmall, Center, default, Module.CastFinishAt(spell, 4)));
                }
                break;
        }
    }
