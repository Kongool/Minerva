...
class BirdTether(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void Update()
    {
        ...
        for (var i = 0; i < Math.Min(birdsLarge.Count, _chains.Length); ++i)
        {
            ...
            {
                _chains[i].Item1 = World.Actors.Find(bird.Tether.Target); // first target found
            }
            if (_chains[i].Item2 == null && (_chains[i].Item1?.Tether.Target ?? 0) != 0)
            {
                _chains[i].Item2 = World.Actors.Find(_chains[i].Item1!.Tether.Target); // second target found
            }
        ...
        foreach ((var bird, (var p1, var p2, var numCharges)) in birdsLarge.Zip(_chains))
        {
            ...
            if (p1 == pc)
            {
                ...
                if (numCharges == 0)
                {
                    ...
                    if (bird.Position != Center)
                    {
                        var safespot = bird.Position + (Center - bird.Position).Normalized() * _chargeMinSafeDistance;
