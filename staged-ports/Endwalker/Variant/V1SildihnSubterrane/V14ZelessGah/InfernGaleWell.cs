...
sealed class InfernGale(ModuleBase module) : Components.GenericKnockback(module)
{
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        ...
        {
            _kb = [new(actor.Position.Quantized(), 20f, World.FutureTime(5.6d))];
        }
    }
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (_kb.Length != 0)
        {
            ...
            if (!IsImmune(slot, act))
            {
                ...
                hints.AddForbiddenZone(new SDKnockbackInAABBRectAwayFromOrigin(Center, kb.Origin, 20f, 14f, 19f), act);
            }
        }
    }
}

sealed class InfernWellPull(ModuleBase module) : Components.GenericKnockback(module)
{
    private Knockback[] _kb = [];
    private bool kbInit;
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        ...
        {
            _kb = [new(actor.Position.Quantized(), 15f, World.FutureTime(7.6d), kind: Kind.TowardsOrigin, minDistance: 1f)]; // min distance: actor (0x233C helper) + player hitbox radius
            kbInit = true;
        }
    }
...
sealed class InfernWellAOE(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        if (status.ID == (uint)SID.MechanicStatus && status.Extra == 0x1CB)
        {
            AOE = [new(circle, actor.Position.Quantized(), default, World.FutureTime(9.1d))];
            AOEInit = true;
        }
    }
