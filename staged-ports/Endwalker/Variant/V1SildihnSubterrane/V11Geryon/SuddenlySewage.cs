...
sealed class SuddenlySewage(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        if (state == 0x00020001u)
        {
            ...
            if (pos != default)
            {
                _aoes.Add(new(square, (pos + new WDir(default, -10f)).Quantized(), Angle.AnglesCardinals[1], World.FutureTime(8d)));
            }
        }
    }
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        hints.AddForbiddenZone(new SDInvertedCircle(Center, 6f), aoe.Activation.AddSeconds(2d));
    }
}
