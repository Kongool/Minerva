...
class Ex3TitanAI(ModuleBase module) : ModuleComponent(module)
{
        ...
        foreach (var e in hints.PotentialTargets)
        {
            ...
            switch ((OID)e.Actor.OID)
            {
                ...
                case OID.TitansHeart:
                    ...
                    e.DesiredPosition = Center - new WDir(0, Bounds.Radius - 6);
                ...
                case OID.GraniteGaoler:
                    e.Priority = 2;
                    e.DesiredPosition = Center + (Bounds.Radius - 4) * 30.Degrees().ToDirection(); // move them away from boss, healer gaol spots and upheaval knockback spots
                    e.ShouldBeTanked = Module.PrimaryActor.TargetID != actor.InstanceID && actor.Role == Role.Tank;
                    break;
                case OID.BombBoulder:
                    e.Priority = KillNextBomb && e.Actor.Position.AlmostEqual(Center, 1) ? 3 : 0; // kill center bomb when needed
        ...
        if (!haveGaolers && !KillNextBomb && actor.InstanceID != Module.PrimaryActor.TargetID && hints.ForbiddenZones.Count == 0)
        {
            if (_rockThrow != null && _rockThrow.PendingFetters[slot])
            {
                var pos = actor.Role == Role.Healer
                    ? Center + Bounds.Radius * (-30).Degrees().ToDirection() // healers should go to the back; 30 degrees will be safe if landslide is baited straight to south (which it should, since it will follow upheaval)
    ...
    private int TankVulnStacks() => World.Actors.Find(Module.PrimaryActor.TargetID)?.FindStatus(SID.PhysicalVulnerabilityUp)?.Extra ?? 0;
}
