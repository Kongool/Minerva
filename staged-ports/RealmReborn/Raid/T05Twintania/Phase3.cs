...
class P3Divebomb(ModuleBase module) : Components.GenericAOEs(module)
{
    public WPos? Target;
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        switch (spell.Action.ID)
        {
            case (uint)AID.DivebombMarker:
                Target = World.Actors.Find(spell.MainTargetID)?.Position;
                HitAt = World.FutureTime(1.7d);
                break;
...
class P3Adds(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        var killHygieia = asclepiusVuln == null || (asclepiusVuln.Value.ExpireAt - World.CurrentTime).TotalSeconds < 10;
...
{
    private readonly DateTime _activation = module.World.FutureTime(6.7d);
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (actor.Role == Role.Ranged)
            hints.ActionsToExecute.Push(ActionID.MakeSpell(ClassShared.AID.Addle), Module.PrimaryActor, ActionQueue.Priority.High, (float)(_activation - World.CurrentTime).TotalSeconds);
    }
