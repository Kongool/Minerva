...
namespace Minerva.RealmReborn.Raid.T01ADS;
...
class HighVoltage(ModuleBase module) : Components.CastHint(module, (uint)AID.HighVoltage, "Interruptible");
...
class DirtyCannons(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DirtyCannons, 5.15f);
...
{
    public T01ADSStates(ModuleBase module) : base(module)
    {
...
[ModuleInfo(CFCID = 93u, NameID = 1459u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")]
public class T01ADS(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-3, 27), new ArenaBoundsRect(7, 28))
{
