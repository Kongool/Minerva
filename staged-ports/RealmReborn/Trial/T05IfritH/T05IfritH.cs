...
namespace Minerva.RealmReborn.Trial.T05IfritH;
...
class Hints(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void AddGlobalHints(GlobalHints hints)
    {
        ...
        if (_nailSpawn != default && nailsActive)
        {
            hints.Add($"Nail enrage in: {Math.Max(55 - (World.CurrentTime - _nailSpawn).TotalSeconds, 0.0f):f1}s");
        }
    }
}

class Incinerate(ModuleBase module) : Components.Cleave(module, (uint)AID.Incinerate, new AOEShapeCone(15f, 60f.Degrees()));
...
class CrimsonCyclone(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.CrimsonCyclone)
{
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        ...
        {
            var c = _casters[i];
            aoes[i] = new(_shape, c.CastInfo?.LocXZ ?? c.Position, c.CastInfo?.Rotation ?? c.Rotation, Module.CastFinishAt(c.CastInfo, 0, World.FutureTime(4d)));
        }
        return aoes;
    }
...
{
    public T05IfritHStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 59u, NameID = 1185u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")]
public class T05IfritH : ModuleBase
{
