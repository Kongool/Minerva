...
class Ex4IfritAICommon(ModuleBase module) : ModuleComponent(module)
{
    ...
    protected DateTime CreatedAt = module.World.CurrentTime;
    ...
    protected void UpdateBossTankingProperties(AIHints.Enemy boss, Actor player, PartyRolesConfig.Assignment assignment)
    {
        ...
        boss.DesiredRotation = Angle.FromDirection(Module.PrimaryActor.Position - Center); // point to the wall
        if (!Module.PrimaryActor.Position.InCircle(Center, 13f)) // 13 == radius (20) - tank distance (2) - hitbox (5)
            boss.DesiredPosition = Center + 13f * boss.DesiredRotation.ToDirection();
        if (player.Role == Role.Tank)
        {
            ...
            //else if (TankVulnStacks(player) == 0 && TankVulnStacks(World.Actors.Find(boss.Actor.TargetID)) >= 2)
            {
                // taunt if safe
    ...
    {
        if (action.IsValid && phaseTime >= minPhaseTime && phaseTime < maxPhaseTime)
        {
...
class Ex4IfritAINormal(ModuleBase module) : Ex4IfritAICommon(module)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        var bossAngle = Angle.FromDirection(Module.PrimaryActor.Position - Center);
        ...
        else if (Module.PrimaryActor.TargetID != actor.InstanceID)
        {
            ...
            if (IsSearingWindTarget(actor))
            {
                ...
                var dir = !actor.Position.InCircle(Center, 10) ? Angle.FromDirection(actor.Position - Center)
                    : bossAngle + 105.Degrees();
                AddPositionHint(hints, Center + 18 * dir.ToDirection());
            }
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        if (Module.PrimaryActor.TargetID == pc.InstanceID)
        {
            ...
            Arena.ZoneConeOutline(Module.PrimaryActor.Position, 0f, 2f, Angle.FromDirection(Module.PrimaryActor.Position - Center), Incinerate.CleaveShape.HalfAngle, Colors.Safe);
        }
    }
}
...
class Ex4IfritAINails : Ex4IfritAINormal
{
    ...
    public Ex4IfritAINails(ModuleBase module, int minNailsForCWSearingWinds, ulong otTankAtIncinerateCounts) : base(module)
    {
        ...
        var startingNail = smallNails.Closest(Center + new WDir(15f, 0f));
        if (startingNail != null)
        {
            NailKillOrder.Add(startingNail);
            var startingDir = Angle.FromDirection(startingNail.Position - Center);
            NailKillOrder.AddRange(smallNails.Exclude(startingNail).Select(n => (n, NailDirDist(n.Position - Center, startingDir))).OrderBy(t => t.Item2.Item1).ThenBy(t => t.Item2.Item2).Select(t => t.n));
        }
        ...
        else
        {
            ...
            else if (Module.PrimaryActor.TargetID != actor.InstanceID)
            {
                ...
                if (IsSearingWindTarget(actor))
                {
                    ...
                    AddPositionHint(hints, Center + 18f * dir.ToDirection());
                }
                else if (assignment == BossTankRole)
                {
                ...
                else
                {
                    ...
                    if (!EruptionActive)
                        hints.AddForbiddenZone(new SDCircle(Center, Eruption.Radius));
                }
            }
...
class Ex4IfritAINails1(ModuleBase module) : Ex4IfritAINails(module, 1, 0x8)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        base.AddAIHints(slot, actor, assignment, hints);
        ...
        {
            var phaseTime = (float)(World.CurrentTime - CreatedAt).TotalSeconds;
            PlanRampart(hints, actor, phaseTime, 10, 20);
        }
    }
}

class Ex4IfritAINails2(ModuleBase module) : Ex4IfritAINails(module, 4, 0x7)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        base.AddAIHints(slot, actor, assignment, hints);
...
class Ex4IfritAINails3(ModuleBase module) : Ex4IfritAINails(module, 7, 0x3C70)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        base.AddAIHints(slot, actor, assignment, hints);
...
class Ex4IfritAIHellfire : Ex4IfritAICommon
{
    ...
    public Ex4IfritAIHellfire(ModuleBase module, Angle safeSpotDir, PartyRolesConfig.Assignment tankRole) : base(module)
    {
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (IsSearingWindTarget(actor))
        {
            AddPositionHint(hints, Center - _safespotOffset);
        }
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        Arena.ZoneCircleOutline(Center + _safespotOffset, 2, Colors.Safe);
    }
}
class Ex4IfritAIHellfire1(ModuleBase module) : Ex4IfritAIHellfire(module, 150f.Degrees(), PartyRolesConfig.Assignment.MT);
class Ex4IfritAIHellfire2(ModuleBase module) : Ex4IfritAIHellfire(module, 110f.Degrees(), PartyRolesConfig.Assignment.OT);
class Ex4IfritAIHellfire3(ModuleBase module) : Ex4IfritAIHellfire(module, 70f.Degrees(), PartyRolesConfig.Assignment.MT);
