...
class Ex3TitanStates : StateMachineBuilder
{
    ...
    private void Geocrush<Crush>(uint id, float delay, float newRadius = 0) where Crush : Geocrush
    {
        ...
        Cast(id + 0x10, (uint)AID.Geocrush, 3.3f, 3, "Geocrush")
            ...
            .OnExit(() => Module.Bounds = new ArenaBoundsCircle(newRadius), newRadius > 0);
    ...
    {
        // BossmodReborn told its autorotation to kill the next bomb here; Minerva runs no rotation, so the
        // state stands and the instruction is dropped rather than half-ported.
