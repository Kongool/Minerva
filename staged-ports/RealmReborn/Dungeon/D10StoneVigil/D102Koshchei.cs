...
namespace Minerva.RealmReborn.Dungeon.D10StoneVigil.D102Koshchei;
...
class SpikedTail(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.SpikedTail);
class SonicStorm(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SonicStorm, 6f);

class Typhoon(ModuleBase module) : Components.Exaflare(module, 3f)
{
...
{
    public D102KoshcheiStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 11u, NameID = 1678u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
public class D102Koshchei(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
