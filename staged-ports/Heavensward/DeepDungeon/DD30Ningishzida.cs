...
namespace Minerva.Heavensward.DeepDungeon.PalaceOfTheDead.DD30Ningishzida;
...
abstract class Voidzones(ModuleBase module, uint aid, uint oid) : Components.VoidzoneAtCastTarget(module, 6, aid, m => GetVoidzones(m, oid), 2.1f)
{
...
class Hints(ModuleBase module) : ModuleComponent(module)
{
    ...
    {
        if (NumCasts < 4)
            hints.AddForbiddenZone(new SDCircle(FearCastSource, 11f), World.FutureTime(10d));
        else
            hints.AddForbiddenZone(new SDInvertedCircle(FearCastSource, 5f), World.FutureTime(10d));
    }
}

class DD30NingishzidaStates : StateMachineBuilder
{
    public DD30NingishzidaStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 176u, NameID = 5012u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "LegendofIceman (ported from BMR)")]
public class DD30Ningishzida(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
