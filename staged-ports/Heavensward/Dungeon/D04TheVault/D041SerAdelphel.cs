...
namespace Minerva.Heavensward.Dungeon.D04TheVault.D041SerAdelphel;
...
sealed class Bloodstain(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Bloodstain, 5f);
sealed class HeavenlySlash(ModuleBase module) : Components.Cleave(module, (uint)AID.HeavenlySlash, new AOEShapeCone(10.2f, 45f.Degrees()));
sealed class HoliestOfHoly(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.HoliestOfHoly);
sealed class HolyShieldBash(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.HolyShieldBash, "Stun + single target damage x2");

sealed class BrightSphere(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class Execution(ModuleBase module) : Components.BaitAwayIcon(module, 5f, (uint)IconID.Spreadmarker, (uint)AID.Execution, 4.8f);

sealed class ShiningBlade(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorNpcYell(Actor actor, ushort id)
    {
        ...
        var activationTimes = GetActivationTimes(World.FutureTime(0.08f));
        var center = Center;
...
{
    public D041SerAdelphelStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 34u, CFCID = 34u, NameID = 3634u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS), Xyzzy (ported from BMR)")]
public sealed class D041SerAdelphel : ModuleBase
{
