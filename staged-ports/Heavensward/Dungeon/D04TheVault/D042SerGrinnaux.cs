...
namespace Minerva.Heavensward.Dungeon.D04TheVault.D042SerGrinnaux;
...
sealed class HeavySwing(ModuleBase module) : Components.Cleave(module, (uint)AID.HeavySwing, new AOEShapeCone(6.5f, 45f.Degrees()), [(uint)OID.SerGrinnauxTheBull]);
sealed class Overpower(ModuleBase module) : Components.Cleave(module, (uint)AID.Overpower, new AOEShapeCone(10.2f, 45f.Degrees()));
sealed class DimensionalRip(ModuleBase module) : Components.VoidzoneAtCastTarget(module, 5f, (uint)AID.DimensionalRip, GetVoidzones, 1.1d)
{
...
sealed class FaithUnmoving(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.FaithUnmoving, 20f, stopAtWall: true)
{
...
sealed class Rive(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Rive, new AOEShapeRect(30.5f, 1f));
sealed class HyperdimensionalSlash(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HyperdimensionalSlash, new AOEShapeRect(47.2f, 4f));
sealed class DimensionalCollapse1(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DimensionalCollapse1, new AOEShapeDonutSector(2.5f, 7.5f, 90f.Degrees()));
sealed class DimensionalCollapse2(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DimensionalCollapse2, new AOEShapeDonutSector(7.5f, 12.5f, 90f.Degrees()));
sealed class DimensionalCollapse3(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DimensionalCollapse3, new AOEShapeDonutSector(12.5f, 17.5f, 90f.Degrees()));
sealed class AetherialTear(ModuleBase module) : Components.Voidzone(module, 7f, GetTears)
{
    private static List<Actor> GetTears(ModuleBase module) => module.Enemies((uint)OID.AetherialTear);
}

sealed class D042SerGrinnauxStates : StateMachineBuilder
{
    public D042SerGrinnauxStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 34u, CFCID = 34u, NameID = 3639u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS), Xyzzy (ported from BMR)")]
public sealed class D042SerGrinnaux(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
