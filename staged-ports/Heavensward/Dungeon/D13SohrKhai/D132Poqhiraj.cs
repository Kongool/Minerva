...
namespace Minerva.Heavensward.Dungeon.D13SohrKhai.D132Poqhiraj;
...
sealed class GallopAOE(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GallopAOE, new AOEShapeRect(40.5f, 1f));

sealed class GallopKB(ModuleBase module) : Components.GenericKnockback(module)
{
...
sealed class GallopKBHint(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly GallopKB _kb = module.FindComponent<GallopKB>()!;
...
sealed class Touchdown(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TouchdownVisual, 25f);

sealed class BurningBright(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.BurningBright, new AOEShapeRect(28.5f, 3f), endsOnCastEvent: true)
{
...
sealed class RearHoof(ModuleBase module) : Components.SingleTargetInstant(module, (uint)AID.RearHoof)
{
...
sealed class CloudCall(ModuleBase module) : Components.GenericBaitAway(module, centerAtTarget: true)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        ...
        {
            CurrentBaits.Add(new(Module.PrimaryActor, actor, Circle, World.FutureTime(4.9d)));
        }
    }
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        {
            hints.AddForbiddenZone(new SDRect(Center, new WDir(default, 1f), 19f, 19f, 5f));
        }
    }
...
sealed class LightningBolt(ModuleBase module) : Components.GenericAOEs(module)
{
    private AOEInstance[] _aoe = [];
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor) => _aoe;
    ...
    public override void OnActorCreated(Actor actor)
    {
        if (actor.OID == (uint)OID.DarkCloud)
        {
            _aoe = [new(CloudCall.Circle, actor.Position, default, World.FutureTime(7.8d))];
        }
    }
...
{
    public D132PoqhirajStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 171u, CFCID = 171u, NameID = 4952u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D132Poqhiraj(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(400f, 104.166f), new ArenaBoundsRect(4.5f, 19.75f));
