...
public sealed class ReplayAnalysis
{
    ...
    private ReplayAnalysis(WorldState ws)
    {
        ...
        ws.Actors.HPMPChanged.Subscribe(this.OnHPMP);
    ...
    private void OnAdded(Actor a)
    {
        ...
        if (!this.objects.TryGetValue(a.OID, out var info))
            this.objects[a.OID] = info = new ObjInfo { Name = a.Name, HitboxRadius = a.HitboxRadius, Type = a.Type, FirstSeen = this.Now, MaxHP = a.HPMP.MaxHP };
        info.Count++;
        info.LastSeen = this.Now;
    }

    /// <summary>
    /// Remember the largest health pool ever seen for each object id.
    /// <para>It is not in the spawn packet — an actor is created with zero and the real figure arrives in a
    /// later update — so reading it at creation reports every hazard and every trash mob alike as zero, and
    /// the voidzone test that leans on it cannot tell them apart.</para>
    /// </summary>
    private void OnHPMP(Actor a)
    {
        if (this.objects.TryGetValue(a.OID, out var info) && a.HPMP.MaxHP > info.MaxHP)
            info.MaxHP = a.HPMP.MaxHP;
    }
    ...
    public GenerationInput BuildGenerationInput()
    {
        ...
        foreach (var (oid, info) in this.objects)
        {
            ...
            // A voidzone has no health bar. That one extra test is what separates a hazard from a
            // bystander, and in open-world content the bystanders are the majority: a foray FATE is not
            // sealed, so players drag trash in and it ends up standing inside the boss's cast radius for
            // the whole fight. Every one of those passed the other four tests — no casts, plausible
            // hitbox, present throughout, ActorType.Enemy — and three per draft were emitted as voidzones
            // that would have told the dodge to avoid a wandering mob. Measured across the North Horn
            // recordings: real hazards report MaxHP 0, dragged-in trash reports ~800,000.
            var voidzone = oid != bossOID && info.Casts == 0 && info.MaxHP == 0
                && info.HitboxRadius is >= 1f and <= 15f
