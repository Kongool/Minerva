...
public sealed class RouteGrid
{
    ...
    ///
    /// <para><see cref="Waypoint"/> answers "where do I steer this frame", which is the right question for
    /// a mover we drive ourselves. A navmesh follower is handed a path and walks it, so it wants the whole
    /// thing: give it one point and it walks the straight chord to that point, straight through whatever
    /// the route was bending around. That is not the follower's fault -- it was told to go there.</para>
    ///
    /// <para>String-pulled the same way <see cref="Waypoint"/> picks its target: from each kept point, skip
    /// ahead to the furthest one still reachable in a straight line. What survives is the corners.</para>
    /// </summary>
