...
public class Gaze(ModuleBase module, uint aid) : ModuleComponent(module)
{
    ...
    /// <summary>
    /// True if <paramref name="actor"/> is looking at the source closely enough to be caught by it.
    /// <para>The same ±45 degree test <see cref="GenericGaze.HitByEye"/> uses, which is the game's own
    /// tolerance. This used to be a front <i>hemisphere</i> (±90 degrees), so it warned for a source
    /// anywhere ahead of the player and disagreed with the arc this component's own
    /// <see cref="AddAIHints"/> forbids — the text said look away while the dodge considered the facing
    /// fine, and neither was wrong about the other's rule.</para>
    /// </summary>
    public static bool FacingToward(Actor actor, WPos source)
        => actor.Rotation.ToDirection().Dot((source - actor.Position).Normalized()) >= GazeCosine;

    /// <summary>cos(45°) — a gaze catches you when its source is within 45 degrees of your facing.</summary>
    private const float GazeCosine = 0.707107f;

    /// <summary>The same 45 degrees, as an angle, so the drawn test and the forbidden arc cannot drift.</summary>
    private static readonly Angle GazeHalfAngle = 45f.Degrees();

    /// <summary>
    /// Mark each source, coloured by whether you are currently caught by it: red while you are looking at
    /// it, green once you are not.
    /// <para>It was unconditionally red, which tells you a gaze exists but not the one thing you need to
    /// know while turning — whether you have turned far enough yet. The information was already here; only
    /// the drawing did not ask for it.</para>
    /// </summary>
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        foreach (var id in this.Sources)
        {
            if (this.World.Actors.Find(id) is { } a)
                this.Arena.Eye(a.Position, 1f, FacingToward(pc, a.Position) ? Colors.Danger : Colors.Safe);
        }
    }
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        {
            if (this.World.Actors.Find(id) is { } a)
                hints.ForbiddenDirections.Add((Angle.FromDirection(a.Position - actor.Position), GazeHalfAngle, default));
        }
    }
}
...
{
    /// <summary>
    /// Draw the eye glyph at a screen position, red when it currently has you and green when it does not.
    /// Static and screen-space because a few modules place eyes themselves rather than through the component
    /// — the glyph must stay a readable size regardless of arena zoom.
    /// </summary>
    public static void DrawEye(Vector2 screenPos, bool dangerous)
    {
        EyeRenderer?.Invoke(screenPos, dangerous);
    }

    /// <summary>Set by the renderer. Null headlessly, where there is nothing to draw on.</summary>
    public static Action<Vector2, bool>? EyeRenderer;
    ...
    /// <summary>BossmodReborn passes the actor by ref here; the reference is never reassigned, so this just
    /// forwards. Carried so ported call sites compile unedited.</summary>
    public static bool HitByEye(ref Actor actor, in Eye eye) => HitByEye(actor, eye);
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        ...
        foreach (ref readonly var eye in eyes)
        {
            // inverted gazes want you looking AT them, so "danger" is the state that needs the red, not
            // the direction you happen to be facing
            var danger = HitByEye(pc, in eye) != eye.Inverted;
            this.Arena.Eye(eye.Position, 1.5f, danger ? Colors.Danger : Colors.Safe);
        }
    }
}
...
#pragma warning disable SA1300, IDE1006 // BossmodReborn member spelling, deliberate
    /// <summary>
    /// BossmodReborn's spelling of <see cref="FallbackTime"/>, which is already wired into the resolve
    /// search below.
    ///
    /// <para>An alias that writes THROUGH, not a second field. A plain field of this name would compile,
    /// accept a module's assignment and change nothing, because the search reads the other one — the
    /// mechanic would resolve on the wrong clock with no error anywhere.</para>
    /// </summary>
    protected float fallbackTime
    {
        get => this.FallbackTime;
        set => this.FallbackTime = value;
    }
#pragma warning restore SA1300, IDE1006

#pragma warning disable SA1300, IDE1006 // BossmodReborn member spelling, deliberate
    /// <summary>BossmodReborn's spelling of <see cref="Casters"/>; its modules read it by this name.</summary>
    protected List<Actor> _casters => this.Casters;
#pragma warning restore SA1300, IDE1006
