...
public sealed class RelSimplifiedComplexPolygon(List<RelPolygonWithHoles> parts)
{
    ...
    /// <summary>
    /// Rebuild the point-in-polygon acceleration index. Call it after mutating the polygon; the index is
    /// built from the vertices and cannot notice them changing underneath it.
    ///
    /// <para>Public rather than internal as in BossmodReborn: there the modules share an assembly with this
    /// type, whereas Minerva's modules live in the plugin assembly and Minerva.Core is a separate one. A
    /// module rebuilding an index it owns is a legitimate call, not a reach into private state.</para>
    /// </summary>
    public PolygonBoundaryIndex2D InitPolygonIndex()
    {
