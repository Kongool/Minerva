...
/// <summary>
/// Real character-steering controller for auto-dodge. It hooks the game's walk-input reader
/// (<c>PlayerMoveController::readInput</c>, "RMIWalk") and, when a dodge target is set and the player
/// isn't already steering themselves, overrides the sampled movement toward that target.
/// <para>
/// This is a deliberately trimmed clean-room port of BossmodReborn's <c>MovementOverride</c>: walk
/// only — no fly, misdirection, spinning, or navmesh handoff. The three signatures and the input
/// convention (sumLeft/sumForward relative to the forward-movement direction) are reused from BMR as
/// hard-won game constants; the logic is our own. Guarded so a stale signature or a detour fault can
/// never take the game's movement down — it just falls back to vanilla input.
/// </para>
/// </summary>
public sealed unsafe class MovementController : IMovementController, IDisposable
{
    ...
    public void MoveTo(WPos target) => this.target = target;
