...
internal sealed class BmrDriver
{
    ...
    /// </summary>
    public static void Apply(BmrBridge bridge, WorldState.Operation op, ulong pov = 0)
    {
        switch (op)
        {
            ...
            case PartyState.OpModify pm: bridge.PartyModify(pm.Slot, pm.Member.ContentID, pm.Member.InstanceID, pov); break;

            case ActorState.OpCreate c:
                bridge.Create(c.InstanceID, c.OID, c.Name, c.NameID, (int)c.Type, c.PosRot, c.HitboxRadius,
                    c.HPMP.CurHP, c.HPMP.MaxHP, c.HPMP.Shield, c.HPMP.CurMP, c.HPMP.MaxMP, c.IsTargetable, c.IsAlly, c.OwnerID);
                break;
            ...
            case ActorState.OpHPMP hp: bridge.HPMP(hp.InstanceID, hp.HPMP.CurHP, hp.HPMP.MaxHP, hp.HPMP.Shield, hp.HPMP.CurMP, hp.HPMP.MaxMP); break;
            case ActorState.OpActorEAnim ea: bridge.EAnim(ea.InstanceID, ea.State); break;
            case ActorState.OpModelState ms: bridge.ModelState(ms.InstanceID, ms.ModelState); break;
            case ActorState.OpEventState es: bridge.EventState(es.InstanceID, es.Value); break;
            case ActorState.OpRenderflags rf: bridge.Renderflags(rf.InstanceID, rf.Value); break;
            case WorldState.OpDirectorUpdate du: bridge.DirectorUpdate(du.DirectorID, du.UpdateID, du.Param1, du.Param2, du.Param3, du.Param4); break;
            case WorldState.OpMapEffect me: bridge.MapEffect(me.Index, me.State); break;
    ...
    public static Result Run(ReplayTimeline timeline, BmrHost host, uint? forceOID = null)
    {
        ...
        {
            Apply(bridge, op, timeline.PlayerInstanceID);
