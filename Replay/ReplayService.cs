    ...
    public string? UpdateRecording(TimeSpan realDt)
    {
        if (!this.IsRecording)
        {
            ...
            if (this.config.AutoRecordEncounters)
            {
                ...
                if (module != null)
                {
                    // The module outlives its boss -- it stays active until the corpse despawns -- and a recording
                    // that has just ended on that death must not start again on the same dead boss. It did, twice,
                    // two seconds each, after Many Mouths to Feed.
