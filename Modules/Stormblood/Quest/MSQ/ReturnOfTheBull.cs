...
namespace Minerva.Stormblood.Quest.MSQ.ReturnOfTheBull;
...
class PathOfLight(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ThePathOfLight, new AOEShapeCone(43.5f, 60f.Degrees()));
class BlissfulSpear(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BlissfulSpear, new AOEShapeCross(40f, 4f));
class ThePallOfLight(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.ThePallOfLight, 6f, 1);
class BlissfulHammer(ModuleBase module) : Components.BaitAwayIcon(module, 7f, 109u, (uint)AID.BlissfulHammer, 12.1f);
class FordolaShield(ModuleBase module) : ModuleComponent(module)
{
    public Actor? Shield => World.Actors.FirstOrDefault(a => a.OID == (uint)OID.FordolaShield);
    ...
    {
        if (Shield != null)
            hints.AddForbiddenZone(new SDInvertedCircle(Shield.Position, 4f), World.FutureTime(5d));
    }
...
class Deflect(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        var deflectAction = World.Client.DutyActions[0].Action;
...
{
    public LakshmiStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 68508u, NameID = 6385u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")]
public class Lakshmi(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(250, -353), new ArenaBoundsSquare(23))
{
