...
namespace Minerva.Stormblood.Quest.MSQ.BestServedWithColdSteel;
...
class OpenFire(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.OpenFire1, 6);

class CermetPile(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.CermetPile, new AOEShapeRect(42.1f, 3f));
class Firebomb(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Firebomb, 4);

class MagitekTurret(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.SelfDetonate)
{
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        ...
        for (var i = 0; i < count; ++i)
        {
            ...
            if (m.target == actor)
            {
                ...
                var distance = m.DistanceLeft(World);
    ...
    {
        if (tether.ID == (uint)TetherID.Mine && World.Actors.Find(tether.Target) is Actor target)
            Mines.Add(new(source, target, source.Position, World.CurrentTime));
    }
...
class MagitekSelfDetonate(ModuleBase module) : Components.CastCounter(module, (uint)AID.SelfDetonate1)
{
...
[ModuleInfo(CFCID = 67989u, NameID = 5650u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")]
public class MagitekVanguardIPrototype(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
