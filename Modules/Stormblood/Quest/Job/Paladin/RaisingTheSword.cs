...
namespace Minerva.Stormblood.Quest.Job.Paladin.RaisingTheSword;
...
class VictorySlash(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.VictorySlash, new AOEShapeCone(6.5f, 60f.Degrees()));
class ShudderingSwipeCone(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ShudderingSwipeAOE, new AOEShapeCone(60f, 15f.Degrees()));

class ShudderingSwipeKB(ModuleBase module) : Components.GenericKnockback(module, stopAtWall: true)
{
...
class NaldsWhisper(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.NaldsWhisper, 20f);
class TheFourWinds(ModuleBase module) : Components.Voidzone(module, 6f, GetVoidzones)
{
...
{
    public AldisSwordOfNaldStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 270u, NameID = 6311u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")]
public class AldisSwordOfNald(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-89.3f, default), new ArenaBoundsCircle(20.5f));
