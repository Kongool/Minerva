...
namespace Minerva.Stormblood.Quest.Job.Samurai.TheFaceOfTrueEvil;
...
class Musojin(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Musojin);

abstract class Hissatsu(ModuleBase module, uint aid) : Components.SimpleAOEs(module, aid, new AOEShapeRect(44.5f, 2f));
class HissatsuKiku(ModuleBase module) : Hissatsu(module, (uint)AID.HissatsuKiku1);
class HissatsuTo(ModuleBase module) : Hissatsu(module, (uint)AID.HissatsuTo);

class Maiogi(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Maiogi, new AOEShapeCone(80f, 25f.Degrees()));
class HissatsuKyuten(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HissatsuKyuten, 5.5f);
class Arashi(ModuleBase module) : Components.GenericAOEs(module)
{
...
class ViolentWind(ModuleBase module) : Components.Adds(module, (uint)OID.ViolentWind);

class MusosaiStates : StateMachineBuilder
{
    public MusosaiStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.Quest, CFCID = 68101u, NameID = 6111u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")]
public class Musosai(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-217.27f, -158.31f), new ArenaBoundsSquare(15f));
