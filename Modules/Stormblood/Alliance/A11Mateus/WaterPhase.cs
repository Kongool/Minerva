...
class Froth(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (count != 0)
        {
            ...
            if (status is ActorStatus breathless && breathless.Extra >= 0x5)
            {
                ...
                if (orbs.Length != 0)
                    hints.AddForbiddenZone(new SDIntersection(orbs), World.FutureTime(10d - breathless.Extra));
            }
        }
    }
...
class Snowpierce(ModuleBase module) : Components.BaitAwayChargeCast(module, (uint)AID.Snowpierce, 1.5f)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
