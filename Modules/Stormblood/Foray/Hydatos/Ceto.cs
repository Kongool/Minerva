...
namespace Minerva.Stormblood.Foray.Hydatos.Ceto;
...
sealed class SickleStrike(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.SickleStrike);
sealed class Petrifaction(ModuleBase module) : Components.CastGazes(module, [(uint)AID.PetrifactionBoss, (uint)AID.PetrifactionAdds], range: 50f);
sealed class AbyssalReaper(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AbyssalReaper, 18f);
sealed class CircleOfFlames(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.CircleOfFlames, 5f);
sealed class TailSlap(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TailSlap, new AOEShapeCone(12f, 60f.Degrees()));
sealed class GrimFate(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GrimFate, new AOEShapeCone(8f, 60f.Degrees()));
sealed class Desolation(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Desolation, new AOEShapeRect(12f, 4f));

sealed class Petrattraction(ModuleBase module) : Components.GenericKnockback(module)
{
...
sealed class CircleBlade(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.CircleBlade1, (uint)AID.CircleBlade2], 7f);
sealed class FireToss(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FireToss, 5f);

sealed class CetoStates : StateMachineBuilder
{
    public CetoStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.EurekaNM, GroupID = 639u, CFCID = 639u, NameID = 1421u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "xan, Malediktus (ported from BMR)")]
