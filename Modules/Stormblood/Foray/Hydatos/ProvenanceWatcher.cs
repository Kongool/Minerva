...
namespace Minerva.Stormblood.Foray.Hydatos.ProvenanceWatcher;
...
class TheScarletPrice(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.TheScarletPrice, 3f, true);
class TheScarletWhisper(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TheScarletWhisper, new AOEShapeCone(22f, 60f.Degrees()));
class EuhedralSwat(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.EuhedralSwat, new AOEShapeRect(100f, 13f));
class Touchdown(ModuleBase module) : Components.RaidwideInstant(module, (uint)AID.Touchdown, 3.1d)
{
...
class PillarPierce(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PillarPierce, new AOEShapeRect(52.5f, 5));
class Thunderstorm(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Thunderstorm, 5f);

class IceAndLevin(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.Chillstorm)
{
...
class Charybdis(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        if (actor.OID == (uint)OID.Charybdis)
            casters.Add((actor.InstanceID, actor.Position.Quantized(), World.FutureTime(4d), 19));
    }
...
class HotTail(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HotTailFirst, new AOEShapeRect(77f, 8f));

class HotTailSecond(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        {
            _aoe = [new(rect, caster.Position, spell.Rotation, World.FutureTime(3.1d))];
        }
...
class AkhMorn(ModuleBase module) : Components.GenericStackSpread(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        {
            Stacks.Add(new(World.Actors.Find(spell.TargetID)!, 6f, activation: Module.CastFinishAt(spell)));
            castsLeft = 3;
        }
    }
...
{
    public ProvenanceWatcherStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.EurekaNM, CFCID = 639u, NameID = 1423u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "xan (ported from BMR)")]
