...
namespace Minerva.Stormblood.Foray.Hydatos.Daphne;
...
class Spellwind(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.SpellwindCast);
class Upburst(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Upburst, 8f);
class RoilingReach(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.RoilingReach, new AOEShapeCross(32f, 3.5f));
class Wallop(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Wallop, new AOEShapeRect(50f, 3.5f));
class ChillingGlare(ModuleBase module) : Components.CastGaze(module, (uint)AID.ChillingGlare);
class Netherwater(ModuleBase module) : Components.StackWithIcon(module, (uint)IconID.Stackmarker, (uint)AID.Netherwater, 6, 4.9f, 4, 24);
class Spiritus(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Spiritus, new AOEShapeCone(5, 30f.Degrees()));

class DaphneStates : StateMachineBuilder
{
    public DaphneStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.EurekaNM, CFCID = 639u, NameID = 1417u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "xan, Malediktus (ported from BMR)")]
