...
namespace Minerva.Stormblood.Foray.Hydatos.Molech;
...
class W11TonzeSwipe(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.W11TonzeSwipe, new AOEShapeCone(9f, 60f.Degrees()));
class W111TonzeSwing(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.W111TonzeSwing, 13f);
class W111TonzeSwingAdds(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.W111TonzeSwingAdds, 13f);
class W111TonzeSwingBig(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.W111TonzeSwingBig, 20f);
class ZoomIn(ModuleBase module) : Components.ChargeAOEs(module, (uint)AID.ZoomIn, 4f);

class MolechStates : StateMachineBuilder
{
    public MolechStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.EurekaNM, CFCID = 639u, NameID = 1414u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "xan, Malediktus (ported from BMR)")]
