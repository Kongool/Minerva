...
sealed class BrightDarkAuroraExplosion(ModuleBase module) : Components.GenericAOEs(module)
{
    private static readonly AOEShapeCircle circle = new(8f);
    private readonly List<(Actor source, ulong target)> tetherByActor = [];
...
abstract class Towers(ModuleBase module, uint oid, uint tid) : Components.GenericTowersOpenWorld(module)
{
    private readonly List<(Actor source, Actor target)> tetherByActor = [];
    ...
    public override void OnActorCreated(Actor actor)
    {
        if (actor.OID == oid)
        {
            Towers.Add(new(actor.Position, 2f, 1, 1, [], World.FutureTime(20d)));
        }
    }
    ...
    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        if (tether.ID == tid)
        {
            tetherByActor.Add((source, World.Actors.Find(tether.Target)!));
        }
    }
    ...
    public override void OnUntethered(Actor source, in ActorTetherInfo tether)
    {
        if (tether.ID == tid)
        {
            tetherByActor.Remove((source, World.Actors.Find(tether.Target)!));
        }
    }
...
sealed class BrightAuroraTether(ModuleBase module) : Towers(module, (uint)OID.DarkAuroraHelper, (uint)TetherID.BrightAurora);
sealed class DarkAuroraTether(ModuleBase module) : Towers(module, (uint)OID.BrightAuroraHelper, (uint)TetherID.DarkAurora);
