...
namespace Minerva.Stormblood.TreasureHunt.ShiftingAltarsOfUznair.AltarTotem;
...
class FlurryOfRage(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FlurryOfRage, new AOEShapeCone(13.06f, 60f.Degrees()));
class WaveOfMalice(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WaveOfMalice, 5f);
class WhorlOfFrenzy(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WhorlOfFrenzy, 11.06f);
class TheWardensVerdict(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TheWardensVerdict, new AOEShapeRect(45.06f, 2f));
class FlamesOfFury(ModuleBase module) : Components.VoidzoneAtCastTarget(module, 10f, (uint)AID.FlamesOfFury, GetVoidzones, 1.2f)
{
...
class FlamesOfFuryBait(ModuleBase module) : Components.GenericBaitAway(module, centerAtTarget: true)
{
...
{
    public AltarTotemStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 586u, NameID = 7586u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
