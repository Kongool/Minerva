...
namespace Minerva.Stormblood.TreasureHunt.ShiftingAltarsOfUznair.AltarKelpie;
...
class Innocence(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Innocence, 5f);
class HydroPush(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HydroPush, new AOEShapeRect(49.4f, 22f));

class BloodyPuddle(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        if (actor.OID == (uint)OID.Hydrosphere)
            AOEs.Add(new(circle, actor.Position.Quantized(), default, World.FutureTime(8.6d)));
    }
...
class RisingSeas(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.RisingSeas);

class RisingSeasKB(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.RisingSeas, 20f, stopAtWall: true)
{
...
{
    public AltarKelpieStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 586u, NameID = 7589u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
