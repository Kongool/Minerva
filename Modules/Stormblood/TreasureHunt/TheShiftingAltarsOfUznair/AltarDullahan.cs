...
namespace Minerva.Stormblood.TreasureHunt.ShiftingAltarsOfUznair.AltarDullahan;
...
class IronJustice(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.IronJustice, new AOEShapeCone(11.8f, 60.Degrees()));
class Cloudcover(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Cloudcover, 6f);
class TerrorEye(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TerrorEye, 6f);
class VillainousRebuke(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.VillainousRebuke, 6f, 8, 8);
class StygianRelease(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.StygianRelease);
class StygianReleaseKB(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.StygianRelease, 20f, stopAtWall: true)
{
...
{
    public AltarDullahanStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 586u, NameID = 7585u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
