...
namespace Minerva.Stormblood.TreasureHunt.ShiftingAltarsOfUznair.AltarSkatene;
...
sealed class Chirp(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Chirp, 12.48f);
sealed class Tornado(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Tornado, 6f);
sealed class VoidCall(ModuleBase module) : Components.CastHint(module, (uint)AID.VoidCall, "Calls adds");
sealed class RecklessAbandon(ModuleBase module) : Components.SingleTargetDelayableCast(module, (uint)AID.RecklessAbandon);
...
{
    public AltarSkateneStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 586u, CFCID = 586u, NameID = 7587u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
