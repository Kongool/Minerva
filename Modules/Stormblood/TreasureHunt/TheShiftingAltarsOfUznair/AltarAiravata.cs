...
namespace Minerva.Stormblood.TreasureHunt.ShiftingAltarsOfUznair.AltarAiravata;
...
sealed class HurlBoss(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HurlBoss, 6f);
sealed class SpinBoss(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SpinBoss, new AOEShapeCone(30f, 60f.Degrees()));
sealed class BarbarousScream(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BarbarousScream, 14f);
sealed class Huff(ModuleBase module) : Components.SingleTargetDelayableCast(module, (uint)AID.Huff);

sealed class Buffet(ModuleBase module) : Components.GenericKnockback(module, (uint)AID.Buffet, stopAtWall: true)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        ...
        {
            target = actor;
            activation = World.FutureTime(3.1d);
        }
    }
...
sealed class Buffet2(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.Buffet, new AOEShapeCone(30f, 60f.Degrees()), true)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
...
sealed class RaucousScritch(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.RaucousScritch, new AOEShapeCone(8.42f, 60f.Degrees()));
sealed class Hurl(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Hurl, 6f);
sealed class Spin(ModuleBase module) : Components.Cleave(module, (uint)AID.Spin, new AOEShapeCone(9.42f, 60f.Degrees()), [(uint)OID.AltarMatanga]);

sealed class AltarAiravataStates : StateMachineBuilder
{
    public AltarAiravataStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 586u, CFCID = 586u, NameID = 7601u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
