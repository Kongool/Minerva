...
namespace Minerva.Stormblood.TreasureHunt.ShiftingAltarsOfUznair.AltarMandragora;
...
class OpticalIntrusion(ModuleBase module) : Components.SingleTargetDelayableCast(module, (uint)AID.OpticalIntrusion);
class Hypnotize(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Hypnotize, new AOEShapeCone(22.85f, 45f.Degrees()));
class SaibaiMandragora(ModuleBase module) : Components.CastHint(module, (uint)AID.SaibaiMandragora, "Calls adds");
class LeafDagger(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.LeafDagger, 3f);
...
{
    public AltarMandragoraStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 586u, NameID = 7600u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
