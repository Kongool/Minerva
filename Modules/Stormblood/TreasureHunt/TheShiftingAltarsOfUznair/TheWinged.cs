...
namespace Minerva.Stormblood.TreasureHunt.ShiftingAltarsOfUznair.TheWinged;
...
class Filoplumes(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Filoplumes, new AOEShapeRect(11.36f, 2f));
class Wingbeat(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Wingbeat, new AOEShapeCone(43.36f, 30f.Degrees()));
class WingbeatKB(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.Wingbeat, 20f, false, 1, new AOEShapeCone(43.36f, 30.Degrees()), stopAtWall: true);
class FeatherSquall(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FeatherSquall, 6f);
class Pinion(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Pinion, new AOEShapeRect(40.5f, 1.5f));
class Sideslip(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Sideslip);
...
{
    public TheWingedStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 586u, NameID = 7595u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
