...
namespace Minerva.Stormblood.TreasureHunt.HiddenCalansOfUznair.Airavata;
...
class Hurl(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.HurlBoss, (uint)AID.Hurl], 6f);
class SpinBoss(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SpinBoss, new AOEShapeCone(20f, 60f.Degrees()));
class BarbarousScream(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BarbarousScream, 14f);
class Buffet(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.Buffet);

class DoubleSmash(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DoubleSmash, new AOEShapeCone(7.95f, 60f.Degrees()));
class AncientAero(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AncientAero, new AOEShapeRect(13.6f, 2f));
class RingOfFire(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.RingOfFire, 5f);
class StoneII(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.StoneII);
...
{
    public AiravataStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 276u, NameID = 6853u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
