...
namespace Minerva.Stormblood.Trial.T04Tsukuyomi;
...
sealed class TormentUntoDeath(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.TormentUntoDeath,
    new AOEShapeCone(15f, 37.5f.Degrees()), tankbuster: true);

sealed class TsukuNoMaiogi(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TsukiNoMaiogi, 10f, 7);

sealed class SteelOfTheUnderworld(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SteelOfTheUnderworld, new AOEShapeCone(70f, 45f.Degrees()));

sealed class Reprimand(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Reprimand);

sealed class MidnightHaze(ModuleBase module) : Components.Adds(module, (uint)OID.MidnightHaze, 1);

sealed class LeadOfTheUnderworld(ModuleBase module) : Components.LineStack(module, aidMarker: (uint)AID.LeadOfUnderworldMark, (uint)AID.LeadOfTheUnderworld, 5d, 70f, 4f, 8, 8);

/*
 * Nightbloom happens and the arena takes the bloody smoke look, same bounds. Multiple adds come out to be killed.
 */
sealed class Nightbloom(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Nightbloom);

sealed class Spectres(ModuleBase module) : Components.AddsMulti(module,
[
 ...
 */
sealed class Nightbloom2(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Nightbloom2);

// Selenomancy triggers the floor change in arena to waxing/waning moon.

//Angle is an estimate
sealed class UnmovingTroika1(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.UnmovingTroika1, new AOEShapeCone(39f, 37.5f.Degrees()));
//Angle is an estimate
sealed class UnmovingTroika2(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.UnmovingTroika2, new AOEShapeCone(39f, 37.5f.Degrees()));

// radius are estimates
sealed class LunarHalo(ModuleBase module)
 ...
 */
sealed class MoonStatus(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        ...
        if (status.ID is (uint)SID.Moonlit or (uint)SID.Moonshadowed && status.Extra >= 3 && actor.InstanceID == Raid.Player()!.InstanceID)
        {
            ...
            if (Center.X > actor.Position.X)
                _westSideDanger = true;
            else if (Center.X < actor.Position.X)
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        foreach (var s in actor.Statuses)
        {
            ...
            {
                if (_eastSideDanger)
                    hints.AddForbiddenZone(new SDRect(new WPos(Center.X + 20.5f, Center.Z), Center, 20f));
                else if (_westSideDanger)
                    hints.AddForbiddenZone(new SDRect(Center, new WPos(Center.X - 20.5f, Center.Z), 20f));
            }
        }
    }
}
...
sealed class Antitwilight(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Antitwilight);

sealed class Lunacy(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.Lunacy, 6f, 8, 8);

sealed class BrightDarkBlade(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.BrightBlade, (uint)AID.DarkBlade], new AOEShapeCone(70f, 105f.Degrees()));

sealed class DanceOfTheDead(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.DanceOfTheDead1);

sealed class ToAshes(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.ToAshes);

[SkipLocalsInit]
sealed class TsukuyomiStates : StateMachineBuilder
{
    public TsukuyomiStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 537u, CFCID = 537u, NameID = 7225u, PrimaryActorOID = (uint)OID.Tsukuyomi, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "wen (ported from BMR)")]
[SkipLocalsInit]
public sealed class Tsukuyomi(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(100f, 100f), new ArenaBoundsCircle(20f));
