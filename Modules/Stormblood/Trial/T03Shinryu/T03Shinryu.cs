...
namespace Minerva.Stormblood.Trial.T03Shinryu;
 ...
 */
sealed class TidalWave(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.TidalWave, 35f, kind: Kind.DirForward)
{
 ...
 */
sealed class Levinbolt(ModuleBase module) : Components.SpreadFromIcon(module, (uint)IconID.LevinMarker, (uint)AID.Levinbolt, 5f, 6f);
...
sealed class WaterPuddles(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        {
            _levinCasting = true;
            _activation = World.FutureTime(6d);
        }

        else if (spell.Action.ID == (uint)AID.HellfireVisual)
        {
            _fireCasting = true;
            _activation = World.FutureTime(10d);
        }
    }
...
// Big bait away cones.
sealed class EarthBreathBait(ModuleBase module) : Components.BaitAwayIcon(module, new AOEShapeCone(60f, 30f.Degrees()), (uint)IconID.EarthBreathIcon, (uint)AID.EarthBreath);

sealed class EarthBreathAOE(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.EarthBreath1, new AOEShapeCone(60f, 30f.Degrees()));

sealed class IcicleAdds(ModuleBase module) : Components.AddsMulti(module, [(uint)OID.Icicle]);

// Icicles land and do circle aoe
sealed class IcicleSummon(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        {
            _aoes.Add(new(circle, caster.Position.Quantized(), default, World.FutureTime(0d)));
        }
    }
 ...
 */
sealed class IcicleThrustAOE(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Spikesicle, new AOEShapeRect(64.5f, 5f), maxCasts: 3);

sealed class IcicleThrustKnockback(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.Spikesicle, 10f, shape: new AOEShapeRect(64.5f, 5f), kind: Kind.DirForward, maxCasts: 3);

/*
 * Initial akh morn stack to share damage
 */
sealed class AkhMornStack(ModuleBase module) : Components.CastSharedTankbuster(module, (uint)AID.AkhMornVisual, 5f);

sealed class BurningChains(ModuleBase module) : Components.StretchTetherSingle(module, (uint)TetherID.BurningTether, 30f, null, (uint)AID.BurningChains);
...
sealed class MeteorImpact(ModuleBase module) : Components.ProximityAOEs(module, (uint)AID.MeteorImpact, 14f);

sealed class Cocoons(ModuleBase module) : Components.AddsMulti(module, [(uint)OID.Cocoon1, (uint)OID.MassiveCocoon]);

/*
 * Summons all the little dragons
 */
sealed class DragonAdds(ModuleBase module) : Components.AddsMulti(module, [(uint)OID.Ginryu, (uint)OID.Hakkinryu], priority: 1);
sealed class Fireball(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Fireball, 4f);
sealed class BlazingTrail(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BlazingTrail, new AOEShapeRect(15f, 5.5f));

// Use fetters as the time to update the arena.
sealed class Fetters(ModuleBase module) : ModuleComponent(module)
{
    public override void OnStatusLose(Actor actor, ref ActorStatus status)
    {
        if (status.ID == (uint)SID.Fetters && Raid.FindSlot(actor.InstanceID) is var slot && slot >= 0)
        {
            Bounds = new ArenaBoundsSquare(30f);
        }
    }
}
 ...
 */
// Phase 3
sealed class GyreCharge(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GyreChargeVisual, new AOEShapeRect(100f, 30f));
 ...
 */
sealed class TailSlap(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.TailSlap1, (uint)AID.TailSlap2], new AOEShapeRect(40f, 10f))
{
        ...
        switch (state)
        {
            ...
            case 0x00040008u: // Initial state
                if (_active.Count < 1)
                    ...
                    _active.Add(new Square(Center, 30f));
                break;
            case 0x00400080u: // Broken platform state.
                ...
                if (actor.OID == (uint)OID.Platform && !actor.Position.AlmostEqual(default, 1f))
                {
                    ...
                    Bounds = tailSlapArena;
                    Center = tailSlapArena.Center;
                }
                break;
        }
    }
...
sealed class Raidwides(ModuleBase module) : Components.RaidwideCasts(module, [(uint)AID.IceStormRaidwide, (uint)AID.Protostar, (uint)AID.DarkMatter, (uint)AID.JudgmentBolt, (uint)AID.EarthenFury]);

// Shinryu slaps fist down in center tile of arena.
sealed class Dragonfist(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DragonfistVisual, 16f);

/*
 * Diamond dust gives players thin ice for some seconds.  They slide around while under status.
 */
sealed class ThinIce(ModuleBase module) : Components.ThinIce(module, 30f, stopAtWall: false);

sealed class EarthenFurySmash(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.EarthenFurySmash, new AOEShapeRect(20f, 10f));

sealed class SuperCyclone(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.SuperCycloneKB, 5f)
{
    // Add a safe zone at knockback origin so ai knows where to run.
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
...
sealed class AerialBlast(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.AerialBlastKB, 15f)
{
    // Add a safe zone at knockback origin so ai knows where to run.
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
...
sealed class Hypernova(ModuleBase module) : Components.StackWithIcon(module, (uint)IconID.HyperNovaStackIcon, (uint)AID.Hypernova, 7f, 6d);

[SkipLocalsInit]
sealed class T03ShinryuStates : StateMachineBuilder
{
    public T03ShinryuStates(ModuleBase module) : base(module)
    {
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 239u, CFCID = 239u, NameID = 5640u, PrimaryActorOID = (uint)OID.Shinryu, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "wen (ported from BMR)")]

[SkipLocalsInit]
public sealed class T03Shinryu(WorldState ws, Actor primary) : ModuleBase(ws, primary, default, new ArenaBoundsSquare(20f, mapResolution: 0.3f));
