...
sealed class UWUStates : StateMachineBuilder
{
    ...
    public UWUStates(UWU module) : base(module)
    {
        ...
        SimplePhase(2u, Phase3Titan, "P3: Titan")
            ...
            .OnExit(() => module.Bounds = new ArenaBoundsCustom([new Polygon(UWU.ArenaCenter, 20f, 64)]))
    ...
    private void P1AwakenedWickedWheelDownburst(uint id, float delay)
    {
        ActorCast(id, _module.Garuda, (uint)AID.WickedWheel, delay, 3, true, "Out")
            .ActivateOnEnter<P1WickedWheel>();
        ComponentCondition<P1WickedWheel>(id + 2, 2.1f, comp => comp.NumCasts >= 2 || comp.Sources.Count == 0 && _module.World.CurrentTime >= comp.AwakenedResolve, "In") // complicated condition handles fucked up awakening
            .DeactivateOnExit<P1WickedWheel>();
    ...
    private State P3LandslideAwakened(uint id, float delay)
    {
        ActorCastMulti(id, _module.Titan, [(uint)AID.LandslideBoss, (uint)AID.LandslideBossAwakened], delay, 2.2f, true, "Landslide (awakened)")
            .ActivateOnEnter<P3Landslide>();
        return ComponentCondition<P3Landslide>(id + 0x10, 2, comp => comp.Awakened ? comp.NumCasts >= 10 : (comp.NumCasts >= 5 && Module.World.CurrentTime >= comp.PredictedActivation), "Landslide second hit")
            .DeactivateOnExit<P3Landslide>();
    }
