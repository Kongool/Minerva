...
class Landslide(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnCastFinished(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID is (uint)AID.LandslideBoss or (uint)AID.LandslideBossAwakened or (uint)AID.LandslideHelper or (uint)AID.LandslideHelperAwakened or (uint)AID.LandslideUltima or (uint)AID.LandslideUltimaHelper)
        {
            ...
            if (spell.Action.ID == (uint)AID.LandslideBoss)
                PredictedActivation = World.FutureTime(2d); // used if boss wasn't awakened when it should've been
        }
    }
}

class P3Landslide(ModuleBase module) : Landslide(module);

class P4Landslide(ModuleBase module) : Landslide(module)
{
    public override void OnActorPlayActionTimelineEvent(Actor actor, ushort id)
    {
        if (actor.OID == (uint)OID.Titan && id == 0x1E43)
        {
            ...
            PredictedActivation = World.FutureTime(8.1d);
        }
    }
}
