...
class WickedWheel(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        switch (spell.Action.ID)
        {
            case (uint)AID.WickedWheel:
                ...
                AwakenedResolve = World.FutureTime(2.1f); // for tornado
...
class P1WickedWheel(ModuleBase module) : WickedWheel(module);

class P4WickedWheel(ModuleBase module) : WickedWheel(module)
{
    public override void OnActorPlayActionTimelineEvent(Actor actor, ushort id)
    {
        if (actor.OID == (uint)OID.Garuda && id == 0x1E43)
            Sources.Add((actor, ShapeCombined, World.FutureTime(8.1d)));
    }
}
