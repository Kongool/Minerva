...
class P3EarthShaker(ModuleBase module) : Components.GenericBaitAway(module, (uint)AID.EarthShakerAOE)
{
...
class P3EarthShakerVoidzone(ModuleBase module) : Components.GenericAOEs(module, default, "GTFO from voidzone!")
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        if (spell.Action.ID == (uint)AID.EarthShaker)
            foreach (var (_, p) in Raid.WithSlot(false, true, true).IncludedInMask(_targets))
                _predicted.Add(new(_shape, p.Position, default, World.FutureTime(1.4d)));
    }
}
