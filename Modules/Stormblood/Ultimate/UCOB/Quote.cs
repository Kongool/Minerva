...
class Quote(ModuleBase module) : ModuleComponent(module)
{
    public Actor? Source;
    ...
    public override void OnActorNpcYell(Actor actor, ushort id)
    {
        ...
        if (aids.Count > 0)
        {
            ...
            NextActivation = World.FutureTime(5.1d);
        }
    }
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        if (PendingMechanics.Count != 0 && spell.Action.ID == PendingMechanics[0])
        {
            ...
            NextActivation = World.FutureTime(3.1d);
        }
    }
}

class QuoteIronChariotLunarDynamo(ModuleBase module) : Components.GenericAOEs(module)
{
...
class QuoteThermionicBeam(ModuleBase module) : Components.UniformStackSpread(module, 4f, default, 8, 8)
{
    private readonly Quote? _quote = module.FindComponent<Quote>();
...
class QuoteRavenDive(ModuleBase module) : Components.UniformStackSpread(module, default, 3f)
{
    private readonly Quote? _quote = module.FindComponent<Quote>();
...
class QuoteMeteorStream(ModuleBase module) : Components.UniformStackSpread(module, default, 4f)
{
    private readonly Quote? _quote = module.FindComponent<Quote>();
...
class QuoteDalamudDive(ModuleBase module) : Components.GenericBaitAway(module, (uint)AID.DalamudDive, true, true)
{
    private readonly Quote? _quote = module.FindComponent<Quote>();
    ...
    public override void Update()
    {
        ...
        if (imminent && CurrentBaits.Count == 0 && Module.Enemies((uint)OID.NaelDeusDarnus)[0] is var source && World.Actors.Find(source?.TargetID ?? 0) is var target && target != null)
