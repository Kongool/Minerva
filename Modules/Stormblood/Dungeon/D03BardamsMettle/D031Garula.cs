...
namespace Minerva.Stormblood.Dungeon.D03BardamsMettle.D031Garula;
...
class CrumblingCrust(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.CrumblingCrust, 3f);
class Heave(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Heave, new AOEShapeCone(13f, 60f.Degrees()));
class WideBlaster(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WideBlaster, new AOEShapeCone(29.15f, 60f.Degrees()));
class Rush(ModuleBase module) : Components.BaitAwayChargeTether(module, 4, 10.1f, (uint)AID.Rush, minimumDistance: 23f)
{
...
class RushYamaa(ModuleBase module) : Components.ChargeAOEs(module, (uint)AID.RushYamaa, 4f);
class Lullaby(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Lullaby, 3.7f);

class D031GarulaStates : StateMachineBuilder
{
    public D031GarulaStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 240u, NameID = 6173u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D031Garula(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
