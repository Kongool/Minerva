...
namespace Minerva.Stormblood.Dungeon.D03BardamsMettle.D032HunterOfBardam;
...
class CometChase(ModuleBase module) : Components.StandardChasingAOEs(module, 4f, (uint)AID.CometFirst, (uint)AID.CometRest, 10f, 1.5d, 9, true, (uint)IconID.ChasingAOE)
{
...
class CometAOE(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.CometFirst, (uint)AID.CometRest], 4f);

class MeteorImpact(ModuleBase module) : Components.CastLineOfSightAOE(module, (uint)AID.MeteorImpact, 50f, safeInsideHitbox: false)
{
...
class Charge(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Charge, new AOEShapeRect(46.25f, 2.5f));
class EmptyGaze(ModuleBase module) : Components.CastGaze(module, (uint)AID.EmptyGaze);
class Sacrifice(ModuleBase module) : Components.CastTowers(module, (uint)AID.Sacrifice, 3f);
class Reconstruct(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Reconstruct, 5f);
class CometImpact(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.CometImpact, 9f);
class BardamsRing(ModuleBase module) : Components.DonutStack(module, (uint)AID.BardamsRing, (uint)IconID.BardamsRing, 10f, 20f, 3.5f, 4, 4);

class Tremblor(ModuleBase module) : Components.ConcentricAOEs(module, _shapes)
{
...
class TremblorFinal(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Tremblor2, new AOEShapeDonut(10f, 20f))
{
...
class HeavyStrike(ModuleBase module) : Components.ConcentricAOEs(module, _shapes)
{
    ...
    public override void OnCastFinished(Actor caster, ActorCastInfo spell)
    {
        if (Sequences.Count > 0)
        {
            ...
            AdvanceSequence(order, spell.LocXZ, World.FutureTime(1.3d), spell.Rotation);
        }
    }
}
...
{
    public D032HunterOfBardamStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 240u, NameID = 6180u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D032HunterOfBardam(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
