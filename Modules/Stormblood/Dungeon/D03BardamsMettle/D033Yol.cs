...
namespace Minerva.Stormblood.Dungeon.D03BardamsMettle.D033Yol;
...
class Flutterfall(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Flutterfall, 6f);
class Pinion(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Pinion, new AOEShapeRect(40.5f, 1f));
class EyeOfTheFierce(ModuleBase module) : Components.CastGaze(module, (uint)AID.EyeOfTheFierce);
class WindUnbound(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.WindUnbound);
class Wingbeat(ModuleBase module) : Components.BaitAwayIcon(module, new AOEShapeCone(42.8f, 45f.Degrees()), (uint)IconID.Wingbeat, (uint)AID.Wingbeat);
class FlutterfallSpread(ModuleBase module) : Components.SpreadFromIcon(module, (uint)IconID.Flutterfall, (uint)AID.FlutterfallSpread, 6f, 5.4f);

class FeatherSquall(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorPlayActionTimelineEvent(Actor actor, ushort id)
    {
        ...
        {
            _aoe = [new(rect, actor.Position.Quantized(), actor.Rotation, World.FutureTime(6.7d))];
        }
    }
...
{
    public D033YolStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 240u, NameID = 6155u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D033Yol(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
