...
namespace Minerva.Stormblood.Dungeon.D13TheBurn.D133MistDragon;
...
sealed class FogPlumeCross(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class FogPlumeCircle(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FogPlumeCircle, 6f);

sealed class ColdFog(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        ...
        {
            _aoe = [new(new AOEShapeCircle(4f + status.Extra), Center.Quantized(), default, activation)];
        }
    }
    ...
    public override void Update()
    {
        if (_aoe.Length != 0 && !reset)
        {
            ...
            aoe.Activation = World.FutureTime(5.6d);
            reset = true;
        }
    }
}

abstract class BaitAway(ModuleBase module) : Components.GenericBaitAway(module)
{
...
sealed class ChillingAspiration(ModuleBase module) : BaitAway(module)
{
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        if (iconID == (uint)IconID.BaitawayRect)
        {
            CurrentBaits.Add(new(Module.PrimaryActor, actor, rect, Module.World.FutureTime(6.1d)));
        }
    }
...
sealed class FrostBreath(ModuleBase module) : BaitAway(module)
{
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        if (iconID == (uint)IconID.BaitawayCone)
        {
            CurrentBaits.Add(new(Module.PrimaryActor, actor, cone, Module.World.FutureTime(4.1d)));
        }
    }
...
sealed class RimeWreath(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.RimeWreath);
sealed class TouchDown(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Touchdown, 15f);
sealed class IceVoidzone(ModuleBase module) : Components.Voidzone(module, 6f, GetVoidzones)
{
...
sealed class Cauterize(ModuleBase module) : Components.GenericAOEs(module)
{
    private AOEInstance[] _aoe = [];
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        {
            _aoe = [new(rect, actor.Position.Quantized(), actor.Rotation, World.FutureTime(7.3d))];
        }
    }
...
{
    public D133MistDragonStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 585u, CFCID = 585u, NameID = 7672u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D133MistDragon(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-300f, -400f), new ArenaBoundsCircle(19.5f))
{
