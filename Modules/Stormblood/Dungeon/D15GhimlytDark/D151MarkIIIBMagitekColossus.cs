...
namespace Minerva.Stormblood.Dungeon.D15TheGhimlytDark.D151MarkIIIBMagitekColossus;
...
class MagitektSlashRotation(ModuleBase module) : Components.GenericRotatingAOE(module)
{
...
class MagitektSlashVoidzone(ModuleBase module) : Components.GenericAOEs(module)
{
...
class JarringBlow(ModuleBase module) : Components.SingleTargetDelayableCast(module, (uint)AID.JarringBlow);
class Exhaust(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Exhaust, new AOEShapeRect(43.5f, 5f));
class WildFireBeam(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.WildFireBeam, 6f);
class MagitekRay(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.MagitekRay, 6f, 4, 4);
...
{
    public D151MarkIIIBMagitekColossusStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 611u, NameID = 7855u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D151MarkIIIBMagitekColossus(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
