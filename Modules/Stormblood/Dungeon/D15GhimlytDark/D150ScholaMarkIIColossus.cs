...
namespace Minerva.Stormblood.Dungeon.D15TheGhimlytDark.D150ScholaMarkIIColossus;
...
class MagitekMissile(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MagitekMissile, 15f);
class Exhaust(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Exhaust, new AOEShapeRect(43.2f, 5f));
class GrandSword(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GrandSword, new AOEShapeCone(18.4f, 60f.Degrees()));
class SelfDetonate(ModuleBase module) : Components.CastHint(module, (uint)AID.SelfDetonate, "Enrage!", true);

class UnbreakableCermetBlade(ModuleBase module) : Components.GenericAOEs(module)
{
...
{
    public D0150ScholaMarkIIColossusStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 611u, NameID = 7888u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D0150ScholaMarkIIColossus(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
