...
namespace Minerva.Stormblood.Dungeon.D15TheGhimlytDark.D153SoranusDuo;
...
class Innocence(ModuleBase module) : Components.SingleTargetDelayableCast(module, (uint)AID.Innocence);
class DeltaTrance(ModuleBase module) : Components.SingleTargetDelayableCast(module, (uint)AID.DeltaTrance);
class Heirsbane(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.Heirsbane, "Single target damage");

class ArtificialPlasma(ModuleBase module) : Components.RaidwideCasts(module, [(uint)AID.ArtificialPlasmaAnnia, (uint)AID.ArtificialPlasmaJulia]);

class Crossbones(ModuleBase module) : Components.BaitAwayChargeCast(module, (uint)AID.Crossbones, 2f);

class CrossbonesKB(ModuleBase module) : Components.GenericKnockback(module, stopAtWall: true)
{
...
class AngrySalamander(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AngrySalamander, new AOEShapeRect(45.6f, 3f));
class Quaternity1(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Quaternity1, new AOEShapeRect(41f, 2f));
class Quaternity2(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Quaternity2, new AOEShapeRect(26f, 2f));
class StunningSweep(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.StunningSweep, 6.6f);
class Bombardment(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Bombardment, 10f);
class MagitekMissile(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MissileImpact, 6f);
class CoveringFire(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.CoveringFire, 8f);

class CeruleumTanks(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        {
            _aoes.Add(new(circleRoundhouse, new WPos(370.992f, -265.034f).Quantized(), default, World.FutureTime(3.1d)));
            for (var i = 0; i < 8; ++i)
            {
                ...
                _aoes.Add(new(circle, origins[i].Quantized(), default, World.FutureTime(activation)));
            }
        }
    }
...
class Crosshatch(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
...
class ImperialAuthority(ModuleBase module) : Components.CastHints(module, [(uint)AID.ImperialAuthorityAnnia, (uint)AID.ImperialAuthorityJulia], "Enrage!", true);

class D153SoranusDuoStates : StateMachineBuilder
{
    public D153SoranusDuoStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 611u, NameID = 7861u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D153SoranusDuo(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
