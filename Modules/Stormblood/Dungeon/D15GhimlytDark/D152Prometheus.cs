...
namespace Minerva.Stormblood.Dungeon.D15TheGhimlytDark.D152Prometheus;
...
sealed class NitrospinArenaChange(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        if (spell.Action.ID == (uint)AID.Nitrospin && Bounds.Radius > 21f)
        {
            var center = Center;
...
sealed class Heat(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorEAnim(Actor actor, uint state)
    {
        if (state == 0x00040008u && actor.OID == (uint)OID.TunnelHelper)
        {
            ...
            if (posrot != default)
            {
                ...
                var rot = posrot.rot;
                _aoe = [new(rect, pos, rot, World.FutureTime(6.7d), shapeDistance: rect.Distance(pos, rot))];
            }
        }
    }
...
sealed class Nitrospin(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Nitrospin);
sealed class UnbreakableCermetDrill(ModuleBase module) : Components.SingleTargetDelayableCast(module, (uint)AID.UnbreakableCermetDrill);
sealed class OilShower(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.OilShower, new AOEShapeCone(47.8f, 135f.Degrees()));
sealed class NeedleGun(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.NeedleGun, new AOEShapeCone(47.8f, 45f.Degrees()));
sealed class FreezingMissile(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FreezingMissile, 8f);

sealed class D152PrometheusStates : StateMachineBuilder
{
    public D152PrometheusStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 611u, CFCID = 611u, NameID = 7856u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D152Prometheus : ModuleBase
{
