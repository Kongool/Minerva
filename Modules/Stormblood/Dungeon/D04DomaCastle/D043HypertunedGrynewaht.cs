...
namespace Minerva.Stormblood.Dungeon.D04DomaCastle.D043HypertunedGrynewaht;
...
class CleanCut(ModuleBase module) : Components.ChargeAOEs(module, (uint)AID.CleanCut, 4f);
class DelayActionCharge(ModuleBase module) : Components.SpreadFromIcon(module, (uint)IconID.Spreadmarker, (uint)AID.DelayActionCharge, 6f, 4);
class ThermobaricCharge(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ThermobaricCharge, 30f);

class Chainsaw(ModuleBase module) : Components.GenericAOEs(module)
{
...
class ThermobaricChargeBait(ModuleBase module) : Components.GenericBaitAway(module, centerAtTarget: true)
{
    ...
    {
        if (CurrentBaits.Count != 0 && CurrentBaits.Ref(0).Target == actor)
            hints.AddForbiddenZone(new SDCircle(Center, 23f));
    }
}

class Gunsaw(ModuleBase module) : Components.GenericBaitAway(module)
{
...
{
    public D043HypertunedGrynewahtStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 241u, NameID = 6205u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D043HypertunedGrynewaht(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-240, -198), new ArenaBoundsSquare(19.5f));
