...
namespace Minerva.Stormblood.Dungeon.D02ShisuiOfTheVioletTides.D023ShisuiYohi;
...
class BlackTide(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        if (spell.Action.ID == (uint)AID.WaterSplashVisual)
        {
            ++NumCasts;
            helper = caster;
            activation = World.FutureTime(4f); // seen activation times from 4-6s
        }
...
class BiteAndRun1(ModuleBase module) : Components.BaitAwayChargeCast(module, (uint)AID.BiteAndRun1, 2.5f);
class BiteAndRun2(ModuleBase module) : Components.BaitAwayChargeCast(module, (uint)AID.BiteAndRun2, 2.5f);
class MadStare(ModuleBase module) : Components.CastGaze(module, (uint)AID.MadStare);
class ThickFog(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.ThickFog);

class D023ShisuiYohiStates : StateMachineBuilder
{
    public D023ShisuiYohiStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 235u, NameID = 6243u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D023ShisuiYohi(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
