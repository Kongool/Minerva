...
namespace Minerva.Stormblood.Dungeon.D02ShisuiOfTheVioletTides.D022RubyPrincess;
...
class SeduceOld(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly AOEShapeCircle circle = new(2.5f);
    private bool active;
    private readonly List<Actor> chests = [];
    ...
    public override void Update()
    {
        if (closedAOE == null)
        {
            ...
            closedAOE = new AOEShapeCustom(Center, [.. closedChests]);
        }
    }
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        ...
        if (closedAOE is AOEShapeCustom aoe)
        {
            ...
            aoes[count] = new(aoe, Center, color: isold || !active ? default : Colors.SafeFromAOE);
        }
        return aoes;
    }
...
sealed class SeduceCoriolisKick(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class AbyssalVolcano(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AbyssalVolcano, 7f);

sealed class GeothermalFlatulence(ModuleBase module) : Components.StandardChasingAOEs(module, 4f, (uint)AID.GeothermalFlatulenceFirst, (uint)AID.GeothermalFlatulenceRest, 3, 0.8d, 10, true, (uint)IconID.ChasingAOE)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (TargetsMask[slot])
        {
            hints.AddForbiddenZone(new SDCircle(Center, 18f), Activation);
        }
    }
}

sealed class Tornadogenesis(ModuleBase module) : Components.Cleave(module, (uint)AID.Tornadogenesis, new AOEShapeCone(9.6f, 60f.Degrees()))
{
...
{
    public D022RubyPrincessStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 235u, CFCID = 235u, NameID = 6241u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D022RubyPrincess : ModuleBase
{
