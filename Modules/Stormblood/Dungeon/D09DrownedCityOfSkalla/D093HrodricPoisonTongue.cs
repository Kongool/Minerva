...
namespace Minerva.Stormblood.Dungeon.D09DrownedCityOfSkalla.D093HrodricPoisonTongue;
 ...
 */
sealed class ChaosBaits(ModuleBase module) : Components.GenericBaitAway(module)
{
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        DateTime _nextActivation = World.FutureTime(4.7f);
        // next target is the pc with the icon.
        Actor? _nextTarget = World.Actors.Find(targetID);
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        foreach (var bait in CurrentBaits)
        {
            ...
            if (bait.Source == actor && (bait.Shape is AOEShapeCircle or AOEShapeCross))
                hints.AddForbiddenZone(new AOEShapeCircle(16f), Center, default, bait.Activation);
            // bait the donut to the center.
            else if (bait.Source == actor && bait.Shape is AOEShapeDonut)
                hints.AddForbiddenZone(new AOEShapeCircle(6f, true), Center, default, bait.Activation);
        }
...
sealed class WordsOfWoe(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WordsOfWoe, new AOEShapeRect(65f, 3f));

[SkipLocalsInit]
sealed class D093HrodricPoisonTongueStates : StateMachineBuilder
{
    public D093HrodricPoisonTongueStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 279u, CFCID = 279u, NameID = 6910u, PrimaryActorOID = (uint)OID.HrodricPoisontongue, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = " (ported from BMR)")]
[SkipLocalsInit]
public sealed class D093HrodricPoisonTongue(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(479f, 4f), new ArenaBoundsCircle(20f));
