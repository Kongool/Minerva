...
namespace Minerva.RealmReborn.Dungeon.D06Haukke.D062ManorSteward;
...
class SoulDrain(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SoulDrain, new AOEShapeCircle(9f));

class D062ManorStewardStates : StateMachineBuilder
{
    public D062ManorStewardStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 6u, NameID = 424u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Chuggalo (ported from BMR)")]
public class D062ManorSteward(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
