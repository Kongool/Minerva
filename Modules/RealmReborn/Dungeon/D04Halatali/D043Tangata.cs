...
namespace Minerva.RealmReborn.Dungeon.D04Halatali.D043Tangata;
...
class StraightPunch(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.StraightPunch);
class Firewater(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Firewater, 3f);

class PlainPound(ModuleBase module) : Components.ConcentricAOEs(module, _shapes)
{
...
{
    public D043TangataStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 7u, NameID = 1194u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D043Tangata(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
    ...
    {
        Arena.Actors(World.Actors.Where(x => !x.IsAlly && x.FindStatus(SID.BurningWard) == null && x.Position.AlmostEqual(Center, Bounds.Radius)));
    }
