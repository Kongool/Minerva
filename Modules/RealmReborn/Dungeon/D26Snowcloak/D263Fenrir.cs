...
namespace Minerva.RealmReborn.Dungeon.D26Snowcloak.D263Fenrir;
...
sealed class PillarImpact(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PillarImpact, 4f);

sealed class PillarShatter1(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PillarShatter1, 8f);
sealed class PillarShatter2(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnCastFinished(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID == (uint)AID.LunarCry)
        {
            ...
            for (var i = 0; i < count; ++i)
            {
                ...
                {
                    _aoes.Add(new(circle, p.Position.Quantized(), default, World.FutureTime(4.5d)));
                }
            }
        }
...
sealed class HeavenswardRoar(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HeavenswardRoar, new AOEShapeCone(50f, 30f.Degrees()));
sealed class EclipticBite(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.EclipticBite);
sealed class ThousandYearStorm(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.ThousandYearStorm);

sealed class D263FenrirStates : StateMachineBuilder
{
    public D263FenrirStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 27u, CFCID = 27u, NameID = 3044u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D263Fenrir(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
