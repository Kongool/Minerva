...
namespace Minerva.RealmReborn.Dungeon.D26Snowcloak.D262Yeti;
...
sealed class FrozenSpike(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.FrozenSpike, 5f);
sealed class HeavySnow(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HeavySnow, 15f);
sealed class LightSnow(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.LightSnow, 2f);
sealed class Buffet(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Buffet, new AOEShapeCone(12f, 60f.Degrees()));

sealed class SpinFrozenCircle(ModuleBase module) : Components.ConcentricAOEs(module, _shapes)
{
...
sealed class Northerlies(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Northerlies);

sealed class D262YetiStates : StateMachineBuilder
{
    public D262YetiStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 27u, CFCID = 27u, NameID = 3040u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D262Yeti(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
