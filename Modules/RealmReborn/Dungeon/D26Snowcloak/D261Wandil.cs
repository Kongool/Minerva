...
namespace Minerva.RealmReborn.Dungeon.D26Snowcloak.D261Wandil;
...
sealed class TundraArenaChange(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class IceGuillotine(ModuleBase module) : Components.Cleave(module, (uint)AID.IceGuillotine, new AOEShapeCone(11.23f, 60f.Degrees()), activeWhileCasting: false);
sealed class SnowDriftRaidwide(ModuleBase module) : Components.RaidwideCastDelay(module, (uint)AID.SnowDriftVisual, (uint)AID.SnowDrift, 2d);
sealed class SnowDriftMove(ModuleBase module) : Components.StayMove(module, 3d)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
...
sealed class ColdWave(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ColdWave, 8f);

sealed class D261WandilStates : StateMachineBuilder
{
    public D261WandilStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 27u, CFCID = 27u, NameID = 3038u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D261Wandil : ModuleBase
{
