...
namespace Minerva.RealmReborn.Dungeon.D26Snowcloak.D260NorthernBateleur;
...
sealed class WallRemoval(ModuleBase module) : ModuleComponent(module)
{
    public override void OnActorEAnim(Actor actor, uint state)
    {
        if (state == 0x00040008u && actor.OID == (uint)OID.WallController1)
        {
            Bounds = D260NorthernBateleur.Arena2;
            Center = D260NorthernBateleur.Arena2.Center;
        }
    }
}

sealed class Frostbite(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorEAnim(Actor actor, uint state)
    {
        ...
        else if (state == 0x00010002u)
        {
            ...
            for (var i = 0; i < count; ++i)
            {
                ...
                if (aoe.ActorID == id)
                {
                    aoe.Activation = World.FutureTime(5d);
    ...
    public override void AddHints(int slot, Actor actor, TextHints hints)
    {
        ...
        for (var i = 0; i < count; ++i)
        {
            ...
            if (aoe.Check(actor.Position))
            {
                ...
                {
                    hints.Add("Leave unsafe area!" + $" {(aoe.Activation - World.CurrentTime).TotalSeconds:F1}s until activation.");
                }
                return;
            }
        }
...
sealed class WingCutter(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WingCutter, new AOEShapeCone(6.9f, 30f.Degrees()));
sealed class DoubleSmash(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DoubleSmash, new AOEShapeCone(7.69f, 60f.Degrees()));
sealed class SicklySneeze(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SicklySneeze, new AOEShapeCone(7.9f, 45f.Degrees()));

sealed class D260NorthernBateleurStates : StateMachineBuilder
{
    public D260NorthernBateleurStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 27u, CFCID = 27u, NameID = 3220u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D260NorthernBateleur(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena1.Center, arena1)
{
