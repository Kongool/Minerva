...
namespace Minerva.RealmReborn.Dungeon.D07Brayflox.D073Hellbender;
...
class BogBubble(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BogBubble, 6);

class PeculiarLight(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PeculiarLight, 8);

class StagnantSpray(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.StagnantSpray, new AOEShapeCone(8, 60.Degrees()));

class PlayerBound(ModuleBase module) : ModuleComponent(module)
{
...
{
    public D120HellbenderStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 8u, NameID = 1286u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Chuggalo (ported from BMR)")]
public class D120Hellbender(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
