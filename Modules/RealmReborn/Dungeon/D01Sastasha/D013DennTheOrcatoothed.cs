...
namespace Minerva.RealmReborn.Dungeon.D01Sastasha.D013DennTheOrcatoothed;
...
class Hydroball(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Hydroball, new AOEShapeCone(8f, 45f.Degrees()));

class TrashSpawning(ModuleBase module) : ModuleComponent(module)
{
...
{
    public D013DennTheOrcatoothedStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 4u, NameID = 1206u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Chuggalo (ported from BMR)")]
public class D013DennTheOrcatoothed(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
