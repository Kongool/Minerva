...
namespace Minerva.RealmReborn.Dungeon.D17PharosSirius.D173Tyrant;
...
sealed class AeroBlast(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.AeroBlast);

sealed class Bombination(ModuleBase module)
    : Components.SimpleAOEs(module, (uint)AID.Bombination, new AOEShapeCircle(6f));

sealed class SailorAdds(ModuleBase module) : Components.Adds(module, (uint)OID.ZombieSailor);

[SkipLocalsInit]
sealed class D173TyrantStates : StateMachineBuilder
{
    public D173TyrantStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 17u, NameID = 2264u, PrimaryActorOID = (uint)OID.Tyrant, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "wen (ported from BMR)")]
// (0, 140, 0) for hyperborea
[SkipLocalsInit]
public sealed class D173Tyrant : ModuleBase
{
