...
namespace Minerva.RealmReborn.Dungeon.D14Praetorium.D142Nero;
...
class IronUprising(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.IronUprising, new AOEShapeCone(7f, 60f.Degrees()));
class SpineShatter(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.SpineShatter);

class AugmentedSuffering(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.AugmentedSuffering, 12f)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (Casters.Count != 0)
        {
            hints.AddForbiddenZone(new SDInvertedCircle(Center, 8f), Casters.Ref(0).Activation);
        }
    }
}

class AugmentedShatter(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.AugmentedShatter, 6f, 4, 4);
class AugmentedUprising(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AugmentedUprising, new AOEShapeCone(45f, 45f.Degrees()));
class WheelOfSuffering(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WheelOfSuffering, 7f);
...
class ArenaChange(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        {
            Bounds = D142Nero.DefaultBounds;
            Center = D142Nero.DefaultBounds.Center;
            _aoe = [];
            begin = true;
        }
    }
    ...
    public override void Update()
    {
        ...
        {
            _aoe = [new(donut, Center, default, World.FutureTime(3.6d))];
        }
    }
}
...
{
    public D142NeroStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 16u, NameID = 2135u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
public class D142Nero(WorldState ws, Actor primary) : ModuleBase(ws, primary, startingBounds.Center, startingBounds)
{
