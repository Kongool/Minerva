...
namespace Minerva.RealmReborn.Dungeon.D16Amdapor.D163Anantaboga;
...
class TheLook(ModuleBase module) : Components.Cleave(module, (uint)AID.TheLook, new AOEShapeCone(11.5f, 45f.Degrees())); // TODO: verify angle
class RottenBreath(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.RottenBreath, new AOEShapeCone(11.5f, 45f.Degrees())); // TODO: verify angle
class TailDrive(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TailDrive, new AOEShapeCone(35.5f, 45f.Degrees()));

class ImminentCatastrophe(ModuleBase module) : Components.CastLineOfSightAOE(module, (uint)AID.ImminentCatastrophe, 100f, true)
{
...
class PlagueDance(ModuleBase module) : ModuleComponent(module)
{
...
class BubonicCloud(ModuleBase module) : Components.Voidzone(module, 11.5f, GetDarkNova)
{
    private static List<Actor> GetDarkNova(ModuleBase module) => module.Enemies((uint)OID.DarkNova);
}

class D163AnantabogaStates : StateMachineBuilder
{
    public D163AnantabogaStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 14u, NameID = 1696u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")]
public class D163Anantaboga(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(10f, default), new ArenaBoundsSquare(25))
{
