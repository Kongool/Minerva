...
namespace Minerva.RealmReborn.Dungeon.D09CuttersCry.D093Chimera;
...
sealed class TheLionsBreath(ModuleBase module) : Components.Cleave(module, (uint)AID.TheLionsBreath, new AOEShapeCone(9.7f, 60f.Degrees()));
sealed class TheRamsDragonsBreath(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.TheRamsBreath, (uint)AID.TheDragonsBreath], new AOEShapeCone(9.7f, 60f.Degrees()));
sealed class TheRamsVoice(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TheRamsVoice, 9.7f);
...
sealed class LightningStorm(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.LightningStorm, 5f);
...
sealed class Cacophony(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        {
            source = actor;
            _aoe = [new(circle, actor.Position, default, World.FutureTime(10.5d))];
            aoeInit = true;
        }
    }
...
{
    public D093ChimeraStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 12u, CFCID = 12u, NameID = 1590u, PrimaryActorOID = (uint)OID.Chimera, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D093Chimera(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
