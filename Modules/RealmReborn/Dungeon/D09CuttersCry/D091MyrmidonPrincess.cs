...
namespace Minerva.RealmReborn.Dungeon.D09CuttersCry.D091MyrmidonPrincess;
...
sealed class MandibleBite(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MandibleBite, new AOEShapeCone(9f, 45f.Degrees()));
sealed class StoneII(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.StoneII, 5f);
sealed class Silence(ModuleBase module) : Components.CastInterruptHint(module, (uint)AID.Silence, showNameInHint: true);

sealed class D091MyrmidonPrincessStates : StateMachineBuilder
{
    public D091MyrmidonPrincessStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 12u, CFCID = 12u, NameID = 1585u, PrimaryActorOID = (uint)OID.MyrmidonPrincess, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus), Chuggalo (ported from BMR)")]
public sealed class D091MyrmidonPrincess(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
