...
namespace Minerva.RealmReborn.Dungeon.D09CuttersCry.D092GiantTunnelWorm;
...
sealed class Sludge(ModuleBase module) : Components.CleansableDebuff(module, (uint)SID.Sludge, "Sludge", "muddied");

sealed class SandPillarEarthbreak(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        switch (spell.Action.ID)
        {
                ...
                if (count == 0)
                {
                    ...
                    for (var i = 0; i < 8; ++i)
                    {
                        ...
                        if (initialPosition.AlmostEqual(cPos, 1f))
                        {
                            ...
                            for (var j = 1; j < 8; ++j)
                            {
                                _aoes.Add(new(j != 7 ? circleSmall : circleBig, (initialPosition + j * dir).Quantized(), default, World.FutureTime(0.9d * j)));
                            }
                            return;
                        }
                    }
            ...
            case (uint)AID.BottomlessDesert:
                _aoes.Add(new(circleBig, caster.Position.Quantized(), default, World.FutureTime(3.1d)));
                break;
        }
    }
}
...
{
    public D092GiantTunnelWormStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 12u, CFCID = 12u, NameID = 1589u, PrimaryActorOID = (uint)OID.GiantTunnelWorm, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D092GiantTunnelWorm(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
