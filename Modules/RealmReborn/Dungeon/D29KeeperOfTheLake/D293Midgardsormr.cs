...
namespace Minerva.RealmReborn.Dungeon.D29TheKeeperoftheLake.D293Midgardsormr;
...
abstract class TurmoilInner(ModuleBase module, uint aid) : Components.SimpleAOEs(module, aid, 22f);
class InnerTurmoil(ModuleBase module) : TurmoilInner(module, (uint)AID.InnerTurmoil);
class PhantomInnerTurmoil(ModuleBase module) : TurmoilInner(module, (uint)AID.PhantomInnerTurmoil);

class PhantomOuterTurmoil(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PhantomOuterTurmoil, new AOEShapeDonutSector(20.5f, 39f, 90f.Degrees()));
class Animadversion(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Animadversion);
class Condescension(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.Condescension);
class Disgust(ModuleBase module) : Components.RaidwideCastDelay(module, (uint)AID.DisgustVisual, (uint)AID.Disgust, 0.5f);

abstract class Admonishments(ModuleBase module, uint aid) : Components.SimpleAOEs(module, aid, new AOEShapeRect(40f, 6f));
class Admonishment(ModuleBase module) : Admonishments(module, (uint)AID.Admonishment);
class PhantomAdmonishment(ModuleBase module) : Admonishments(module, (uint)AID.PhantomAdmonishment);

class MirageAdmonishment(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorPlayActionTimelineEvent(Actor actor, ushort id)
    {
        if (id == 0x11D3 && actor.OID is (uint)OID.MirageDragon1 or (uint)OID.MirageDragon2)
            _aoes.Add(new(rect, actor.Position.Quantized(), actor.Rotation, World.FutureTime(12d)));
    }
...
class Antipathy(ModuleBase module) : Components.ConcentricAOEs(module, _shapes)
{
...
{
    public D293MidgardsormrStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 32u, NameID = 3374u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D293Midgardsormr(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
