...
namespace Minerva.RealmReborn.Dungeon.D29TheKeeperoftheLake.D292MagitekGunship;
...
class GarleanFire(ModuleBase module) : Components.VoidzoneAtCastTarget(module, 8f, (uint)AID.GarleanFire, GetVoidzones, 0.2f)
{
...
class DrillCannons(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DrillCannons, new AOEShapeRect(32.8f, 2.5f));
class CarpetBomb(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.CarpetBomb, 5f);
class Overcharge(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Overcharge, new AOEShapeCone(10.8f, 60f.Degrees()));

class Flamethrower(ModuleBase module) : Components.GenericAOEs(module)
{
...
{
    public D292MagitekGunshipStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 32u, NameID = 3373u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D292MagitekGunship(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
