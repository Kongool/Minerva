...
namespace Minerva.RealmReborn.Dungeon.D02TamTaraDeepcroft.D021VoidSoulcounter;
...
class Condemnation(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Condemnation, new AOEShapeCone(7f, 45f.Degrees()));

class D021VoidSoulcounterStates : StateMachineBuilder
{
    public D021VoidSoulcounterStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 2u, NameID = 455u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Chuggalo (ported from BMR)")]
public class D021VoidSoulcounter(WorldState ws, Actor primary) : ModuleBase(ws, primary, IsFirstArena(primary) ? FirstArena.Center : (IsSecondArena(primary) ? SecondArena.Center : ThirdArena.Center), IsFirstArena(primary) ? FirstArena : (IsSecondArena(primary) ? SecondArena : ThirdArena))
{
