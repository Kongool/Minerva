...
namespace Minerva.RealmReborn.Dungeon.D13CastrumMeridianum.D131BlackEft;
...
class IncendiarySupport(ModuleBase module) : Components.RaidwideCastDelay(module, (uint)AID.IncendiarySupportVisual, (uint)AID.IncendiarySupport, 1f, "Raidwide x3");
class HighPoweredMagitekRay(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HighPoweredMagitekRay, new AOEShapeRect(50f, 2f));
class MagitekCannon(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MagitekCannon, 6f);

class D131BlackEftStates : StateMachineBuilder
{
    public D131BlackEftStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 15u, NameID = 557u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
public class D131BlackEft(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
