...
namespace Minerva.RealmReborn.Raid.T01Caduceus;
...
class HoodSwing(ModuleBase module) : Components.Cleave(module, (uint)AID.HoodSwing, new AOEShapeCone(11, 60.Degrees())) // TODO: verify angle
{
    private DateTime _lastBossCast; // assume boss/add cleaves are synchronized?..
    public float SecondsUntilNextCast() => Math.Max(0, 18 - (float)(World.CurrentTime - _lastBossCast).TotalSeconds);
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        if (spell.Action.ID == WatchedAction && caster == Module.PrimaryActor)
            _lastBossCast = World.CurrentTime;
    }
}

class WhipBack(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WhipBack, new AOEShapeCone(9, 60.Degrees()));
class Regorge(ModuleBase module) : Components.VoidzoneAtCastTarget(module, 4, (uint)AID.Regorge, m => m.Enemies((uint)OID.Regorge).Where(z => z.EventState != 7), 2.1f);
class Syrup(ModuleBase module) : Components.VoidzoneAtCastTarget(module, 4, (uint)AID.Syrup, m => m.Enemies((uint)OID.Syrup).Where(z => z.EventState != 7), 0.3f);
...
class CloneMerge(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void Update()
    {
        ...
        if (Clone != null)
            CloneSpawnTime = World.CurrentTime;
    }
    ...
    public override void AddGlobalHints(GlobalHints hints)
    {
        ...
        if (clone != null && !Module.PrimaryActor.IsDestroyed && !Module.PrimaryActor.IsDead && Module.PrimaryActor.IsTargetable)
        {
            ...
            var checkIn = Math.Max(0, 20 - (World.CurrentTime - CloneSpawnTime).TotalSeconds);
...
{
    public T01CaduceusStates(ModuleBase module) : base(module)
    {
...
[ModuleInfo(CFCID = 93u, NameID = 1466u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")]
public class T01Caduceus : ModuleBase
{
