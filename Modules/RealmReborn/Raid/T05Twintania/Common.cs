...
{
    public Plummet(ModuleBase module) : base(module, (uint)AID.Plummet, new AOEShapeRect(20, 6)) // TODO: verify shape
    {
        NextExpected = World.FutureTime(6.5f);
    }

    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        base.AddAIHints(slot, actor, assignment, hints);
        if ((NextExpected - World.CurrentTime).TotalSeconds < 3)
        {
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        if (spell.Action.ID == WatchedAction)
            NextExpected = World.FutureTime(12.5f);
    }
}
...
class DeathSentence(ModuleBase module) : Components.CastCounter(module, (uint)AID.DeathSentence)
{
    public DateTime NextCastStart { get; private set; } = module.World.FutureTime(18);
    ...
    {
        hints.Add($"Next death sentence in ~{(NextCastStart - World.CurrentTime).TotalSeconds:f1}s");
    }
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if ((NextCastStart - World.CurrentTime).TotalSeconds < 1)
        {
            boss.AttackStrength += 0.3f;
        }
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID == WatchedAction)
        {
            NextCastStart = World.FutureTime(36d);
