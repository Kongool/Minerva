...
class AllaganRot(ModuleBase module) : ModuleComponent(module)
{
    ...
    {
        if (_rotHolderSlot >= 0)
            hints.Add($"Rot: {(_rotExpiration[_rotHolderSlot] - World.CurrentTime).TotalSeconds:f1}s");
    }
    ...
    private bool WantToPickUpRot(PartyRolesConfig.Assignment assignment)
    {
        ...
        if ((deadline - World.CurrentTime).TotalSeconds > 7)
