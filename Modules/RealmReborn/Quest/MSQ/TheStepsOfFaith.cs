...
namespace Minerva.RealmReborn.Quest.MSQ.TheStepsOfFaith;
...
class RipperClaw(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.RipperClaw, new AOEShapeCone(9f, 45f.Degrees()));
class Levinshower(ModuleBase module) : Components.Cleave(module, (uint)AID.Levinshower, new AOEShapeCone(6f, 60f.Degrees()),
[(uint)OID.HordeBiast2, (uint)OID.HordeBiast4, (uint)OID.HordeBiast5, (uint)OID.HordeBiast6, (uint)OID.HordeBiast7]);
class EarthShakerAOE(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.EarthshakerAOE, 31f);
class Earthshaker(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Earthshaker, new AOEShapeCone(80f, 15f.Degrees()), 2);

class EarthrisingAOE(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.EarthrisingAOE, 31f);
class Earthrising(ModuleBase module) : Components.Exaflare(module, 8f)
{
...
class SidewiseSlice(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SidewiseSlice, new AOEShapeCone(50f, 60f.Degrees()));

class FireballSpread(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.FireballSpread, 6f);
class FireballAOE(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FireballAOE, 6f);
class Flamisphere(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Flamisphere, 10f);

class BodySlam(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.BodySlam, 20f, kind: Kind.DirForward, stopAtWall: true);

class FlameBreath(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.FlameBreathChannel)
{
...
class FlameBreath2(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.FlameBreathChannel)
{
    private AOEInstance[] _aoe = [];
...
class Cauterize(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.Cauterize)
{
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        ...
        if (Center.Z > 218f)
            return new AOEInstance[1] { new(MoveIt, Center) };
        else
...
class Touchdown(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.Touchdown, 10f, stopAtWall: true);

class ScorchingBreath(ModuleBase module) : Components.GenericAOEs(module)
{
...
class ScrollingBounds(ModuleBase module) : ModuleComponent(module)
{
    public const float HalfHeight = 40f;
    public const float HalfWidth = 22f;

    // Not named Bounds: that is the module's live geometry, inherited from ModuleComponent, and this
    // is the fixed rect the scrolling window is cut from.
    public static readonly ArenaBoundsRect Shape = new(HalfWidth, HalfHeight);
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (Phase == 1 && !actor.InCombat && actor.FindStatus(1268u) == null)
            hints.AddForbiddenZone(new SDRect(Center, new WDir(default, 1f), 38f, 22f, 40f));
        // subsequent state transitions don't trigger until player moves into the area
            ...
            hints.AddForbiddenZone(new SDRect(Center, new WDir(default, 1f), 40f, 22f, 38f));
    }

    public override void Update()
    {
        if (World.Party.Player() is not Actor p)
            return;
        Center = new(0, Math.Clamp(p.PosRot.Z, ZBounds.Min + HalfHeight, ZBounds.Max - HalfHeight));
    }
}

class VishapStates : StateMachineBuilder
{
    public VishapStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.Quest, CFCID = 70127u, NameID = 3330u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")]
public class Vishap(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(default, 245f), ScrollingBounds.Shape)
{
