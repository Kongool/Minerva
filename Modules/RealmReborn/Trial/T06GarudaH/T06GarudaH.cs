...
namespace Minerva.RealmReborn.Trial.T06GarudaH;
...
class MistralShriek(ModuleBase module) : Components.CastLineOfSightAOE(module, (uint)AID.MistralShriek, 24.7f, true)
{
    public override ReadOnlySpan<Actor> BlockerActors() => CollectionsMarshal.AsSpan(((T06GarudaH)Module).ActiveMonoliths);
}

class MistralSong(ModuleBase module) : Components.CastLineOfSightAOE(module, (uint)AID.MistralSong, 31.7f, true)
{
    public override ReadOnlySpan<Actor> BlockerActors() => CollectionsMarshal.AsSpan(((T06GarudaH)Module).ActiveMonoliths);
}

class AerialBlast(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.AerialBlast);
class GreatWhirlwind(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GreatWhirlwind, 8f);
class EyeOfTheStorm(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.EyeOfTheStorm, new AOEShapeDonut(12f, 25f));

class T06GarudaHStates : StateMachineBuilder
{
    public T06GarudaHStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 61u, NameID = 1644u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")]
public class T06GarudaH : ModuleBase
{
