...
namespace Minerva.RealmReborn.Trial.T07TitanH;
...
class MountainBuster(ModuleBase module) : Components.Cleave(module, (uint)AID.MountainBuster, new AOEShapeCone(21.25f, 60f.Degrees())); // TODO: verify angle

class WeightOfTheLand(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WeightOfTheLandAOE, 6f);
class Landslide(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Landslide, new AOEShapeRect(40.25f, 3f));

class Geocrush(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.Geocrush)
{
    ...
    {
        if (_outer != null)
            return new AOEInstance[1] { new(_outer, Center) };
        if (_inner != null)
            return new AOEInstance[1] { new(_inner, Center, default, _innerFinish) };
        return [];
    }
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID == WatchedAction)
        {
            ...
            _outer = new AOEShapeDonut(outerRadius, Bounds.Radius);
...
class Burst(ModuleBase module) : Components.SimpleAOEGroupsByTimewindow(module, [(uint)AID.Burst], 6.3f, 2.25d);

class T07TitanHStates : StateMachineBuilder
{
    public T07TitanHStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 60u, NameID = 1801u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")]
public class T07TitanH : ModuleBase
{
