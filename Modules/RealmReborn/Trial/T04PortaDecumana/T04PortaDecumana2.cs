...
namespace Minerva.RealmReborn.Trial.T04PortaDecumana.Phase2;
...
sealed class TankPurge(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.TankPurge);
sealed class HomingLasers(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.HomingLasers);

sealed class MagitekRay(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.MagitekRayAOEForward, (uint)AID.MagitekRayAOERight,
(uint)AID.MagitekRayAOELeft], new AOEShapeRect(40f, 3f));

sealed class HomingRay(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.HomingRayAOE, 6f);
sealed class LaserFocus(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.LaserFocusAOE, 6f, 4, 4);

sealed class AethericBoom(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.AethericBoom, 30f, stopAtWall: true)
{
    ...
    {
        // BossmodReborn queued Arm's Length / Surecast here. Minerva executes no actions — it is a radar
        // and a dodge, not a rotation — so the mitigation half of this mechanic is the player's to press.
        _ = Casters.Count;
    }
}
...
sealed class AssaultCannon(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AssaultCannon, new AOEShapeRect(40f, 2f));
sealed class CitadelBuster(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.CitadelBuster, new AOEShapeRect(40f, 6f));

sealed class Explosion(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Explosion, 16f) // TODO: verify falloff
{
...
sealed class Ultima(ModuleBase module) : Components.CastHint(module, (uint)AID.Ultima, "Enrage!", true);

sealed class T04PortaDecumana2States : StateMachineBuilder
{
    public T04PortaDecumana2States(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 830u, CFCID = 830u, NameID = 2137u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
public sealed class T04PortaDecumana2(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-704f, 480f), new ArenaBoundsCircle(19.5f))
{
