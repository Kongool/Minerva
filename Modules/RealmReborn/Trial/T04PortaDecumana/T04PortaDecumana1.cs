...
namespace Minerva.RealmReborn.Trial.T04PortaDecumana.Phase1;
...
class EarthenFury(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.EarthenFuryAOE);
class Geocrush(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Geocrush, 25); // TODO: verify falloff...

abstract class Landslide(ModuleBase module, uint aid) : Components.SimpleAOEs(module, aid, new AOEShapeRect(40, 3));
class Landslide1(ModuleBase module) : Landslide(module, (uint)AID.Landslide1);
class Landslide2(ModuleBase module) : Landslide(module, (uint)AID.Landslide2);

class WeightOfTheLand(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WeightOfTheLand, 6);
class AerialBlast(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.AerialBlastAOE);
class EyeOfTheStorm(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.EyeOfTheStormAOE, new AOEShapeDonut(12.5f, 25));
class MistralShriek(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MistralShriek, 23);
class Hellfire(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.HellfireAOE);
class RadiantPlume(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.RadiantPlumeAOE, 8);

class VulcanBurst(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.VulcanBurst, 15, stopAtWall: true)
{
...
{
    public T04PortaDecumana1States(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 830u, NameID = 2137u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")]
public class T04PortaDecumana1(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-772f, -600f), new ArenaBoundsCircle(19.5f))
{
