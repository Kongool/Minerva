...
class PomMeteor(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (_cometsLeft > 0)
        {
            foreach (var i in _activeTowers.SetBits())
                hints.AddForbiddenZone(new SDCircle(Center + _towerOffsets[i], _cometAvoidRadius));
        }
        else
        {
            ...
            if (soakedTower >= 0)
            {
                if (!_activeTowers[soakedTower])
                    soakedTower += 4;
                hints.AddForbiddenZone(new SDInvertedCircle(Center + _towerOffsets[soakedTower], _towerRadius), _towerActivation);
            }
            else
            {
                foreach (var i in _activeTowers.SetBits())
                    hints.AddForbiddenZone(new SDCircle(Center + _towerOffsets[i], _towerRadius), _towerActivation);
            }
        }
    }
    ...
    {
        foreach (var i in _activeTowers.SetBits())
            Arena.ZoneCircleOutline(Center + _towerOffsets[i], _towerRadius, _soakedTowers[i] ? Colors.Safe : 0);
    }
        ...
        if (index is >= 8 and < 16)
        {
            ...
            switch (state)
            {
                case 0x00020001:
                    ...
                    _towerActivation = World.FutureTime(_activationDelay);
                    break;
                case 0x00200010:
