...
class SpinningMogshield(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SpinningMogshield, 6f);
class ThousandKuponzeSwipe(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ThousandKuponzeSwipe, new AOEShapeCone(20f, 45f.Degrees()));
class MograinOfDeath(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.MograinOfDeathAOE, 6f);
class PomHoly(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.PomHoly);
class MoggledayNightFever(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MoggledayNightFever, new AOEShapeCone(30f, 60f.Degrees()));
class MoogleThrust(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.MoogleThrust);
class MementoMoogle(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.MementoMoogle);
class PomHolyBoss(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.PomHolyBoss);
class ThousandKuponzeCharge(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.ThousandKuponzeCharge);
class PomBog(ModuleBase module) : Components.VoidzoneAtCastTarget(module, 8f, (uint)AID.PomBog, GetVoidzones, 0.8f)
{
...
class MogStone(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.MogStoneAOE, 6f, 8, 8);
class TwinPomMeteor(ModuleBase module) : Components.CastSharedTankbuster(module, (uint)AID.TwinPomMeteorAOE, 6f);
class MogComet(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MogCometAOE, 6);
class MogCreation(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MogCreation, new AOEShapeRect(50f, 5f));
...
{
    public T08ThornmarchHStates(ModuleBase module) : base(module)
    {
...
[ModuleInfo(CFCID = 66u, NameID = 725u, PrimaryActorOID = (uint)OID.WhiskerwallKupdiKoop, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")]
public class T08ThornmarchH(WorldState ws, Actor primary) : ModuleBase(ws, primary, default, new ArenaBoundsCircle(21))
{
