...
class MistralSongVulcanBurst(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.MistralSong)
{
    public bool Active;
    ...
    {
        Active = _garuda != null && (_resolve - World.CurrentTime).TotalSeconds <= 6d; // note that garuda continues rotating for next ~0.5s
    }
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (_burstImminent)
        {
            var p1 = Center + Bounds.Radius * (_garuda!.Rotation + _shape.HalfAngle).ToDirection();
            var p2 = Center + Bounds.Radius * (_garuda!.Rotation - _shape.HalfAngle).ToDirection();
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        ...
        var adjPos = _burstImminent ? ClampToBounds(Components.GenericKnockback.AwayFromSource(pc.Position, Module.PrimaryActor, 30f)) : pc.Position;
    ...
    public override void OnActorPlayActionTimelineEvent(Actor actor, ushort id)
    {
        ...
        {
            _garuda = actor;
            _resolve = World.FutureTime(6.6d);
            _burstImminent = true;
        }
    }
}
