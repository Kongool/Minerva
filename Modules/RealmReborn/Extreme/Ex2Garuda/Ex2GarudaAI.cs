...
class Ex2GarudaAI(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
            ...
            switch ((OID)e.Actor.OID)
            {
                case OID.Boss:
                    ...
                    if (_aerialBlast?.NumCasts > 0)
                    {
                        ...
                        e.DesiredPosition = Center + 18 * e.DesiredRotation.ToDirection();
                    }
                    else
                    {
                        e.DesiredRotation = 180.Degrees();
                        e.DesiredPosition = Center + 8 * e.DesiredRotation.ToDirection();
                    }
                    break;
                case OID.Chirada:
                    e.Priority = 2;
                ...
                case OID.SpinyPlume:
                    ...
                    e.AttackStrength = 0;
                    e.ShouldBeTanked = false;
                    if (actor.Role == Role.Tank && e.Actor.TargetID != actor.InstanceID && (World.Actors.Find(e.Actor.TargetID)?.FindStatus(SID.ThermalLow)?.Extra ?? 0) >= 2)
                    {
                        e.Priority = 6;
        ...
        if (haveMonoliths && actor.Role is Role.Healer or Role.Ranged)
        {
            ...
            hints.AddForbiddenZone(new SDInvertedCircle(Center, 7), DateTime.MaxValue);
        }
    }
}
