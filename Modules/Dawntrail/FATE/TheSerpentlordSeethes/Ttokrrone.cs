...
sealed class Devour(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Devour, 8f);
sealed class Touchdown(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Touchdown);

sealed class TtokrroneStates : StateMachineBuilder
{
    public TtokrroneStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.Fate, GroupID = 1871u, CFCID = 1871u, NameID = 12733u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class Ttokrrone(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
