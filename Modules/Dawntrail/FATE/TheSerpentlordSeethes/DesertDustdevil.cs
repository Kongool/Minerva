...
sealed class DesertDustdevil(ModuleBase module) : Components.GenericRotatingAOE(module)
{
        ...
        if (Sequences.Count != 0)
        {
            switch (spell.Action.ID)
            {
                ...
                case (uint)AID.LeftwardSandspoutDDRest:
                    ...
                    if (Sequences.Count != 0)
                    {
                        _aoe.SetAOE(spell.LocXZ, World.FutureTime(2.6d));
                    }
                    else
                    {
                        _aoe.ClearAOE();
...
sealed class DustcloakDustdevil(ModuleBase module) : Components.GenericAOEs(module)
{
