...
namespace Minerva.Dawntrail.FATE.MicaTheMagicalMu;
...
sealed class Draw(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    private readonly List<int> cachedCardIndices = [];
...
sealed class FlourishingBow(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class DoubleMisdirect(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.DoubleMisdirectAOELong, (uint)AID.DoubleMisdirectAOEShort], new AOEShapeCone(40f, 30f.Degrees()), 3, 6);

sealed class RollingStarlight(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
...
sealed class MagicalHat(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TwinkleToss, new AOEShapeRect(42f, 2.5f), 4);
sealed class Shimmerstorm(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ShimmerstormAOE, 6f);
sealed class Shimmerstrike(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.ShimmerstrikeAOE, 6f, tankbuster: true);
sealed class SparkOfImagination(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.SparkOfImaginationAOE);

sealed class MicaTheMagicalMuStates : StateMachineBuilder
{
    public MicaTheMagicalMuStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.Fate, GroupID = 1922u, CFCID = 1922u, NameID = 13049u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")]
public sealed class MicaTheMagicalMu(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(791f, 593f), new ArenaBoundsRect(20.5f, 19.5f))
{
