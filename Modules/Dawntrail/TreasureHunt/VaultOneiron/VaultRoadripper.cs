...
namespace Minerva.Dawntrail.TreasureHunt.VaultOneiron.VaultRoadripper;
...
sealed class HomingShot(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HomingShot, 6f);
sealed class ThrownFlames(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ThrownFlames, 8f);
sealed class IncendiaryCircle(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.IncendiaryCircle, new AOEShapeDonut(3f, 12f));
sealed class RunAmok(ModuleBase module) : Components.BaitAwayChargeCast(module, (uint)AID.RunAmok, 4f);
sealed class Wheel(ModuleBase module) : Components.SingleTargetDelayableCast(module, (uint)AID.Wheel);
sealed class WheelingShot(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WheelingShot, new AOEShapeCone(40f, 90f.Degrees()));
sealed class Electroflame(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Electroflame);
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 1060u, CFCID = 1060u, NameID = 13995u, PrimaryActorOID = (uint)OID.VaultBot, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
