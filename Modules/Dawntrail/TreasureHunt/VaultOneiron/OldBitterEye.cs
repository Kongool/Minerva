...
namespace Minerva.Dawntrail.TreasureHunt.VaultOneiron.OldBitterEye;
...
sealed class TenStoneSwing(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TenStoneSwing, 11f);
sealed class LumberingLeap(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.LumberingLeap, 10f);
sealed class HundredStoneSmash(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.HundredStoneSmash, new AOEShapeRect(65f, 4f), tankbuster: true, endsOnCastEvent: true);
sealed class HundredStoneSwipe(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HundredStoneSwipe, new AOEShapeCone(40f, 30f.Degrees()));
sealed class HyperchargedGlower(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HyperchargedGlower, new AOEShapeRect(20f, 3.5f));
sealed class Glower(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Glower, new AOEShapeRect(19f, 3.5f));
sealed class ThousandStoneSwingHint(ModuleBase module) : Components.CastInterruptHint(module, (uint)AID.ThousandStoneSwing, showNameInHint: true);
sealed class ThousandStoneSwing(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.ThousandStoneSwing);

sealed class PredatorialInstinct(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.PredatorialInstinct, 50f, kind: Kind.TowardsOrigin, minDistance: 4.9f);

sealed class HundredStoneSwingEyeOfTheThunderstorm(ModuleBase module) : Components.GenericAOEs(module)
{
...
{
    public OldBitterEyeStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 1060u, CFCID = 1060u, NameID = 14001u, PrimaryActorOID = (uint)OID.OldBitterEye, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
