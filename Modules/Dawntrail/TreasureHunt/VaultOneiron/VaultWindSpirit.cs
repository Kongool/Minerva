...
namespace Minerva.Dawntrail.TreasureHunt.VaultOneiron.VaultWindSpirit;
...
sealed class Leafcutter(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Leafcutter, new AOEShapeRect(14f, 2f));
sealed class Entangle(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Entangle, 4f);
sealed class StumpGust(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.Stump, (uint)AID.Gust], 6f);
sealed class Whipwind(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Whipwind, new AOEShapeRect(60f, 20f));
sealed class ChangelessWinds(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.ChangelessWinds, new AOEShapeRect(40f, 4f), tankbuster: true, endsOnCastEvent: true);
sealed class WhirlingGaol(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.WhirlingGaol);
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 1060u, CFCID = 1060u, NameID = 13989u, PrimaryActorOID = (uint)OID.VaultAgavoides, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
