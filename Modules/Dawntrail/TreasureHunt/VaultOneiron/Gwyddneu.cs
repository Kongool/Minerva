...
namespace Minerva.Dawntrail.TreasureHunt.VaultOneiron.Gwyddneu;
...
sealed class VioletBolt(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.VioletBolt, new AOEShapeCone(70f, 22.5f.Degrees()), 4);
sealed class CracklingHowlFulgurousHowl(ModuleBase module) : Components.RaidwideCasts(module, [(uint)AID.CracklingHowl, (uint)AID.FulgurousHowl]);
sealed class Thundercrack(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.Thundercrack, 5f, tankbuster: true);
sealed class LightningBolt(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.LightningBolt, 6f);
sealed class ShockElectrify(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        if (shape != null)
        {
            _aoes.Add(new(shape, actor.Position.Quantized(), default, World.FutureTime(7.9d)));
        }
    }
...
{
    public GwyddneuStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 1060u, CFCID = 1060u, NameID = 13996u, PrimaryActorOID = (uint)OID.Gwyddneu, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
