...
namespace Minerva.Dawntrail.TreasureHunt.VaultOneiron.VaultFluidSoul;
...
sealed class FluidSwing(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.FluidSwing, new AOEShapeCone(13f, 45f.Degrees()), tankbuster: true, endsOnCastEvent: true);
sealed class Batter(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Batter, 6f);
sealed class TwistedPool(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TwistedPool, new AOEShapeDonut(6f, 40f));
sealed class BrineBomb(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BrineBomb, 5f);
sealed class WaterIIIFlood(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.WaterIII, (uint)AID.Flood], 8f);
sealed class ProteanWave(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ProteanWave, new AOEShapeCone(39f, 15f.Degrees()));
sealed class Splash(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Splash);
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 1060u, CFCID = 1060u, NameID = 13983u, PrimaryActorOID = (uint)OID.VaultTorbalan, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
