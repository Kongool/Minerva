...
namespace Minerva.Dawntrail.TreasureHunt.VaultOneiron.GreatGimmeCat;
...
sealed class PreeningPrism(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.PreeningPrism1, (uint)AID.PreeningPrism2], new AOEShapeTriCone(20f, 45f.Degrees()), 4, 8);
sealed class GlitterboxAOE(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GlitterboxAOE, 5f);
sealed class GlitterboxSpread(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.GlitterboxSpread, 5f);
sealed class SparklingIntensifies(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.SparklingIntensifies);
sealed class TormentingTwinkle(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.TormentingTwinkle, 5f, tankbuster: true);

sealed class BaskingBeam(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        {
            _aoes.Add(new(rect, (actor.Position + new WDir(default, -21f)).Quantized(), actor.Rotation, World.FutureTime(7.7d)));
        }
    }
...
{
    public GreatGimmeCatStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 1060u, CFCID = 1060u, NameID = 14003u, PrimaryActorOID = (uint)OID.GreatGimmeCat, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
