...
namespace Minerva.Dawntrail.TreasureHunt.VaultOneiron.SpiritOfThunder;
...
sealed class Electray(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.Electray1, (uint)AID.Electray2], new AOEShapeRect(20f, 5f), 4, 8);
sealed class TidalThunder(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TidalThunder, new AOEShapeCone(30f, 15f.Degrees()));
sealed class Electrowave(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Electrowave);
sealed class ElectricVortex(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.ElectricVortex, 5f);
sealed class HypercondensedThunder(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.HypercondensedThunder, 6f, tankbuster: true);

sealed class HighVoltageElectray : Components.SimpleAOEGroups
{
    public HighVoltageElectray(ModuleBase module) : base(module, [(uint)AID.HighVoltageElectray1, (uint)AID.HighVoltageElectray2,
...
sealed class PowerLine(ModuleBase module) : Components.GenericAOEs(module)
{
...
{
    public SpiritOfThunderStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 1060u, CFCID = 1060u, NameID = 14012u, PrimaryActorOID = (uint)OID.SpiritOfThunder, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
