...
namespace Minerva.Dawntrail.TreasureHunt.VaultOneiron.Paeonia;
...
sealed class Hypnotize(ModuleBase module) : Components.CastGazes(module, [(uint)AID.Hypnotize1, (uint)AID.Hypnotize2]);
sealed class LightningStorm(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.LightningStorm, 6f);
sealed class LightningCrossing(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.LightningCrossing1, (uint)AID.LightningCrossing2], new AOEShapeCone(40f, 22.5f.Degrees()), 4, 8);
sealed class Mandrastorm(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Mandrastorm);
sealed class LightningBolt(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.LightningBolt, 6f);
sealed class Ram(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.Ram);

sealed class EpicenterShock(ModuleBase module) : Components.ConcentricAOEs(module, _shapes)
{
...
sealed class LevinrootRing(ModuleBase module) : Components.ConcentricAOEs(module, _shapes)
{
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 1060u, CFCID = 1060u, NameID = 14005u, PrimaryActorOID = (uint)OID.Paeonia, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
