...
namespace Minerva.Dawntrail.TreasureHunt.VaultOneiron.MobileFortress;
...
sealed class AntiPersonnelMissile(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AntiPersonnelMissile, 6f);
sealed class SurfaceMissile(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.SurfaceMissile);
sealed class Elimination(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.Elimination, new AOEShapeRect(46f, 5f), tankbuster: true, endsOnCastEvent: true);
sealed class GuidedMissile(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.GuidedMissile, 6f);

sealed class CarpetBomb : Components.SimpleAOEGroups
{
    public CarpetBomb(ModuleBase module) : base(module, [(uint)AID.CarpetBomb1, (uint)AID.CarpetBomb2,
...
sealed class QuietWorld(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.QuietWorld, new AOEShapeRect(10f, 20f))
{
...
sealed class SystematicBombardment(ModuleBase module) : Components.GenericAOEs(module)
{
...
{
    public MobileFortressStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 1060u, CFCID = 1060u, NameID = 14009u, PrimaryActorOID = (uint)OID.MobileFortress, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
