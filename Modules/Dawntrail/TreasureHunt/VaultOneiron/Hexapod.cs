...
namespace Minerva.Dawntrail.TreasureHunt.VaultOneiron.Hexapod;
...
sealed class ShockRect(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ShockRect, new AOEShapeRect(40f, 3f));
sealed class ShockCircle(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ShockCircle, 10f);
sealed class DiffusionRay(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DiffusionRay, new AOEShapeCone(30f, 60f.Degrees()));
sealed class Electroblast(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Electroblast);
sealed class ElectricExcess(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ElectricExcess, 5f);
sealed class AntiHeroArtillery(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.AntiHeroArtillery, 5f, tankbuster: true);
...
sealed class Electromine(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class Brainjack(ModuleBase module) : Components.StatusDrivenForcedMarch(module, 3f, (uint)SID.ForwardMarch, (uint)SID.AboutFace, (uint)SID.LeftFace, (uint)SID.RightFace, stopAtWall: true)
{
...
sealed class PoleShift(ModuleBase module) : Components.GenericKnockback(module, stopAtWall: true)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (_kbs[slot] != null)
        {
            ...
            {
                hints.AddForbiddenZone(new SDKnockbackInAABBSquareAwayFromOrigin(Center, pos, 7f, 13f), act); // arena square 20 - 6 rect width - 1 safety margin
            }
        }
    }
}
...
{
    public HexapodStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 1060u, CFCID = 1060u, NameID = 14010u, PrimaryActorOID = (uint)OID.Hexapod, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
