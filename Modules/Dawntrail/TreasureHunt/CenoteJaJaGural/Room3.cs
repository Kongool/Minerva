...
namespace Minerva.Dawntrail.TreasureHunt.CenoteJaJaGural.Room3;
...
sealed class Wingbeat(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Wingbeat, new AOEShapeCone(18f, 30f.Degrees()));
sealed class Feathercut(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Feathercut, new AOEShapeRect(40f, 4f));
sealed class GoldDust(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GoldDust, 8f);
sealed class BloodyCaress(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BloodyCaress, new AOEShapeCone(12f, 60f.Degrees()));
sealed class SwiftSough(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SwiftSough, new AOEShapeCone(13f, 30f.Degrees()));
sealed class FireBreak(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FireBreak, new AOEShapeCone(8f, 45f.Degrees()));

sealed class TornadoIncubusRottenSpores(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.Tornado, (uint)AID.Incubus, (uint)AID.RottenSpores], 6f);
sealed class FireIIBitterNectar(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.FireII, (uint)AID.BitterNectar], 5f);
...
{
    public Room3States(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
public sealed class Room3(WorldState ws, Actor primary) : ModuleBase(ws, primary, ArenaBounds.Center, ArenaBounds)
{
