...
namespace Minerva.Dawntrail.TreasureHunt.CenoteJaJaGural.Room2;
...
sealed class GravelShower(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GravelShower, new AOEShapeRect(10f, 2f));
sealed class Flatten(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Flatten, new AOEShapeCone(8f, 45f.Degrees()));

sealed class WaterIIIPollenCorona(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.WaterIII, (uint)AID.PollenCorona], 8f);
sealed class DisseverNepenthicPlunge(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.Dissever, (uint)AID.NepenthicPlunge], new AOEShapeCone(10f, 45f.Degrees()));
sealed class DoubleSmashCriticalBite(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.DoubleSmash, (uint)AID.CriticalBite], new AOEShapeCone(10f, 60f.Degrees()));
sealed class TornadoTornadicSerenadeRottenSpores(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.Tornado, (uint)AID.TornadicSerenade, (uint)AID.RottenSpores], 6f);
...
{
    public Room2States(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
public sealed class Room2(WorldState ws, Actor primary) : ModuleBase(ws, primary, ArenaBounds.Center, ArenaBounds)
{
