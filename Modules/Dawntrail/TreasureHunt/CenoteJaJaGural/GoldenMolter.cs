...
namespace Minerva.Dawntrail.TreasureHunt.CenoteJaJaGural.GoldenMolter;
...
sealed class Lap(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.Lap);
sealed class Lightburst(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Lightburst);

sealed class Crypsis(ModuleBase module) : ModuleComponent(module)
{
...
sealed class GoldenGall(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GoldenGall, new AOEShapeCone(40f, 90f.Degrees()));
sealed class GoldenRadiance(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GoldenRadiance, 5f);
sealed class BlindingLight(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.BlindingLight, 6f);
...
sealed class Vasoconstrictor(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.Vasoconstrictor1, (uint)AID.Vasoconstrictor2, (uint)AID.Vasoconstrictor3], 6f);

sealed class VasoconstrictorPool(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class RottenSpores(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.RottenSpores, 6f);
...
{
    public GoldenMolterStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
            .ActivateOnEnter<Lap>()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 993u, CFCID = 993u, NameID = 13248u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Kismet, Malediktus (ported from BMR)")]
