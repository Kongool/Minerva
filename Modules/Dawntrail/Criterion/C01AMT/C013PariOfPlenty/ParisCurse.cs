...
class ParisCurse(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnCastFinished(Actor caster, ActorCastInfo spell)
    {
        ...
        {
            var actor = Module.World.Actors.FirstOrDefault(a => a.OID == (uint)OID.IcyBauble);
            if (actor != null)
            {
                blueCrystal = actor;
            }
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        ...
        {
            aoes.Add(new AOEInstance(cellShape, gridMap.GridToWorld(assignment.Value), default, World.CurrentTime, Colors.SafeFromAOE, false));
            return CollectionsMarshal.AsSpan(aoes);
        }
        ...
        if (safeTiles.Count > 0)
        {
            foreach (var tile in safeTiles[0])
            {
                ...
                aoes.Add(new AOEInstance(cellShape, gridMap.GridToWorld(tile), default, World.CurrentTime, colour, false));
            }
        }
    ...
    private bool CheckAssignmentRoles(PartyState partyState)
    {
        ...
        {
            Service.Log.Debug("Missing unique roles - each player must have a unique role!");
            return false;
        }
...
class Fableflight(ModuleBase module) : Fireflight(module)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
