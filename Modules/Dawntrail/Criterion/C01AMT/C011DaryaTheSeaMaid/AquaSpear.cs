...
class AquaSpear(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID == (uint)AID.WaterZone1)
        {
            ...
            aoes.Add(new AOEInstance(cellShape, pos, default, World.CurrentTime, Colors.Danger));
        }
    }
    ...
    private WPos IndexToPosition(int index)
    {
        ...
        const float halfcell = cellSize * 0.5f;
        var startX = Center.X - halftotal + halfcell;
        var startZ = Center.Z - halftotal + halfcell;
