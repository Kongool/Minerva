...
class AlluringOrder(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.AlluringOrder);

class AlluringOrderForcedMarch(ModuleBase module) : Components.StatusDrivenForcedMarch(module, 3.0f, (uint)SID.ForwardMarch, (uint)SID.AboutFace, (uint)SID.LeftFace, (uint)SID.RightFace);
...
class Tidalspout(ModuleBase module) : Components.StatusStackSpread(module, (uint)SID.TidalspoutTarget, 0, 6, 0)
{
...
class SwimmingInTheAir(ModuleBase module) : Components.GenericAOEs(module)
{
...
class SunkenTreasure(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> aoes = [];
...
{
    public SunkenTreasure2(ModuleBase module) : base(module)
    {
        maxShow = 6;
    }
}