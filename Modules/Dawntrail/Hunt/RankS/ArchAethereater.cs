...
namespace Minerva.Dawntrail.Hunt.RankS.ArchAethereater;
...
sealed class Heatstroke(ModuleBase module) : Components.StayMove(module, 3f)
{
    ...
    public override void AddHints(int slot, Actor actor, TextHints hints)
    {
        ...
        if (_heatstroke[slot])
        {
            var remaining = (PlayerStates[slot].Activation - World.CurrentTime).TotalSeconds;
            hints.Add($"Heatstroke on you in {remaining:f1}s. (Pyretic!)", remaining < 3d);
        }
    }
}

sealed class ColdSweats(ModuleBase module) : Components.StayMove(module, 3)
{
...
sealed class Aethermodynamics(ModuleBase module) : Components.RaidwideCasts(module, [(uint)AID.Aethermodynamics1, (uint)AID.Aethermodynamics2,
(uint)AID.Aethermodynamics3, (uint)AID.Aethermodynamics4]);

sealed class Obliterate(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.Obliterate, 6f, 8);
sealed class Meltdown(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Meltdown, new AOEShapeRect(40f, 5f));
sealed class BlizzardIV(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.BlizzardIV1, (uint)AID.BlizzardIV5], new AOEShapeDonut(6f, 40f));
sealed class FireIV(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.FireIV1, (uint)AID.FireIV5], 15f);

sealed class SoullessStreamFireBlizzardCombo(ModuleBase module) : Components.GenericAOEs(module)
{
...
{
    public ArchAethereaterStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.Hunt, CFCID = 0u, NameID = 13406u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
