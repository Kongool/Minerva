...
namespace Minerva.Dawntrail.Hunt.RankS.AtticusThePrimogenitor;
...
sealed class PyricBlast(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.PyricBlast, 6f, 8);
sealed class Intimidation(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Intimidation);
sealed class Brutality(ModuleBase module) : Components.CastHint(module, (uint)AID.Brutality, "Applies Haste");

sealed class BreathSequence(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorNpcYell(Actor actor, ushort id)
    {
        if (id > 16903)
            return;
        void AddAOE(Angle offset = default) => _aoes.Add(new(cone, actor.Position, actor.Rotation + offset, World.FutureTime(10.5d))); // activation is a placeholder, gets replaced when sequence starts
        switch (id)
        {
...
{
    public AtticusThePrimogenitorStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.Hunt, CFCID = 0u, NameID = 13156u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
