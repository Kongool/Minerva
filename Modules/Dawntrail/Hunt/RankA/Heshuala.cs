...
namespace Minerva.Dawntrail.Hunt.RankA.Heshuala;
...
sealed class SpinShock(ModuleBase module) : Components.GenericRotatingAOE(module)
{
...
sealed class ShockingCrossXMarksTheShock(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void Update()
    {
        if (!aoeInit && _rotation.Sequences.Count != 0)
        {
            ...
            var activation = World.FutureTime(11.5d + _rotation.Spins * 2d);
...
sealed class LightningBolt(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.LightningBolt, 5f);
sealed class ElectricalOverload(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.ElectricalOverload);

sealed class HeshualaStates : StateMachineBuilder
{
    public HeshualaStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.Hunt, CFCID = 0u, NameID = 13157u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
