...
sealed class FTB3MarbleDragonStates : StateMachineBuilder
{
    ...
    private void DraconiformMotion(uint id, float delay)
    {
        ComponentCondition<DraconiformMotion>(id, delay, comp => comp.NumCasts == 2, "Front/back cone AOEs")
            ...
            .ExecOnEnter<DraconiformMotionBait>(comp => comp.Activation = _module.World.FutureTime(8.6d))
    ...
    private void ImitationRain1(uint id, float delay)
    {
        ComponentCondition<ImitationRain>(id, delay, comp => comp.NumCasts != 0, "Raidwide")
            ...
            .ExecOnExit<DraconiformMotionBait>(comp => comp.Activation = _module.World.FutureTime(11.2d))
            .ExecOnEnter<ImitationRain>(comp => comp.Activation = _module.World.FutureTime(4.9d))
            .SetHint(StateMachine.StateHint.Raidwide)
    ...
    private void ImitationRain2(uint id, float delay)
    {
        ComponentCondition<ImitationRain>(id, delay, comp => comp.NumCasts != 0, "Raidwide")
            ...
            .ActivateOnExit<DraconiformMotionBait>()
            .ExecOnExit<DraconiformMotionBait>(comp => comp.Activation = _module.World.FutureTime(9.8d))
            .ActivateOnExit<BallOfIce>()
            .ExecOnEnter<ImitationRain>(comp => comp.Activation = _module.World.FutureTime(5.8d))
            .SetHint(StateMachine.StateHint.Raidwide)
            .DeactivateOnExit<ImitationRain>();
    ...
    private void ImitationRain3(uint id, float delay, bool first = true)
    {
        if (!first)
        {
            ComponentCondition<ImitationRain>(id, delay, comp => comp.NumCasts != 0, "Raidwide")
                .ActivateOnEnter<ImitationRain>()
                .ActivateOnExit<BallOfIce>()
                .ExecOnEnter<ImitationRain>(comp => comp.Activation = _module.World.FutureTime(4.6d))
                .SetHint(StateMachine.StateHint.Raidwide)
                .DeactivateOnExit<ImitationRain>();
        }
        else
    ...
    private void ImitationRain4(uint id, float delay)
    {
        ...
        ComponentCondition<WickedWater>(id + 0x10u, 1.8f, comp => comp.NumCasts != 0, "Raidwide")
            .ActivateOnExit<DraconiformMotionBait>()
            .ExecOnExit<DraconiformMotionBait>(comp => comp.Activation = _module.World.FutureTime(11d))
            .SetHint(StateMachine.StateHint.Raidwide)
    ...
    private void ImitationRain5(uint id, float delay, bool first = true)
    {
        var cond = ComponentCondition<ImitationRain>(id, delay, comp => comp.NumCasts != 0, "Raidwide")
            .ActivateOnEnter<ImitationRain>()
            .ActivateOnExit<BallOfIce>()
            .ExecOnEnter<ImitationRain>(comp => comp.Activation = _module.World.FutureTime(first ? 12d : 2.1d))
            .SetHint(StateMachine.StateHint.Raidwide)
        ...
        if (first)
        {
            cond
                .ActivateOnExit<DraconiformMotionBait>()
                .ExecOnExit<DraconiformMotionBait>(comp => comp.Activation = _module.World.FutureTime(9.7d));
        ...
        else
        {
            cond
                .ActivateOnEnter<DraconiformMotionBait>()
                .ExecOnEnter<DraconiformMotionBait>(comp => comp.Activation = _module.World.FutureTime(3.8d));
        }
