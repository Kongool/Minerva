...
sealed class ImitationBlizzard(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void Update()
    {
        ...
        {
            var isCCW = (referenceIcewind.Position - Center).OrthoR().Dot(referenceIcewind.LastFrameMovement) < 0f;
            if (!IsRain4)
            {
                WPos[] positions =
                [
...
sealed class ImitationBlizzardTowers(ModuleBase module) : Components.GenericTowersOpenWorld(module)
{
...
sealed class BallOfIce(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        if (circle != null)
        {
            _aoes.Add(new(circle, actor.Position.Quantized(), default, World.FutureTime(12.6d))); // activation time depends on mechanic, this is only the lowest possible
        }
    }
