...
sealed class DemonographTowers(ModuleBase module) : Components.CastTowersOpenWorld(module, (uint)AID.Explosion1, 4f, 4, 8, true);

// this is tricky to show in a useful way on radar since there is a grounded and floating tower in each position
sealed class GravityTowers(ModuleBase module) : Components.GenericTowersOpenWorld(module)
{
    private readonly HashSet<Actor> levitatingPlayers = [];
    private readonly HashSet<Actor> groundedPlayers = Soakers(module);
    private readonly List<Tower> towersGround = [];
    private readonly List<Tower> towersFlying = [];
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        if (spell.Action.ID == (uint)AID.EraseGravity)
        {
            ...
            for (var i = 0; i < len; ++i)
            {
                ...
                if (World.Actors.Find(targ.ID) is Actor t)
                {
...
sealed class EraseGravity(ModuleBase module) : Components.GenericAOEs(module)
{
