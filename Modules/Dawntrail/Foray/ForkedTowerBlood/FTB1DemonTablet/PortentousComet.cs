...
sealed class PortentousCometeor(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PortentousCometeor, 43f);

sealed class PortentousCometeorBait(ModuleBase module) : Components.GenericBaitAway(module, onlyShowOutlines: true, centerAtTarget: true)
{
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        ...
        {
            CurrentBaits.Add(new(Module.PrimaryActor, actor, circle, World.FutureTime(12d)));
        }
    }
...
sealed class PortentousCometKnockback(ModuleBase module) : Components.GenericKnockback(module)
{
    private readonly AOEShapeCircle circle = new(4f);
    private readonly List<(Actor target, Angle dir)> targets = [];
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        if (spell.Action.ID is (uint)AID.PortentousComet1 or (uint)AID.PortentousComet2 && World.Actors.Find(spell.TargetID) is Actor target)
        {
...
sealed class PortentousComet(ModuleBase module) : Components.GenericStackSpread(module)
{
    public int NumFinishedStacks;

    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID is (uint)AID.PortentousComet1 or (uint)AID.PortentousComet2 && World.Actors.Find(spell.TargetID) is Actor t)
        {
