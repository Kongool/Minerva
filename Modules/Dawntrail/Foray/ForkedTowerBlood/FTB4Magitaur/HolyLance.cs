...
sealed class HolyLance(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        if (actor.OID == (uint)OID.LuminousLance)
        {
            ...
            var act = count == 0 ? World.FutureTime(13d) : _aoes[0].Activation.AddSeconds(2d * count);
            for (var i = 0; i < 3; ++i)
            {
            ...
            var center = Center;
            var shape = FTB4Magitaur.GetCircleMinusSquares(center);
            _aoes.Add(new(shape, Center, default, act, shapeDistance: shape.Distance(center, default)));
        }
    }
...
sealed class HolyIV(ModuleBase module) : Components.GenericStackSpread(module)
{
    public readonly List<(int Order, Actor Actor, DateTime expireAt, int squareIndex)> Status = [];
    private readonly WPos[] squarePositions = FTB4Magitaur.GetSquarePositions();
    private readonly WDir[] squareDirs = FTB4Magitaur.GetSquareAnglesDirs();
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        if (status.ID == (uint)SID.PreyLancepoint)
        {
            ...
            var order = (expire - World.CurrentTime).TotalSeconds switch
            {
                < 20d => 0,
                < 30d => 1,
                _ => 2
...
sealed class HolyIVHints(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly HolyIV _status = module.FindComponent<HolyIV>()!;
    public readonly AOEShapeCustom[] StackOutsideSquare = GenerateOutsideStackHints(module.Center);
    public readonly AOEShapeCustom[] StackInsideSquare = GenerateInsideStackHints(module.Center);
        ...
        if (_status.Stacks.Count != 0)
        {
            ...
            for (var i = 0; i < count; ++i)
            {
                ...
                if (s.squareIndex == index) // there might be no spread for the square if player died
                {
                    ...
                    return new AOEInstance[1] { new(isOutsideStack ? StackOutsideSquare[index] : StackInsideSquare[index], Center, default, _status.Stacks.Ref(0).Activation) };
                }
            }
        }
        return [];
