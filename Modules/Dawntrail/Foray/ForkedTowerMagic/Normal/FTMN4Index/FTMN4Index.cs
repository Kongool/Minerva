...
sealed class ArenaChange(ModuleBase module) : ModuleComponent(module)
{
    public override void OnMapEffect(byte index, uint state)
    {
        if (index == 0x00)
        {
            ...
            {
                case 0x00020001:
                    Bounds = Index.OmniElementsBounds;
                    Center = Index.OmniElementsCenter;
                    break;
                case 0x00080004:
                    Bounds = Index.InitialBounds;
                    Center = Index.InitialCenter;
                    break;
            }
        }
    }
}
sealed class OmniElementPanels(ModuleBase module) : ModuleComponent(module)
{
...
sealed class Flare(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Flare);
sealed class Bombs(ModuleBase module) : Components.Adds(module, (uint)OID.SummonedBomb, 2)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (ActiveActors.Count != 0)
        {
            ...
            if (_config.ForceAddTargeting && World.Actors.Find(actor.TargetID) is var target && target?.Type != ActorType.Player && target?.OID != (uint)OID.SummonedBomb)
            {
                hints.ForcedTarget = ActiveActors.MinBy(actor.DistanceToHitbox);
            }
        }
        else if (_config.ForceBossTargeting && World.Actors.Find(actor.TargetID) == null)
        {
            hints.ForcedTarget = Module.PrimaryActor;
        }
    }
}
sealed class Aim(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Aim, 11f)
{
...
sealed class RomeosBallad(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.RomeosBallad, 15f)
{
    // resolves after shockwave and predict; ignore until predict AOEs are gone
    public override void AddHints(int slot, Actor actor, TextHints hints)
    {
...
sealed class ElementaryChemistry(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ElementaryChemistry, new AOEShapeRect(15f, 7.5f));
sealed class ElementaryEvocation(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        if (degrees < 0f)
        {
            ...
            var activation = World.FutureTime(8d + 2.4d * delay);
...
sealed class ElementaryExpansion(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly OmniElementPanels _panels = module.FindComponent<OmniElementPanels>()!;
    private readonly List<AOEInstance> _aoes = [];
    private readonly AOEShapeCone _cone = new(30f, 30f.Degrees());
...
sealed class Shockwave(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.Shockwave, 9f, shape: new AOEShapeCircle(15f), stopAfterWall: true)
{
    ...
    private void AddHints(int slot, Actor actor, TextHints? textHints, AIHints? aiHints)
    {
        ...
        if (kbs.Length != 0)
        {
            ...
            if (aim.Length == 0 && romeo.Length == 0)
            {
                if (textHints != null)
                {
                    var sd = new SDKnockbackInComplexPolygonAwayFromOrigin(Center, kb.Origin, Distance, Polygon);
                ...
                {
                    var sd = new SDKnockbackInComplexPolygonAwayFromOrigin(Center, kb.Origin, Distance + 1f, Polygon);
            ...
            else
            {
                ...
                if (textHints != null)
                {
                    var sd = new SDKnockbackInComplexPolygonAwayFromOriginPlusAOECircles(Center, kb.Origin, Distance, Polygon, origins, radius, origins.Length);
                    if (sd.Contains(actor.Position))
                    {
                        textHints?.Add("About to be knocked into danger!");
                    }
                ...
                else
                {
                    ...
                    var sd = new SDKnockbackInComplexPolygonAwayFromOriginPlusAOECircles(Center, kb.Origin, Distance + 1f, Polygon, origins, radius + 1f, origins.Length);
                    aiHints?.AddForbiddenZone(sd, kb.Activation);
                }
            }
        }
...
sealed class DuologyOfImplements(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
    private readonly AOEShapeCone _cone = new(30f, 30f.Degrees());
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
...
sealed class AllConsumingFlames(ModuleBase module) : Components.SpreadFromIcon(module, (uint)IconID.Spread, (uint)AID.AllConsumingFlames, 6f, 5d);
sealed class Predict(ModuleBase module) : Components.GenericAOEs(module)
{
...
[ModuleInfo(Group = ModuleGroup.TheForkedTowerMagic, CFCID = 1093u, NameID = 14717u, PrimaryActorOID = (uint)OID.Index, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "gynorhino (ported from BMR)")]
[SkipLocalsInit]
public sealed class Index(WorldState ws, Actor primary) : ModuleBase(ws, primary, InitialCenter, InitialBounds)
{
    ...
    protected override bool CheckPull() => base.CheckPull() && Raid.Player()!.Position.InCircle(Center, 28f);
}
