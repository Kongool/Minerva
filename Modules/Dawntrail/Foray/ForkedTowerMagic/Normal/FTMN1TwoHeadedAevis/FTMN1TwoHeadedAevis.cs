...
sealed class StormsBreath(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.StormsBreathCast, 14f)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        {
            Casters.Add(new(Center, Distance, Module.CastFinishAt(spell), Shape, spell.Rotation, KnockbackKind, 0, [], caster.InstanceID, IgnoreImmunes));
        }
    }
        ...
        if (hissing == null || hissing.ActiveKnockbacks(slot, actor).Length == 0)
        {
            ...
            if (kbs.Length != 0)
            {
                ...
                if (!IsImmune(slot, act))
                {
                    ...
                    hints.AddForbiddenZone(new SDKnockbackInAABBSquareAwayFromOrigin(Center, kb.Origin, 19f, 20f), act);
                }
            }
        }
    }
...
sealed class TwoTerrors(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TwoTerrors, new AOEShapeRect(40f, 5f));

sealed class LightningIcePoison(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (knockbacks == null || knockbacks.ActiveKnockbacks(slot, actor).Length == 0)
        {
            ...
            if (aoes.Count != 0)
            {
                ...
                hints.AddForbiddenZone(new SDInvertedRect(Center, 0f.Degrees(), 18f, 18f, 18f));
            }
        }
    }
}

sealed class BlazeLoop(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class ArcaneBeacon(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ArcaneBeacon, new AOEShapeRect(60f, 2.5f), 8);
sealed class Archaeofury1(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.Archaeofury1, 6f);
sealed class Archaeofury2(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.Archaeofury2, 6f);

[ModuleInfo(Group = ModuleGroup.TheForkedTowerMagic, GroupID = 1093u, CFCID = 1093u, NameID = 14489u, PrimaryActorOID = (uint)OID.GreenHead, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Equilius + gynorhino (ported from BMR)")]
[SkipLocalsInit]
public sealed class TwoHeadedAevis(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-900f, 700f), new ArenaBoundsSquare(20f))
{
