...
sealed class HissingReprise(ModuleBase module) : Components.GenericKnockback(module)
{
    ...
    public override ReadOnlySpan<Knockback> ActiveKnockbacks(int slot, Actor actor)
    {
        ...
        if (easterly[slot] || westerly[slot])
        {
            ...
            kb.Add(new(new(posx, Center.Z), 21f, activation, kind: kind));
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (count != 0)
        {
            ...
            if (count == 2)
            {
                hints.AddForbiddenZone(new SDKnockbackInAABBSquareFixedDirectionIntoCircle(Center, direction, 19f, Center, 5f), kbs[0].Activation);
                return;
            }
            ...
            if (!IsImmune(slot, kb.Activation))
            {
                ...
                {
                    hints.AddForbiddenZone(new SDKnockbackInAABBSquareFixedDirection(Center, direction, 19f), kb.Activation);
                }
                else
                {
                    hints.AddForbiddenZone(new SDKnockbackInAABBSquareFixedDirectionPlusAOECircles(Center, direction, 19f, origins, aoeinfo.Radius, aoecount), kb.Activation);
                }
            }
        }
    }
