...
[SkipLocalsInit]
sealed class FreezingFugue(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.FreezingFugue1, (uint)AID.FreezingFugue2, (uint)AID.FreezingFugue3], 20f);
[SkipLocalsInit]
sealed class PoisonBreath(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PoisonBreath, 18f);
[SkipLocalsInit]
sealed class FulgurousFugue(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.FulgurousFugue1, (uint)AID.FulgurousFugue2, (uint)AID.FulgurousFugue3], new AOEShapeDonut(20f, 60f));
[SkipLocalsInit]
sealed class FreezingFulgurousFugue(ModuleBase module) : Components.GenericAOEs(module)
{
...
[SkipLocalsInit]
sealed class ThunderfrostTempest(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.ThunderfrostTempest);
[SkipLocalsInit]
sealed class Archaeofury(ModuleBase module) : Components.SpreadFromIcon(module, (uint)IconID.Tankbuster, default, 6f, 5f)
{
...
[SkipLocalsInit]
sealed class TwoTerrorsWide(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TwoTerrors1, new AOEShapeRect(40f, 10f));
[SkipLocalsInit]
sealed class TwoTerrorsThin(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TwoTerrors2, new AOEShapeRect(40f, 5f));
[SkipLocalsInit]
sealed class ArcaneRevelation(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        switch (spell.Action.ID)
        {
            case (uint)AID.TwoTerrors1:
                AddArcaneBeacon(spell.LocXZ.X < Module.Center.X ? _blue : _green);
                break;
            case (uint)AID.FreezingFugue1:
            case (uint)AID.FreezingFugue2:
            case (uint)AID.FreezingFugue3:
...
[ModuleInfo(Group = ModuleGroup.TheForkedTowerMagic, CFCID = 1114u, NameID = 14490u, PrimaryActorOID = (uint)OID.TwoHeadedAevis, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "gynorhino (ported from BMR)")]
[SkipLocalsInit]
public sealed class FTME1TwoHeadedAevis(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-900f, 700f), new ArenaBoundsSquare(20f))
{
    ...
    {
        return PrimaryActor.InCombat && Raid.Player()!.Position.InSquare(Module.Center, 20f);
    }
}
