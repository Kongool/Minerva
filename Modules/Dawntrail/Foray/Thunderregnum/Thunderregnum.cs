...
namespace Minerva.Dawntrail.Foray.FATE.Thunderregnum;
...
sealed class RegalFulguration(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.RegalFulguration, (uint)AID.RegalFulguration1], new AOEShapeCone(40.0f, 90.0f.Degrees()));
sealed class Thunderbolt(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Thunderbolt, 10f);
sealed class NobleBlaster(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.NobleBlaster, new AOEShapeRect(50.0f, 2.5f));

sealed class ThunderboltPuddle : Components.SimpleAOEGroups
{
    public ThunderboltPuddle(ModuleBase module) : base(module, [(uint)AID.ThunderboltPuddle, (uint)AID.ThunderboltPuddle1, (uint)AID.ThunderboltPuddle2,
...
{
    public ThunderregnumStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
// (Cresceregina, recording 2026-08-24: 53 ground casts spanning 38x37y) with BMR's own 30y FATE radius.
public sealed class Thunderregnum(WorldState ws, Actor primary)
    : ModuleBase(ws, primary, new WPos(141.1f, -707.5f), new ArenaBoundsCircle(30f));
