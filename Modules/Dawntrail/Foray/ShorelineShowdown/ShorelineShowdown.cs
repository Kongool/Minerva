...
namespace Minerva.Dawntrail.Foray.FATE.ShorelineShowdown;
...
sealed class TheRamsVoice(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TheRamsVoice, new AOEShapeCircle(9.0f));
sealed class TheDragonsVoice(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TheDragonsVoice1, new AOEShapeDonut(8.0f, 30.0f));
sealed class ChaoticChorus(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ChaoticChorus, new AOEShapeCircle(6.0f));

sealed class Breath(ModuleBase module) : Components.GenericRotatingAOE(module)
{
...
sealed class GlacipotentOrb(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class TheDragonsVoiceBoss(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TheDragonsVoice, new AOEShapeDonut(8.0f, 30.0f))
{
...
sealed class Cacophony(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<Actor> orbs = [];
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        ...
        foreach (var orb in orbs)
        {
            aoes.Add(new(shape, orb.Position, orb.Rotation, World.FutureTime(1.5f)));
        }
...
sealed class Duobreath(ModuleBase module) : Components.GenericAOEs(module)
{
...
{
    public RegnantChimeraStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
// BMR's own 30y FATE radius stands in for it -- comfortably containing the 30y donut and the 40y duobreath.
public sealed class RegnantChimera(WorldState ws, Actor primary)
    : ModuleBase(ws, primary, new WPos(91.2f, 471.2f), new ArenaBoundsCircle(30f));
