...
namespace Minerva.Dawntrail.Foray.FATE.ARottenAffair;
...
sealed class Glory(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Glory, new AOEShapeCone(50.0f, 45.0f.Degrees()));
sealed class StoneIII(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.StoneIII, (uint)AID.StoneIII1],
    new AOEShapeCone(40.0f, 22.5f.Degrees()));
sealed class AeroIII(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.AeroIII, (uint)AID.AeroIII1], new AOEShapeRect(40.0f, 4.0f));
sealed class Holy(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.HolyStart, (uint)AID.HolyNext], new AOEShapeCircle(6.0f));
sealed class ShortswordAndSorcery(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.ShortswordAndSorcery, (uint)AID.ShortswordAndSorcery1], new AOEShapeCircle(15.0f));
sealed class LongswordAndSorcery(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.LongswordAndSorcery, (uint)AID.LongswordAndSorcery1],
    new AOEShapeDonut(10.0f, 25.0f));

[SkipLocalsInit]
sealed class ARottenAffairStates : StateMachineBuilder
{
    public ARottenAffairStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.ForayFATE, GroupID = 1093u, CFCID = 1093u, NameID = 2081u, PrimaryActorOID = (uint)OID.PatientKuribu, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Equilius (ported from BMR)")]
[SkipLocalsInit]
