...
namespace Minerva.Dawntrail.Foray.FATE.RoughWaters;
...
sealed class Tideline(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class TwinTentacle(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
...
sealed class RecedingTwinTides(ModuleBase module) : Components.ConcentricAOEs(module, _shapes)
{
...
sealed class EncroachingTwinTides(ModuleBase module) : Components.ConcentricAOEs(module, _shapes)
{
...
sealed class VoidWaterIII(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.VoidWaterIII, 6f);
sealed class VoidWaterIV(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.VoidWaterIV);

sealed class RoughWatersStates : StateMachineBuilder
{
    public RoughWatersStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.ForayFATE, CFCID = 1018u, NameID = 1962u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
