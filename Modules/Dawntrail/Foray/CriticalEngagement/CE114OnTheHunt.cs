...
namespace Minerva.Dawntrail.Foray.CriticalEngagement.CE114OnTheHunt;
...
sealed class Shockwave(ModuleBase module) : Components.RaidwideCastDelay(module, (uint)AID.ShockwaveVisual1, (uint)AID.Shockwave, 0.8f, "Raidwide x3");
sealed class FearsomeGlint(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FearsomeGlint, new AOEShapeCone(60f, 45f.Degrees()));
sealed class FallingRock(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FallingRock, 10f);
sealed class Decompress(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Decompress, 12f);

sealed class AetherialRay(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        if (tether.ID == (uint)TetherID.BeaconTimer)
        {
            ...
            if (target is Actor t)
            {
                ...
                _aoes.Add(new(rect, Module.Center, Angle.FromDirection(t.Position - CE114OnTheHunt.ArenaCenter) + 200f.Degrees(), World.FutureTime(5.2d)));
            }
        }
    }
...
sealed class BrightPulse(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        if (status.ID == (uint)SID.RoundelTimer && actor.OID == (uint)OID.RadiantRoundel)
        {
            ...
            if (dir.OrthoR().Dot(actor.Rotation.ToDirection()) > 0f)
                angle = -angle;
            _aoes.Add(new(circle, CE114OnTheHunt.ArenaCenter + dir.Rotate(angle), default, World.FutureTime(5.2d)));
        }
    }
...
{
    public CE114OnTheHuntStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, CFCID = 1018u, NameID = 42u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class CE114OnTheHunt(WorldState ws, Actor primary) : ModuleBase(ws, primary, ArenaCenter.Quantized(), new ArenaBoundsCircle(26f))
{
