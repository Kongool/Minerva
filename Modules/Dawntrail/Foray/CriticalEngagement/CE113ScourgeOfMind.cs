...
namespace Minerva.Dawntrail.Foray.CriticalEngagement.CE113ScourgeOfMind;
...
sealed class WallopNarrow(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.Wallop2, (uint)AID.Wallop3], new AOEShapeRect(60f, 5f));
sealed class WallopWide(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Wallop1, new AOEShapeRect(60f, 10f));
sealed class SurpriseAttack(ModuleBase module) : Components.ChargeAOEs(module, (uint)AID.SurpriseAttack, 3f);
sealed class DarkII(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DarkII, new AOEShapeCone(65f, 45f.Degrees()));
sealed class MindBlast(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.MindBlast);
sealed class ArcaneBlast(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.ArcaneBlast);
sealed class VoidThunderIII(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.VoidThunderIII);

sealed class FireIceTrap(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [], _firePlayers = [], _icePlayers = [];
    ...
    private readonly List<(WPos position, bool isFire)> jesters = [];
        ...
        switch (status.ID)
        {
            case (uint)SID.ImpElement:
                if (actor.OID == (uint)OID.JestingJackanapes)
                {
                    ...
                    if (first)
                    {
                        AddTraps(position.Quantized(), World.FutureTime(10d), isFire);
                    }
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, CFCID = 1018u, NameID = 33u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class CE113ScourgeOfMind(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
