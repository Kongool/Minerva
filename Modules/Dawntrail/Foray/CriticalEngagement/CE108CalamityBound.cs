...
namespace Minerva.Dawntrail.Foray.CriticalEngagement.CE108CalamityBound;
...
sealed class SundersealRoar(ModuleBase module) : Components.RaidwideCastDelay(module, (uint)AID.SundersealRoarVisual, (uint)AID.SundersealRoar, 0.9d);
sealed class Gigaflare(ModuleBase module) : Components.RaidwideCastDelay(module, (uint)AID.GigaflareVisual, (uint)AID.Gigaflare, 0.9d, "Raidwide + bleed");
sealed class VoidThunderIII(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.VoidThunderIII, new AOEShapeRect(60f, 4f), 4);
sealed class TidalBreath(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TidalBreath, new AOEShapeCone(40f, 90f.Degrees()));
sealed class KarmicDrain(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.KarmicDrain, new AOEShapeCone(60f, 30f.Degrees()), 3);
sealed class Explosion(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Explosion, 22f);
sealed class Flare(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Flare, 10f);
sealed class SealAsunder(ModuleBase module) : Components.CastHints(module, [(uint)AID.SealAsunder1, (uint)AID.SealAsunder2, (uint)AID.SealAsunder3], "Enrage!", true);

sealed class Seals(ModuleBase module) : Components.GenericTowersOpenWorld(module)
{
...
{
    public CE108CalamityBoundStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, CFCID = 1018u, NameID = 37u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class CE108CalamityBound(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
