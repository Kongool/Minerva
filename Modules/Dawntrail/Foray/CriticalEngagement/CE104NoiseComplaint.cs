...
namespace Minerva.Dawntrail.Foray.CriticalEngagement.CE104NoiseComplaint;
...
sealed class LightningCrossingMammothBoltEpicenterShock(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class Heave(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.Heave1, (uint)AID.Heave2], new AOEShapeCone(60f, 60f.Degrees()));
sealed class Squash(ModuleBase module) : Components.SingleTargetDelayableCast(module, (uint)AID.Squash);
sealed class AgitatedGroan(ModuleBase module) : Components.RaidwideCastDelay(module, (uint)AID.AgitatedGroanVisual, (uint)AID.AgitatedGroan, 0.9d);

sealed class RushingRumbleRampage(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
    private bool? lightningIsCardinal;
    private readonly List<Actor> activebirds = [];
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        ...
        {
            var dir = activebirds[0].Position - Module.Center;
            AOEInstance[] bait = [new(new AOEShapeRect(dir.Length(), 4f), Module.Center.Quantized(), Angle.FromDirection(dir), activation)];
            return bait;
        }
    }
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        if (iconID == (uint)IconID.Chatterbird)
        {
            ...
            {
                showBait = true;
                activation = World.FutureTime(6.3d);
            }
        }
    }
    ...
    private void InitIfReady()
    {
        if (_aoes.Count == 0 && activebirds.Count != 0 && lightningIsCardinal != null)
        {
            ...
            _aoes.Add(new(new AOEShapeRect(dir.Length(), 4f), primaryPos.Quantized(), angle, World.FutureTime(6.3d)));
            _aoes.Add(new(LightningCrossingMammothBoltEpicenterShock.CircleBig, destination.Quantized(), default, World.FutureTime(9.4d)));
            var act = World.FutureTime(10.5d);
...
{
    public CE104NoiseComplaintStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, CFCID = 1018u, NameID = 44u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class CE104NoiseComplaint(WorldState ws, Actor primary) : ModuleBase(ws, primary, ArenaCenter.Quantized(), new ArenaBoundsCircle(23f))
{
    public static readonly WPos ArenaCenter = new(461f, -363f);

    protected override bool CheckPull() => base.CheckPull() && Raid.Player()!.Position.InCircle(Module.Center, 23f);
}
