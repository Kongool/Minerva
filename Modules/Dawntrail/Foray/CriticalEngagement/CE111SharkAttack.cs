...
namespace Minerva.Dawntrail.Foray.CriticalEngagement.CE111SharkAttack;
...
sealed class TidalGuillotine(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        {
            _aoes.Add(new(circle, spell.TargetXZ, default, World.FutureTime(_aoes.Count == 1 ? 8.7d : 9.9d)));
        }
    }
...
sealed class MarineMayhem(ModuleBase module) : Components.RaidwideCastDelay(module, (uint)AID.MarineMayhemVisual, (uint)AID.MarineMayhem, 1.2f);

sealed class PelagicCleaverHydrocleave : Components.SimpleAOEGroups
{
    public PelagicCleaverHydrocleave(ModuleBase module) : base(module, [(uint)AID.Hydrocleave, (uint)AID.PelagicCleaver], new AOEShapeCone(50f, 30f.Degrees()))
    {
        MaxDangerColor = 1;
    }
}

abstract class OpenWater(ModuleBase module, int maxCasts, float timeToMove, Angle increment, bool isInner) : Components.GenericAOEs(module)
{
...
sealed class OpenWaterInside(ModuleBase module) : OpenWater(module, 35, 1.2f, 22.5f.Degrees(), true);
sealed class OpenWaterOutside(ModuleBase module) : OpenWater(module, 59, 0.8f, 12.5f.Degrees(), false);

sealed class CE111SharkAttackStates : StateMachineBuilder
{
    public CE111SharkAttackStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, GroupID = 1018u, CFCID = 1018u, NameID = 41u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class CE111SharkAttack(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
