...
namespace Minerva.Dawntrail.Foray.CriticalEngagement.CE107Unbridled;
...
sealed class ArenaChange(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        if (spell.Action.ID == (uint)AID.BoilOverVisual && Module.Bounds.Radius > 25f)
        {
            _aoe = [new(donut, Module.Center, default, Module.CastFinishAt(spell, 1d))];
        }
    }
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        {
            Module.Bounds = CE107Unbridled.DefaultArena;
            Module.Center = Module.Center.Quantized();
            _aoe = [];
        }
    }
}

sealed class BoilOverChanneledHeightenedRage(ModuleBase module) : Components.RaidwideCasts(module, [(uint)AID.BoilOver, (uint)AID.ChanneledRage, (uint)AID.HeightenedRage]);
sealed class WhiteHotRage(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WhiteHotRage, 6f);
sealed class HeatedOutburst(ModuleBase module) : Components.SimpleAOEGroupsByTimewindow(module, [(uint)AID.HeatedOutburst], 13f, riskyWithSecondsLeft: 4d);
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, CFCID = 1018u, NameID = 35u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class CE107Unbridled(WorldState ws, Actor primary) : ModuleBase(ws, primary, startingArena.Center, startingArena)
{
