...
namespace Minerva.Foray.CriticalEngagement.CE202AcceptNoImitators;
...
sealed class BlackenedRain(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.BlackenedRain);
sealed class TongueOfFlame(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TongueOfFlame, new AOEShapeCircle(15.0f));
sealed class DarkDealing(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.DarkDealing);
sealed class CyclonicRing(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.CyclonicRing, new AOEShapeDonut(10.0f, 30.0f));
sealed class CycloneCrossing(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.CycloneCrossing1, new AOEShapeCross(60.0f, 8.0f));
sealed class WindSphere(ModuleBase module) : Components.Voidzone(module, 17.5f, module => module.Enemies((uint)OID.WindSphere).Where(z => z.EventState != 7));

sealed class HellfireFetch : Components.SimpleAOEs
{
    public HellfireFetch(ModuleBase module) : base(module, (uint)AID.HellfireFetch, new AOEShapeCircle(6.0f), 8, 5.0f)
    {
        MaxDangerColor = 4;
    }
}
...
{
    public HellwardBoundCharge(ModuleBase module) : base(module, (uint)AID.HellwardBoundStart, 5.0f)
    {
        Color = Colors.Danger;
    }
}

sealed class HellwardBound(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class HellishBreath(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> aoes = [];
...
sealed class ShapeshiftingSupercellRings(ModuleBase module) : Components.GenericAOEs(module)
{
...
{
    private readonly List<ActorCastInfo> rotations = [];
    private readonly AOEShapeCone shape = new(60.0f, 45.0f.Degrees());
    private Angle increment;

    public ShapeshiftingSupercell(ModuleBase module) : base(module)
    {
...
{
    public CE202AcceptNoImitatorsStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, CFCID = 1093u, NameID = 63u, PrimaryActorOID = (uint)OID.Metamorph, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Equilius (ported from BMR)")]
[SkipLocalsInit]
public sealed class CE202AcceptNoImitators(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(500.000f, -310.000f), new ArenaBoundsCircle(25f))
{
