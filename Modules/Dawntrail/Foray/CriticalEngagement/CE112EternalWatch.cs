...
namespace Minerva.Dawntrail.Foray.CriticalEngagement.CE112EternalWatch;
...
{
    public AncientStoneAncientAeroIIISlow(ModuleBase module) : base(module, [(uint)AID.AncientAeroIII2, (uint)AID.AncientStoneIII2], WindStoneLightSurge.Cone, 4)
    {
        MaxDangerColor = 2;
    }
}

sealed class AncientStoneAncientAeroIIIFast(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.AncientAeroIII1, (uint)AID.AncientStoneIII1], WindStoneLightSurge.Cone)
{
...
sealed class AncientHoly(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AncientHoly1, 26f);
sealed class RadiantWave(ModuleBase module) : Components.RaidwideCastDelay(module, (uint)AID.RadiantWaveVisual, (uint)AID.RadiantWave, 1.2f);
sealed class Scratch(ModuleBase module) : Components.SingleTargetDelayableCast(module, (uint)AID.Scratch);
sealed class HolyBlaze(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HolyBlaze, new AOEShapeRect(60f, 2.5f));

sealed class WindStoneLightSurge(ModuleBase module) : Components.GenericAOEs(module)
{
    public readonly List<AOEInstance> AOEs = [];
    private static readonly AOEShapeCircle circle = new(15f);
    public static readonly AOEShapeCone Cone = new(40f, 30f.Degrees());
    private readonly List<Actor> spheres = [];
    private readonly List<Actor> spheresStone = [];
    private readonly List<Actor> spheresWind = [];
...
{
    public CE112EternalWatchStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, CFCID = 1018u, NameID = 46u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class CE112EternalWatch(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
