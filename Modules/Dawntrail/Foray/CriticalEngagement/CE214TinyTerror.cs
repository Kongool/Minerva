...
namespace Minerva.Dawntrail.Foray.CriticalEngagement.CE214TinyTerror;
...
sealed class DiminutiveDualcast(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> Casters = [];
...
sealed class TinyMeteor(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TinyMeteor, new AOEShapeCircle(6.0f))
{
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        var count = Casters.Count;
...
sealed class FlareHolyMerge(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        ...
        {
            var sphere = World.Actors.Find(tether.Target);
            if (sphere != null)
            {
        ...
        {
            var activationStart = World.FutureTime(9.1d);
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        for (var i = 0; i < max; i++)
        {
            ...
            if (!combination.IsFlare)
            {
                ...
                hints.AddForbiddenZone(new SDKnockbackInCircleAwayFromOriginPlusAOECircles(Module.Center, combination.Origin, holyKnockBackDistance, 19.0f,
...
sealed class SphereGrowable(ModuleBase module) : ModuleComponent(module)
{
    private static readonly AOEShapeCircle flareShape = new(18.0f);
    private const float holyKnockBackDistance = 15.0f;
    ...
    public override void OnActorCreated(Actor actor)
    {
        if (actor.OID == (uint)OID.TinyApprentice)
        {
            ...
            if (mages.Count == 4)
            {
                mages.Sort(delegate (Actor x, Actor y)
                {
                    ...
                    var xAngle = (x.Position - Module.Center).ToAngle();
                    var yAngle = (y.Position - Module.Center).ToAngle();
        ...
        {
            orb = actor;
            activation = World.FutureTime(15.6);
        }
    }
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (orb.OID == (uint)OID.HolySphereGrow)
        {
            hints.AddForbiddenZone(new SDKnockbackInCircleAwayFromOrigin(Module.Center, target.Position, holyKnockBackDistance, 19.0f), activation);
        }
    }
...
{
    public CE214TinyTerrorStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, CFCID = 1093u, NameID = 60u, PrimaryActorOID = (uint)OID.TinyMage, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Equilius (ported from BMR)")]
[SkipLocalsInit]
public sealed class CE214TinyTerror(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(152.000f, 716.000f), new ArenaBoundsCircle(20f))
{
