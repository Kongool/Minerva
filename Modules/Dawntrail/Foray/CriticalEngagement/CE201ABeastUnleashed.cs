...
namespace Minerva.Dawntrail.Foray.CriticalEngagement.CE201ABeastUnleashed;
...
[SkipLocalsInit]
sealed class TopazRay(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.TopazRay1, (uint)AID.TopazRay2], 4f)
{
...
{
    public RubyReflection(ModuleBase module) : base(module)
    {
        var center = Center;
        Reflection1Zero =
        [
    ...
    public override void OnActorEAnim(Actor actor, uint state)
    {
        if (actor.OID == (uint)OID.TopazOutline1)
        {
            ...
            {
                var act = World.FutureTime(11.8d);
        ...
        else if (actor.OID == (uint)OID.TopazOutline2)
        {
            ...
            {
                var act = World.FutureTime(14.8d);
...
[SkipLocalsInit]
sealed class SpinebreakingStampede(ModuleBase module) : Components.GenericKnockback(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID is var id && id == (uint)AID.SpinebreakingStampedeMiddleVisual)
        {
            ...
            void AddSource(Angle offset) => _kbs.Add(new(Center, 15f, act, rect, rot + offset, Kind.DirForward));
        }
            ...
            case 3:
                {
                    ...
                    if (!firstImmune)
                    {
                        hints.AddForbiddenZone(isAlongXAxis ? new SDKnockbackInAABBSquareLeftRightAlongXAxisTowardsGoal(Center, 15f, 19f, last.Origin, 5f)
                            : new SDKnockbackInAABBSquareLeftRightAlongZAxisTowardsGoal(Center, 15f, 19f, last.Origin, 5f), firstActivation);
                        return;
                    }
                ...
                void AddAwayFromOrigin(WPos origin, DateTime activation) => hints.AddForbiddenZone(new SDKnockbackInAABBSquareAwayFromOrigin(Center, origin, 30f, 19f), activation);
        }
    }
}
...
{
    public CE201ABeastUnleashedStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, GroupID = 1093u, CFCID = 1093u, NameID = 56u, PrimaryActorOID = (uint)OID.AtlasCarbuncle, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "gynorhino (ported from BMR)")]
[SkipLocalsInit]
public sealed class CE201ABeastUnleashed(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(238f, 352f), new ArenaBoundsSquare(20f))
{
