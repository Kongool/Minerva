...
namespace Minerva.Dawntrail.Foray.CriticalEngagement.CE203AheadoftheCompetition;
...
sealed class ElementalCascade(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.ElementalCascade, (uint)AID.ElementalCascade1,
            (uint)AID.ElementalCascade2, (uint)AID.ElementalCascade3, (uint)AID.ElementalCascade4], new AOEShapeCircle(8.0f));
sealed class Discordance(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Discordance);
sealed class ElementalCascadeElements(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.ElementalCascadeFire, (uint)AID.ElementalCascadePoison,
    (uint)AID.ElementalCascadeLightning, (uint)AID.ElementalCascadeLight, (uint)AID.ElementalCascadeIce ], new AOEShapeCircle(6.0f));
sealed class StunningSheen(ModuleBase module) : Components.CastGaze(module, (uint)AID.StunningSheen);

sealed class ScarletThread : Components.SimpleAOEs
{
    public ScarletThread(ModuleBase module) : base(module, (uint)AID.ScarletThread, new AOEShapeRect(70.0f, 2.0f))
    {
        Color = Colors.Danger;
    }
}
...
{
    public IceBurst(ModuleBase module) : base(module, (uint)AID.IceBurst, new AOEShapeCone(40.0f, 10.0f.Degrees()))
    {
        Color = Colors.Danger;
    }
}
...
sealed class ManyHeadedBreath(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class Dissipate(ModuleBase module) : Components.Voidzone(module, 8.5f, module => module.Enemies((uint)OID.PoisonOrb).Where(z => z.EventState != 7))
{
...
{
    public CE203AheadoftheCompetitionStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, CFCID = 1093u, NameID = 62u, PrimaryActorOID = (uint)OID.PhantomHydra, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Equilius (ported from BMR)")]
[SkipLocalsInit]
public sealed class CE203AheadoftheCompetition(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-82.000f, 485.000f), new ArenaBoundsCircle(20f))
{
