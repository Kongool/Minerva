...
namespace Minerva.Dawntrail.Foray.CriticalEngagement.CE110FlameOfDusk;
...
sealed class DreadDive(ModuleBase module) : Components.SingleTargetDelayableCast(module, (uint)AID.DreadDive);
sealed class Lamplight(ModuleBase module) : Components.RaidwideCastDelay(module, (uint)AID.LamplightVisual, (uint)AID.Lamplight, 0.9f);

sealed class MoltAOEs(ModuleBase module) : Components.GenericAOEs(module)
{
    public readonly List<AOEInstance> AOEs = [];
    private readonly List<(WPos position, AOEShape? shape, bool knockback, DateTime activation)> pendingMechanics = [];
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        {
            var pos = World.Actors.Find(spell.MainTargetID)?.Position;
...
sealed class MoltKB(ModuleBase module) : Components.GenericKnockback(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (_kb.Length != 0)
        {
            ...
            if (!IsImmune(slot, act))
            {
                ...
                hints.AddForbiddenZone(new SDKnockbackInCircleAwayFromOrigin(Center, kb.Origin, 20f, 18f), act);
            }
        }
    }
...
{
    public CE110FlameOfDuskStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, GroupID = 1018u, CFCID = 1018u, NameID = 47u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class CE110FlameOfDusk(WorldState ws, Actor primary) : ModuleBase(ws, primary, new WPos(-570f, -160f).Quantized(), new ArenaBoundsCircle(20f))
{
