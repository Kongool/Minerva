...
namespace Minerva.Dawntrail.Foray.CriticalEngagement.CE115CursedConcern;
...
sealed class Earthquake(ModuleBase module) : Components.RaidwideCastDelay(module, (uint)AID.EarthquakeVisual, (uint)AID.Earthquake, 0.9f);
sealed class Waterspout : Components.SimpleAOEs
{
    public Waterspout(ModuleBase module) : base(module, (uint)AID.Waterspout, 12f, 8)
    {
        MaxDangerColor = 1;
    }
}
sealed class HoardWealth(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HoardWealth, new AOEShapeCone(40f, 30f.Degrees()), 3);

sealed class CostOfLiving(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.CostOfLiving, 30f, maxCasts: 1)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (Casters.Count != 0)
        {
            ...
            if (!IsImmune(slot, act))
            {
                ...
                hints.AddForbiddenZone(new SDKnockbackInCircleAwayFromOrigin(Center, c.Origin, 30f, 23f), act);
            }
        }
    }
}

sealed class BuyersRemorseForcedMarch(ModuleBase module) : Components.GenericKnockback(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (affectedPlayers[slot])
        {
            hints.AddForbiddenZone(new SDCircle(Center, 15f), activation);
            var dir = actor.Position - Center;
            var len = dir.Length();
...
sealed class BuyersRemorseECFreeze(ModuleBase module) : Components.StayMove(module)
{
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        switch (status.ID)
...
sealed class WhatreYouBuying(ModuleBase module) : Components.GenericAOEs(module)
{
...
{
    public CE115CursedConcernStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, GroupID = 1018u, CFCID = 1018u, NameID = 45u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class CE115CursedConcern(WorldState ws, Actor primary) : ModuleBase(ws, primary, new WPos(72f, -545f).Quantized(), new ArenaBoundsCircle(25f))
{
