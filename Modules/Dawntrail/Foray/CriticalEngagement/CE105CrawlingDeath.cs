...
namespace Minerva.Dawntrail.Foray.CriticalEngagement.CE105CrawlingDeath;
...
sealed class TheGripOfPoison(ModuleBase module) : Components.RaidwideCastDelay(module, (uint)AID.TheGripOfPoisonVisual, (uint)AID.TheGripOfPoison, 0.9f, "Raidwide + bleed");
sealed class DirtyNails(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.DirtyNails);

sealed class Clawmarks(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        if (id is (uint)OID.Clawmarks1 or (uint)OID.Clawmarks2 or (uint)OID.Clawmarks3)
        {
            ...
            {
                AOEs.Add(new(rect, actor.Position.Quantized(), actor.Rotation, World.FutureTime(activation), actorID: actor.InstanceID));
            }
        }
    }
...
sealed class ClawingShadow(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorEAnim(Actor actor, uint state)
    {
        ...
        {
            _aoes.Add(new(Cone, actor.Position.Quantized(), actor.Rotation, World.FutureTime(_aoes.Count < 3 ? 8.1d : 10.6d)));
        }
    }
...
sealed class Crosshatch(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
...
{
    public CE105CrawlingDeathStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, CFCID = 1018u, NameID = 36u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class CE105CrawlingDeath(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(681, 534f), new ArenaBoundsSquare(21f))
{
    protected override bool CheckPull() => base.CheckPull() && Raid.Player()!.Position.InSquare(Module.Center, 21f);
}
