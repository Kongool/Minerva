...
namespace Minerva.Dawntrail.Foray.CriticalEngagement.CE106TrialByClaw;
...
sealed class PrimalRoar(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.PrimalRoar);
sealed class PrismaticWingMadeMagicCircle(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.PrismaticWingCircle, (uint)AID.MadeMagicCircle], 22f, riskyWithSecondsLeft: 4d);
sealed class PrismaticWingMadeMagicDonut(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.PrismaticWingDonut, (uint)AID.MadeMagicDonut], new AOEShapeDonut(5f, 31f), riskyWithSecondsLeft: 5d);
sealed class CrystallizedEnergy(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.CrystallizedEnergy1, (uint)AID.CrystallizedEnergy2, (uint)AID.CrystallizedEnergy3], 7f, riskyWithSecondsLeft: 5d);
sealed class CrystallizedChaosDonut1(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.CrystallizedChaos1, (uint)AID.CrystallizedChaos4, (uint)AID.CrystallizedChaos7], new AOEShapeDonut(7f, 13f), riskyWithSecondsLeft: 5d);
sealed class CrystallizedChaosDonut2(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.CrystallizedChaos2, (uint)AID.CrystallizedChaos5, (uint)AID.CrystallizedChaos8], new AOEShapeDonut(13f, 19f), riskyWithSecondsLeft: 5d);
sealed class CrystallizedChaosDonut3(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.CrystallizedChaos3, (uint)AID.CrystallizedChaos6, (uint)AID.CrystallizedChaos9], new AOEShapeDonut(19f, 25f), riskyWithSecondsLeft: 5d);

sealed class CE106TrialByClawStates : StateMachineBuilder
{
    public CE106TrialByClawStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, CFCID = 1018u, NameID = 38u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class CE106TrialByClaw(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
    private static readonly ArenaBoundsCustom arena = new([new Polygon(new(-413.9f, 75f), 24.5f, 32)]);

    protected override bool CheckPull() => base.CheckPull() && Raid.Player()!.Position.InCircle(Module.Center, 25f);
}
