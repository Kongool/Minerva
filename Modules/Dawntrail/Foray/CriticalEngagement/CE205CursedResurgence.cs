...
namespace Minerva.Dawntrail.Foray.CriticalEngagement.CE205CursedResurgence;
...
sealed class HowlingDarkness(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.HowlingDarkness);
sealed class SnakingNecrobreath(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SnakingNecrobreath, new AOEShapeCone(60f, 135f.Degrees()));
sealed class GraveMold(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GraveMold, 8f);
sealed class Cauterize(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Cauterize, new AOEShapeRect(40f, 5f));
sealed class Catching(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        {
            var actors = World.Actors.ToArray();
...
sealed class BreathInThrees(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.BreathInThrees, (uint)AID.BreathInThrees1], new AOEShapeCone(60f, 60f.Degrees()))
{
...
sealed class Necrohaze(ModuleBase module) : Components.GenericAOEs(module)
{
    // regular voidzone
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            if (_isMoving)
            {
                ...
                var offset = position - Module.Center;
                ...
                aoes.Add(new(new AOEShapeArcCapsule(5.0f, lengthDirection, Module.Center), position, rotation));
            }
            else
            {
...
sealed class AetherialWard(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        if (_angles.Count != 0)
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                Arena.ZoneCone(Module.Center, 4f, 5f, adjustedAngle, 45f.Degrees(), Colors.Danger);
            }
        }
    }
    ...
    public override void AddHints(int slot, Actor actor, TextHints hints)
    {
        if (_angles.Count != 0)
        {
            ...
            {
                ref var angle = ref angles[i];
                if (actor.Position.InCone(Module.Center, angle, 45f.Degrees()))
                {
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (_angles.Count != 0)
        {
            ...
            for (var i = 0; i < 4; i++)
            {
                ...
                if (!found)
                {
                    hints.GoalZones.Add(GoalCone(Module.Center, 5f, 15f, cardinal, 45f.Degrees(), 5f));
                    break;
                }
            }
        }
        ...
        Func<WPos, float> GoalCone(WPos center, float innerRadius, float outerRadius, Angle centerDirection, Angle halfAngle, float weight = 1f)
        {
            ...
            return p =>
            {
                ...
                return p.InDonutCone(Module.Center, innerR, outerR, centerDirection, halfAngle) ? weight : default;
            };
        }
    }
}
...
{
    public CE205CursedResurgenceStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, CFCID = 1093u, NameID = 53u, PrimaryActorOID = (uint)OID.ClaretDragon, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "gynorhino (ported from BMR)")]
[SkipLocalsInit]
public sealed class CE205CursedResurgence(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-688f, 150f), new ArenaBoundsSquare(20f))
{
