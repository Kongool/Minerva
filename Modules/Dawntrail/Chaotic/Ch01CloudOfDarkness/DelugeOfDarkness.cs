...
sealed class Phase2InnerCells(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        if (!_config.ShowOccupiedTiles)
            return [];
        var cell = CellIndex(actor.Position - Center) - 3;
        ...
        for (var i = 0; i < 28; ++i)
        {
            ...
            if (breaktime != default)
            {
                if (i == cell)
                {
                    if ((breaktime - World.CurrentTime).TotalSeconds < 6d)
                    {
    ...
    public override void AddHints(int slot, Actor actor, TextHints hints)
    {
        ...
        if (breakTime != default)
        {
            var remaining = Math.Max(0d, (breakTime - World.CurrentTime).TotalSeconds);
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        {
            0x00200010u => World.FutureTime(44d),
            0x00800040u => World.FutureTime(6d),
            0x00080004u => World.CurrentTime,
            _ => default,
        };
    }
...
sealed class Phase2AIHints(ModuleBase module) : ModuleComponent(module)
{
