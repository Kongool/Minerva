...
sealed class ArenaChanges(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        if (index == 0x00)
        {
            ...
            {
                case 0x00200010u:
                    SetAOE(new AOEShapeCustom(Center, GetDefaultShape(), GetDiamondShape()));
                    break;
                case 0x00020001u:
                    SetAOE(new AOEShapeCustom(Center, GetDefaultShape(), [.. GetP2ShapeNoDonut(), .. GetP2Donut()]));
                    break;
            }
        }
        else if (index == 0x02)
    ...
    public override void OnEventDirectorUpdate(uint updateID, uint param1, uint param2, uint param3, uint param4)
    {
        ...
        switch (param1)
        {
            case 0x10000000u: // default arena
                Bounds = new ArenaBoundsCircle(40f);
    ...
    {
        var pos = Center;
        _aoe = [new(shape, pos, default, World.FutureTime(9d), shapeDistance: shape.Distance(pos, default))];
    }
}
