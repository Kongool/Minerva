...
sealed class RuinfallKB(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.RuinfallKB, 21f, stopAfterWall: true, kind: Kind.DirForward)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (!IsImmune(slot, act))
        {
            ...
            {
                hints.AddForbiddenZone(new SDKnockbackInAABBRectFixedDirection(Center, 21f * c.Direction.ToDirection(), 19f, 14f), act); // rect intentionally slightly smaller to prevent sus knockback
                return;
            }
            ...
            var isDelayDeltaLow = (act - World.CurrentTime).TotalSeconds < 5d;
...
sealed class RuinfallTower(ModuleBase module) : Components.CastTowers(module, (uint)AID.RuinfallTower, 6f, 2, 2)
{
