...
namespace Minerva.Dawntrail.Trial.T08Enuo;
...
class Meteorain(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Meteorain);

sealed class NaughtGrowsAOE(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.NaughtGrowsAOE, new AOEShapeCircle(40f));

// cone degree is an estimate.
sealed class GazeOfTheVoidCones(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.GazeOfTheVoidCones, (uint)AID.GazeOfTheVoid1], new AOEShapeCone(20f, 22.5f.Degrees()), 7, 10);

// Flare marker : tank buster magic damage
sealed class DeepFreeze(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.DeepFreeze, new AOEShapeCircle(8), true, true);

//Naughthunts
/** The solution credit here goes to @Endings.  Any mistakes in implementation from
 * here belong to @wen.
*/
sealed class NaughtHunts(ModuleBase module) : Components.StandardChasingAOEs(module, new AOEShapeCircle(6f), (uint)AID.EndlessChaseRest, (uint)AID.EndlessChaseRest, 2.9f, 0.7d, 13, icon: (uint)IconID.EndlessChaseIcon, activationDelay: 6d)
{
    ...
    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        ...
        {
            var p = World.Actors.Find(tether.Target);
            if (_tethercount == 2)
            {
                Chasers.Clear();
            }
            if (p != null)
            {
                Chasers.Add(new(Shape, p, source.Position, 0, MaxCasts, World.FutureTime(ActivationDelay), SecondsBetweenActivations));
                ++_tethercount;
            }
            if (_tethercount == 4)
            {
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell) // Don't count the first action so that we can have the same count between the two.
    {
        ...
        {
            var pos = spell.MainTargetID == caster.InstanceID ? caster.Position.Quantized() : World.Actors.Find(spell.MainTargetID)?.Position ?? spell.TargetXZ;
            Advance(pos, MoveDistance, World.CurrentTime);
...
sealed class ShroudedHoly(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.ShroudedHolyStack, 7, 4);

sealed class MeltdownSpread(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.MeltdownSpread, 5f);
sealed class MeltdownAOE(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MeltdownAOE, new AOEShapeCircle(5f));

sealed class SilentTorrentSmall(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SilentTorrentSmall, new AOEShapeDonutSector(17f, 19f, 10f.Degrees()));
sealed class SilentTorrentMedium(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SilentTorrentMedium, new AOEShapeDonutSector(17f, 19f, 20f.Degrees()));
sealed class SilentTorrentLarge(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SilentTorrentLarge, new AOEShapeDonutSector(17f, 19f, 30f.Degrees()));

sealed class VacuumCircle(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.VacuumCircle, new AOEShapeCircle(7f));

class ArenaSwitcher : ModuleComponent
{
    public Actor? Beacon;

    public ArenaSwitcher(ModuleBase module) : base(module)
    {
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        if (index == 0 && state == 0x00020001)
            ...
            Bounds = new ArenaBoundsSquare(80);
...
// Void arena actors
class LoomingShadow(ModuleBase module) : Components.Adds(module, (uint)OID.LoomingShadow);
class Shadows(ModuleBase module) : Components.Adds(module, (uint)OID.UncastShadow, 1);
class Beacon(ModuleBase module) : Components.Adds(module, (uint)OID.BeaconInTheDark, 1, true);

class EmptyShadow(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.EmptyShadowAOE, new AOEShapeCircle(10f));
// big explosion For when Looming emptiness spawns. Went a little small: proximity AOE
class LoomingEmptinessAOE(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.LoomingEmptinessAOE, 42f);

class Nothingness(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Nothingness, new AOEShapeRect(100f, 2f));

// back to circle arena

sealed class LightlessWorld(ModuleBase module) : Components.CastLineOfSightAOE(module, (uint)AID.LightlessWorld, 40f, false)
{
...
//almagest
class Almagest(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Almagest);

sealed class NaughtGrows(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.NaughtGrowsAOESmall, 12f);

sealed class DimensionZero(ModuleBase module) : Components.LineStack(module, iconID: (uint)IconID.DimensionZeroIcon, (uint)AID.DimensionZeroRect, 0d, 60f, 4f, 8);

[SkipLocalsInit]
sealed class EnuoStates : StateMachineBuilder
{
    public EnuoStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 1115u, CFCID = 1115u, NameID = 14749u, PrimaryActorOID = (uint)OID.Enuo, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Wen (ported from BMR)")]
[SkipLocalsInit]
public sealed class Enuo(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(100f, 100f), new ArenaBoundsCircle(20f));
