...
namespace Minerva.Dawntrail.Alliance.A35ShinryuParadox;
...
abstract class FloorAOE(ModuleBase module, uint action) : Components.GenericAOEs(module, action)
{
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        foreach (var c in Casters)
        {
            ...
            if (danger == 1 && floor == 0)
                ...
                return new ReadOnlySpan<AOEInstance>([new AOEInstance(new AOEShapeCircle(2), Center - new WDir(0, 6), default, activation)]);
            else if (danger == 1 && floor == 1)
                // drop down to lower floor from top floor
                return new ReadOnlySpan<AOEInstance>([new AOEInstance(new AOEShapeDonut(6, 100), Center - new WDir(0, 6), default, activation)]);
            else if (danger == 0 && floor == 0)
                // hop up to top floor from lower floor
                return new ReadOnlySpan<AOEInstance>([new AOEInstance(new AOEShapeDonut(2, 100), Center - new WDir(0, 6), default, activation)]);
            else if (danger == 0 && floor == 1)
                // stay on top floor to avoid danger on lower floor
                return new ReadOnlySpan<AOEInstance>([new AOEInstance(new AOEShapeCircle(6), Center - new WDir(0, 6), default, activation)]);
        }
        return default;
    }
...
class CosmicBreath(ModuleBase module) : FloorAOE(module, (uint)AID.CosmicBreath)
{
    protected override int GetDangerFloor(int slot, Actor actor) => 1;
}

class CosmicTail(ModuleBase module) : FloorAOE(module, (uint)AID.CosmicTail)
{
    protected override int GetDangerFloor(int slot, Actor actor) => 0;
}

class AtomicTail(ModuleBase module) : FloorAOE(module, (uint)AID.AtomicTail)
{
    protected override int GetDangerFloor(int slot, Actor actor) => 0;
}

class AtomicTailArena(ModuleBase module) : ModuleComponent(module)
{
    public override void OnMapEffect(byte index, uint state)
    {
        if (index == 0x00 && state == 0x00200010)
        {
            Shape[] arenaOutline = [new Rectangle(Center, 30f, 20f)];
            // Set a boundary to keep pc from jumping down into atomic tail.
            Shape[] circleOfDanger = [new Circle(Center - new WDir(0, 6), 6)];
            ...
            Bounds = atomicTailArenaBounds;
        }

        if (index == 0x00 && state == 0x02000100)
            Bounds = new ArenaBoundsRect(30, 20);
    }
}

class TwilightNebula(ModuleBase module) : FloorAOE(module, (uint)AID.TwilightNebula1)
{
...
// between floors.
class StarflareTimeGroups(ModuleBase module) : Components.SimpleAOEGroupsByTimewindow(module,
...
class VortexStayMove(ModuleBase module) : Components.StayMove(module)
{
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        Requirement r;
        ...
        if (p >= 0)
        {
            var state = new PlayerState(r, World.FutureTime(7));
            SetState(p, in state);
        }
    }
...
class UpDownCounter(ModuleBase module) : Components.CastCounterMulti(module, [(uint)AID.CosmicBreath, (uint)AID.CosmicTail]);

// There are more spells for DarkNova p2, I haven't seen them yet.  Will add if they are confirmed with a log.
class DarkNova(ModuleBase module) : Components.BaitAwayIcon(module, new AOEShapeCircle(6), (uint)IconID.Tankbuster, (uint)AID.DarkNova, centerAtTarget: true);
 ...
 */
class CelestialTrail(ModuleBase module) : Components.CastTowers(module, (uint)AID.CelestialTrailTower, 2, 1, 10)
{
    BitMask _forbidden;
...
class EmptyProclamation(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.EmptyProclamation);
class Swordscross1(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.RightSwordscross1, (uint)AID.LeftSwordscross1], new AOEShapeRect(60, 15));
class Swordscross2(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.RightSwordscross2, (uint)AID.LeftSwordscross2], new AOEShapeRect(70, 18));
class TwinBlaze1(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TwinBlazeIn, new AOEShapeDonutSector(20, 60, 45.Degrees()));
class TwinBlaze2(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TwinBlazeOut, new AOEShapeCone(35, 45.Degrees()));
class CataclysmicBlade(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.CataclysmicBladeCone, new AOEShapeCone(60, 22.5f.Degrees()));

class Burst(ModuleBase module) : Components.ConcentricAOEs(module, [new AOEShapeCircle(10), new AOEShapeDonut(10, 20), new AOEShapeDonut(20, 30)])
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        AdvanceSequence(order, caster.Position, World.FutureTime(2));
    }
}

class CosmicFlame(ModuleBase module) : Components.Exaflare(module, new AOEShapeCircle(6))
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
...
class AtomicRay(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.AtomicRay)
{
...
class AtomicRayCast(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AtomicRay, new AOEShapeRect(60, 7.5f));

class GyreCharge(ModuleBase module) : Components.RaidwideCastDelay(module, (uint)AID.AtomicTailVisual1, (uint)AID.GyreCharge, 6.3f);

class SuperNova(ModuleBase module) : Components.StackWithIcon(module, (uint)IconID.Stack, (uint)AID.SuperNova, 6, 6.1f, minStackSize: 8)
{
...
[ModuleInfo(CFCID = 1117u, NameID = 14729u, PrimaryActorOID = (uint)OID.ShinryuParadox, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Xan, ported by wen (ported from BMR)")]
...
public class A35ShinryuParadox(WorldState ws, Actor primary)
    : ModuleBase(ws, primary, new(820f, -820f), new ArenaBoundsRect(30f, 20f))
{
    Actor? Groin;
    Actor? _bossP2;
