...
sealed class ArenaChanges(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        {
            _aoe = [];
            Bounds = Bounds.Radius == 29.5f ? new ArenaBoundsCustom(A23Kamlanaut.BuildP1Circle(), BuildVoidzone()) : BuildP2ArenaWithBridgesDonut();
        }
    }
    ...
    public override void OnActorRenderflagsChange(Actor actor, int renderflags)
    {
        ...
        {
            Bounds = Bounds.Radius == 29.5f ? A23Kamlanaut.BuildArena().arena : BuildP2ArenaWithBridges();
        }
    }
        ...
        switch (index)
        {
            case 0x00: // p1/p2 transition
                switch (state)
                {
                    case 0x00020001u:
                        ...
                        _aoe = [new(shape, center, default, World.FutureTime(5.1d), shapeDistance: shape.Distance(center, default))];
                        break;
                    case 0x00200010u:
            ...
            case 0x63: // bridges
                switch (state)
                {
                    case 0x00200010u:
                        var center = new WPos(-200f, 150f);
                        _aoe = [new(donut, center, default, World.FutureTime(4.3d), shapeDistance: donut.Distance(center, default))];
                        break;
                    case 0x00020001u:
