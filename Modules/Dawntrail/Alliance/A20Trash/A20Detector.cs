...
namespace Minerva.Dawntrail.Alliance.A20Detector;
...
sealed class Electray(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Electray, new AOEShapeRect(40f, 2.5f));
sealed class Electroswipe(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Electroswipe, new AOEShapeCone(50f, 60f.Degrees()));
sealed class ElectroswipeHint(ModuleBase module) : Components.CastInterruptHint(module, (uint)AID.Electroswipe, showNameInHint: true);

sealed class A20DetectorStates : StateMachineBuilder
{
    public A20DetectorStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 1058u, CFCID = 1058u, NameID = 14072u, PrimaryActorOID = (uint)OID.Detector1, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class A20Detector(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
