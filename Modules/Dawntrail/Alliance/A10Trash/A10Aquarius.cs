...
namespace Minerva.Dawntrail.Alliance.A10Aquarius;
...
sealed class CursedSphere(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.CursedSphere, 3f);
sealed class WaterIII(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WaterIII, 7f);
sealed class BubbleShower(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BubbleShower, new AOEShapeCone(6f, 30f.Degrees()));
sealed class Scoop(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Scoop, new AOEShapeCone(15f, 60f.Degrees()));
sealed class Agaricus(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Agaricus, 5f);
sealed class Beatdown(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Beatdown, new AOEShapeRect(9f, 1.5f));
sealed class SpiderWeb(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SpiderWeb, 6f);
sealed class HundredFists(ModuleBase module) : Components.CastInterruptHint(module, (uint)AID.HundredFists, showNameInHint: true);

public sealed class A10AquariusStates : StateMachineBuilder
{
    public A10AquariusStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 1015u, CFCID = 1015u, NameID = 13605u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class A10Aquarius(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
