...
namespace Minerva.Dawntrail.Alliance.A10Despot;
...
sealed class IsleDrop(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.IsleDrop, 6f);
sealed class WingCutter(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WingCutter, new AOEShapeCone(6f, 60f.Degrees()));
sealed class PanzerfaustHint(ModuleBase module) : Components.CastInterruptHint(module, (uint)AID.Panzerfaust, showNameInHint: true);
sealed class Panzerfaust(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.Panzerfaust);
sealed class ScraplineStorm(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.ScraplineStorm, 10f, kind: Kind.TowardsOrigin)
{
...
sealed class ScraplineTyphoon(ModuleBase module) : Components.GenericAOEs(module)
{
...
{
    public A10DespotStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 1015u, CFCID = 1015u, NameID = 13608u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class A10Despot(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
