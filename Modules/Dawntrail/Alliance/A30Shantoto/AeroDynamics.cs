...
sealed class AeroDynamics(ModuleBase module) : Components.GenericKnockback(module, stopAtWall: false)
{
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        if (status.ID == (uint)SID.WesterlyWinds)
        {
            ...
            if (p >= 0)
            {
                ...
                //Service.Log.Debug($"Adding West: {p}");
            }
        }
        if (status.ID == (uint)SID.EasterlyWinds)
        {
            ...
            if (p >= 0)
            {
                ...
                //Service.Log.Debug($"Adding East: {p}");
            }
        }
    }
        ...
        if (_statuskbs.Count != 0)
        {
            ...
            if (!IsImmune(slot, World.CurrentTime))
            {
                ...
                if (len > 0)
                {
                    ...
                    hints.AddForbiddenZone(new SDKnockbackFixedDirectionAgainstSafewalls(dir, swalls, 60f, len), World.CurrentTime);
                }
            }
        }
    }
