...
sealed class InsaneAirSnaps(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        ...
        {
            var origin = World.Actors.Find(p.OriginID);
            var target = World.Actors.Find(p.TargetID);
            ...
            _active.Add(new(InsaneAirData.SnapCone, origin.Position, rot, World.CurrentTime, color, true, origin.InstanceID));
        }
...
// -----------------------------
sealed class BlastingSnapPersistent(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.BlastingSnap1)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        switch (spell.Action.ID)
        {
            ...
            case (uint)AID.BlastingSnap1:
                _aoes.Add(new(InsaneAirData.SnapCone, caster.Position, caster.Rotation, World.CurrentTime, Colors.AOE, true, caster.InstanceID));
                break;
