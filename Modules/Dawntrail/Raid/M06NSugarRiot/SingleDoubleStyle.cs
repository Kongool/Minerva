...
sealed class SingleDoubleStyle(ModuleBase module) : Components.GenericAOEs(module)
{
        ...
        switch (source.OID)
        {
            ...
            case (uint)OID.ThrowUpTarget:
                ...
                for (var i = 0; i < count; ++i)
                {
                    ...
                    var direction = Angle.FromDirection(Center - sourceP).ToDirection();
    ...
    private void HandleSweetShot(Actor source, double time)
    {
        ...
        var direction = target ? Angle.FromDirection(Center - sourceP).ToDirection() : source.Rotation.ToDirection();
        var dirVector = (sourceP + 60f * direction).Quantized() - sourceP;
        _aoes.Add(new(new AOEShapeRect(dirVector.Length(), 3.5f), sourceP.Quantized(), Angle.FromDirection(dirVector), World.FutureTime(time)));
    }

    private void AddAOE(WPos position, double time)
    {
        _aoes.Add(new(circle, position.Quantized(), default, World.FutureTime(time)));
    }
