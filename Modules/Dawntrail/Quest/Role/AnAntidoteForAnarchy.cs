...
namespace Minerva.Dawntrail.Quest.Role.AnAntidoteForAnarchy;
...
sealed class StingingMalady(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.StingingMalady, StingingMaladyBait.Cone);
sealed class StingingMaladyBait(ModuleBase module) : Components.GenericBaitAway(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        if (spell.Action.ID == (uint)AID.StingingMaladyMarker)
            CurrentBaits.Add(new(caster, World.Actors.Find(spell.MainTargetID)!, Cone, World.FutureTime(5d)));
        else if (spell.Action.ID == (uint)AID.StingingMaladyBait)
            CurrentBaits.Clear();
    }
}

sealed class BewilderingBlight(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.BewilderingBlight, 6f);
sealed class BewilderingBlightTM(ModuleBase module) : Components.TemporaryMisdirection(module, (uint)AID.BewilderingBlight);

sealed class SkineaterSurge(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.SkineaterSurge1, (uint)AID.SkineaterSurge2], new AOEShapeCone(40f, 90f.Degrees()));
sealed class SkineaterSurgePoisonCloud(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.Burst1, (uint)AID.SkineaterSurgePoisonCloud], 10f);

sealed class SelfDestruct(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SelfDestruct, 5f);
sealed class SelfDestructKBMammet(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SelfDestructKBMammet, 7f, 8);
sealed class Fester(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.Fester);

sealed class TriDisaster(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.TriDisasterFirst, 5f, 2, 2)
{
...
sealed class SuffocatingCloud(ModuleBase module) : Components.Voidzone(module, 9f, GetVoidzone)
{
    private static List<Actor> GetVoidzone(ModuleBase module) => module.Enemies((uint)OID.PoisonVoidzone);
}

sealed class StingingAnguishStingScornOfTheScorpion(ModuleBase module) : Components.RaidwideCasts(module, [(uint)AID.StingingAnguish, (uint)AID.StingOfTheScorpion,
(uint)AID.ScornOfTheScorpion], "Raidwide + bleed (Esuna!)");

sealed class Esuna(ModuleBase module) : ModuleComponent(module)
{
...
{
    public AnAntidoteForAnarchyStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.Quest, GroupID = 70365u, CFCID = 70365u, NameID = 12743u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class AnAntidoteForAnarchy(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
