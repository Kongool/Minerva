...
namespace Minerva.Dawntrail.Quest.Role.DreamsOfANewDay;
...
sealed class Bladestorm(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Bladestorm, new AOEShapeCone(20f, 45f.Degrees()));
sealed class KeenTempest(ModuleBase module) : Components.SimpleAOEGroupsByTimewindow(module, [(uint)AID.KeenTempest], 8f);

sealed class AethericBurst(ModuleBase module) : Components.RaidwideCastDelay(module, (uint)AID.AethericBurstVisual, (uint)AID.AethericBurst, 0.9f);
sealed class AetherialExposure(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.AetherialExposure, 6f, 3, 3);
sealed class Conviction(ModuleBase module) : Components.CastTowers(module, (uint)AID.Conviction, 4f)
{
...
sealed class AetherialRay(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AetherialRay, new AOEShapeRect(40f, 2f), 10);
sealed class Aethershot(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.Aethershot, 5f);
sealed class BloodyTrinity(ModuleBase module) : Components.SingleTargetDelayableCast(module, (uint)AID.BloodyTrinity);
sealed class PoisonDaggers(ModuleBase module) : Components.CastInterruptHint(module, (uint)AID.PoisonDaggersVisual);
sealed class Daggerflight(ModuleBase module) : Components.InterceptTether(module, (uint)AID.DaggerflightVisual)
{
...
sealed class CradleOfTheSleepless(ModuleBase module) : Components.GenericAOEs(module)
{
...
{
    public DreamsOfANewDayStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
public abstract class DreamsOfANewDay(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
...
sealed class DreamsOfANewDayP1States(ModuleBase module) : DreamsOfANewDayStates(module) { }
sealed class DreamsOfANewDayP2States(ModuleBase module) : DreamsOfANewDayStates(module) { }

[ModuleInfo(Group = ModuleGroup.Quest, GroupID = 70359u, CFCID = 70359u, NameID = 13046u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class DreamsOfANewDayP1(WorldState ws, Actor primary) : DreamsOfANewDay(ws, primary);

[ModuleInfo(Group = ModuleGroup.Quest, GroupID = 70359u, CFCID = 70359u, NameID = 13046u, PrimaryActorOID = (uint)OID.BossP2, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
