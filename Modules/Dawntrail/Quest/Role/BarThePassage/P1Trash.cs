...
namespace Minerva.Dawntrail.Quest.Role.BarThePassage.Trash1;
...
sealed class NopalFan(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.NopalFan, new AOEShapeRect(15f, 2f));
sealed class PulverizingPound(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PulverizingPound, 5f);
sealed class BodySlamPredatorySwoop(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.BodySlam, (uint)AID.PredatorySwoop], 6f);
sealed class ZillionNeedles(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ZillionNeedles, 4f);
sealed class FlockFrenzy(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.FlockFrenzy);

sealed class ToxicSpit(ModuleBase module) : Components.InterceptTether(module, (uint)AID.ToxicSpitVisual, excludedAllies: [(uint)OID.Boss])
{
...
{
    public Trash1States(ModuleBase module) : base(module)
    {
        TrivialPhase()
            ...
            .Raw.Update = () => !module.PrimaryActor.IsTargetable || module.World.CurrentCFCID != 1016u;
    }
}

[ModuleInfo(CFCID = 1016u, NameID = 13720u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class Trash1(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(default, -336.3f), new ArenaBoundsRect(34.6f, 43.7f))
{
