...
namespace Minerva.Dawntrail.Quest.Role.BarThePassage.Apyaahi;
...
sealed class ArenaChanges(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        void AddAOE(double delay) => _aoe = [new(donut, Center, default, Module.CastFinishAt(spell, delay))];
...
sealed class FlamesOfTheFallingOrder(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FlamesOfTheFallingOrder, 12f);
sealed class RottingRoarMycocyclone(ModuleBase module) : Components.RaidwideCasts(module, [(uint)AID.RottingRoar, (uint)AID.Mycocyclone]);

sealed class PathogenicPowerAOE(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class PathogenicPowerKB(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.PathogenicPowerKB, 13f)
{
...
sealed class SporeBurst(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SporeBurst, 6f);
sealed class FleshBoilingPunishmentLake(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FleshBoilingPunishmentLake, new AOEShapeDonut(5f, 40f));
sealed class BlazingBlazeOfGlory(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.BlazingBlazeOfGlory, 6f);
sealed class MyceliumStomp(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.MyceliumStomp, 6f);
sealed class SpoilingSmashEnrage(ModuleBase module) : Components.CastHint(module, (uint)AID.SpoilingSmashVisual2, "Enrage!", true);

sealed class Burst(ModuleBase module) : Components.Exaflare(module, 6f)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
...
{
    public ApyaahiTheArchitectStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 1016u, NameID = 13669u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class ApyaahiTheArchitect(WorldState ws, Actor primary) : ModuleBase(ws, primary, StartingArena.Center, StartingArena)
{
