...
namespace Minerva.Dawntrail.Quest.Role.BarThePassage.Trash2;
...
sealed class ArenaChange(ModuleBase module) : ModuleComponent(module)
{
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        {
            Bounds = Trash2.Arena2;
            Center = Trash2.Arena2Center;
        }
    }
}

sealed class IceAegis(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class SavageSwipe(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SavageSwipe, new AOEShapeCone(9f, 60f.Degrees()));
sealed class OneOneOneTonzeSwing(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.OneOneOneTonzeSwing, 10f);
sealed class OneOneTonzeSwipe(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.OneOneTonzeSwipe, new AOEShapeCone(10f, 60f.Degrees()));

sealed class Trash2States : StateMachineBuilder
{
    public Trash2States(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 1016u, NameID = 13676u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class Trash2(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(default, 28.5f), new ArenaBoundsRect(34.6f, 23.5f))
{
