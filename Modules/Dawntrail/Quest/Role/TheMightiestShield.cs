...
namespace Minerva.Dawntrail.Quest.Role.TheMightiestShield;
...
sealed class Fractures(ModuleBase module) : Components.DirectionalParry(module, [(uint)OID.CrackedMettle1, (uint)OID.CrackedMettle2])
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (ActorStates.Count != 0)
        {
            ...
            var target = World.Actors.Find(first.Key)!;
...
sealed class SteelhogsRevenge(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SteelhogsRevenge, 12f);
sealed class RuthlessBombardment1(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.RuthlessBombardment1, 8f);
sealed class Bombardment(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.RuthlessBombardment2, (uint)AID.AreaBombardment], 4f);

sealed class RagingArtillery(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.RagingArtilleryFirst);
sealed class MagitekCannon(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.MagitekCannon, 6f);
sealed class MagitekMissile(ModuleBase module) : Components.Voidzone(module, 3f, GetVoidzones, 5f)
{
...
sealed class NeedleGunOilShower(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class HeavySurfaceMissiles(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.HeavySurfaceMissiles1, (uint)AID.HeavySurfaceMissiles2,
(uint)AID.HeavySurfaceMissiles3, (uint)AID.HeavySurfaceMissiles4], 14f, 2, 8);

sealed class HomingLaser(ModuleBase module) : Components.GenericBaitAway(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID == (uint)AID.HomingLaserMarker)
            CurrentBaits.Add(new(caster, World.Actors.Find(spell.TargetID)!, rect, World.FutureTime(5.7d)));
    }
...
{
    public TheMightiestShieldStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.Quest, GroupID = 70377u, CFCID = 70377u, NameID = 12923u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class TheMightiestShield(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
