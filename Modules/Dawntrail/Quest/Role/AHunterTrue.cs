...
namespace Minerva.Dawntrail.Quest.Role.AHunterTrue;
...
sealed class ScathingSunshot(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ScathingSunshot, new AOEShapeCone(70f, 60f.Degrees()));
sealed class Foxflare(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Foxflare, new AOEShapeRect(50f, 5f));
sealed class NinefoldCurse(ModuleBase module) : Components.RaidwideCastDelay(module, (uint)AID.NinefoldCurseVisual, (uint)AID.NinefoldCurse, 2f);
sealed class NinetyNinefoldCurse(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.NinetyNinefoldCurse);
sealed class Roar(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Roar);
sealed class SonicStorm(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.SonicStorm, 6f);
sealed class RushBait(ModuleBase module) : Components.BaitAwayChargeTether(module, 4f, 8.3d, (uint)AID.RushBait, (uint)AID.RushBaitFail, (uint)TetherID.TetherBad, (uint)TetherID.TetherGood, (uint)OID.Dzo, 30f);
sealed class RushLineStack(ModuleBase module) : Components.LineStack(module, aidMarker: (uint)AID.RushLineStackMarker, (uint)AID.RushLineStack, 4.9d, markerIsFinalTarget: false);
sealed class Trample(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Trample, 15f);

sealed class FallingDusk(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FallingDusk, 15f)
{
...
sealed class DancingWind(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.DancingWind, 8f, kind: Kind.TowardsOrigin)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (Casters.Count != 0)
...
sealed class DawnlitBolt(ModuleBase module) : Components.GenericAOEs(module)
{
...
{
    public AHunterTrueStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.Quest, GroupID = 70371u, CFCID = 70371u, NameID = 12846u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class AHunterTrue(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
