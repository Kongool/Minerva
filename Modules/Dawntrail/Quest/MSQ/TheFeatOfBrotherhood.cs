...
namespace Minerva.Dawntrail.Quest.MSQ.TheFeatOfBrotherhood;
...
sealed class DualPyresSteelfoldStrike(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class RoaringStarRect(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
    ...
    public override void OnActorPlayActionTimelineEvent(Actor actor, ushort id)
    {
        if (id == 0x1E46)
        {
            _aoes.Add(new(rect, actor.Position.Quantized(), actor.Rotation, World.FutureTime(8.5d)));
        }
    }
...
sealed class SublimeHeat(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        {
            _aoes.Add(new(circle, caster.Position, default, World.FutureTime(7.5d)));
        }
    }
...
sealed class NobleTrail(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class LayOfTheSun(ModuleBase module) : Components.UniformStackSpread(module, 6f, default, 8)
{
...
sealed class RoaringStar(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.RoaringStarRaidwide);
...
sealed class Burn(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Burn, new AOEShapeRect(46f, 2.5f), 8);
sealed class FallenStar(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.FallenStar, 6f);
sealed class FirstLight(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FirstLight, 6f);
sealed class InnerWake(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.InnerWake, 10f);
sealed class OuterWake(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.OuterWake, new AOEShapeDonut(6f, 40f));
sealed class BattleBreaker(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.BattleBreaker);
sealed class HeartOfTuralRaidwides(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.HeartOfTural, "Raidwides x7");

sealed class HeartOfTural : Components.SimpleAOEs
{
    public HeartOfTural(ModuleBase module) : base(module, (uint)AID.HeartOfTural, new AOEShapeRect(20f, 20f, invertForbiddenZone: true)) { Color = Colors.SafeFromAOE; }
...
{
    public TheFeatOfBrotherhoodStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.Quest, GroupID = 70444u, CFCID = 70444u, NameID = 12734u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class TheFeatOfBrotherhood(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(353.5f, 596.4f), new ArenaBoundsSquare(19.75f, -77.5f.Degrees()))
{
