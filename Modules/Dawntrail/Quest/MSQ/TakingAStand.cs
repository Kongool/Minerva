...
namespace Minerva.Dawntrail.Quest.MSQ.TakingAStand;
...
sealed class RoarArenaChange(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        {
            _aoe = [];
            Bounds = TakingAStand.DefaultArena;
        }
    }
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        {
            _aoe = [new(donut, Center, default, Module.CastFinishAt(spell, 0.9d))];
        }
    }
}

sealed class MagickedStandard(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        if (shape != null)
        {
            AOEs.Add(new(shape, actor.Position.Quantized(), default, World.FutureTime(12.6d)));
        }
    }
...
sealed class GreatLeap(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class SelfSacrifice(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class Kickdown(ModuleBase module) : Components.GenericKnockback(module)
{
    private DateTime activation;
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        {
            hints.AddForbiddenZone(new SDKnockbackInCircleAwayFromOriginMixedAOEs(Center, Module.PrimaryActor.Position, 18f, 20f, [.. _aoe.AOEs], _aoe.AOEs.Count), activation);
        }
    }
}

sealed class RiotousRampage(ModuleBase module) : Components.CastTowers(module, (uint)AID.RiotousRampage, 4f);
sealed class Roar(ModuleBase module) : Components.RaidwideCasts(module, [(uint)AID.Roar1, (uint)AID.Roar2]);
sealed class LethalSwipe(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.LethalSwipe, new AOEShapeCone(45f, 90f.Degrees()));
sealed class Fireshower(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Fireshower, 6f);
sealed class RunThrough(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.RunThrough1, (uint)AID.RunThrough2], new AOEShapeRect(45f, 2.5f));
sealed class Fireflood(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Fireflood, 18f);
...
{
    public TakingAStandStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 70438u, NameID = 12677u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class TakingAStand(WorldState ws, Actor primary) : ModuleBase(ws, primary, arenaCenter, startingArena)
{
