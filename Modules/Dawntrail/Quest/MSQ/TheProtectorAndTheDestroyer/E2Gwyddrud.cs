...
using Minerva.Dawntrail.Quest.MSQ.TheProtectorAndTheDestroyer.Otis;

namespace Minerva.Dawntrail.Quest.MSQ.TheProtectorAndTheDestroyer.Gwyddrud;
...
sealed class Gnaw(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.Gnaw);
sealed class CracklingHowl(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.CracklingHowl);
sealed class UntamedCurrentRaidwide(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.UntamedCurrent);

sealed class VioletVoltage(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class RoaringBoltKB(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.RoaringBoltKB, 12f, stopAtWall: true)
{
...
{
    public RollingThunder(ModuleBase module) : base(module, (uint)AID.RollingThunder, new AOEShapeCone(20f, 22.5f.Degrees()), 6) { MaxDangerColor = 2; }
}

sealed class RoaringBolt(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.RoaringBolt, 6f);
sealed class UntamedCurrentSpread(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.UntamedCurrentSpread, 5f);
sealed class UntamedCurrentStack(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.UntamedCurrentStack, 6f, 5, 5);
sealed class UntamedCurrentAOEs(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.UntamedCurrentAOE1, (uint)AID.UntamedCurrentAOE2, (uint)AID.UntamedCurrentAOE3,
...
{
    public GwyddrudStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.Quest, CFCID = 70478u, NameID = 13170u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class Gwyddrud(WorldState ws, Actor primary) : ModuleBase(ws, primary, OtisOathbroken.ArenaCenter, OtisOathbroken.ArenaBounds)
{
