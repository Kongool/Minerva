...
namespace Minerva.Dawntrail.Quest.MSQ.TheProtectorAndTheDestroyer.Otis;
...
sealed class StormlitShockwave(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.StormlitShockwave);
sealed class ValorousAscension(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.ValorousAscension);
sealed class RendPower(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.RendPower, new AOEShapeCone(40f, 15f.Degrees()), 6);

sealed class BastionBreaker(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.BastionBreaker, 6f);
sealed class HolyBlade(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.HolyBlade, 6f);

sealed class SearingSlashThrownFlames(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.SearingSlash, (uint)AID.ThrownFlames], 8f);

sealed class Electrobeam(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Electrobeam, new AOEShapeRect(40f, 2f));

sealed class Rush : Components.SimpleChargeAOEGroups
{
    public Rush(ModuleBase module) : base(module, [(uint)AID.Rush], 2.5f, 4)
    {
        MaxDangerColor = 2;
        MaxRisky = 2;
    }
}
sealed class SteadfastWill(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.SteadfastWill);

sealed class OtisOathbrokenStates : StateMachineBuilder
{
    public OtisOathbrokenStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.Quest, CFCID = 70478u, NameID = 13168u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class OtisOathbroken(WorldState ws, Actor primary) : ModuleBase(ws, primary, ArenaCenter, ArenaBounds)
{
