...
namespace Minerva.Dawntrail.Quest.Job.Viper.VengeanceOfTheViper;
...
sealed class Razorwind(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.Razorwind, 7f, 2, 2);
sealed class Explosion(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Explosion, 15f);

sealed class SwoopingFrenzy1(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SwoopingFrenzy1, 15f);

sealed class SwoopingFrenzy2(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class BrutalStroke(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BrutalStroke, 25f);
sealed class CatchingChaos(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.CatchingChaos);
sealed class Galeripper(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Galeripper, new AOEShapeCone(60f, 45f.Degrees()));
sealed class BitingScratch(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.BitingScratch1, (uint)AID.BitingScratch2], new AOEShapeCone(40f, 45f.Degrees()));
sealed class FervidImpact(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FervidImpact, 12f);
sealed class FrigidPulse(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FrigidPulse, new AOEShapeDonut(12f, 60f));

sealed class FirestormCycle(ModuleBase module) : Components.ConcentricAOEs(module, _shapes)
{
    ...
    public override void OnCastFinished(Actor caster, ActorCastInfo spell)
    {
        if (Sequences.Count != 0)
        {
            ...
            AdvanceSequence(order, spell.LocXZ, World.FutureTime(2.4d));
        }
    }
}

sealed class StormkissedFlames(ModuleBase module) : Components.ConcentricAOEs(module, _shapes)
{
...
sealed class FervidPulse(ModuleBase module) : Components.GenericRotatingAOE(module)
{
...
{
    public VengeanceOfTheViperStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.Quest, GroupID = 70389u, CFCID = 70389u, NameID = 12829u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class VengeanceOfTheViper(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
