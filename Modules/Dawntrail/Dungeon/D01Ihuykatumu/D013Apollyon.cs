...
namespace Minerva.Dawntrail.Dungeon.D01Ihuykatumu.D013Apollyon;
...
sealed class Whirlwind(ModuleBase module) : Components.Voidzone(module, 4.5f, GetWhirlwind, 5f)
{
    private static List<Actor> GetWhirlwind(ModuleBase module) => module.Enemies((uint)OID.Whirlwind);
}
sealed class Blade(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.Blade);
sealed class HighWind(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.HighWind);
sealed class RazorZephyrBladesOfFamine(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.RazorZephyr, (uint)AID.BladesOfFamine], new AOEShapeRect(50f, 6f));

sealed class Levinsickle(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Levinsickle, 4f);
sealed class LevinsickleSpark(ModuleBase module) : Components.VoidzoneAtCastTarget(module, 4f, (uint)AID.LevinsickleSpark, GetVoidzones, 0.7d)
{
...
sealed class WingOfLightning(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WingOfLightning, new AOEShapeCone(40f, 22.5f.Degrees()), 8);

sealed class ThunderIII2(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.ThunderIII, 6f);
sealed class BladeTB(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.BladeTB, 6f, tankbuster: true);

sealed class WindSickle(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WindSickle, new AOEShapeDonut(5f, 60f));
sealed class RazorStorm(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.RazorStorm, new AOEShapeRect(40f, 20f));

sealed class CuttingWind(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        void AddWhirlwind(WPos[] pos)
        {
            for (var i = 0; i < 3; ++i)
            {
                var delay = World.FutureTime(delays[i]);
...
{
    public D013ApollyonStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 826u, NameID = 12711u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS) (ported from BMR)")]
public sealed class D013Apollyon(WorldState ws, Actor primary) : ModuleBase(ws, primary, DefaultBounds.Center, DefaultBounds)
{
