...
//
// Replaces Minerva's original hand-written version of this fight — its own header noted that the
// tether-bait / spread / concentric mechanics were left out because those components did not exist yet.
// They all do now, so this is the full port: Inhale, HydrowaveBait, PunutiyFlop1/2 and ShoreShaker are
// the five mechanics the old module was missing.
using System;
...
sealed class HydrowaveBait(ModuleBase module) : Components.BaitAwayTethers(module, new AOEShapeCone(60f, 15f.Degrees()), (uint)TetherID.BaitAway, (uint)AID.HydrowaveBait, (uint)OID.Punutiy, 8.6d)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        {
            hints.AddForbiddenZone(new SDRect(Module.Center - new WDir(default, -18f), Module.Center - new WDir(default, 18f), 18f), World.FutureTime(ActivationDelay));
        }
    }
...
sealed class Resurface(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Resurface, Inhale.Cone);
sealed class Inhale(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class PunutiyPress(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.PunutiyPress);
...
sealed class PunutiyFlop1(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.PunutiyFlop1, 14f);
sealed class PunutiyFlop2(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.PunutiyFlop2, 6f);

sealed class ShoreShaker(ModuleBase module) : Components.ConcentricAOEs(module, _shapes)
{
...
[ModuleInfo(CFCID = 826u, NameID = 12723u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS) (ported from BMR)")]
public sealed class D011PrimePunutiy(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(35f, -95f), new ArenaBoundsSquare(19.5f))
{
