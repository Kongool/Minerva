...
namespace Minerva.Dawntrail.Dungeon.D02WorqorZormor.D021RyoqorTerteh;
...
sealed class ArenaChange(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        if (spell.Action.ID == (uint)AID.FrostingFracas && Bounds.Radius != 20f)
        {
            _aoe = [new(donut, Center, default, Module.CastFinishAt(spell, 0.6d))];
        }
    }
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        {
            Bounds = D021RyoqorTerteh.DefaultBounds;
            _aoe = [];
        }
    }
}

sealed class IceScreamFrozenSwirl(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        if (tether.ID == (uint)TetherID.Freeze)
        {
            ...
            for (var i = 0; i < count; ++i)
            {
                ...
                if (aoe.ActorID == id)
                {
                    aoe.Activation = World.FutureTime(14.9d);
                    ++tetherCount;
                    var isTutorial = tutorial < 2u;
...
sealed class FrostingFracas(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.FrostingFracas);
sealed class SnowBoulder(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SnowBoulder, new AOEShapeRect(50f, 3f), 6);
sealed class SparklingSprinkling(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.SparklingSprinkling, 5f);

sealed class D021RyoqorTertehStates : StateMachineBuilder
{
    public D021RyoqorTertehStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 824u, NameID = 12699u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS) (ported from BMR)")]
public sealed class D021RyoqorTerteh(WorldState ws, Actor primary) : ModuleBase(ws, primary, StartingBounds.Center, StartingBounds)
{
