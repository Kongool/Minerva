...
namespace Minerva.Dawntrail.Dungeon.D02WorqorZormor.D023Gurfurlur;
...
sealed class ArenaChange(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        if (spell.Action.ID == (uint)AID.HeavingHaymaker && Module.Bounds.Radius > 21f)
        {
...
sealed class AuraSphere(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (count != 0)
        {
            ...
            // BMR also queues Sprint here via its action queue; Minerva does not execute actions.
            for (var i = 0; i < count; ++i)
            {
                var o = orbs[i];
...
sealed class SledgeHammer(ModuleBase module) : Components.LineStack(module, aidMarker: (uint)AID.SledgeHammerMarker, (uint)AID.Sledgehammer3, 4.9d);
sealed class HeavingHaymaker(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.HeavingHaymaker);
sealed class LithicImpact(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.LithicImpact, new AOEShapeRect(4f, 2f));
sealed class Whirlwind(ModuleBase module) : Components.Voidzone(module, 5f, GetVoidzones, 7f)
{
...
sealed class GreatFlood(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.GreatFlood, 25f, kind: Kind.DirForward)
{
...
sealed class Allfire(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class VolcanicDrop(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.VolcanicDrop, 6f);
sealed class EnduringGloryGreatFloodWindswrathRaidwide(ModuleBase module) : Components.RaidwideCasts(module, [(uint)AID.EnduringGlory, (uint)AID.GreatFlood,
(uint)AID.Windswrath1, (uint)AID.Windswrath2]);

class Windswrath(ModuleBase module, uint aid) : Components.SimpleKnockbacks(module, aid, 15f);
sealed class Windswrath1(ModuleBase module) : Windswrath(module, (uint)AID.Windswrath1)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (Casters.Count == 0)
        ...
        ref readonly var c = ref Casters.Ref(0);
        hints.AddForbiddenZone(new SDKnockbackInAABBSquareAwayFromOrigin(Module.Center, c.Origin, 15f, 19f), c.Activation);
    }
}

sealed class Windswrath2(ModuleBase module) : Windswrath(module, (uint)AID.Windswrath2)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (_aoe.ActiveAOEs(slot, actor).Length != 0)
        {
            ...
            if ((act - World.CurrentTime).TotalSeconds <= 3d)
            {
...
{
    public D023GurfurlurStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 824u, CFCID = 824u, NameID = 12705u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS) (ported from BMR)")]
public sealed class D023Gurfurlur(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-54f, -195f), new ArenaBoundsSquare(24.5f));