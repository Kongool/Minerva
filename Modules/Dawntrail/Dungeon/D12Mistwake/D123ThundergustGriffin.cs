...
namespace Minerva.Dawntrail.Dungeon.D12Mistwake.D123ThundergustGriffin;
...
sealed class ArenaChange(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        if (spell.Action.ID == (uint)AID.Thunderspark && Bounds.Radius > 20f)
        {
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        {
            Bounds = new ArenaBoundsCustom([new Polygon(Center, 20f, 64)], MapResolution: 0.35f);
            _aoe = [];
        }
    }
}
...
[SkipLocalsInit]
sealed class Thunderspark(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Thunderspark);

[SkipLocalsInit]
sealed class LightningBoltSpread(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.LightningBoltSpread, 5f);

[SkipLocalsInit]
sealed class LightningBoltAOE(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.LightningBoltAOE, 5f);

[SkipLocalsInit]
sealed class Thunderbolt(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.Thunderbolt1, (uint)AID.Thunderbolt2, (uint)AID.Thunderbolt3], new AOEShapeRect(92f, 3f));

[SkipLocalsInit]
sealed class GoldenTalons(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.GoldenTalons);

[SkipLocalsInit]
sealed class Rush(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Rush, new AOEShapeRect(40f, 5f));

[SkipLocalsInit]
sealed class ElectrogeneticForce(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ElectrogeneticForce, new AOEShapeRect(40f, 9f))
{
...
[SkipLocalsInit]
sealed class StormSurge(ModuleBase module) : Components.GenericAOEs(module)
{
    private AOEInstance[] _aoe = [];
    ...
    public override void OnActorCreated(Actor actor)
    {
        if (actor.OID == (uint)OID.ShockStorm)
        {
            ...
            _aoe = [new(rect, pos, rot, World.FutureTime(6d), shapeDistance: rect.Distance(pos, rot))];
        }
    }
...
sealed class ElectrifyingFlight(ModuleBase module) : Components.GenericKnockback(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID == (uint)AID.ElectrifyingFlight)
        {
            ...
            distance = isAlongZAxis ? new SDKnockbackInCircleLeftRightAlongZAxis(Center, 12f, 19f) : new SDKnockbackInCircleLeftRightAlongXAxis(Center, 12f, 19f);
        }
    }
