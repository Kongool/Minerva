...
namespace Minerva.Dawntrail.Dungeon.D06Alexandria.D061AntivirusX;
...
sealed class ImmuneResponseArenaChange(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        if (spell.Action.ID == (uint)AID.ImmuneResponseVisualSmall && Bounds.Radius > 20f)
        {
            _aoe = [new(rect, Center, default, Module.CastFinishAt(spell, 0.8d))];
        }
    }
...
sealed class PathoCircuitCrossPurge(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    private void UpdateAOE()
    {
        ...
        if (count > 1)
        {
            ...
            var isFrontDonutAndConeSmall = aoe1.Origin.AlmostEqual(Center, 1f) && aoe1.Shape == donut && aoe0.Shape == coneSmall;
    ...
    public override void OnActorCreated(Actor actor)
    {
        if (actor.OID is (uint)OID.InterferonR or (uint)OID.InterferonC)
        {
            ...
            var act = _aoes.Count == 0 ? World.FutureTime(9.9d) : _aoes.Ref(0).Activation.AddSeconds(2.5d * _aoes.Count);
...
sealed class Cytolysis(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Cytolysis);

sealed class Quarantine(ModuleBase module) : Components.StackWithIcon(module, (uint)IconID.Quarantine, (uint)AID.Quarantine, 6f, 5.1d, 3, 3);

sealed class Disinfection(ModuleBase module) : Components.BaitAwayIcon(module, 6f, (uint)IconID.Disinfection, (uint)AID.Disinfection, tankbuster: true)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (CurrentBaits.Count != 0)
        {
            ...
            if (b.Activation.AddSeconds(-2d) >= World.CurrentTime)
            {
                if (b.Target == actor)
                {
...
{
    public D061AntivirusXStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 827u, NameID = 12844u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS), erdelf (ported from BMR)")]
public sealed class D061AntivirusX(WorldState ws, Actor primary) : ModuleBase(ws, primary, ArenaCenter, new ArenaBoundsRect(22.5f, 17.5f))
{
