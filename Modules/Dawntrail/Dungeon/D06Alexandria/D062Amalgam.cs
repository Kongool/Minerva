...
namespace Minerva.Dawntrail.Dungeon.D06Alexandria.D062Amalgam;
...
sealed class ElectrowaveDisassembly(ModuleBase module) : Components.RaidwideCasts(module, [(uint)AID.Electrowave, (uint)AID.Disassembly]);
sealed class CentralizedCurrent(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.CentralizedCurrent, new AOEShapeRect(90f, 7.5f));

sealed class SplitCurrent(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.SplitCurrent1, (uint)AID.SplitCurrent2], new AOEShapeRect(90f, 12.5f));
sealed class SupercellMatrix1(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SupercellMatrix1, new AOEShapeTriCone(40f, 45.Degrees()));
sealed class SupercellMatrix2(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SupercellMatrix2, new AOEShapeRect(55f, 4f));
sealed class StaticSpark(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.StaticSpark, 6f);
sealed class Amalgamight(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.Amalgamight);
sealed class Voltburst(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Voltburst, 6f);
sealed class Superbolt(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.Superbolt, 6f, 4, 4);

sealed class TernaryCharge(ModuleBase module) : Components.ConcentricAOEs(module, [new AOEShapeCircle(10f), new AOEShapeDonut(10f, 20f), new AOEShapeDonut(20f, 30f)])
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
...
{
    public D062AmalgamStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 827u, CFCID = 827u, NameID = 12864u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS), erdelf (ported from BMR)")]
public sealed class D062Amalgam(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-533f, -373f), new ArenaBoundsSquare(23f));