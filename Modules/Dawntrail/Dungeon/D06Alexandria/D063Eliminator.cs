...
namespace Minerva.Dawntrail.Dungeon.D06Alexandria.D063Eliminator;
...
sealed class DisruptionArenaChange(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        if (spell.Action.ID == (uint)AID.Disruption && Module.Bounds.Radius > 15f)
        {
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        {
            Module.Bounds = new ArenaBoundsSquare(15f);
            _aoe = [];
        }
    }
}

sealed class Disruption(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Disruption);

sealed class Partition(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.Partition1, (uint)AID.Partition2, (uint)AID.Partition3], new AOEShapeCone(40f, 90f.Degrees()));
sealed class Terminate(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Terminate, new AOEShapeRect(40f, 5f));
sealed class HaloOfDestruction(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HaloOfDestruction, new AOEShapeDonut(6f, 40f));

sealed class Electray(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.Electray, 6f)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        else
        {
            ...
            if (Spreads.Count != 0)
            {
                hints.AddForbiddenZone(new SDCircle(Module.Center - new WDir(default, 15f), 15f), Spreads.Ref(0).Activation);
            }
        }
    }
}
...
{
    public Explosion(ModuleBase module) : base(module, (uint)AID.Explosion, new AOEShapeRect(50f, 4f))
    {
        MaxDangerColor = 2;
        MaxRisky = 4;
    }
...
sealed class Impact(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.Impact, 15f)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (Casters.Count != 0)
        {
            ...
            hints.AddForbiddenZone(new SDKnockbackInAABBSquareAwayFromOrigin(Module.Center, c.Origin, 15f, 14f), c.Activation);
        }
    }
}

sealed class Compression(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Compression, 6f);

sealed class Overexposure(ModuleBase module) : Components.LineStack(module, aidMarker: (uint)AID.OverexposureMarker, (uint)AID.Overexposure, 5d, 40f, 3f);
sealed class LightOfDevotion(ModuleBase module) : Components.LineStack(module, aidMarker: (uint)AID.LightOfDevotionMarker, (uint)AID.LightOfDevotion, 5.5d, 40f, 3f)
{
    public override void OnMapEffect(byte index, uint state)
    {
...
sealed class LightOfSalvation(ModuleBase module) : Components.GenericBaitAway(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        {
            CurrentBaits.Add(new(caster, World.Actors.Find(spell.TargetID)!, rect, Module.CastFinishAt(spell, 0.2d)));
        }
    }
...
{
    public D063EliminatorStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 827u, CFCID = 827u, NameID = 12729u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS), erdelf (ported from BMR)")]
public sealed class D063Eliminator(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-759f, -648f), new ArenaBoundsSquare(15.5f))
{
