...
namespace Minerva.Dawntrail.Dungeon.D07TenderValley.D073YokHuyAttestant;
...
sealed class TectonicShift(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TectonicShift, 8f);
sealed class BoulderToss(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.BoulderToss);
sealed class SunToss(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SunToss, 6f);

sealed class AncientWrath(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        if (tether.ID == (uint)TetherID.StatueActivate)
        {
            ...
            _aoes.Add(new(aoeShape, source.Position.Quantized(), source.Rotation, World.FutureTime(8.1d)));
        }
    }
...
{
    public D073YokHuyAttestantStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 834u, NameID = 12801u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D073YokHuyAttestant : ModuleBase
{
