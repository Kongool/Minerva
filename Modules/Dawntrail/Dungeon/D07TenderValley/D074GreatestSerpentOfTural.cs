...
sealed class DubiousTulidisasterArenaChange(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        if (spell.Action.ID == (uint)AID.DubiousTulidisaster && Bounds.Radius > 12f)
        {
            _aoe = [new(square, Center, default, Module.CastFinishAt(spell, 4.8d))];
        }
    }
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        {
            Bounds = new ArenaBoundsSquare(12f);
            _aoe = [];
        }
    }
}
...
sealed class GreatestFlood(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.GreatestFlood, 15f)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (Casters.Count != 0)
        {
            ...
            {
                hints.AddForbiddenZone(new SDKnockbackInAABBSquareAwayFromOrigin(Center, c.Origin, 15f, 11f), act);
            }
        }
    }
}
...
sealed class GreatestLabyrinth(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        void AddAOEs(int index)
        {
            ...
            _aoes.Add(new(safeShapes[index], Center, default, World.FutureTime(10d), Colors.SafeFromAOE));
        }
        switch (state)
        {
            case 0x01000080u:
