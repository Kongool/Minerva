...
namespace Minerva.Dawntrail.Dungeon.D07TenderValley.D071Barreltender;
...
sealed class ArenaChange(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        if (spell.Action.ID == (uint)AID.HeavyweightNeedlesVisual && Bounds.Radius > 20f)
        {
...
sealed class HeavyweightNeedles(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HeavyweightNeedles, new AOEShapeCone(36f, 25f.Degrees()))
{
...
sealed class NeedleStormSuperstorm(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class Prickly(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.PricklyRight, (uint)AID.PricklyLeft], new AOEShapeCone(36f, 165f.Degrees()));
sealed class SucculentStomp(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.SucculentStomp, 6f, 4, 4);
sealed class BarrelBreaker(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.BarrelBreaker, 20f)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (!IsImmune(slot, act))
        {
            ...
            hints.AddForbiddenZone(new SDKnockbackInAABBSquareAwayFromOriginPlusAOECirclesMixedRadii(Center, c.Origin, 20f, 19f, circles, max), act);
        }
    }
...
sealed class TenderFury(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.TenderFury);
sealed class BarbedBellow(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.BarbedBellow);

sealed class D071BarreltenderStates : StateMachineBuilder
{
    public D071BarreltenderStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 834u, NameID = 12889u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS) (ported from BMR)")]
public sealed class D071Barreltender(WorldState ws, Actor primary) : ModuleBase(ws, primary, ArenaCenter, new ArenaBoundsSquare(24.5f))
{
