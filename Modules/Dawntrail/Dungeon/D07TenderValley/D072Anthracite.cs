...
namespace Minerva.Dawntrail.Dungeon.D07TenderValley.D072Anthracite;
...
sealed class Anthrabomb(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.Anthrabomb1, (uint)AID.Anthrabomb2], 10f);
sealed class AnthrabombSpread(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.AnthrabombSpread, 6f);
sealed class HotBlast(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.HotBlast1, (uint)AID.HotBlast2], new AOEShapeRect(40f, 3f));
sealed class CarbonaceousCombustion(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.CarbonaceousCombustion);
sealed class ChimneySmack(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.ChimneySmack);
sealed class BurningCoals(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.BurningCoals, 6f, 4, 4);

sealed class D072AnthraciteStates : StateMachineBuilder
{
    public D072AnthraciteStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 834u, NameID = 12853u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS) (ported from BMR)")]
public sealed class D072Anthracite : ModuleBase
{
