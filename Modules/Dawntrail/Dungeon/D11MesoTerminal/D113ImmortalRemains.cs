...
namespace Minerva.Dawntrail.Dungeon.D11MesoTerminal.D113ImmortalRemains;
...
sealed class RecollectionImpression(ModuleBase module) : Components.RaidwideCasts(module, [(uint)AID.Recollection, (uint)AID.ImpressionKB]);
sealed class MemoryOfThePyre(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.MemoryOfThePyre);
sealed class Electray(ModuleBase module) : Components.SimpleAOEGroupsByTimewindow(module, [(uint)AID.Electray], new AOEShapeRect(45f, 4f));
sealed class MemoryOfTheStorm(ModuleBase module) : Components.LineStack(module, iconID: (uint)IconID.MemoryOfTheStorm, (uint)AID.MemoryOfTheStorm, 5.2d, 60f, 3f, markerIsFinalTarget: false);

sealed class Bombardment(ModuleBase module) : Components.GenericAOEs(module)
{
    public readonly List<AOEInstance> AOEs = [];
    private readonly List<Actor> terrors = [];
    ...
    public override void OnActorNpcYell(Actor actor, ushort id)
    {
        if (terrors.Count > 6 && id is >= 18705 and <= 18708)
        {
            ...
            AOEs.Count == 0 ? World.FutureTime(9.8d) : AOEs.Ref(0).Activation, actorID: big ? 1ul : default, shapeDistance: shape.Distance(loc, default)));
        }
    }
...
sealed class Impression(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Impression, 10f)
{
...
sealed class ImpressionKB(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.Impression, 11f)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        var innerCircle = new SDCircle(Module.Center.Quantized(), 10f);
        if (!IsImmune(slot, act))
        {
            ...
            hints.AddForbiddenZone(new SDKnockbackInAABBSquareAwayFromOriginPlusAOECirclesMixedRadiiPlusAvoidShape(Module.Center, c.Origin, 11f, 19f, circles, len, innerCircle), act);
        }
        else
        {
...
sealed class Turmoil(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        if (offset != default)
        {
            _aoe = [new(rect, (caster.Position + offset * new WDir(10f, default)).Quantized(), default, World.FutureTime(4.4d))];
        }
...
sealed class Keraunography(ModuleBase module) : Components.GenericAOEs(module)
{
...
{
    public D113ImmortalRemainsStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 1028u, CFCID = 1028u, NameID = 13974u, PrimaryActorOID = (uint)OID.ImmortalRemains, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D113ImmortalRemains(WorldState ws, Actor primary) : ModuleBase(ws, primary, default, new ArenaBoundsSquare(20f));
