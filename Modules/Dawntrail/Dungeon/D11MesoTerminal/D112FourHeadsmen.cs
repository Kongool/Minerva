...
namespace Minerva.Dawntrail.Dungeon.D11MesoTerminal.D112FourHeadsmen;
...
{
    private readonly List<DonutV> borders = [];
    ...
    private void UpdateArena() => Module.Bounds = borders.Count == 0 ? D112FourHeadsmen.DefaultBounds : new ArenaBoundsCustom(D112FourHeadsmen.DefaultRect, [.. D112FourHeadsmen.Differences, .. borders]);

    public override void DrawArenaBackground(int pcSlot, Actor pc)
    {
        var isDefault = Module.Bounds == D112FourHeadsmen.DefaultBounds;
        if (_cell.AssignedBoss[pcSlot] == null)
        {
            if (!isDefault)
            {
                Module.Bounds = D112FourHeadsmen.DefaultBounds;
            }
        }
        else if (isDefault)
        {
...
sealed class CellBlock(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class DismembermentExecutionWheelFlayingFlailChoppingBlock(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class HeadSplittingRoar(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.HeadSplittingRoar);
sealed class DeathPenalty(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.DeathPenalty, "Healerbuster");
sealed class Doom(ModuleBase module) : Components.CleansableDebuff(module, (uint)SID.Doom);
sealed class WillBreaker(ModuleBase module) : Components.CastInterruptHint(module, (uint)AID.WillBreaker, showNameInHint: true);
// BMR tracks the three hits and drops the prediction when caster/target dies, to time mitigation.
// Minerva's SingleTargetCastDelay is hint-only (no damage prediction), so that bookkeeping is dropped.
sealed class RelentlessTorment(ModuleBase module) : Components.SingleTargetCastDelay(module, (uint)AID.RelentlessTormentVisual, (uint)AID.RelentlessTorment, 0.4d, "Tankbuster x3");

sealed class PealOfJudgment(ModuleBase module) : Components.Exaflare(module, new AOEShapeRect(2f, 2f))
{
    private readonly D112FourHeadsmen bossmod = (D112FourHeadsmen)module;
    ...
    public override void OnActorCreated(Actor actor)
    {
        if (actor.OID == (uint)OID.SwordOfJustice)
        {
            ...
            Lines.Add(new(actor.Position, 0.24118f * rot.ToDirection(), World.FutureTime(5.5d), 0.2d, 51, 12, rot));
        }
    }
        ...
        if (spell.Action.ID == (uint)AID.PealOfJudgment)
        {
            ...
            for (var i = 0; i < count; ++i)
            {
                ...
                if (line.Next.AlmostEqual(pos, 1f)) // 0.1 should suffice, but depending on the current server tick casts can be skipped since the time between steps is only 0.2s and an actor can't do more than one cast event per tick
                {
                    ...
                    if (line.ExplosionsLeft == 0) // BMR also ends the line when the cast event lists targets; Minerva's cast events carry MainTargetID only
                    {
                        Lines.RemoveAt(i);
                    }
                    return;
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 1028u, CFCID = 1028u, NameID = 14049u, PrimaryActorOID = (uint)OID.PaleHeadsman, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D112FourHeadsmen(WorldState ws, Actor primary) : ModuleBase(ws, primary, DefaultBounds.Center, DefaultBounds)
{
