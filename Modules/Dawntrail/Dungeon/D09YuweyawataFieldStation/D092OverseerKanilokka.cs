...
namespace Minerva.Dawntrail.Dungeon.D09YuweyawataFieldStation.D092OverseerKanilokka;
...
sealed class ArenaChanges(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        void AddAOE(AOEShape shape) => _aoe = [new(shape, Module.Center.Quantized(), default, Module.CastFinishAt(spell))];
    }
...
sealed class Soulweave(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class FreeSpirits(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.FreeSpirits);
sealed class Bloodburst(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Bloodburst);
sealed class DarkSouls(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.DarkSouls);
sealed class TelltaleTears(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.TelltaleTears, 5f);
sealed class SoulDouse(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.SoulDouse, 6f, 4, 4);

sealed class LostHope(ModuleBase module) : Components.TemporaryMisdirection(module, (uint)AID.LostHope)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (prepare)
        {
            hints.GoalZones.Add(AIHints.GoalSingleTarget(Module.Center, 1f, 9f));
        }
    }
}

sealed class Necrohazard(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Necrohazard, 18f);
sealed class DarkII(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.DarkII1, (uint)AID.DarkII2], new AOEShapeCone(35f, 15f.Degrees()), 6, 12);

sealed class D092OverseerKanilokkaStates : StateMachineBuilder
{
    public D092OverseerKanilokkaStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 1008u, CFCID = 1008u, NameID = 13634u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D092OverseerKanilokka : ModuleBase
{
