...
namespace Minerva.Dawntrail.Dungeon.D09YuweyawataFieldStation.D091LindblumZaghnal;
...
sealed class LineVoltage(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class LightningStorm(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.LightningStorm, 5f);
sealed class ElectricalOverloadSparkingFissure(ModuleBase module) : Components.RaidwideCasts(module, [(uint)AID.ElectricalOverload, (uint)AID.SparkingFissureFirst, (uint)AID.SparkingFissure]);

sealed class CellShock(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        if (state is 0x00020001u or 0x00200010u)
        {
            ...
            _aoe = [new(circle, position.Quantized(), default, World.FutureTime(8d))];
        }
    }
...
{
    public D091LindblumZaghnalStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 1008u, NameID = 13623u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D091LindblumZaghnal(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
