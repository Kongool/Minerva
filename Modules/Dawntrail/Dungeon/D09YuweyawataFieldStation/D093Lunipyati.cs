...
namespace Minerva.Dawntrail.Dungeon.D09YuweyawataFieldStation.D093Lunipyati.cs;
...
sealed class ArenaChanges(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        {
            Bounds = bounds;
            Center = D093Lunipyati.ArenaCenter;
            _aoe = [];
        }
    }
}

sealed class CraterCarve(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.CraterCarve, 11f, riskyWithSecondsLeft: 2.5d);

sealed class RagingClaw(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class BoulderDance(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class JaggedEdge(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.JaggedEdge, 6f)
{
...
sealed class TuraliStoneIV(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.TuraliStoneIV, 6f, 4, 4);
sealed class LeporineLoafSonicHowlBeastlyRoar(ModuleBase module) : Components.RaidwideCasts(module, [(uint)AID.LeporineLoaf, (uint)AID.BeastlyRoar, (uint)AID.SonicHowl]);
sealed class BeastlyRoar(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BeastlyRoar, 25f);
sealed class Slabber(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.Slabber);

sealed class LeapingEarth(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        else if (spell.Action.ID == (uint)AID.LeapingEarthVisual2)
        {
            ...
            for (var i = 0; i < 20; ++i)
            {
                _aoes.Add(new(circle, rotatedPoints[i], default, World.FutureTime(4.5d + 0.25d * i)));
            }
            maxCasts = 10;
        }
    }
    ...
    private void AddAOEs(WPos[] points)
    {
        for (var i = 0; i < 4; ++i)
        {
            _aoes.Add(new(circle, points[i], default, World.FutureTime(6.5d + 0.2d * i)));
        }
    }
}

sealed class RockBlast(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
    private static readonly AOEShapeCircle circle = new(5f);
...
{
    public D093LunipyatiStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 1008u, NameID = 13610u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D093Lunipyati(WorldState ws, Actor primary) : ModuleBase(ws, primary, startingBounds.Center, startingBounds)
{
