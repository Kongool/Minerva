...
namespace Minerva.Dawntrail.Dungeon.D08StrayboroughDeadwalk.D083Träumerei;
...
sealed class ImpactArenaChange(ModuleBase module) : ModuleComponent(module)
{
    private bool active;
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        if (index == 0x0B)
        {
            ...
            {
                active = false;
                Module.Bounds = new ArenaBoundsSquare(19.5f);
            }
        }
    }
    public override void DrawArenaBackground(int pcSlot, Actor pc)
    {
        if (active)
        {
            var isSquare = Module.Bounds is ArenaBoundsSquare;
            var ghostly = ghost.Ghostly[pcSlot];
            Module.Bounds = ghostly && !isSquare ? new ArenaBoundsSquare(19.5f)
            : !ghostly && isSquare ? new ArenaBoundsCustom([new Square(Module.Center, 19.5f)], [new Cross(Module.Center, 20f, 1.5f)])  // for some reason the obstacle cross is smaller than the AOE;
            : Module.Bounds;
        }
    }
}

sealed class GhostlyGuise(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    private SDIntersection shapeDistances = null!; // set in ActiveAOEs before any AI-hint use, same as D123ThundergustGriffin
...
sealed class MaliciousMistRaidwide(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.MaliciousMistRaidwide);
sealed class IllIntentMaliciousMist(ModuleBase module) : Components.StretchTetherDuo(module, 20f, 10f)
{
...
sealed class BitterRegret1(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BitterRegret1, new AOEShapeRect(50f, 8f));
sealed class BitterRegret2(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BitterRegret2, new AOEShapeRect(50f, 6f));
sealed class BitterRegret3(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BitterRegret3, new AOEShapeRect(40f, 2f), 5);
sealed class Impact(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Impact, new AOEShapeRect(40f, 2f));
sealed class Ghostcrusher(ModuleBase module) : Components.LineStack(module, aidMarker: (uint)AID.GhostcrusherMarker, (uint)AID.Ghostcrusher, 5d, 80f, maxStackSize: 4);
sealed class Ghostduster(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.Ghostduster, 8f)
{
    private GhostlyGuise? ghost;
...
{
    public D083TräumereiStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 981u, CFCID = 981u, NameID = 12763u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS) (ported from BMR)")]
public sealed class D083Träumerei(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(148f, -433f), new ArenaBoundsSquare(19.5f));