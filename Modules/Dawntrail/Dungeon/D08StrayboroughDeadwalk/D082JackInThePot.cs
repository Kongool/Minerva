...
namespace Minerva.Dawntrail.Dungeon.D08StrayboroughDeadwalk.D082JackInThePot;
...
sealed class PipingPour(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class TeaAwhirl : Components.GenericAOEs
{
    ...
    public TeaAwhirl(ModuleBase module) : base(module)
    {
        cupPositions = new Dictionary<uint, Action>
        {
            ...
            { 0x00100001u, () => AddAOEs(World.FutureTime(16d), [_cups[0].Position, _cups[1].Position]) },
    ...
    {
        var activation = World.FutureTime(futureTime);
...
sealed class SordidSteam(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.SordidSteam);
sealed class LastDrop(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.LastDrop);

class D082JackInThePotStates : StateMachineBuilder
{
    public D082JackInThePotStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 981u, NameID = 12760u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS) (ported from BMR)")]
public sealed class D082JackInThePot(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
