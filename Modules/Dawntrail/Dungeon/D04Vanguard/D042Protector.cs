...
namespace Minerva.Dawntrail.Dungeon.D04Vanguard.D042Protector;
...
sealed class ArenaChanges(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        if (spell.Action.ID == (uint)AID.Electrowave && Module.Bounds.Radius > 21f)
        {
    ...
    {
        if (preparedArena != null && activation <= World.CurrentTime)
        {
            Module.Bounds = preparedArena;
            _aoe = [];
        }
    }
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        void ResetArena() => Module.Bounds = new ArenaBoundsRect(12f, 20f);
        ...
        void AddAOEAndPrepareArenaChange(ReadOnlySpan<int> rectangleIndices, ReadOnlySpan<int> circleIndices)
        {
            ...
            activation = World.FutureTime(3d);
...
sealed class BatteryCircuit(ModuleBase module) : Components.GenericRotatingAOE(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        {
            AdvanceSequence(caster.Position, caster.Rotation, World.CurrentTime);
        }
    }
}

sealed class HeavyBlastCannon(ModuleBase module) : Components.LineStack(module, aidMarker: (uint)AID.HeavyBlastCannonMarker, (uint)AID.HeavyBlastCannon, 8d, 36f);
sealed class RapidThunder(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.RapidThunder);
...
{
    public BlastCannon(ModuleBase module) : base(module, (uint)AID.BlastCannon, new AOEShapeRect(26f, 2f), 4)
    {
        MaxDangerColor = 2;
        MaxRisky = 2;
    }
}
sealed class Shock(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Shock, 3f);

sealed class HomingCannon : Components.SimpleAOEs
{
    public HomingCannon(ModuleBase module) : base(module, (uint)AID.HomingCannon, new AOEShapeRect(50f, 1f))
    {
        MaxDangerColor = 4;
    }
}

sealed class Bombardment(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Bombardment, 5f);
sealed class Electrowhirl(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.Electrowhirl1, (uint)AID.Electrowhirl2], 6f);

sealed class TrackingBolt(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.TrackingBolt, 8f);

sealed class AccelerationBomb(ModuleBase module) : Components.StayMove(module, 3f)
{
...
{
    public D042ProtectorStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 831u, CFCID = 831u, NameID = 12757u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS) (ported from BMR)")]
public sealed class D042Protector(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(0f, -100f), new ArenaBoundsRect(14.5f, 22.5f));
