...
sealed class ElectrowaveArenaChange(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        if (spell.Action.ID == (uint)AID.Electrowave && Bounds.Radius > 19f)
        {
            _aoe = [new(new AOEShapeCustom([new Square(D041CommanderR8.ArenaCenter, 20f)], [new Square(D041CommanderR8.ArenaCenter, 17f)]), Center, default, Module.CastFinishAt(spell, 0.4d))];
        }
    }
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        {
            Bounds = D041CommanderR8.DefaultBounds;
            _aoe = [];
        }
    }
}
