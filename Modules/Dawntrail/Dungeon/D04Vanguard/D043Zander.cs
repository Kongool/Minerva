...
namespace Minerva.Dawntrail.Dungeon.D04Vanguard.D043Zander;
...
sealed class ElectrothermiaArenaChange(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        if (spell.Action.ID == (uint)AID.Electrothermia && Module.Bounds.Radius > 17f)
        {
            _aoe = [new(donut, Module.Center, default, Module.CastFinishAt(spell, 0.5d))];
        }
    }
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        {
            Module.Bounds = D043Zander.DefaultBounds;
            Module.Center = D043Zander.DefaultBounds.Center;
            _aoe = [];
        }
    }
}

sealed class SlitherbaneBurstCombo(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class Electrothermia(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Electrothermia);
sealed class Screech(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Screech);
sealed class Burst1(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Burst1, new AOEShapeRect(20f, 20f));
// BMR clears its predicted-damage Targets list here when the phase change cancels the tankbuster.
// Minerva's SingleTargetDelayableCast is hint-only (no damage prediction), so there is nothing to clear.
sealed class SaberRush(ModuleBase module) : Components.SingleTargetDelayableCast(module, (uint)AID.SaberRush);

sealed class ShadeShot(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.ShadeShot);
sealed class SoulbaneShock(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.SoulbaneShock, 5f);

sealed class SlitherbaneSoulbaneSaber(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.SlitherbaneForeguardRect, (uint)AID.SlitherbaneRearguardRect, (uint)AID.SoulbaneSaber], new AOEShapeRect(20f, 2f));
sealed class Syntheslither(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.Syntheslean, (uint)AID.Syntheslither1, (uint)AID.Syntheslither2,
...
{
    public D043ZanderStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 831u, CFCID = 831u, NameID = 12752u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS) (ported from BMR)")]
public sealed class D043Zander(WorldState ws, Actor primary) : ModuleBase(ws, primary, StartingBounds.Center, StartingBounds)
{
