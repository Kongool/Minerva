...
namespace Minerva.Dawntrail.Dungeon.D05Origenics.D052Deceiver;
...
sealed class ArenaChanges(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        if (spell.Action.ID == (uint)AID.Electrowave && Module.Bounds == StartingBounds)
        {
...
sealed class BionicThrash(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BionicThrash, new AOEShapeCone(30f, 45f.Degrees()));
sealed class Synchroshot(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SynchroshotReal, new AOEShapeRect(40f, 2f));
sealed class InitializeTurrets(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.InitializeTurretsReal, new AOEShapeRect(4f, 5f));
sealed class LaserLash(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.LaserLashReal, new AOEShapeRect(40f, 5f));
sealed class Electray(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.Electray, 5f);

sealed class Surge(ModuleBase module) : Components.GenericKnockback(module)
{
...
sealed class SurgeHint(ModuleBase module) : Components.GenericAOEs(module)
{
    private static readonly AOEShapeRect rect = new(15.5f, 5);
    private readonly List<AOEInstance> _hints = [];
...
{
    public D052DeceiverStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 825u, NameID = 12693u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS) (ported from BMR)")]
public sealed class D052Deceiver(WorldState ws, Actor primary) : ModuleBase(ws, primary, ArenaChanges.ArenaCenter, ArenaChanges.StartingBounds)
{
