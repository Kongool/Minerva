...
namespace Minerva.Dawntrail.Dungeon.D05Origenics.D053Ambrose;
...
sealed class PsychicWaveArenaChange(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        if (spell.Action.ID == (uint)AID.PsychicWave && Module.Bounds.Radius > 30f)
        {
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        {
            Module.Bounds = new ArenaBoundsRect(15f, 19.5f);
            _aoe = [];
        }
    }
}

sealed class PsychicWave(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.PsychicWave);
sealed class Psychokinesis(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Psychokinesis, new AOEShapeRect(70f, 6.5f));

sealed class ExtrasensoryExpulsion(ModuleBase module) : Components.GenericKnockback(module, maxCasts: 1)
{
    public readonly List<Knockback> KBs = [];
...
sealed class VoltaicSlash(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.VoltaicSlash);

sealed class OverwhelmingCharge(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        ...
        var componentActive = count != 0; // BMR also ORs actor.PendingKnockbacks; Minerva has no action prediction
        ...
        else if (componentActive)
        {
            ...
            {
                ref var recti = ref kbs[i];
                if (recti.Shape is AOEShapeRect rect && rect.LenFront == 20f)
                {
...
sealed class Electrolance(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Electrolance, 22f);
sealed class WhorlOfTheMind(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.WhorlOfTheMind, 5f);
...
{
    public D053AmbroseStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 825u, CFCID = 825u, NameID = 12695u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS) (ported from BMR)")]
public sealed class D053Ambrose(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(190f, 0f), new ArenaBoundsRect(32.5f, 24f))
{
