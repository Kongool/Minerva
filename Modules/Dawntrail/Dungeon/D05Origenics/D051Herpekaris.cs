...
namespace Minerva.Dawntrail.Dungeon.D05Origenics.D051Herpekaris;
...
sealed class CollectiveAgony(ModuleBase module) : Components.LineStack(module, aidMarker: (uint)AID.CollectiveAgonyMarker, (uint)AID.CollectiveAgony, 5.6d);
sealed class StridentShriek(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.StridentShriek);
sealed class ConvulsiveCrush(ModuleBase module) : Components.SingleTargetDelayableCast(module, (uint)AID.ConvulsiveCrush);
sealed class PoisonHeartSpread(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.PoisonHeartSpread, 5f);
sealed class PoisonHeartVoidzone(ModuleBase module) : Components.VoidzoneAtCastTarget(module, 2f, (uint)AID.PoisonHeartVoidzone, GetVoidzones, 0.9d)
{
...
sealed class PodBurst(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.PodBurst1, (uint)AID.PodBurst2], 6f);

sealed class WrithingRiot(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        void AddAOE(AOEShape shape) => _aoes.Add(new(shape, spell.LocXZ, spell.Rotation, World.FutureTime(7.3d - _aoes.Count * 1d)));
...
{
    public D051HerpekarisStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 825u, CFCID = 825u, NameID = 12741u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS) (ported from BMR)")]
public sealed class D051Herpekaris(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-88f, -180f), new ArenaBoundsSquare(17.5f));
