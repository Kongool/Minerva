...
sealed class DynamicDominanceArenaChange(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        if (spell.Action.ID == (uint)AID.DynamicDominance && Bounds.Radius > 20f)
        {
            _aoe = [new(square, Center, default, Module.CastFinishAt(spell, 0.6d))];
        }
    }
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        {
            Bounds = D032Firearms.DefaultBounds;
            _aoe = [];
        }
    }
}
