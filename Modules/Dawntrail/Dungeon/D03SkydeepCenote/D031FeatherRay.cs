...
namespace Minerva.Dawntrail.Dungeon.D03SkydeepCenote.D031FeatherRay;
...
sealed class HydroRing(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        if (index == 0x13)
        {
            ...
            {
                Module.Bounds = D031FeatherRay.CircleBounds;
                _aoe = [];
            }
            else if (state == 0x00080004u)
            {
                Module.Bounds = D031FeatherRay.NormalBounds;
            }
        }
    }
}

sealed class AiryBubble(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (active)
        {
            hints.AddForbiddenZone(new SDCircle(Module.Center, 5f));
        }
        if (count == 0)
        {
            return;
...
sealed class Immersion(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Immersion);
sealed class WorrisomeWaveBoss(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WorrisomeWave1, WorrisomeWavePlayer.Cone);

sealed class WorrisomeWavePlayer(ModuleBase module) : Components.GenericBaitAway(module, centerAtTarget: true)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID == (uint)AID.WorrisomeWave1)
        {
            ...
            {
                var p = party[i];
                CurrentBaits.Add(new(p, p, Cone, World.FutureTime(6.3d)));
            }
        }
    }
...
{
    public D031FeatherRayStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 829u, CFCID = 829u, NameID = 12755u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS) (ported from BMR)")]
public sealed class D031FeatherRay(WorldState ws, Actor primary) : ModuleBase(ws, primary, arenaCenter, NormalBounds)
{
