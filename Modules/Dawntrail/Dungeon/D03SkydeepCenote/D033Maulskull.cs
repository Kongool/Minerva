...
namespace Minerva.Dawntrail.Dungeon.D03SkydeepCenote.D033Maulskull;
...
sealed class Stonecarver(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (AOEs.Count != 0)
        {
            hints.AddForbiddenZone(new SDInvertedRect(Module.Center, new WDir(1f, default), 1.5f, 1.5f, 40f), _kb!.Casters.Count != 0 ? _kb.Casters.Ref(0).Activation : AOEs.Ref(0).Activation);
        }
    }
}

sealed class Shatter(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        ...
        if (aoes[0].Activation.AddSeconds(-6d) <= World.CurrentTime)
        {
            return aoes;
        }
        return [];
...
abstract class Impact(ModuleBase module, uint aid, float distance) : Components.SimpleKnockbacks(module, aid, distance, stopAfterWall: true)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (Casters.Count != 0)
        {
            ...
            hints.AddForbiddenZone(new SDKnockbackInAABBSquareAwayFromOrigin(Module.Center, c.Origin, dist, halfWidth), c.Activation);
        }
    }
}

sealed class Impact1(ModuleBase module) : Impact(module, (uint)AID.Impact1, 18f);

sealed class Impact2(ModuleBase module) : Impact(module, (uint)AID.Impact2, 18f)
{
...
sealed class ColossalImpactSkullcrush(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.ColossalImpact, (uint)AID.Skullcrush1, (uint)AID.Skullcrush2], 10f);

sealed class DestructiveHeat(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.DestructiveHeat, 6f)
{
...
sealed class Landing(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Landing, 8f);

abstract class DeepThunder(ModuleBase module, uint aid) : Components.CastTowers(module, aid, 6f, 4, 4);
sealed class DeepThunder1(ModuleBase module) : DeepThunder(module, (uint)AID.DeepThunderTower1);
sealed class DeepThunder2(ModuleBase module) : DeepThunder(module, (uint)AID.DeepThunderTower2);

sealed class WroughtFire(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.WroughtFire, 6f, tankbuster: true);
sealed class BuildingHeat(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.BuildingHeat, 6f, 4, 4);
sealed class Ashlayer(ModuleBase module) : Components.RaidwideCastDelay(module, (uint)AID.AshlayerVisual, (uint)AID.Ashlayer, 2.2d);

sealed class D033MaulskullStates : StateMachineBuilder
{
    public D033MaulskullStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 829u, NameID = 12728u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS) (ported from BMR)")]
public sealed class D033Maulskull(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(100f, -430f), new ArenaBoundsSquare(20f));
