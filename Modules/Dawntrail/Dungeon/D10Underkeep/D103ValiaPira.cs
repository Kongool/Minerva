...
namespace Minerva.Dawntrail.Dungeon.D10Underkeep.D103ValiaPira;
...
sealed class EntropicSphere(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.EntropicSphere);
sealed class Electray(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Electray, new AOEShapeRect(40f, 4.5f));
sealed class ConcurrentField(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ConcurrentField, new AOEShapeCone(26f, 25f.Degrees()));
sealed class ElectricField(ModuleBase module) : Components.BaitAwayIcon(module, new AOEShapeCone(26f, 25f.Degrees()), (uint)IconID.ElectricField, (uint)AID.ElectricField, 7.4d);
sealed class NeutralizeFrontLines(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.NeutralizeFrontLines, new AOEShapeCone(30f, 90f.Degrees()));
sealed class HyperchargedLight(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.HyperchargedLight, 5f);
sealed class DeterrentPulse(ModuleBase module) : Components.LineStack(module, (uint)IconID.DeterrentPulse, (uint)AID.DeterrentPulse, 5.3d, 40f, 4f, 4, 4, 1, false);

sealed class EnforcementRay(ModuleBase module) : Components.GenericAOEs(module)
{
    private static readonly AOEShapeCross cross = new(36f, 4.5f);
    private readonly List<AOEInstance> _aoes = [];
    private bool teleported;
    private readonly List<WPos> startingpositions = [];
    private WPos center;
    ...
    public override void OnActorPlayActionTimelineEvent(Actor actor, ushort id)
    {
        ...
        void UpdateAOEs()
        {
            ...
            if (count > 1)
            {
                ...
                aoes[0].Activation = World.FutureTime(activationTime);

                if (NumCasts == 0)
                    aoes[1].Activation = World.FutureTime(11.4d);
            }
        }
    }
    ...
    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        if (source.OID == (uint)OID.CoordinateBit2 && tether.ID == (uint)TetherID.OrbTeleport)
        {
            ...
            var aoes = CollectionsMarshal.AsSpan(_aoes);
            var len = aoes.Length;
            var position = World.Actors.Find(tether.Target)?.Position;
            ...
            for (var i = 0; i < max; ++i)
            {
                aoes[i].Activation = World.FutureTime(timings[i]);
            }
        }
    }
...
{
    public D103ValiaPiraStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 1027u, CFCID = 1027u, NameID = 13749u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D103ValiaPira(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(0f, -331f), new ArenaBoundsSquare(17.5f));
