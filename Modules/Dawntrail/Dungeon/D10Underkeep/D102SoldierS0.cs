...
namespace Minerva.Dawntrail.Dungeon.D10Underkeep.D102SoldierS0;
...
sealed class FieldOfScorn(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.FieldOfScorn);
sealed class ThunderousSlash(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.ThunderousSlash);
sealed class OrderedFire(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.OrderedFire, new AOEShapeRect(55f, 4f));
sealed class ElectricExcess(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.ElectricExcess, 6f);
sealed class StaticForce(ModuleBase module) : Components.BaitAwayIcon(module, new AOEShapeCone(60f, 15f.Degrees()), (uint)IconID.StaticForce, (uint)AID.StaticForce, 5.1d);

sealed class SectorBisector(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    private readonly List<(Actor source, Actor target)> tethers = [];
    ...
    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        ...
        {
            tethers.Add((source, World.Actors.Find(tether.Target)!));
        }
    }
    ...
    public override void Update()
    {
        if (active && firstClone != null && _aoe.Length == 0 && tethers.Count < cloneCount)
        {
            ...
            for (var i = 0; i < count; ++i)
            {
                ...
                {
                    active = false;
                    _aoe = [new(cone, tether.target.Position, tether.target.Rotation + (direction ? -1f : 1f) * 90f.Degrees(), World.FutureTime(cloneCount == 6 ? 4.2d : 5.9d))];
                    return;
                }
            }
        }
...
{
    public D102SoldierS0States(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 1027u, NameID = 13757u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D102SoldierS0(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(default, -182f), new ArenaBoundsSquare(15.5f));
