...
namespace Minerva.Dawntrail.Dungeon.D13TheClyteum.D131EyeOfTheScorpion;
...
sealed class PetrifyingBeam(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.PetrifyingBeam, (uint)AID.PetrifyingBeam2], new AOEShapeCone(70f, 50f.Degrees()), maxCasts: 2);

sealed class EyesOnMe(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.EyesOnMe);

sealed class PenetratorMissile(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.PenetratorMissile, 6f, 4);

sealed class SurfaceMissile(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SurfaceMissile, 5f);

sealed class AntiPersonnelMissile(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.AntiPersonnelMissile, 5f);

sealed class MotionTracker(ModuleBase module) : Components.StayMove(module)
{
    ...
    public override void DrawArenaBackground(int pcSlot, Actor pc)
    {
        if (TrackingBeam != null)
        {
            ...
            {
                PlayerStates[pcSlot] = new(Requirement.Stay, World.CurrentTime);
            }
            else
...
{
    public D131EyeOfTheScorpionStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
