...
namespace Minerva.Modules.Dawntrail.Dungeon.D13TheClyteum;
...
[SkipLocalsInit]
sealed class RubbishDisposal(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.RubbishDisposal);

sealed class VoidDark(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.VoidDark, new AOEShapeCone(30f, 90f.Degrees()));

sealed class Goekinesis(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Goekinesis, new AOEShapeRect(80f, 2f));
...
sealed class MetallicMiasma(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.MetallicMiasma1, (uint)AID.MetallicMiasma3], new AOEShapeCone(60f, 15f.Degrees()));

sealed class CastOffHalo(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.CastOffHalo1, (uint)AID.CastOffHalo3], new AOEShapeCircle(7f));

sealed class ShadowPlay(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.ShadowPlay, 6f);

sealed class GluttonousWire(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.GluttonousWire, 6f, 4);

sealed class WrathfulWire(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.WrathfulWire, 5f);
...
sealed class StringUp(ModuleBase module) : Components.StayMove(module)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (caster.OID == (uint)OID.Malphas && spell.Action.ID == (uint)AID.StringUp)
        {
            ...
            {
                PlayerStates[slot] = new(Requirement.Move, World.CurrentTime.AddSeconds(5d));
            }
        }
    }
...
{
    public D133MalphasStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
