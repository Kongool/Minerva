...
sealed class DebuffTracker(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void OnStatusLose(Actor actor, ref ActorStatus status)
    {
        if (status.ID is >= (uint)SID.MaleficE and <= (uint)SID.MaleficNSEW)
        {
            var remaining = (status.ExpireAt - World.CurrentTime).TotalSeconds;
            if (remaining < 2d)
            {
...
sealed class SteelsbreathRelease(ModuleBase module) : Components.RaidwideCasts(module, [(uint)AID.SteelsbreathRelease, (uint)AID.SteelsbreathReleaseArena]);
...
sealed class NearFarFromHeaven(ModuleBase module) : Components.UniformStackSpread(module, 5f, 5f)
{
...
sealed class WolfsCrossing(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WolfsCrossing, new AOEShapeCross(40f, 4f));
// 4 at a time, leaves puddles, happens 4 times, echoing eight happen at position of 1st 3 sets (up to 12 spots)
sealed class EchoingHush(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.EchoingHush, (uint)AID.EchoingHush1], 8f);
sealed class EchoingHushPuddle(ModuleBase module) : Components.Voidzone(module, 8f, GetPuddles)
{
    public static Actor[] GetPuddles(ModuleBase module)
    {
...
sealed class EchoingEight(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.EchoingEight, new AOEShapeCross(40f, 4f));
...
sealed class SteelsbreathReleaseArena(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorEAnim(Actor actor, uint state)
    {
        if (actor.OID == (uint)OID.SteelsbreathPuddles)
        {
            if (state == 0x00010002)
            {
                _aoes.Add(new(_shape, Center + new WDir(0f, -15f), 90f.Degrees()));
                _aoes.Add(new(_shape, Center + new WDir(15f, 0f), 0f.Degrees()));
                _aoes.Add(new(_shape, Center + new WDir(0f, 15f), -90f.Degrees()));
                _aoes.Add(new(_shape, Center + new WDir(-15f, 0f), 180f.Degrees()));
            }
...
sealed class ChainTether(ModuleBase module) : Components.StretchTetherDuo(module, 20f, 30d, (uint)TetherID.Chains)
{
...
sealed class MaleficPortent(ModuleBase module) : Components.CastCounter(module, (uint)AID.MaleficPortent)
{
...
sealed class MaleficAlignment(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MaleficAlignment, new AOEShapeCone(40f, 45f.Degrees()))
{
...
*/
class GenericWillUnderworld(ModuleBase module, uint aid, AOEShape shape) : Components.SimpleAOEs(module, aid, shape)
{
    private readonly DebuffTracker _debuffs = module.FindComponent<DebuffTracker>()!;
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
...
sealed class WillOfTheUnderworld(ModuleBase module) : GenericWillUnderworld(module, (uint)AID.WillOfTheUnderworld, new AOEShapeRect(40f, 10f));
sealed class WillOfTheUnderworld1(ModuleBase module) : GenericWillUnderworld(module, (uint)AID.WillOfTheUnderworld1, new AOEShapeRect(40f, 5f));
sealed class SilentEight(ModuleBase module) : Components.GenericBaitAway(module)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        ...
        {
            CurrentBaits.Add(new(actor, actor, new AOEShapeCircle(8f), World.FutureTime(5d)));
        }
    }
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        ...
        Arena.ZoneCircleOutline(Center - new WDir(-18f, -18f), 2f);
        Arena.ZoneCircleOutline(Center - new WDir(-18f, 18f), 2f);
        Arena.ZoneCircleOutline(Center - new WDir(18f, -18f), 2f);
        Arena.ZoneCircleOutline(Center - new WDir(18f, 18f), 2f);
    }
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 1084u, CFCID = 1084u, NameID = 14323u, PrimaryActorOID = (uint)OID.LoneSwordmaster, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = " (ported from BMR)")]
[SkipLocalsInit]
public sealed class LoneSwordmaster(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(170f, -815f), new ArenaBoundsSquare(20f));
