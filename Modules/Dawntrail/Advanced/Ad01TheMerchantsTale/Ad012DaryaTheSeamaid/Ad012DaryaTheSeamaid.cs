...
sealed class SurgingCurrent(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SurgingCurrent, new AOEShapeCone(60f, 45f.Degrees()));
...
sealed class TidalSpout(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.Tidalspout, 6f, 1, 3)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        if (id == StackAction && World.Actors.Find(spell.TargetID) is Actor stackTarget)
        {
...
sealed class Hydrobullet(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.Hydrobullet, 15f)
{
...
sealed class SunkenTreasure(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorEAnim(Actor actor, uint state)
    {
        if (state == 0x00100020)
        {
            ...
            {
                _aoes.Add(new(new AOEShapeDonut(4f, 20f), actor.Position, activation: World.CurrentTime.AddSeconds(7.7d)));
            }
        }
    }
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 1079u, CFCID = 1084u, NameID = 14291u, PrimaryActorOID = (uint)OID.DaryaTheSeaMaid, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = " (ported from BMR)")]
[SkipLocalsInit]
public sealed class DaryaTheSeaMaid(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(375f, 530f), new ArenaBoundsSquare(20f));
