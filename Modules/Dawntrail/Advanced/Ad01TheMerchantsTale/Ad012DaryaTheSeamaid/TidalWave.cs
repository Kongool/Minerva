...
sealed class SeaShackles(ModuleBase module) : Components.StretchTetherDuo(module, 25f, 30d, (uint)TetherID.TetherBad, (uint)TetherID.TetherGood)
{
    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        ...
        if (player != null && target != null)
        {
            if (!ActivationDelayOnActor.Any(x => x.Item1 == target))
                ActivationDelayOnActor.Add((target, World.FutureTime(ActivationDelay)));
    ...
    public override void OnUntethered(Actor source, in ActorTetherInfo tether)
    {
        ...
        if (player != null && target != null)
        {
            ...
            var removed = TetherOnActor.Remove((World.Actors.Find(tether.Target)!, tether.ID));
        }
    }
}
...
sealed class TidalWave(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.TidalWave, 25f, kind: Kind.DirForward)
{
