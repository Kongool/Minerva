...
sealed class VirtualShiftIce(ModuleBase module) : Components.GenericAOEs(module, default, "GTFO from broken bridge!")
{
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        void UpdateArena() => Bounds = new ArenaBoundsCustom(Ex3QueenEternal.GetIceRects(), [.. _destroyedBridges]);
    }
}

sealed class LawsOfIce(ModuleBase module) : Components.StayMove(module)
{
    public int NumCasts;
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        ...
        {
            PlayerState state = new(Requirement.Move, World.FutureTime(4.2d));
...
sealed class Rush(ModuleBase module) : Components.GenericBaitAway(module)
{
    ...
    {
        if (tether.ID is (uint)TetherID.RushShort or (uint)TetherID.RushLong && World.Actors.Find(tether.Target) is var target && target != null)
        {
            RemoveBait(source);
...
sealed class IceDart(ModuleBase module) : Components.BaitAwayTethers(module, 16f, (uint)TetherID.IceDart, (uint)AID.IceDart)
{
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
...
sealed class RaisedTribute(ModuleBase module) : Components.GenericWildCharge(module, 4f, (uint)AID.RaisedTribute, 80f)
{
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
