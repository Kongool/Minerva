...
sealed class AbsoluteAuthorityPuddles(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AbsoluteAuthorityPuddlesAOE, 8f);

sealed class AbsoluteAuthorityExpansionBoot(ModuleBase module) : Components.UniformStackSpread(module, 6f, 15f, 4) // TODO: verify falloff
{
    public int NumCasts;
...
sealed class AbsoluteAuthorityHeel(ModuleBase module) : Components.GenericStackSpread(module)
{
    public int NumCasts;
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        if (iconID == (uint)IconID.AuthoritysHeel && Stacks.Count == 0)
            Stacks.Add(new(actor, 1.5f, 8, 8, activation: World.FutureTime(5.1d)));
    }
...
sealed class AbsoluteAuthorityKnockback(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.AbsoluteAuthorityKnockback, 30f, kind: Kind.DirForward);
