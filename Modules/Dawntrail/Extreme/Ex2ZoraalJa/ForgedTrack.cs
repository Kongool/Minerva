...
sealed class ForgedTrack(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        var casterOffset = caster.Position - Center;
        ...
        if (casterOffset.Z < 0f)
        {
            ...
            {
                // wide
                WideAOEs.Add(new(_shapeWide, Center + rightDir * adjustedLaneOffset, spell.Rotation, Module.CastFinishAt(spell, 1.4d)));
            }
            else
            {
                // knockback
                KnockbackAOEs.Add(new(_shape, Center + rightDir * adjustedLaneOffset, spell.Rotation, Module.CastFinishAt(spell, 1.9d)));
            }
        }
        else
        {
            ...
            NarrowAOEs.Add(new(_shape, Center + rightDir * adjustedLaneOffset, spell.Rotation, Module.CastFinishAt(spell, 1.3d)));
        }
    }
...
sealed class ForgedTrackKnockback(ModuleBase module) : Components.GenericKnockback(module, (uint)AID.StormyEdgeKnockback)
{
