...
sealed class MutedStruggle(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.MutedStruggle, ChokingGrasp.Rect, endsOnCastEvent: true, tankbuster: true);
...
sealed class SpreadingFearEnrage(ModuleBase module) : Components.CastHint(module, (uint)AID.SpreadingFear1, "Enrage!", true)
{
    public override bool KeepOnPhaseChange => true;
}
sealed class SpreadingFearInterrupt(ModuleBase module) : Components.CastInterruptHint(module, (uint)AID.SpreadingFear2)
{
    public override bool KeepOnPhaseChange => true;
}
sealed class ChokingGraspTB(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.ChokingGraspTB, ChokingGrasp.Rect, endsOnCastEvent: true, tankbuster: true)
{
    public override bool KeepOnPhaseChange => true;
}
sealed class ChokingGraspHealer(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.ChokingGraspHealer, ChokingGrasp.Rect, endsOnCastEvent: true)
{
    public override bool KeepOnPhaseChange => true;
}
sealed class ChillingFingers(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.ChillingFingers, ChokingGrasp.Rect, endsOnCastEvent: true)
{
    public override bool KeepOnPhaseChange => true;
}
sealed class ChokingGrasp3(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ChokingGraspAOE3, ChokingGrasp.Rect)
{
    public override bool KeepOnPhaseChange => true;
}
sealed class Slow(ModuleBase module) : Components.CleansableDebuff(module, (uint)SID.Slow, "Slow", "slowed")
{
    public override bool KeepOnPhaseChange => true;
}
...
sealed class Prisons(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void DrawArenaBackground(int pcSlot, Actor pc)
    {
        ...
        {
            var playerPos = pc.Position;
            if (playerPos.InSquare(Center, 50f))
            {
                return;
            }
            for (var i = 0; i < 8; ++i)
        ...
        else if (Bounds.Radius == 17.5f)
        {
            ...
            Center = Trial.T05Necron.Necron.ArenaCenter;
        }
    }
...
sealed class LimitBreakAdds(ModuleBase module) : Components.AddsMulti(module, [(uint)OID.IcyHands2, (uint)OID.BeckoningHands])
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
...
sealed class PrisonAdds(ModuleBase module) : Components.AddsMulti(module, [(uint)OID.IcyHands3, (uint)OID.IcyHands5, (uint)OID.IcyHands6])
{
    public override bool KeepOnPhaseChange => true;
}
