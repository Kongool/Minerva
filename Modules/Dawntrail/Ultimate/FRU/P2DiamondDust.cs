...
sealed class P2AxeKick(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AxeKick, 16f);
sealed class P2ScytheKick(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ScytheKick, new AOEShapeDonut(4f, 20f));

sealed class P2IcicleImpact(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.IcicleImpact)
{
...
sealed class P2FrigidNeedleCircle(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FrigidNeedleCircle, 5f);
sealed class P2FrigidNeedleCross(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FrigidNeedleCross, new AOEShapeCross(40f, 2.5f));

sealed class P2FrigidStone : Components.BaitAwayIcon
{
    public P2FrigidStone(ModuleBase module) : base(module, 5f, (uint)IconID.FrigidStone, (uint)AID.FrigidStone, 8.1f)
    {
        EnableHints = false;
...
sealed class P2DiamondDustHouseOfLight(ModuleBase module) : Components.GenericBaitAway(module, (uint)AID.HouseOfLight)
{
...
sealed class P2DiamondDustSafespots(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (_safeOffs[slot] != default)
        {
            ...
            hints.AddForbiddenZone(new SDPrecisePosition(Center + _safeOffs[slot], new WDir(default, 1f), Bounds.MapResolution, actor.Position, 0.1f));
        }
    }
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        if (_safeOffs[pcSlot] != default)
            Arena.ZoneCircleOutline(Center + _safeOffs[pcSlot], 1f, Colors.Safe);
    }
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        switch (spell.Action.ID)
        {
            case (uint)AID.IcicleImpact:
                ...
                {
                    _conesAtCardinals = IsCardinal(caster.Position - Center);
                    InitIfReady();
                }
                break;
...
sealed class P2HeavenlyStrike(ModuleBase module) : Components.GenericKnockback(module, (uint)AID.HeavenlyStrike)
{
    private readonly WDir[] _safeDirs = BuildSafeDirs(module);
    private readonly DateTime _activation = module.World.FutureTime(3.9d);

    public override ReadOnlySpan<Knockback> ActiveKnockbacks(int slot, Actor actor)
    {
        return new Knockback[1] { new(Center, 12, _activation) };
    }

    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (_safeDirs[slot] != default)
            hints.AddForbiddenZone(new SDPrecisePosition(Center + 6f * _safeDirs[slot], new(1f, default), Bounds.MapResolution, actor.Position, 0.25f), _activation);
    }
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        ...
        if (_safeDirs[pcSlot] != default)
            Arena.ZoneCircleOutline(Center + 18f * _safeDirs[pcSlot], 1f, Colors.Safe);
    }

    private static WDir[] BuildSafeDirs(ModuleBase module)
    {
...
sealed class P2SinboundHoly(ModuleBase module) : Components.UniformStackSpread(module, 6f, default, 4, 4)
{
    ...
    private static WDir CalculateDestination(ModuleBase module)
    {
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (master != null && ((master.Position - actor.Position).LengthSq() > 100f || (master.Position - Center).LengthSq() < 196f))
        ...
        var preferredDir = !moveQuickly ? _destinationDir : (actor.Position - Center).Normalized().OrthoR();
        ...
        var hintTime = World.FutureTime(50d);

        // stay near border
        hints.AddForbiddenZone(new SDCircle(Center, 16f), hintTime);
        ...
        hints.AddForbiddenZone(new SDHalfPlane(Center + planeOffset * preferredDir, preferredDir), hintTime);
    }
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        if (spell.Action.ID == (uint)AID.SinboundHolyAOE && World.CurrentTime > _nextExplosion)
        {
            if (NumCasts == 0)
            ...
            ++NumCasts;
            _nextExplosion = World.FutureTime(0.5f);
        }
    }
}

sealed class P2SinboundHolyVoidzone(ModuleBase module) : Components.Voidzone(module, 6f, GetVoidzones)
{
...
sealed class P2ShiningArmor(ModuleBase module) : Components.GenericGaze(module, (uint)AID.ShiningArmor)
{
    private Actor? _source;
    private DateTime _activation;
    ...
    public override void OnActorPlayActionTimelineEvent(Actor actor, ushort id)
    {
        ...
        {
            _source = actor;
            _activation = World.FutureTime(7.2d);
        }
    }
}

sealed class P2TwinStillnessSilence(ModuleBase module) : Components.GenericAOEs(module)
{
        ...
        if (!_thinIce[slot])
        {
            ...
            if (closest != default)
            {
                ...
                if (halfWidth.Deg > 5)
                {
                    ...
                    halfWidth = 5.Degrees();
                    var sourceDir = Angle.FromDirection(_source.Position - Center);
                ...
                hints.AddForbiddenZone(new SDCircle(Center, 16f), World.FutureTime(50));
                hints.AddForbiddenZone(new SDInvertedCone(Center, 100f, desiredDir, halfWidth), DateTime.MaxValue);
            }
        }
        else if (actor.LastFrameMovement == default)
        {
            // at this point, we have thin ice, so we can either stay or move fixed distance
            var sourceOffset = _source.Position - Center;
            var needToMove = AOEs.Count > 0 ? AOEs[0].Check(actor.Position) : NumCasts == 0 && sourceOffset.Dot(actor.Position - Center) > 0;
            if (!needToMove)
                return;

            var zoneList = new ArcList(actor.Position, SlideDistance);
            zoneList.ForbidInverseCircle(Center, Bounds.Radius);
...
sealed class P2ThinIce(ModuleBase module) : Components.ThinIce(module, 32f)
{
