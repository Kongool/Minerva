...
sealed class P5FulgentBlade : Components.Exaflare
{
    ...
    public P5FulgentBlade(ModuleBase module) : base(module, new AOEShapeRect(5f, 40f))
    {
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID is (uint)AID.PathOfLightFirst or (uint)AID.PathOfDarknessFirst)
        {
            ...
            var distanceToBorder = Intersect.RayCircle(caster.Position - Center, dir, 22f);
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        {
            if (World.CurrentTime > _nextBundle)
            {
                ++NumCasts;
