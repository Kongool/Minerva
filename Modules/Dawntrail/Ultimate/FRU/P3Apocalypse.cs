...
sealed class P3Apocalypse(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public void Show(float delay)
    {
        ...
        {
            addAOE(Center + offset, activation);
            addAOE(Center - offset, activation);
        }
        ...
        var activation = World.FutureTime(delay);
        for (var i = -1; i < 5; ++i)
        {
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        {
            if (actor.Position.AlmostEqual(Center, 1f))
            {
                if (Starting == null)
            ...
            {
                var rot = 0.5f * (actor.Rotation - Angle.FromDirection(actor.Position - Center)).Normalized();
                if (Rotation == default)
                    Rotation = rot;
            }
        }
...
sealed class P3ApocalypseDarkWater(ModuleBase module) : Components.UniformStackSpread(module, 6f, default, 4, 4, includeDeadTargets: true)
{
    public struct State
    {
        public int Order;
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        if (status.ID == (uint)SID.SpellInWaitingDarkWater && Raid.FindSlot(actor.InstanceID) is var slot && slot >= 0)
        {
            ...
            States[slot].Order = (status.ExpireAt - World.CurrentTime).TotalSeconds switch
            {
                < 15d => 1,
                < 34d => 2,
                _ => 3,
...
sealed class P3ApocalypseSpiritTaker(ModuleBase module) : SpiritTaker(module)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        base.AddAIHints(slot, actor, assignment, hints);
        hints.AddForbiddenZone(new SDCircle(Center, 6), DateTime.MaxValue); // don't dodge into center...
    }
}

sealed class P3ApocalypseDarkEruption(ModuleBase module) : Components.SpreadFromIcon(module, (uint)IconID.DarkEruption, (uint)AID.DarkEruption, 6, 5.1f)
{
    private readonly FRUConfig _config = Service.Config.Get<FRUConfig>();
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (safeSpot != default)
        {
            ...
            hints.AddForbiddenZone(new SDPrecisePosition(Center + safeSpot, new(0, 1), Bounds.MapResolution, actor.Position, 0.1f), Spreads.Count != 0 ? Spreads.Ref(0).Activation : DateTime.MaxValue);
        }
    }
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        ...
        if (safeSpot != default)
        {
            Arena.ZoneCircleOutline(Center + safeSpot, 1f, Colors.Safe);
            if (refSafeSpot != safeSpot)
                Arena.ZoneCircleOutline(Center + refSafeSpot, 1f);
        }
...
sealed class P3DarkestDanceBait(ModuleBase module) : Components.GenericBaitAway(module, (uint)AID.DarkestDanceBait, centerAtTarget: true)
{
...
sealed class P3DarkestDanceKnockback(ModuleBase module) : Components.GenericKnockback(module, (uint)AID.DarkestDanceKnockback)
{
    public Actor? Caster;
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        switch (spell.Action.ID)
        {
            case (uint)AID.DarkestDanceBait:
                Caster = caster;
                Activation = World.FutureTime(2.8d);
                break;
...
sealed class P3ApocalypseAIWater1(ModuleBase module) : ModuleComponent(module)
{
    private readonly FRUConfig _config = Service.Config.Get<FRUConfig>();
    private readonly P3ApocalypseDarkWater? _water = module.FindComponent<P3ApocalypseDarkWater>();
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (state.AssignedGroup == 2)
            dir += 180f.Degrees();
        hints.AddForbiddenZone(new SDInvertedCircle(Center + range * dir.ToDirection(), 1f), _water.Stacks.Count > 0 ? _water.Stacks.Ref(0).Activation : DateTime.MaxValue);
    }
}

// position for second dark water & darkest dance - for simplicity, we position in the direction tank would take darkest dance
sealed class P3ApocalypseAIWater2(ModuleBase module) : ModuleComponent(module)
{
    private readonly FRUConfig _config = Service.Config.Get<FRUConfig>();
    private readonly P3Apocalypse? _apoc = module.FindComponent<P3Apocalypse>();
    private readonly P3ApocalypseDarkWater? _water = module.FindComponent<P3ApocalypseDarkWater>();
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        hints.AddForbiddenZone(new SDInvertedCircle(Center + destOff, 1f), DateTime.MaxValue);
    }
}

// position for darkest dance knockback & third dark water
sealed class P3ApocalypseAIWater3(ModuleBase module) : ModuleComponent(module)
{
    private readonly P3ApocalypseDarkWater? _water = module.FindComponent<P3ApocalypseDarkWater>();
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        var toCenter = Center - _knockback.Caster.Position;
        ...
        if (_knockback.NumCasts == 0)
        {
            ...
            hints.AddForbiddenZone(new SDPrecisePosition(_knockback.Caster.Position + 2f * dir.ToDirection(), new(0, 1), Bounds.MapResolution, actor.Position, 0.1f), _knockback.Activation);
        }
