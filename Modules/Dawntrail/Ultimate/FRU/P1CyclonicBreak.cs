...
sealed class P1CyclonicBreakSpreadStack(ModuleBase module) : Components.UniformStackSpread(module, 6f, 6f, 2, 2)
{
...
sealed class P1CyclonicBreakProtean(ModuleBase module) : Components.BaitAwayEveryone(module, module.PrimaryActor, P1CyclonicBreakCone.Shape, (uint)AID.CyclonicBreakAOEFirst)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints) { } // handled by dedicated component
}

sealed class P1CyclonicBreakCone(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        {
            case (uint)AID.CyclonicBreakAOEFirst:
                AOEs.Add(new(Shape, caster.Position.Quantized(), spell.Rotation, World.FutureTime(2d)));
                break;
            case (uint)AID.CyclonicBreakAOERest:
                if (World.CurrentTime > _currentBundle)
                {
                    _currentBundle = World.CurrentTime.AddSeconds(1d);
                    ++NumCasts;
...
sealed class P1CyclonicBreakAIBait(ModuleBase module) : ModuleComponent(module)
{
...
sealed class P1CyclonicBreakAIDodgeSpreadStack(ModuleBase module) : ModuleComponent(module)
{
    private readonly FRUConfig _config = Service.Config.Get<FRUConfig>();
    private readonly P1CyclonicBreakSpreadStack? _spreadStack = module.FindComponent<P1CyclonicBreakSpreadStack>();
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (_spreadStack.Activation < World.FutureTime(0.5f))
        {
...
sealed class P1CyclonicBreakAIDodgeRest(ModuleBase module) : ModuleComponent(module)
{
    private readonly P1CyclonicBreakCone? _cones = module.FindComponent<P1CyclonicBreakCone>();
