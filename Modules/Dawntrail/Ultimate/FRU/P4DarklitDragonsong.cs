...
sealed class P4DarklitDragonsong(ModuleBase module) : ModuleComponent(module)
{
...
sealed class P4DarklitDragonsongBrightHunger(ModuleBase module) : Components.GenericTowers(module, (uint)AID.BrightHunger)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        var towerPos = Center + new WDir(0, _darklit.AssignS[slot] ? +TowerOffset : -TowerOffset);
        hints.AddForbiddenZone(new SDInvertedCircle(towerPos, 3f), Towers[0].Activation);
        hints.AddForbiddenZone(new SDCircle(Center, TowerOffset), Towers[0].Activation);
        hints.AddForbiddenZone(new SDHalfPlane(Center, new(_darklit.AssignE[slot] ? +1 : -1, default)), Towers[0].Activation);
    }
    ...
    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        if (tether.ID == (uint)TetherID.LightRampantChains && ++_numTethers == 4 && _darklit != null)
        {
            ...
            var act = World.FutureTime(10.4d);
            Towers.Add(new(Center - towerOffset, 4f, 2, 2, new BitMask(0xFF) ^ allowedN, act));
            Towers.Add(new(Center + towerOffset, 4f, 2, 2, new BitMask(0xFF) ^ allowedS, act));
        }
    }
}

sealed class P4DarklitDragonsongPathOfLight(ModuleBase module) : Components.GenericBaitAway(module, (uint)AID.PathOfLightAOE)
{
    private readonly P4DarklitDragonsong? _darklit = module.FindComponent<P4DarklitDragonsong>();
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        hints.AddForbiddenZone(new SDPrecisePosition(Center + 4f * dir.ToDirection(), new(default, 1f), Bounds.MapResolution, actor.Position, 0.1f), _activation);
    }
...
sealed class P4DarklitDragonsongSpiritTaker(ModuleBase module) : SpiritTaker(module)
{
    //private readonly P4DarklitDragonsong? _darklit = module.FindComponent<P4DarklitDragonsong>();

    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        //if (_darklit != null && _darklit.AssignS.Any() && Spreads.Count > 0 && Spreads[0].Activation > World.FutureTime(1))
        //{
        //    // preposition
        //    hints.AddForbiddenZone(new SDInvertedCircle(Center + PrepositionOffset(_darklit, slot, actor), 1)), Spreads[0].Activation);
        //}
        //else
        {
...
sealed class P4DarklitDragonsongDarkWater(ModuleBase module) : Components.UniformStackSpread(module, 6f, default, 4, includeDeadTargets: true)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        for (var i = 0; i < len; ++i)
        {
            ...
            {
                if (s.Target == actor || s.Activation > World.FutureTime(1.5d))
                {
                    // preposition
...
sealed class P4SomberDance(ModuleBase module) : Components.GenericBaitAway(module, centerAtTarget: true)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        {
            // go far east/west
            var pos = Center + new WDir(actor.PosRot.X > Center.X ? 19f : -19f, default);
