...
sealed class P2AbsoluteZero(ModuleBase module) : Components.CastCounter(module, (uint)AID.AbsoluteZeroAOE);

sealed class P2SwellingFrost(ModuleBase module) : Components.GenericKnockback(module, (uint)AID.SwellingFrost)
{
    private readonly DateTime _activation = module.World.FutureTime(3.2d);

    public override ReadOnlySpan<Knockback> ActiveKnockbacks(int slot, Actor actor)
    {
        return new Knockback[1] { new(Center, 10f, _activation, ignoreImmunes: true) };
    }
}

sealed class P2SinboundBlizzard(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SinboundBlizzardAOE, new AOEShapeCone(50f, 10f.Degrees()));

sealed class P2HiemalStorm(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HiemalStormAOE, 7f)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        var deadline = _slowDodges ? World.FutureTime(1.5d) : DateTime.MaxValue;
...
sealed class P2HiemalRay(ModuleBase module) : Components.VoidzoneAtCastTarget(module, 4f, (uint)AID.HiemalRay, GetVoidzones, 0.7d)
{
...
sealed class P2Intermission(ModuleBase module) : Components.GenericBaitAway(module)
{
        ...
        if (index == 0x18)
        {
            switch (state)
            {
                case 0x00020001u: // crystal appears
                    Bounds = new ArenaBoundsCustom([new Polygon(Center, 20f, 64)], [new Polygon(new(100.5f, 100f), 6f, 16)]); // crystal collision is slightly off center for some reason
                    break;
                case 0x00080004u: // crystal destroyed
                    Bounds = FRU.BuildArena().arena;
                    break;
            }
        }
    }
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        foreach (var c in CrystalsOfLight)
            hints.AddForbiddenZone(new SDCircle(c.Position, 4f), World.FutureTime(30d));
        ...
        else if ((clockSpot & 1) == 0)
        {
            ...
            var assignedPosition = Center + 15f * assignedDir.ToDirection(); // crystal is at R=15
            var assignedCrystal = CrystalsOfLight.FirstOrDefault(c => c.Position.AlmostEqual(assignedPosition, 2f));
            if (assignedCrystal != null)
            {
                hints.AddForbiddenZone(new SDInvertedCircle(assignedPosition, 5f), World.FutureTime(60d));
                hints.AddForbiddenZone(new SDCircle(Center, 17f), DateTime.MaxValue); // prefer to stay near border, unless everything else is covered with aoes
            }
            else
            {
                // go to the ice veil
                ...
                hints.AddForbiddenZone(new SDInvertedCone(Center, 7f, assignedDir, 10f.Degrees()), DateTime.MaxValue);
            }
        }
        else
        {
            ...
            {
                var assignedPosition = Center + 9f * (180f - 45f * clockSpot).Degrees().ToDirection(); // crystal is at R=8
                var assignedCrystal = CrystalsOfDarkness.FirstOrDefault(c => c.Position.AlmostEqual(assignedPosition, 2f));
                if (assignedCrystal != null)
                    hints.AddForbiddenZone(new SDPrecisePosition(assignedPosition, new WDir(default, 1f), Bounds.MapResolution, actor.Position, 0.1f));
            }
    ...
    private List<Actor> CrystalsOfLight => ModuleBase.GetActiveActors(_crystalsOfLight);
    private List<Actor> CrystalsOfDarkness => ModuleBase.GetActiveActors(_crystalsOfDarkness);
    private Actor? IceVeil => ModuleBase.GetActiveActor(_iceVeil);
    ...
    {
        var offset = crystal.Position - Center;
