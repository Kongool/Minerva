...
sealed class P5ParadiseRegainedTowers(ModuleBase module) : Components.GenericTowers(module, (uint)AID.WingsDarkAndLightExplosion)
{
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        if (index is >= 51 and <= 53 && state == 0x00020001u)
        {
            ...
            Towers.Add(new(Center + 7f * dir.ToDirection(), 3f, 2, 2, forbidden, World.FutureTime(9.5d)));
        }
    }
...
sealed class P5ParadiseRegainedBaits(ModuleBase module) : Components.GenericBaitAway(module)
{
    ...
    public override void Update()
    {
        ...
        {
            var cleaveTarget = NumCasts == 0 ? _firstTarget : World.Actors.Find(_source.TargetID);
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (!ForbiddenPlayers[slot])
        {
            ...
            hints.AddForbiddenZone(new SDInvertedCircle(Center + SafeOffset(slot, actor), 1f));
        }
    }
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        if (shape != null)
        {
            ...
            _source = caster;
            _firstTarget = World.Actors.Find(caster.TargetID);
            _curCleave = shape;
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        if (nextShape != null)
        {
            ...
            _activation = World.FutureTime(3.7d);
