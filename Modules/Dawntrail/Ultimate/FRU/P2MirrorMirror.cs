...
sealed class P2MirrorMirrorReflectedScytheKickBlue : Components.GenericAOEs
{
    ...
    public P2MirrorMirrorReflectedScytheKickBlue(ModuleBase module) : base(module, (uint)AID.ReflectedScytheKickBlue)
    {
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (_aoe.Length == 0 && Module.Enemies((uint)OID.BossP2) is var bossP2 && bossP2.Count != 0 && bossP2[0] is var boss && boss.TargetID == actor.InstanceID)
        {
            ...
            hints.AddForbiddenZone(new SDInvertedCircle(Center - 16f * _blueMirror, 1), DateTime.MaxValue);
        }
    }
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        ...
        {
            Arena.Actor(Center + 20f * _blueMirror, Angle.FromDirection(-_blueMirror), Colors.Object);
            if (_aoe.Length == 0)
            {
                // draw preposition hint
                var distance = _rangedSpots[pcSlot] ? 19f : -11f;
                Arena.ZoneCircleOutline(Center + distance * _blueMirror, 1f, Colors.Safe);
            }
        }
    }
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        {
            _aoe = [new(_shape, Center + 20f * _blueMirror, default, Module.CastFinishAt(spell))];
        }
    }
...
sealed class P2MirrorMirrorReflectedScytheKickRed(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ReflectedScytheKickRed, new AOEShapeDonut(4f, 20f))
{
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
...
sealed class P2MirrorMirrorHouseOfLight(ModuleBase module) : Components.GenericBaitAway(module, (uint)AID.HouseOfLight)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        {
            dir = Angle.FromDirection(origin.Actor.Position - Center) + group switch
            {
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        switch (spell.Action.ID)
        {
            case (uint)AID.ScytheKick:
                ...
                var mirror = _blueMirror != null ? Module.Enemies((uint)OID.FrozenMirror).Closest(Center + 20f * _blueMirror.Value.ToDirection()) : null;
                if (mirror != null)
            ...
            case (uint)AID.ReflectedScytheKickRed:
                ...
                if (SecondSources.Count == 2 && _blueMirror != null)
                {
                    ...
                    var d1 = (Angle.FromDirection(SecondSources[0].Actor.Position - Center) - _blueMirror.Value).Normalized();
                    var d2 = (Angle.FromDirection(SecondSources[1].Actor.Position - Center) - _blueMirror.Value).Normalized();
                    var d1abs = d1.Abs();
                    var d2abs = d2.Abs();
                    ...
                    RedRangedLeftOfMelee = (SecondSources[0].Actor.Position - Center).OrthoL().Dot(SecondSources[1].Actor.Position - Center) > 0;
                }
                break;
        }
    }
...
sealed class P2MirrorMirrorBanish : P2Banish
{
    ...
    public P2MirrorMirrorBanish(ModuleBase module) : base(module)
    {
        ...
        if (proteans != null && proteans.FirstSources.Count == 2 && proteans.SecondSources.Count == 2)
        {
            ...
            _anchorRanged = Center + 0.5f * (proteans.SecondSources[1].Actor.Position - Center);
    ...
    private WPos? PrepositionLocation(int slot, PartyRolesConfig.Assignment assignment)
        => Stacks.Count > 0 && Stacks.Ref(0).Activation > World.FutureTime(2.5d) ? CalculatePrepositionLocation(_aroundRanged[slot], _leftSide[slot], 90f.Degrees())
        : Spreads.Count > 0 && Spreads.Ref(0).Activation > World.FutureTime(2.5d) ? CalculatePrepositionLocation(_aroundRanged[slot], _leftSide[slot], (_closerToCenter[slot] ? 135f : 45f).Degrees())
        : null;
    ...
    private WPos CalculatePrepositionLocation(bool aroundRanged, bool leftSide, Angle angle)
    {
        ...
        var offset = Angle.FromDirection(anchor - Center) + (leftSide ? angle : -angle);
