...
sealed class P3UltimateRelativity(ModuleBase module) : Components.CastCounter(module, default)
{
    ...
    private DateTime _nextImminent = module.World.FutureTime(21.9f - 2.5f); // approx 2.5s before next step resolves
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (States[slot].AssignedDir != default)
        {
            ...
            {
                case RangeHintOut:
                    if (World.CurrentTime < _nextImminent)
                    {
                ...
                case RangeHintEye:
                    ...
                    hints.AddForbiddenZone(new SDPrecisePosition(SafeSpot(slot, range), new(default, 1f), Bounds.MapResolution, actor.Position, 0.1f), _nextImminent);
                    break;
                default:
                    ...
                    var dest = NumCasts >= 6 ? SafeSpot(slot, range) : Center;
                    hints.AddForbiddenZone(new SDPrecisePosition(dest, new(default, 1f), Bounds.MapResolution, actor.Position, 0.1f), _nextImminent);
                    break;
            }
        }
    }
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        ...
        {
            Arena.AddLine(Center, Center + 20f * assignedDir, Colors.Safe);
        ...
        switch (status.ID)
        {
            case (uint)SID.SpellInWaitingDarkFire:
                ...
                if (slot >= 0)
                {
                    ...
                    state.FireOrder = (status.ExpireAt - World.CurrentTime).TotalSeconds switch
                    {
                        < 15d => 1,
                        < 25d => 2,
                        _ => 3
            ...
            case (uint)SID.SpellInWaitingReturn:
                slot = Raid.FindSlot(actor.InstanceID);
                if (slot >= 0)
                    States[slot].RewindOrder = (status.ExpireAt - World.CurrentTime).TotalSeconds < 20 ? 1 : 2;
                break;
    ...
    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        ...
        {
            _relNorth += source.Position - Center;
    ...
    {
        if (spell.Action.ID is (uint)AID.UltimateRelativityUnholyDarkness or (uint)AID.UltimateRelativitySinboundMeltdownAOEFirst && World.CurrentTime > _nextImminent)
        {
            ++NumCasts;
            _nextImminent = World.FutureTime(2.5d);
        }
    }
    ...
    private WPos SafeSpot(int slot, float range)
    {
        ...
        var safespot = Center + range * assignedDir;
...
sealed class P3UltimateRelativityDarkFireUnholyDarkness(ModuleBase module) : Components.UniformStackSpread(module, 6f, 8f, 5)
{
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        switch (status.ID)
        {
            case (uint)SID.SpellInWaitingUnholyDarkness:
                if ((status.ExpireAt - World.CurrentTime).TotalSeconds < 10d)
...
sealed class P3UltimateRelativitySinboundMeltdownBait(ModuleBase module) : Components.GenericBaitAway(module, (uint)AID.UltimateRelativitySinboundMeltdownAOEFirst)
{
    ...
    private WPos AssignedHourglass(int slot) => Center + 9.5f * (_rel?.States[slot].AssignedDir ?? default);
}

sealed class P3UltimateRelativitySinboundMeltdownAOE(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly P3UltimateRelativity? _rel = module.FindComponent<P3UltimateRelativity>();
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        switch (spell.Action.ID)
        {
            case (uint)AID.UltimateRelativitySinboundMeltdownAOEFirst:
                ...
                for (var i = 1; i < 10; ++i)
                    _aoes.Add(new(_shape, caster.Position, spell.Rotation + i * rot, World.FutureTime(i + 1)));
                break;
...
sealed class P3UltimateRelativityDarkBlizzard(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.UltimateRelativityDarkBlizzard)
{
...
sealed class P3UltimateRelativityShadoweye(ModuleBase module) : ModuleComponent(module)
{
    private readonly P3UltimateRelativity? _rel = module.FindComponent<P3UltimateRelativity>();
...
sealed class P3ShellCrusher(ModuleBase module) : Components.UniformStackSpread(module, 6f, default, 8, 8, includeDeadTargets: true)
{
