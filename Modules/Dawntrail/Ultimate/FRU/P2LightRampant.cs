...
sealed class P2LightRampant(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        if (tether.ID is (uint)TetherID.LightRampantChains or (uint)TetherID.LightRampantCurse && Raid.FindSlot(source.InstanceID) is var slot && slot >= 0)
            _tetherTargets[slot] = World.Actors.Find(tether.Target);
    }
...
sealed class P2LuminousHammer(ModuleBase module) : Components.BaitAwayIcon(module, 6f, (uint)IconID.LuminousHammer, (uint)AID.LuminousHammer, 7.1f)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (predictedDamage.Any())
            hints.AddPredictedDamage(predictedDamage, CurrentBaits[0].Activation);
    }
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        {
            ++NumCasts;
            PrevBaitOffset[slot] = (Raid[slot]?.Position ?? Center) - Center;
...
sealed class P2BrightHunger1(ModuleBase module) : Components.GenericTowers(module, (uint)AID.BrightHunger)
{
    ...
    private void RebuildTowers()
    {
        ...
        for (var i = 0; i < 6; ++i)
        {
            ...
            Towers.Add(new(Center + 16f * dir.ToDirection(), 4, 1, 1, forbidden, World.FutureTime(10.3d)));
        }
    }
}

// note: we can start showing aoes ~3s earlier if we check spawns, but it's not really needed
sealed class P2HolyLightBurst(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HolyLightBurst, 11f, 3)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints) { } // there are dedicated components for hints
}

sealed class P2PowerfulLight(ModuleBase module) : Components.UniformStackSpread(module, 5f, default, 4, 4)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints) { } // there are dedicated components for hints
...
sealed class P2BrightHunger2(ModuleBase module) : Components.GenericTowers(module, (uint)AID.BrightHunger)
{
    private BitMask _forbidden;
    ...
    public override void OnCastFinished(Actor caster, ActorCastInfo spell)
    {
        if (Towers.Count == 0 && spell.Action.ID == (uint)AID.HolyLightBurst)
            Towers.Add(new(Center, 4f, 1, 8, _forbidden, World.FutureTime(6.5d)));
    }
...
sealed class P2LightRampantBanish(ModuleBase module) : P2Banish(module)
{
    private readonly FRUConfig _config = Service.Config.Get<FRUConfig>();
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (_tower?.Towers.Count > 0)
        {
            ...
            if (t.ForbiddenSoakers[slot])
            {
                ...
                hints.AddForbiddenZone(new SDCircle(t.Position, 6f), World.FutureTime(30d));
                ...
                if (prepos != null)
                {
                    ...
                    hints.AddForbiddenZone(new SDInvertedCone(Center, 50f, Angle.FromDirection(prepos.Value - Center), 15f.Degrees()), World.FutureTime(60d));
                }
            }
            else
            {
    ...
    private WPos? PrepositionLocation(PartyRolesConfig.Assignment assignment)
    {
        ...
        var assignedDirection = (180f - 45f * clockspot).Degrees();
        if (Stacks.Count > 0 && Stacks[0].Activation > World.FutureTime(1d))
        {
            ...
            return Center + 10f * assignedDirection.ToDirection();
        }
        else if (Spreads.Count > 0 && Spreads[0].Activation > World.FutureTime(1d))
        {
            return Center + 13f * assignedDirection.ToDirection();
        }
        return null;
    }
}

sealed class P2HouseOfLightBoss(ModuleBase module) : Components.GenericBaitAway(module, (uint)AID.HouseOfLightBossAOE, false)
{
...
sealed class P2LightRampantAITowers(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (bait.Target != null)
        {
            if (_puddles.BaitsPerPlayer[slot] == 0)
            {
                ...
                var preposSpot = Center + new WDir(default, north ? -BaitOffset : BaitOffset);
            ...
            else
            {
                ...
                //var nextSpot = Center + BaitOffset * _puddles.PrevBaitOffset[slot].Normalized().Rotate(-45.Degrees());
                //hints.AddForbiddenZone(new SDInvertedCircle(nextSpot, 3));
                hints.AddForbiddenZone(new SDInvertedDonutSector(Center, BaitOffset - 1f, BaitOffset + 2f, Angle.FromDirection(_puddles.PrevBaitOffset[slot]) - 45f.Degrees(), 30f.Degrees()), DateTime.MaxValue);
            }
        }
        else
        {
            ...
            if (assignedTowerIndex >= 0 && _towers.Towers.FindIndex(assignedTowerIndex + 1, t => !t.ForbiddenSoakers[slot]) < 0)
            {
                ...
                hints.AddForbiddenZone(new SDInvertedCircle(Center + (t.Position - Center) * 1.125f, 2f), t.Activation); // center is at R16, x1.125 == R18
            }
...
sealed class P2LightRampantAIStackPrepos(ModuleBase module) : ModuleComponent(module)
{
    private readonly P2LuminousHammer? _puddles = module.FindComponent<P2LuminousHammer>();
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        var northCamp = isPuddleBaiter ? actor.PosRot.X < Center.X : actor.PosRot.Z < Center.Z; // this assumes CW movement for baiter
        var dest = Center + new WDir(default, northCamp ? -18f : 18f);
        if (isPuddleBaiter)
        {
...
sealed class P2LightRampantAIStackResolve(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        for (var i = 0; i < len; ++i)
        {
            var offset = _orbs.ActiveCasters[i].Origin.Z - Center.Z;
        ...
        var destPos = Center + Radius * destDir.ToDirection();
    ...
    private bool IsNorthCamp(Actor actor) => actor.PosRot.Z < Center.Z;
}

// movement to dodge orbs after resolving stack
sealed class P2LightRampantAIOrbs(ModuleBase module) : ModuleComponent(module)
{
    private readonly P2HolyLightBurst? _orbs = module.FindComponent<P2HolyLightBurst>();
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (_orbs.NumCasts == 0)
        {
            ...
            hints.AddForbiddenZone(new SDCircle(Center, 16f));
        ...
        else
        {
            ...
            hints.AddForbiddenZone(new SDInvertedCircle(Center, 16f), World.FutureTime(30d));
            hints.AddForbiddenZone(new SDInvertedCircle(Center, 13f), World.FutureTime(40d));
            hints.AddForbiddenZone(new SDInvertedCircle(Center, 10f), World.FutureTime(50d));
            hints.AddForbiddenZone(new SDInvertedCircle(Center, 7f), DateTime.MaxValue);
        }
    }
}
