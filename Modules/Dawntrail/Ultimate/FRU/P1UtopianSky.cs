...
sealed class P1UtopianSkyBlastingZone(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.BlastingZoneAOE)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (World.FutureTime(1d) > Activation)
    ...
    public override void OnActorModelStateChange(Actor actor, byte modelState, byte animState1, byte animState2)
    {
        ...
        {
            Activation = World.FutureTime(9.1d);
            AOEs.Add(new(_shape, actor.Position, actor.Rotation, Activation));
            DangerousSpots.Set((int)MathF.Round((-Angle.FromDirection(actor.Position - Center).Deg + 180f) / 45f) & 7);
        }
    }
...
sealed class P1UtopianSkySpreadStack(ModuleBase module) : Components.UniformStackSpread(module, 6f, 5f, 4, 4)
{
    ...
    {
        // micro adjust when activation is imminent; before that we have dedicated ai component
        if (World.FutureTime(1.5d) > Activation)
            base.AddAIHints(slot, actor, assignment, hints);
    }
...
sealed class P1UtopianSkyAIInitial(ModuleBase module) : ModuleComponent(module)
{
    private readonly FRUConfig _config = Service.Config.Get<FRUConfig>();

    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        hints.AddForbiddenZone(new SDCircle(Center, 18f)); // stay on edge
        ...
        if (clockspot >= 0)
        {
            ...
            hints.AddForbiddenZone(new SDInvertedCone(Center, 50f, assignedDirection, 5f.Degrees()), DateTime.MaxValue);
        }
    }
}
...
sealed class P1UtopianSkyAIResolve(ModuleBase module) : ModuleComponent(module)
{
    private readonly FRUConfig _config = Service.Config.Get<FRUConfig>();
    ...
    public override void Update()
    {
        ...
        if (World.FutureTime(6d) > _aoes.Activation)
        {
        ...
        {
            var spot = group & 3;
            if (folded[spot] && !_seenDangerSpot[spot] && Raid[slot] is var p && p != null && !p.Position.InDonutCone(Center, 12f, 20f, (180f - 45f * group).Degrees(), 30f.Degrees()))
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (numDangerSpots < 3)
        {
            if (clockSpot >= 0 && (_aoes.DangerousSpots[clockSpot] || _seenDangerSpot[clockSpot & 3]))
            {
                ...
                hints.AddForbiddenZone(new SDInvertedCircle(Center, 5f), _aoes.Activation);
            }
            // else: we don't have a reason to move, stay where we are...
        }
        else if (numDangerSpots == 3/* && World.FutureTime(1) <= _aoes.Activation*/)
        {
            ...
            hints.AddForbiddenZone(new SDPrecisePosition(Center + range * direction.ToDirection(), new(0, 1), Bounds.MapResolution, actor.Position, 0.1f), _aoes.Activation);
        }
    }
}
