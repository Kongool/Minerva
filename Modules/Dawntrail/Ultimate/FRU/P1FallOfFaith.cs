...
sealed class P1FallOfFaith(ModuleBase module) : Components.CastCounter(module, default)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (baitOrder == 0 || _playerOrder[slot] >= 5 && World.CurrentTime < _minHelpMove)
            return;
        var dest = TetherSpot(baitOrder);
        if (_playerOrder[slot] != baitOrder)
            dest += BaitOffset(_playerOrder[slot], _fireTethers[baitOrder - 1]);
        hints.AddForbiddenZone(new SDPrecisePosition(dest, new(0, 1), Bounds.MapResolution, actor.Position, 0.1f));
    }
    ...
    private void InitAssignments()
    {
        ...
        _minHelpMove = World.FutureTime(1d);
    }
    ...
    private WPos TetherSpot(int order) => order <= _numFetters ? _tetherTargets[order - 1].Position : Center + 5.5f * GroupDirection(order);
