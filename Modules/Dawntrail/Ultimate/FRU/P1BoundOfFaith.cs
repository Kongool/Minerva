...
abstract class BurntStrike(ModuleBase module, uint aid) : Components.SimpleAOEs(module, aid, new AOEShapeRect(80f, 5f));
sealed class P1TurnOfHeavensBurntStrikeFire(ModuleBase module) : BurntStrike(module, (uint)AID.TurnOfHeavensBurntStrikeFire);
sealed class P1TurnOfHeavensBurntStrikeLightning(ModuleBase module) : BurntStrike(module, (uint)AID.TurnOfHeavensBurntStrikeLightning);

abstract class BurntOut(ModuleBase module, uint aid) : Components.SimpleAOEs(module, aid, new AOEShapeRect(80f, 10f));
sealed class P1TurnOfHeavensBurnout(ModuleBase module) : BurntOut(module, (uint)AID.TurnOfHeavensBurnout);

sealed class P1BrightfireSmall(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BrightfireSmall, 5f);
sealed class P1BrightfireLarge(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BrightfireLarge, 10f);
...
sealed class P1BoundOfFaith(ModuleBase module) : Components.UniformStackSpread(module, 6f, default, 4, 4)
{
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        ...
        {
            Arena.ZoneCircleOutline(Center + SafeSide * 18.2f + AssignedLane(pcSlot), 1, Colors.Safe);
        }
    }
    ...
    {
        if (tether.ID == (uint)TetherID.Fire && World.Actors.Find(tether.Target) is var target && target != null)
        {
            Activation = World.FutureTime(10.6d);
    ...
    private void InitAssignments()
    {
        if (_safeHalo != default)
        {
            ...
            foreach (var aoe in Module.Enemies(_safeHalo))
                averageOffset += aoe.Position - Center;
...
sealed class P1BoundOfFaithAIKnockback(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        hints.AddForbiddenZone(new SDHalfPlane(Center + sideOffset * _comp.SafeSide, _comp.SafeSide), _comp.Activation);

        var lane = _comp.AssignedLane(slot);
        if (_horizDone && lane.Z != 0f)
        {
            hints.AddForbiddenZone(new SDInvertedRect(Center + lane, new WDir(1f, default), 20f, 20f, 0.7f), _comp.Activation);
        }
    }
...
sealed class P1BoundOfFaithAIStack(ModuleBase module) : ModuleComponent(module)
{
    private readonly P1BoundOfFaith? _comp = module.FindComponent<P1BoundOfFaith>();
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (_haveFetters)
        {
            ...
            hints.GoalZones.Add(AIHints.GoalSingleTarget(Center, 7.5f, 0.5f));
        }
        else
        {
            // just go to center
            hints.AddForbiddenZone(new SDInvertedRect(Center, new WDir(1f, default), 1f, 1f, 20f), DateTime.MaxValue);
        }
    }
