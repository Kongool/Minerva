...
sealed class P4CrystallizeTime(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        switch (status.ID)
        {
            ...
            case (uint)SID.Wyrmclaw:
                var duration = (status.ExpireAt - World.CurrentTime).TotalSeconds; // 40s for aero, 17s for claw
                if (duration > 25d)
    ...
    {
        if (tether.ID == (uint)TetherID.UltimateRelativitySlow && source.PosRot.Z < Center.Z)
            NorthSlowHourglass = source.Position - Center;
    }
...
sealed class P4CrystallizeTimeDragonHead(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        switch (actor.OID)
        {
            case (uint)OID.DrachenWanderer:
                Heads.Add((actor, actor.PosRot.X > Center.X ? 1 : -1));
                break;
            case (uint)OID.DragonPuddle:
                // TODO: this is very arbitrary
                var mechanic = actor.PosRot.X < Center.X
...
sealed class P4CrystallizeTimeMaelstrom(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.CrystallizeTimeMaelstrom)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        {
            AOEs.Add(new(_shape, actor.Position.Quantized(), actor.Rotation, World.FutureTime(13.2d)));
    ...
    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        ...
        if (delay != 0)
        {
            ...
            for (var i = 0; i < count; ++i)
            {
                ...
                {
                    AOEs.Ref(i).Activation = World.FutureTime(delay);
                    SortHelpers.SortAOEByActivation(AOEs);
                    return;
                }
            }
...
sealed class P4CrystallizeTimeDarkWater(ModuleBase module) : Components.UniformStackSpread(module, 6f, default, 4, 4)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints) { } // handled by other components
...
sealed class P4CrystallizeTimeDarkEruption(ModuleBase module) : Components.GenericBaitAway(module, (uint)AID.DarkEruption, centerAtTarget: true)
{
...
sealed class P4CrystallizeTimeDarkAero(ModuleBase module) : Components.GenericKnockback(module, (uint)AID.CrystallizeTimeDarkAero) // TODO: not sure whether it actually ignores immunes, if so need to warn about immunity
{
...
sealed class P4CrystallizeTimeUnholyDarkness(ModuleBase module) : Components.UniformStackSpread(module, 6f, default, 5, 5)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints) { } // handled by other components
...
sealed class P4CrystallizeTimeTidalLight : Components.Exaflare
{
    ...
    public P4CrystallizeTimeTidalLight(ModuleBase module) : base(module, new AOEShapeRect(10f, 20f))
    {
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID == (uint)AID.TidalLightAOEFirst)
        {
            ...
            StartingOffsetSum += caster.Position - Center;
        }
    }
...
sealed class P4CrystallizeTimeQuietus(ModuleBase module) : Components.CastCounter(module, (uint)AID.Quietus);

sealed class P4CrystallizeTimeHints(ModuleBase module) : ModuleComponent(module)
{
    [Flags]
    public enum Hint
    {
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (hint.offset != default)
        {
            ...
            {
                hints.AddForbiddenZone(new SDInvertedCircle(Center + hint.offset, 1f), DateTime.MaxValue);
            }
            if (hint.hint.HasFlag(Hint.SafespotPrecise))
            {
                hints.PathfindMapBounds = FRU.PathfindHugBorderBounds;
                hints.AddForbiddenZone(new SDPrecisePosition(Center + hint.offset, new(default, 1f), Bounds.MapResolution, actor.Position, 0.1f));
            }
            ...
            if (hint.hint.HasFlag(Hint.Knockback) && _ct != null)
            {
                ...
                var dest = Center + SafeOffsetDarknessStack(_ct.NorthSlowHourglass.X > 0 ? 1 : -1);
                var pos = source != null ? source.Position + 2 * (dest - source.Position).Normalized() : Center + hint.offset;
                hints.AddForbiddenZone(new SDPrecisePosition(pos, new(default, 1f), Bounds.MapResolution, actor.Position, 0.1f));
            }
            if (hint.hint.HasFlag(Hint.Mid) && _hourglass != null)
            {
                ...
                var dest = Center + new WDir(default, hint.offset.Z > 0f ? 18f : -18f);
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        ...
        if (safeOffset != default)
            Arena.ZoneCircleOutline(Center + safeOffset, 1f, Colors.Safe);
    }
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        switch (spell.Action.ID)
        {
            case (uint)AID.CrystallizeTimeDarkAero:
                KnockbacksResolve = World.FutureTime(1d); // it takes ~0.8s to resolve knockbacks
                break;
    ...
    private (WDir offset, Hint hint) HintClawAir(int clawSide, int numHourglassesDone, int northSlowSide)
    {
        ...
        if (head != null)
        {
            var headOff = head.Position - Center;
            var headDir = Angle.FromDirection(headOff) * clawSide; // always decreases as head moves
            var hint = clawSide != northSlowSide && World.CurrentTime < KnockbacksResolve ? Hint.None : Hint.SafespotPrecise; // Hint.KnockbackFrom?.. depends on how new pending knockbacks work for others
...
sealed class P4CrystallizeTimeRewind(ModuleBase module) : Components.GenericKnockback(module)
{
    ...
    private WPos SafeCorner() => _exalines != null ? Center + 0.5f * _exalines.StartingOffsetSum : default;
...
sealed class P4CrystallizeTimeSpiritTaker(ModuleBase module) : SpiritTaker(module);
