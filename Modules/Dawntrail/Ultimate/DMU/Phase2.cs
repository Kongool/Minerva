...
sealed class UltimateEmbrace(ModuleBase module) : Components.CastSharedTankbuster(module, (uint)AID.UltimateEmbrace, 5.0f);

sealed class Forsaken(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Forsaken);

sealed class LightOfJudgmentP2(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.LightOfJudgmentP2);

// Used for towers' spawn locations and marking them as SW or SE depending on the spawn point.
sealed class PathOfLight(ModuleBase module) : Components.GenericTowers(module, (uint)AID.ThePathOfLight)
{
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        if (index >= 1 && index <= 8 && state == 131073)
        {
            ...
            Towers.Add(new(Center + angle.ToDirection() * 8, 4, 2, 2, default, World.FutureTime(10d)));
        }
    }
    ...
    public override void Update()
    {
        ...
        var southDirection = (middleOfTowers - Center).Normalized();
...
sealed class ForsakenShapes(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        if (spell.Action.ID == (uint)AID.ThePathOfLight)
        {
            if (!towerSetLocked)
            {
                lastTowerSetChange = World.CurrentTime;
                towerSetLocked = true;
                currentTowerSet++;
            }
    ...
    public override void Update()
    {
        ...
        if (World.CurrentTime - lastTowerSetChange > TimeSpan.FromSeconds(1.0) && towerSetLocked)
        {
...
sealed class ForsakenBaitsSpreadStacks(ModuleBase module) : Components.UniformStackSpread(module, 5, 5, 3)
{
...
sealed class ForsakenBaitsCone(ModuleBase module) : Components.GenericBaitAway(module, (uint)AID.Spellwave)
{
    private readonly ForsakenShapes? shapes = module.FindComponent<ForsakenShapes>();
    private readonly PathOfLight? towers = module.FindComponent<PathOfLight>();
...
sealed class ForsakenBaitsBossClones(ModuleBase module) : Components.UniformStackSpread(module, 5, 5)
{
...
sealed class ForsakenSolverSet1(ModuleBase module) : ModuleComponent(module)
{
    private readonly ForsakenShapes? shapes = module.FindComponent<ForsakenShapes>();
    private readonly PathOfLight? towers = module.FindComponent<PathOfLight>();
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        ...
        var newSouth = (midpoint - Center).Normalized();
        ...
        if (shapes.swSoakers[pcSlot])
        {
            ...
            {
                var toCenter = (Center - swPosition).Normalized();
        ...
        if (shapes.seSoakers[pcSlot])
        {
            ...
            if (dmuConfig.P2Forsaken == DMUConfig.P2ForsakenStrategy.Kroxy_Rinon_Melee_Flex)
            {
                var toCenter = (Center - sePosition).Normalized();
                if (toCenter.LengthSq() <= 0)
                {
                    return;
                }
...
sealed class ForsakenSolverSet2(ModuleBase module) : ModuleComponent(module)
{
    private readonly ForsakenShapes? shapes = module.FindComponent<ForsakenShapes>();
    private readonly PathOfLight? towers = module.FindComponent<PathOfLight>();
    private readonly PartyRolesConfig partyConfig = Service.Config.Get<PartyRolesConfig>();
...
sealed class WingsOfDestructionLeftRight(ModuleBase module) : Components.SimpleAOEGroups(module,
    [(uint)AID.WingsOfDestructionLeft, (uint)AID.WingsOfDestructionRight], new AOEShapeRect(80, 20));

sealed class WingsOfDestructionTB(ModuleBase module) : Components.GenericBaitAway(module, (uint)AID.WingsOfDestructionTB, true, true)
{
...
sealed class Trine(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.Trine)
{
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        ...
        var waymarkA = World.Waymarks.GetFieldMark((int)Waymark.A);
        var waymark1 = World.Waymarks.GetFieldMark((int)Waymark.N1);
        ...
        var waymarkAAngle = (waymarkA.Value.ToWPos() - Center).ToAngle();
        var waymark1Angle = (waymark1.Value.ToWPos() - Center).ToAngle();
        ...
        {
            Arena.ZoneCircleOutline(Center + (ccwSpot - Center).Normalized() * Bounds.Radius, 1f, Colors.Safe, 2f);
        }
