...
sealed class RevoltingRuinIII(ModuleBase module) : Components.GenericBaitAway(module, (uint)AID.RevoltingRuinIII, tankbuster: true)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        if (spell.Action.ID == (uint)AID.RevoltingRuinIII)
        {
            NumCasts++;
            secondTB = true;
            activation = World.FutureTime(0.3f);
        }
...
sealed class GravenImage(ModuleBase module) : Components.GenericKnockback(module, (uint)AID.PulseWave)
{
    ...
    public override ReadOnlySpan<Knockback> ActiveKnockbacks(int slot, Actor actor)
    {
        ...
        foreach (var target in tethers)
        {
            ...
            var source = World.Actors.Find(target.SourceID);
            if (source == null)
            {
                continue;
            }
...
sealed class StackSpreadOrbs(ModuleBase module) : Components.UniformStackSpread(module, 6f, 5f, 4, 4)
{
...
sealed class BlizzardSafeSpots(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.BlizzardIIIBlowout, (uint)AID.BlizzardIIIBlowout2], new AOEShapeCone(40f, 45f.Degrees()));

sealed class WaveCannon(ModuleBase module) : Components.BaitAwayEveryone(module, module.Enemies((uint)OID.StatueBodyOrb).FirstOrDefault(), new AOEShapeRect(100f, 3f), (uint)AID.WaveCannon)
{
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
...
sealed class WaveCannonTowers(ModuleBase module) : Components.CastTowers(module, (uint)AID.TowerExplosion, 4f)
{
...
class LightningSafeSpots(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class DoubleTroubleTrapStacks(ModuleBase module) : Components.UniformStackSpread(module, 6f, 5f, 4, 4)
{
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
...
sealed class DoubleTroubleTrapKnockback(ModuleBase module) : Components.GenericKnockback(module, (uint)AID.DoubleTroubleTrap1)
{
    private readonly List<Knockback> knockbacks = [];
...
sealed class HyperDrive(ModuleBase module) : Components.GenericBaitAway(module, (uint)AID.Hyperdrive, centerAtTarget: true, tankbuster: true)
{
    private DateTime? activation;
    ...
    public override void Update()
    {
        ...
        if (activation != null)
        {
            var target = World.Actors.Find(Module.PrimaryActor.TargetID);
            if (target != null)
            {
...
sealed class Gravitas(ModuleBase module) : Components.UniformStackSpread(module, 5, 5, 4, 4)
{
        ...
        if (Stacks.Count != 0 && totalStacks == 4)
        {
            if (dmuConfig.P1GravenImage2 == DMUConfig.P1GravenImage2Strategy.GravenImage2Normal)
            {
                if (partyConfigRolesSet)
                {
                    ...
                    {
                        Arena.ZoneCircleOutline(Center + new WDir(0f, -19.5f), 1.0f, Colors.Safe, 2);
                    }
    ...
    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        ...
        {
            AddStack(target, World.FutureTime(6.5f));
            totalStacks++;
        }

        if (source.Position.AlmostEqual(new(126.000f, 41.500f), 5))
        {
            spreadsIncoming.Add(new(target, 5, World.FutureTime(10.6f)));
...
sealed class GravitasPuddles(ModuleBase module) : Components.PersistentInvertibleVoidzoneByCast(module, 5f, _ => [], (uint)AID.Gravitas)
{
...
sealed class GravitationalWave(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorEAnim(Actor actor, uint state)
    {
        ...
        {
            _aoe = [new(rect, Center.Quantized(), (actor.OID == (uint)OID.YellowOrb ? 1f : -1f) * 90.Degrees())];
        }
    }
...
sealed class TeleTrouncing(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        ...
        var duration = (status.ExpireAt - World.CurrentTime).TotalSeconds;
        if (duration > 8)
        {
...
sealed class GravenImage3(ModuleBase module) : Components.UniformStackSpread(module, 5f, 5f, 1, 1)
{
    ...
    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        ...
        else if (source.Position.AlmostEqual(new(107.000f, 43.000f), 5f))
        {
            ...
            AddSpread(target, World.FutureTime(6.5f));
        }
    }
...
sealed class Gaze(ModuleBase module) : Components.GenericGaze(module)
{
    private Eye[] _eye = [];
    ...
    public override void OnActorEAnim(Actor actor, uint state)
    {
        ...
        {
            _eye = [new Eye(actor.Position, World.FutureTime(9.9d), inverted: actor.OID == (uint)OID.StatueYellowEye)];
        }
    }
