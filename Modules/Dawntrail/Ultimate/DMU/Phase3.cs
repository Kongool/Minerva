...
sealed class AeroIIIAssault(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.AeroIIIAssault, 15f);

sealed class TheDecisiveBattle(ModuleBase module) : ModuleComponent(module)
{
...
sealed class BowelsOfAgony(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.BowelsOfAgony);

sealed class Crystals(ModuleBase module) : ModuleComponent(module)
{
...
sealed class ThunderIII(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ThunderIII, new AOEShapeCircle(15.0f));

sealed class WaterCrystal(ModuleBase module) : Components.GenericBaitProximity(module)
{
...
sealed class FireCrystal(ModuleBase module) : Components.GenericBaitProximity(module)
{
    private readonly Crystals? crystals = module.FindComponent<Crystals>();
    private readonly PartyRolesConfig partyConfig = Service.Config.Get<PartyRolesConfig>();
...
sealed class LongitudinalLatitudinalImplosion(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.Shockwave)
{
...
sealed class ThunderIIITB(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.ThunderIIITBCast, new AOEShapeCircle(5.0f), true)
{
...
sealed class UmbraSmash(ModuleBase module) : Components.GenericBaitProximity(module)
{
    private readonly Crystals? crystals = module.FindComponent<Crystals>();
    private readonly PartyRolesConfig partyConfig = Service.Config.Get<PartyRolesConfig>();
    ...
    public override void Update()
    {
        ...
        var chaosBoss = World.Actors.FirstOrDefault(a => a.OID == (uint)OID.Chaos);
        if (chaosBoss == null)
        {
            return;
        }
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        ...
        {
            Arena.ZoneCircleOutline((Center - (windCrystal.actor.Position - Center).Normalized() * 20f) + new WDir(0, 1.0f), 1.0f, Colors.Safe, 2.0f);
        }
    }
}

sealed class UltimaBlaster(ModuleBase module) : Components.RaidwideInstant(module, (uint)AID.UltimaBlaster);

sealed class UltimaBlasterLimitCut(ModuleBase module) : Components.GenericBaitAway(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        if (spell.Action.ID == (uint)AID.UltimaBlaster)
        {
            ...
            {
                angleRotate = (startPosition - Center).OrthoL().Dot(caster.Position - Center) > 0 ? -45.Degrees() : 45.Degrees();
            }
        }
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        ...
        if (orbNumber >= 0)
        {
            ...
            if (slot >= 0)
            {
                ...
                var player = World.Actors.Find(targetID);
                if (player == null)
                {
                    return;
                }

                CurrentBaits.Add(new(Center + (startPosition - Center).Rotate(angleRotate * orbNumber), player, new AOEShapeRect(100.0f, 3.0f), World.FutureTime(12.1f + 0.2f * orbNumber)));
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        ...
        {
            Arena.ZoneCircleOutline(Center + (Center - bait.Source.Position).Normalized().Rotate(angleRotate * 0.5f) * 19.0f, 0.75f, Colors.Safe);
        }
    }
}

sealed class HeadTailWind(ModuleBase module) : Components.GenericKnockback(module)
{
    ...
    public override void Update()
    {
        if (wave.EventHappened)
        {
            ...
            {
                if (player.LastFrameMovement.Length() / World.Frame.Duration > 15)
                {
                    wave = default;
                    break;
                }
...
sealed class Cyclone(ModuleBase module) : Components.GenericStackSpread(module)
{
...
sealed class KefkaMax(ModuleBase module) : ModuleComponent(module)
{
...
sealed class SlapHappy(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> aoes = [];
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        {
            aoes.Add(new(new AOEShapeCircle(13.0f), Center + spell.Rotation.ToDirection().OrthoR() * 10.0f + (spell.Rotation.ToDirection().OrthoR() * 10.0f).OrthoR()));
            aoes.Add(new(new AOEShapeCircle(13.0f), Center + spell.Rotation.ToDirection().OrthoR() * 10.0f));
            aoes.Add(new(new AOEShapeCircle(13.0f), Center + spell.Rotation.ToDirection().OrthoR() * 10.0f + (spell.Rotation.ToDirection().OrthoR() * 10.0f).OrthoL()));
            aoes.Add(new(new AOEShapeCircle(6.0f), Center));
        }

        if (spell.Action.ID == (uint)AID.SlapHappyLeftHand)
        {
            aoes.Add(new(new AOEShapeCircle(13.0f), Center + spell.Rotation.ToDirection().OrthoL() * 10.0f + (spell.Rotation.ToDirection().OrthoL() * 10.0f).OrthoL()));
            aoes.Add(new(new AOEShapeCircle(13.0f), Center + spell.Rotation.ToDirection().OrthoL() * 10.0f));
            aoes.Add(new(new AOEShapeCircle(13.0f), Center + spell.Rotation.ToDirection().OrthoL() * 10.0f + (spell.Rotation.ToDirection().OrthoL() * 10.0f).OrthoR()));
            aoes.Add(new(new AOEShapeCircle(6.0f), Center));
        }
    }
...
sealed class SlapHappyBaits(ModuleBase module) : Components.GenericBaitStack(module)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID == (uint)AID.SlapHappyLeftHand)
...
sealed class DamningEdict(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DamningEdict, new AOEShapeRect(60.0f, 40.0f));

sealed class LookUponMeAndDespairAOE(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.LookUponMeAndDespairAOE, new AOEShapeRect(100.0f, 8.0f));

sealed class WhiteHole(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.WhiteHole);

sealed class EarthquakeRaidwide(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.EarthquakeRaidwide);

sealed class BlackHoleActors(ModuleBase module) : Components.Voidzone(module, 2.0f, enemies => enemies.Enemies((uint)OID.BlackHole));

sealed class BlackHole(ModuleBase module) : Components.BaitAwayTethers(module, new AOEShapeRect(125.0f, 3.0f), (uint)TetherID.BlackHoleTether)
{
...
sealed class P3BlizzardBaits(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BlizzardIIIBaitCast, new AOEShapeCircle(6.0f))
{
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
...
sealed class P3Blizzard(ModuleBase module) : Components.GenericBaitAway(module, centerAtTarget: true, onlyShowOutlines: true)
{
    private Actor? boss = null;
    private readonly PartyRolesConfig partyConfig = Service.Config.Get<PartyRolesConfig>();
...
sealed class P3BlizzardMove(ModuleBase module) : Components.StayMove(module, 5d)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID == (uint)AID.BlizzardIIIRaidwide)
        {
            ...
            {
                PlayerStates[slot] = new(Requirement.Move, World.FutureTime(spell.RemainingTime));
            }
        }
    }
...
sealed class KnockDown(ModuleBase module) : Components.GenericStackSpread(module)
{
...
sealed class BigBang(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class StompAMole(ModuleBase module) : Components.GenericTowers(module)
{
    private Actor? boss = null;
    private readonly KnockDown? stacks = module.FindComponent<KnockDown>();
...
sealed class P3Enrage(ModuleBase module) : ModuleComponent(module)
{
    public bool enrage;
