...
sealed class UltimaRepeater(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.UltimaRepeaterCast)
{
...
sealed class FellForces(ModuleBase module) : Components.GenericBaitStack(module)
{
...
sealed class ChaoticFlood(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ChaoticFloodAOE, new AOEShapeRect(40.0f, 5.0f))
{
...
sealed class ChaoticFloodStack(ModuleBase module) : Components.GenericStackSpread(module)
{
...
sealed class MaddeningOrchestra(ModuleBase module) : Components.GenericBaitAway(module, centerAtTarget: true)
{
    ...
    public override void Update()
    {
        ...
        BitMask forbiddenPlayers = Raid.WithSlot(true, true, true).Where(p => magicVulnerability[p.Item1] > World.CurrentTime || p.Item2.Role == Role.Tank).Mask();
...
sealed class ChaoticFlareTB(ModuleBase module) : Components.GenericBaitStack(module, (uint)AID.ChaoticFlareTB)
{
    public bool active = false;
...
sealed class ChaoticHolyFlareDiffusion(ModuleBase module) : Components.GenericBaitAway(module, centerAtTarget: true, onlyShowOutlines: true)
{
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
...
sealed class Celestriad(ModuleBase module) : Components.GenericTowers(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        if (allTowers.Count == 9)
        {
            allTowers.Sort(delegate ((Actor actor, Elements element) a, (Actor actor, Elements element) b)
            {
                ...
                var xAngle = (a.actor.Position - Center).ToAngle();
                var yAngle = (b.actor.Position - Center).ToAngle();
        ...
        if (actor.OID is ((uint)OID.IceTower) or ((uint)OID.FireTower) or ((uint)OID.ThunderTower))
        {
            if (state == (uint)Animations.TowerGlow)
            {
                ...
                if (Towers.Count >= 4)
                {
                    ...
                    {
                        var north = Angle.AnglesCardinals[2];
                        var xAngle = (a.Position - Center).ToAngle();
                        var yAngle = (b.Position - Center).ToAngle();
...
sealed class CatastrophicChoice(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> aoes = [];
...
sealed class StrayApocalypse(ModuleBase module) : Components.Exaflare(module, 6f)
{
...
sealed class StrayEntropy(ModuleBase module) : Components.UniformStackSpread(module, 0f, 5.0f)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
...
sealed class P5ForsakenRaidWide(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.ForsakenCast);

// TODO update to use MapEffects maybe?
sealed class P5ForsakenGround(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ForsakenGround, new AOEShapeCircle(8.0f))
{
    private readonly List<AOEInstance> aoes = [];
...
sealed class P5ForsakenBait(ModuleBase module) : Components.GenericBaitProximity(module)
{
...
sealed class P5ForsakenStack(ModuleBase module) : Components.StackTogether(module, (uint)IconID.StackShare, 5.0f, 6.0f);
