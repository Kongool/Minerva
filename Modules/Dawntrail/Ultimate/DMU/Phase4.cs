...
sealed class GrandCrossRaidwide(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.GrandCross);

// GrandCross will be casted three times in a row with a fake or real
sealed class GrandCrossOrder(ModuleBase module) : ModuleComponent(module)
{
            ...
            ((uint)SID.AccelerationBomb) or ((uint)SID.CursedShriek))
        {
            ...
            if (slot >= 0)
            {
                ...
                foreach (var entry in grandCross)
                {
                    ...
                    Service.Log.Debug("Slot:" + slot + "Removed status effect: " + status.ID + " at world time: " + status.ExpireAt);
                }
            }
        }
    }
...
sealed class TsunamiRaidwide(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.P4Tsunami);
sealed class InfernoRaidwide(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.P4Inferno);

// Elements will be casted two times in a row with a fake or real
sealed class TsunamiInfernoOrder(ModuleBase module) : ModuleComponent(module)
{
...
sealed class KefkaOrder(ModuleBase module) : ModuleComponent(module)
{
...
sealed class EdgeOfDeath(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.EdgeOfDeath, new AOEShapeRect(47.0f, 1.0f));
...
sealed class AntiLight(ModuleBase module) : ModuleComponent(module)
{
...
sealed class ForkedWater(ModuleBase module) : Components.UniformStackSpread(module, 8.0f, 8.0f, 3, 3)
{
    private readonly GrandCrossOrder? grandCrossOrder = module.FindComponent<GrandCrossOrder>();
    // NumCasts (inherited): 4 casts always go off, so counting them says whether the aoes are live
...
sealed class AccelerationBomb(ModuleBase module) : Components.StayMove(module)
{
    private readonly GrandCrossOrder? grandCrossOrder = module.FindComponent<GrandCrossOrder>();
    ...
    public override void OnStatusLose(Actor actor, ref ActorStatus status)
    {
        if (status.ID == (uint)SID.AccelerationBomb)
        {
            ...
            if (slot >= 0)
            {
                ...
                Service.Log.Debug($"Slot {slot} resetting");
                Service.Log.Debug($"NumCasts {NumCasts}");
            }
        }
    }
    ...
    public override void Update()
    {
        ...
        {
            if ((expireAt - World.CurrentTime).TotalSeconds > 7d)
            {
                continue;
            }
            ...
            if (tellingTruth == true)
            {
                ...
                Service.Log.Debug($"Slot {slot} real bomb - stay still, which expires at {expireAt}"); // todo debug - remove later
            }

            if (tellingTruth == false)
            {
                PlayerStates[slot] = new(Requirement.Move, expireAt);
                Service.Log.Debug($"Slot {slot} fake bomb - move, which expires at {expireAt}"); // todo debug - remove later
            }
        }
    }
}
...
sealed class CursedShriek(ModuleBase module) : Components.GenericGaze(module)
{
    private readonly GrandCrossOrder? grandCrossOrder = module.FindComponent<GrandCrossOrder>();
    ...
    public override void Update()
    {
        ...
        {
            if ((players[i].expireAt - World.CurrentTime).TotalSeconds > 8d)
            {
                continue;
            }
...
sealed class InfernoBaits(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class Inferno(ModuleBase module) : Components.GenericBaitProximity(module, onlyShowOutlines: true)
{
...
sealed class TsunamiBaits(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> aoes = [];
...
sealed class Tsunami(ModuleBase module) : Components.GenericBaitProximity(module, onlyShowOutlines: true)
{
    private readonly TsunamiInfernoOrder? tsunamiInfernoOrder = module.FindComponent<TsunamiInfernoOrder>();
    public bool active = false;
...
sealed class UltimaUpsurge(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.UltimaUpsurge);

// Customize version of P1 lightning safe spots function that uses the truth and lies stored throughout the phase instead
sealed class P4LightningSafeSpots(ModuleBase module) : LightningSafeSpots(module)
{
