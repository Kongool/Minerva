...
sealed class PiercingPlunge(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.PiercingPlunge);
sealed class SurgingCurrent(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SurgingCurrent, new AOEShapeCone(60f, 90f.Degrees()));
sealed class AquaBall(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.AquaBall, (uint)AID.AquaBall1], 5f);
...
//sealed class BigWave(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.BigWave, 36f, kind: Kind.DirForward);
sealed class BigWave(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.BigWave, 36f, kind: Kind.DirForward)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        hints.AddForbiddenZone(new SDKnockbackInAABBRectFixedDirectionPlusAOECircles(Center, dir, 20f, 20f, aquaballs, 5f, len), _activation);
    }
...
sealed class AlluringOrder(ModuleBase module) : Components.StatusDrivenForcedMarch(module, 3f, (uint)SID.ForwardMarch, 0, 0, 0);
sealed class SirenSphere(ModuleBase module) : Components.Voidzone(module, 2f, GetSirenSpheres, 3f)
{
    public static Actor[] GetSirenSpheres(ModuleBase module)
    {
...
sealed class CeaselessCurrent(ModuleBase module) : Components.Exaflare(module, new AOEShapeRect(4f, 20f, 4f))
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
...
sealed class AquaSpear(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class Hydrofall(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor) => CollectionsMarshal.AsSpan(_aoes);
...
sealed class EchoedSerenade(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class SunkenTreasure(ModuleBase module) : Components.GenericAOEs(module)
{
    // spawns 4 orbs
    ...
    public override void OnActorEAnim(Actor actor, uint state)
    {
        ...
        {
            _aoes.Add(new(new AOEShapeCircle(18f), actor.Position, activation: World.CurrentTime.AddSeconds(7.7d)));
        }
    }
    public override void OnCastFinished(Actor caster, ActorCastInfo spell)
    {
...
sealed class NearFarTide(ModuleBase module) : Components.GenericAOEs(module)
{
...
[ModuleInfo(CFCID = 1066u, NameID = 14291u, PrimaryActorOID = (uint)OID.DaryaTheSeaMaid, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "gynorhino (ported from BMR)")]
public class V13DaryaTheSeamaid(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(375, 530), new ArenaBoundsSquare(20f));
