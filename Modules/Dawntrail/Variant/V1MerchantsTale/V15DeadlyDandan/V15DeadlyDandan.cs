...
sealed class MurkyWaters(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.MurkyWaters);
sealed class Spit(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Spit, new AOEShapeCone(50f, 60f.Degrees())); // could draw early during 2nd devour 
sealed class Dropsea(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.Dropsea, 5f);
sealed class MawOfTheDeep(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MawOfTheDeep, 8f, 5);
sealed class TidalGuillotine(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TidalGuillotine, 20f);
sealed class SwallowedSea(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SwallowedSea, new AOEShapeCone(50f, 60f.Degrees())); // could draw earlier by checking action timeline 0x11D1
sealed class UnfathomableHorror(ModuleBase module) : Components.ConcentricAOEs(module, _shapes)
{
...
sealed class AiryBubbles(ModuleBase module) : Components.Voidzone(module, 5f, GetBubbles, 4f)
{
    public static Actor[] GetBubbles(ModuleBase module)
    {
...
sealed class Devour(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID == (uint)AID.DevourCast)
        {
            ...
            var midDist = Intersect.RayCircle(startPos, dir, Center, 30f);
            ...
            var endDist = Intersect.RayCircle(midPos, endDir, Center, 30f);
...
sealed class StingingTentacle(ModuleBase module) : Components.GenericAOEs(module)
{
...
[ModuleInfo(CFCID = 1066u, NameID = 14475u, PrimaryActorOID = (uint)OID.DeadlyDandan, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "gynorhino (ported from BMR)")]
public class V15DeadlyDandan(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(805f, 670f), new ArenaBoundsCircle(20f));
