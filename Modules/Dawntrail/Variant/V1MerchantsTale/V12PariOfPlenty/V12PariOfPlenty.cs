...
sealed class BurningBleam(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.BurningGleam1, (uint)AID.BurningGleam2], new AOEShapeCross(40f, 5f));
sealed class SpurningFlames(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.SpurningFlames);
sealed class ImpassionedSparks(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ImpassionedSparks, 8f, 4);
sealed class ScouringScorn(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.ScouringScorn);
sealed class PredatorySwoop(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PredatorySwoop, 12f);
sealed class TranscendentFlight(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TranscendentFlight, 12f);
sealed class CarpetRide(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnUntethered(Actor source, in ActorTetherInfo tether)
    {
        if (tether.ID == (uint)TetherID.Fireflight)
        {
            ...
            var initial = len == 0 ? Center : _nextLanding;
            _nextLanding = target.Position;

            if (_firstActivation == default)
            {
                _firstActivation = World.FutureTime(11.25d);
            }
...
sealed class LeftRightFireflight(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.LeftFireflightTwoNights, (uint)AID.LeftFireflightThreeNights, (uint)AID.RightFireflightTwoNights, (uint)AID.RightFireflightThreeNights], new AOEShapeRect(40f, 2f));
sealed class WheelOfFireflight(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        if (iconID is (uint)IconID.TurningRight or (uint)IconID.TurningLeft or (uint)IconID.TurningRRight or (uint)IconID.TurningRLeft)
        {
            ...
            _aoes.Add(new(new AOEShapeCone(40f, 90f.Degrees()), Center, _currentRot));
            _prevIcon = iconID;
        }
    }
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (_aoes.Count > 0)
        {
            hints.AddForbiddenZone(new AOEShapeDonut(3f, 60f), Center);
        }
    }
}

sealed class GaleForce(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class StrongWind(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class ThievesWeaves(ModuleBase module) : Components.GenericAOEs(module)
{
...
[ModuleInfo(CFCID = 1066u, NameID = 14274u, PrimaryActorOID = (uint)OID.PariOfPlenty, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "gynorhino (ported from BMR)")]
public class V12PariOfPlenty(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-760, -805), new ArenaBoundsSquare(20f));
