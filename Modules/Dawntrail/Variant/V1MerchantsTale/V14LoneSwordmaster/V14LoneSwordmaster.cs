...
sealed class LashOfLight(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.LashOfLight, new AOEShapeCone(40f, 45f.Degrees()), 2);
sealed class EarthRendingEight(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.EarthRendingEight, 8f);
// could display early since crosses happen at circle location but could sacrifice precious AI uptime
sealed class EarthRendingEightCross(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.EarthRendingEightCross, new AOEShapeCross(40f, 4f));
sealed class WaitingWounds(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WaitingWounds, 10f, 6);
sealed class SteelsbreathRelease(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.SteelsbreathRelease);
sealed class SteelsbreathRelease1(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.SteelsbreathRelease1);
sealed class MawOfTheWolf(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MawOfTheWolf, new AOEShapeRect(80f, 40f));
sealed class StingOfTheScorpion(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.StingOfTheScorpion);
sealed class PlummetSmall(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PlummetSmall, 3f);
sealed class Plummet(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Plummet, 5f, 4);
sealed class PlummetBig(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PlummetBig, 10f, 4);
sealed class PlummetProximity(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PlummetProximity, 25f);
sealed class Concentrativity(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Concentrativity);
sealed class ConcentrativityKnockback(ModuleBase module) : Components.GenericKnockback(module, (uint)AID.Concentrativity)
{
...
sealed class ConcentravityMagnetFloor(ModuleBase module) : Components.GenericAOEs(module, warningText: "Move to safe square!")
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID == (uint)AID.Concentrativity)
        {
            ...
            _aoes.Add(new(_shape, Center + new WDir(-10f, isNegative ? 10f : -10f)));
            _aoes.Add(new(_shape, Center + new WDir(10f, isNegative ? -10f : 10f)));
        }
    }
    public override void OnCastFinished(Actor caster, ActorCastInfo spell)
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        hints.AddForbiddenZone(new AOEShapeRect(16f, 16f, 16f, invertForbiddenZone: true), Center);
    }
}
sealed class ConcentrativityRocks(ModuleBase module) : Components.GenericKnockback(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID is (uint)AID.Repel or (uint)AID.Attract)
        {
            if (spell.TargetID == Raid.Player()!.InstanceID)
            {
                ...
                Casters.Add(new(Center, 17f, _finishAt.AddSeconds(4d)));
            }
        }
    }
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (kbs[0].Kind == Kind.AwayFromOrigin)
            hints.AddForbiddenZone(new SDKnockbackInAABBSquareAwayFromOriginIntoCircle(Center, kbs[0].Origin, 20f, 20f, Center, 2.5f), _finishAt);
        else
            hints.AddForbiddenZone(new SDKnockbackTowardsOriginIntoCircle(kbs[0].Origin, 20f, Center, 2.5f), _finishAt);
    }
...
sealed class HeavensConfluenceIcon(ModuleBase module) : Components.BaitAwayIcon(module, 5f, (uint)IconID.HeavensConfluence)
{
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
...
sealed class HeavensConfluence(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class WillOfTheUnderworld(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WillOfTheUnderworld, new AOEShapeRect(40f, 10f))
{
...
sealed class WillOfTheUnderworldRocks(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.WillOfTheUnderworldRock, "Move behind safe rock!")
{
...
sealed class CrusherOfLions(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.CrusherOfLions, new AOEShapeCone(40f, 45f.Degrees()))
{
    // safe spot depends on player Malefic debuff
    private readonly DebuffTracker _debuffs = module.FindComponent<DebuffTracker>()!;
...
sealed class UnyieldingWill(ModuleBase module) : Components.GenericBaitAway(module)
{
    // safe spot depends on player Malefic debuff
...
[ModuleInfo(CFCID = 1066u, NameID = 14323u, PrimaryActorOID = (uint)OID.LoneSwordmaster, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "gynorhino (ported from BMR)")]
public class V14LoneSwordmaster(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(170f, -815f), new ArenaBoundsSquare(20f));
