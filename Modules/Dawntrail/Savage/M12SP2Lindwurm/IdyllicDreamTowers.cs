...
sealed class IdyllicDreamArena(ModuleBase module) : Components.GenericAOEs(module, warningText: "Go to correct plattform!")
{
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        switch (state)
        {
            ...
            case 0x02000040u:
                _aoe = [];
                Bounds = new ArenaBoundsCustom(splitArena);
                State = 1;
                break;
            case 0x01000001u:
                Bounds = M12S2TheLindwurm.BuildArena().arena;
                State = 0;
                break;
        }
    }
    ...
    public void Predict(double seconds)
    {
        ...
        _aoe = [new(shape, center, default, World.FutureTime(seconds), shapeDistance: shape.Distance(center, default))];
    }
}

// Shared state for Idyllic Dream to persist tower assignments across component lifecycles
sealed class IdyllicDreamSharedState : ModuleComponent
{
    ...
    public IdyllicDreamSharedState(ModuleBase module) : base(module)
    {
...
sealed class ArcadianArcanum(ModuleBase module) : Components.UniformStackSpread(module, 0, 6)
{
    public int NumCasts;
...
sealed class IdyllicDreamElementalMeteor(ModuleBase module) : Components.GenericTowers(module)
{
    ...
    public void CreateTowers()
    {
        ...
        var activation = World.FutureTime(8.4d);
    ...
    public override void AddHints(int slot, Actor actor, TextHints hints)
    {
        ...
        if (assignedTower >= 0 && assignedTower < Towers.Count && DrawIcons)
        {
            ...
            if (!inAssignedTower && tower.Activation > World.CurrentTime)
            {
...
sealed class IdyllicDreamWindTower(ModuleBase module) : Components.GenericKnockback(module)
{
...
sealed class IdyllicDreamLindwurmsDarkII(ModuleBase module) : Components.GenericBaitAway(module)
{
    ...
    public override void Update()
    {
        ...
        if (_sources.Count > 0 && World.CurrentTime >= _sources[0].Activation)
            return;
...
sealed class IdyllicDreamDoom(ModuleBase module) : ModuleComponent(module)
{
...
sealed class IdyllicDreamHotBlooded : Components.StayMove
{
    ...
    public IdyllicDreamHotBlooded(ModuleBase module) : base(module)
    {
...
sealed class LindwurmsStoneIII(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.LindwurmsStoneIII, 4);

sealed class LindwurmsPortent(ModuleBase module) : Components.GenericBaitAway(module)
{
    struct Portent
    {
        public Actor Source;
