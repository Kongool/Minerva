...
sealed class SnakingKick(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SnakingKick, new AOEShapeCone(40, 90.Degrees()));

sealed class WingedScourge(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WingedScourge, new AOEShapeCone(50, 15.Degrees()));

sealed class MightyMagicTopTierSlamFirstBait(ModuleBase module)
...
class Replication1SecondBait(ModuleBase module) : ModuleComponent(module)
{
        ...
        for (var i = 0; i < clones.Length; ++i)
        {
            if (clones[i].Actor == actor)
            {
                ...
                for (var e = 0; e < evs.Length; ++e)
                {
                    ...
                    if (ev.ID == 0x11D3)
                    {
                        var child = World.Actors.Find(ev.InstanceID);
                        if (child != null)
...
class WingedScourgeSecond(ModuleBase module) : Components.GenericAOEs(module)
{
        ...
        for (var i = 0; i < clones.Length; ++i)
        {
            if (clones[i] == actor)
            {
                ...
                for (var e = 0; e < evs.Length; ++e)
                {
                    ...
                    if (ev.ID == 0x11D3)
                    {
                        var clone = World.Actors.Find(ev.InstanceID);
                        if (clone != null)
                            _clones.Add(clone);
                    }
                }
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        for (var i = 0; i < clones.Length; ++i)
        {
            ...
            {
                _draw = true;
                _sources.Add((spell.TargetXZ, World.FutureTime(7.2f)));
                return;
            }
        }
    }
}
class MightyMagicTopTierSlamSecondBait(ModuleBase module)
    : Components.UniformStackSpread(module, 5, 5, minStackSize: 2, maxStackSize: 2)
{
            ...
            case AID.Dash:
                {
                    ...
                    for (var i = 0; i < clones.Length; ++i)
                    {
                        ...
                        if (clone.Actor == caster && clone.Element != Replication1SecondBait.Assignment.None)
                        {
                            ...
                            _sources.Add(new(spell.TargetXZ, isStack, World.FutureTime(10))); // FIXME timing
                            break;
                        }
                    }
                    break;
...
sealed class DoubleSobatBuster(ModuleBase module) : Components.BaitAwayIcon(module, new AOEShapeCone(40, 90.Degrees()), (uint)IconID.DoubleSobat)
{
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        switch ((AID)spell.Action.ID)
...
sealed class DoubleSobatRepeat(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DoubleSobatRepeat, new AOEShapeCone(40, 90.Degrees()));

sealed class EsotericFinisher : Components.GenericBaitAway
{
    static readonly AOEShapeCircle _shape = new(10);

    public EsotericFinisher(ModuleBase module)
        : base(module, (uint)AID.EsotericFinisher, centerAtTarget: true)
    {
        ...
        var activation = World.FutureTime(10);
...
sealed class Replication1Guidance : ModuleComponent
{
    ...
    public Replication1Guidance(ModuleBase module) : base(module)
    {
        _bait = module.FindComponent<Replication1SecondBait>()!;
        _assignments = _prc.AssignmentsPerSlot(Module.World.Party);
