...
sealed class ArenaChanges(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        if (index == 0x01)
        {
            ...
            {
                case 0x00200010u:
                    _aoe = [new(circle, Center, default, World.FutureTime(5.6d))];
                    break;
                case 0x00020001u:
                    _aoe = [];
                    Bounds = M08SHowlingBlade.DonutArena;
                    break;
                case 0x00080004u:
                    Bounds = M08SHowlingBlade.StartingArena;
                    break;
            }
        }
        if (index is >= 0x10 and <= 0x13)
        {
            ...
            else if (state is 0x04000004u or 0x00200004u && Bounds != M08SHowlingBlade.StartingArena)
            {
                polygons.Clear();
                Bounds = M08SHowlingBlade.StartingArena;
                Center = M08SHowlingBlade.ArenaCenter;
            }
        }
...
sealed class Teleporters(ModuleBase module) : ModuleComponent(module)
{
    private readonly List<WPos> activeTeleporters = [];
    private static readonly WPos[] teleporters = CalculateTeleporterPositions();
    private readonly List<Pathfinding.Teleporter> activeTeleporterPairs = [];
