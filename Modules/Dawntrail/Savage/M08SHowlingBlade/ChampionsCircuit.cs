...
sealed class GleamingBarrage(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GleamingBarrage, GleamingBeam.Rect);

sealed class ChampionsCircuit(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnCastFinished(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID is (uint)AID.ChampionsCircuitDonutFirst or (uint)AID.ChampionsCircuitDonutRest)
        {
            if (++NumCasts < 5)
            {
                ...
                var activation = World.FutureTime(4.4d);
