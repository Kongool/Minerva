...
sealed class CruelCoil(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        {
            _active = false;
            Bounds = M12SLindwurm.DefaultBounds;
        }
    }
    ...
    {
        if (_active)
            _skinsplitterDonut.Draw(Arena, Center);
    }
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        ...
        if (betaExpiry != default && (betaExpiry - World.CurrentTime).TotalSeconds < 2.5f)
        {
            var isInside = _centerTower.Check(pc.Position, Center);
            var color = isInside ? Colors.Danger : Colors.Safe;
            _centerTower.Outline(Arena, Center, default, color);
        }
        // when player has Unbreakable Flesh and coil is active, draw tower at opposite end
        else if (hasUnbreakable && _active)
        {
            var towerPos = Center + _oppositeTowerOffset * _openingDir.ToDirection();
    ...
    {
        if (_active && _skinsplitterDonut.Check(actor.Position, Center))
        ...
        if (betaExpiry != default && (betaExpiry - World.CurrentTime).TotalSeconds < 2.5f)
        {
            if (!_centerTower.Check(actor.Position, Center))
    ...
    {
        if (_active)
            hints.AddForbiddenZone(_skinsplitterDonut, Center);
    }
    ...
    private void UpdateArena(int openingIndex = 0)
    {
        ...
        Bounds = new ArenaBoundsCustom(
            [new Rectangle(Center, 20f, 15f)],
            [new DonutSegmentV(Center, 9.5f, 13.5f, _openingDir, openingHalfAngle, 128)]);
    }
}
