...
sealed class StageCombo(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        if (first != null && last != null)
        {
            ...
            _aoes.Add(new(first, Center, 180f.Degrees(), firstActivation));
            AddAOEs(firstRot, Angle.AnglesCardinals, firstActivation);
            _aoes.Add(new(_shapeCross, Center, 180f.Degrees(), Module.CastFinishAt(spell, 4.2d)));
            _aoes.Add(new(last, Center, 180f.Degrees(), lastActivation));
            ...
            void AddAOEs(Angle firstRot, Angle[] angles, DateTime activation)
            {
                for (var i = 0; i < 4; ++i)
                {
                    _aoes.Add(new(_shapeCone, Center, firstRot + angles[i], activation));
                }
            }
        }
    }
