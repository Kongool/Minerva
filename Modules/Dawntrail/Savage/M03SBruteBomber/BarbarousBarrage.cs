...
sealed class BarbarousBarrageTowers(ModuleBase module) : Components.GenericTowers(module)
{
    ...
    private void SetState(State state, int soakers, double activation)
    {
        if (CurState != state)
        {
            ...
            var count = positions.Count;
            for (var i = 0; i < count; ++i)
                Towers.Add(new(positions[i], 4f, soakers, soakers, default, World.FutureTime(activation)));
        }
    }
...
sealed class BarbarousBarrageMurderousMist(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BarbarousBarrageMurderousMist, new AOEShapeCone(40f, 135f.Degrees()));

sealed class BarbarousBarrageLariatCombo(ModuleBase module) : Components.GenericAOEs(module)
{
