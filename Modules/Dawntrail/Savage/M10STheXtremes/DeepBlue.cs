...
sealed class SickestTakeOff(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SickestTakeOff, new AOEShapeRect(50f, 7.5f));
sealed class SickSwell1(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.SickSwell1, 10f, kind: Kind.DirForward);

sealed class AwesomeSplashSlab(ModuleBase module) : Components.GenericStackSpread(module, false)
{
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        if (status.ID == (uint)SID._Gen_)
        {
            ...
            var act = World.FutureTime(15.8d);
            switch (status.Extra)
            {
...
sealed class AlleyOopWater(ModuleBase module) : Components.GenericBaitAway(module, alwaysDrawOtherBaits: true)
{
    private readonly DebuffTracker _debuff = module.FindComponent<DebuffTracker>()!;
    ...
    private void DrawPlayerPosition(int pcSlot, Actor pc)
    {
        ...
        else if (CurrentState == State.SplitArena)
        {
            ...
            var isWest = (position - Center).ToAngle().AlmostEqual(west, 0.1f);
        ...
        else
        {
            ...
            var targetPos = Center + targetRot.ToDirection() * (group % 2 == 0 ? 18f : 25.5f);
            var targetSd = new SDCircle(targetPos, radius);
            Arena.ZoneCircleOutline(targetPos, radius, targetSd.Contains(pc.Position) ? Colors.Safe : Colors.Danger);
        }
    }
...
sealed class AlleyOopWaterAfter(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        {
            _aoes.Add(new(new AOEShapeCone(60f, 15f.Degrees()), caster.Position, spell.Rotation, World.FutureTime(_delay)));
        }

        // reverse does casts in both directions
        if (spell.Action.ID == (uint)AID.ReverseAlleyOop1)
        {
            _aoes.Add(new(new AOEShapeCone(60f, 7.5f.Degrees()), caster.Position, spell.Rotation + _offset, World.FutureTime(_delay)));
            _aoes.Add(new(new AOEShapeCone(60f, 7.5f.Degrees()), caster.Position, spell.Rotation - _offset, World.FutureTime(_delay)));
        }
...
sealed class DeepImpact(ModuleBase module) : Components.GenericBaitProximity(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID == (uint)AID.DeepImpactCast)
        {
            ...
            Bait bait = new(caster, new AOEShapeCircle(6f), activation, 1, false, centerAtTarget: true, tankbuster: true, caster: (uint)OID.DeepBlue, forbidden: _debuff.FirePlayers);
...
sealed class DeepImpactKnockback(ModuleBase module) : Components.GenericKnockback(module)
{
...
sealed class DiversDareBlue(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.DiversDareBlue);
...
//sealed class DeepVarial(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DeepVarial, new AOEShapeCone(60f, 60f.Degrees()));
sealed class DeepVarial(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DeepVarial, new AOEShapeCone(60f, 60f.Degrees()))
{
    private readonly M10STheXtremesConfig _config = Service.Config.Get<M10STheXtremesConfig>();
    AOEInstance _aoe;
    ...
    public override void OnActorPlayActionTimelineEvent(Actor actor, ushort id)
    {
        if (actor.OID == (uint)OID.SickSwell && id == 0x2487)
        {
            ...
            var origin = Center - rotation.ToDirection() * 20f;
            var activation = World.FutureTime(14.2d);
...
sealed class AwesomeSplashSlabAerial(ModuleBase module) : Components.GenericStackSpread(module)
{
        ...
        for (var i = 0; i < len; i++)
        {
            ...
            switch (state)
            {
                case 0x00800040:
                    ...
                    {
                        Stacks.Add(new(player, 6f, 4, 4, World.FutureTime(4.7d), _debuff.FirePlayers));
                    }
                    break;
                case 0x08000400:
                    Spreads.Add(new(player, 5f, World.FutureTime(4.7d)));
                    break;
            }
            /*
            if (state == 0x00800040)
