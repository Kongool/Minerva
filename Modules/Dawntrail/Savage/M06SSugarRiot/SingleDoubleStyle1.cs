...
sealed class SingleDoubleStyle1(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        ...
        void AddAOE(AOEShape shape, WPos position, double time, Angle rotation = default) => AOEs.Add(new(shape, position.Quantized(), rotation, World.FutureTime(time)));
    }
    ...
    private void HandleSweetShot(Actor source, double time)
    {
        ...
        AOEs.Add(new(new AOEShapeRect(dirVector.Length(), 3.5f), sourceP.Quantized(), Angle.FromDirection(dirVector), World.FutureTime(time)));
    }
