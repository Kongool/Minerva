...
sealed class CometTethers(ModuleBase module) : Components.TankbusterTether(module, (uint)AID.ForegoneFatality, (uint)TetherID.CometTether, 0f);
sealed class CosmicKiss(ModuleBase module) : Components.SpreadFromIcon(module, (uint)IconID.CosmicKissIcon, (uint)AID.CosmicKiss, 4f, 5d);
sealed class Tether1(ModuleBase module) : Components.StretchTetherSingle(module, (uint)TetherID.ShortTether, 40f, new AOEShapeRect(60f, 5f));
sealed class Tether2(ModuleBase module) : Components.StretchTetherSingle(module, (uint)TetherID.LongTether, 40f, new AOEShapeRect(60f, 5f));
sealed class FireBreath(ModuleBase module) : Components.BaitAwayIcon(module, new AOEShapeRect(60f, 3f), (uint)IconID.CosmicKissIcon, (uint)AID.FireBreath1);
sealed class MajesticMeteor : Components.SimpleAOEs
{
    public MajesticMeteor(ModuleBase module) : base(module, (uint)AID.MajesticMeteorBaits, 6f, 24)
    {
        MaxDangerColor = 8;
    }
}

sealed class MeteorainComets(ModuleBase module) : ModuleComponent(module)
{
...
sealed class TripleTyrannhilation(ModuleBase module)
    : Components.CastLineOfSightAOE(module, (uint)AID.TripleTyrannhilation2, 60f)
{
    ...
    private void RebuildSafezone()
    {
        ...
        Modify(_origin, _blockerData, World.FutureTime(2));

        Safezones.Clear();
        AddSafezone(World.FutureTime(2));
    }
}

sealed class CosmicKissTowers(ModuleBase module) : Components.CastTowers(module, (uint)AID.CosmicKissTower, 4f, 1, 1)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID == WatchedAction)
...
sealed class WeightyImpactTowers(ModuleBase module) : Components.CastTowers(module, (uint)AID.WeightyImpactTower, 4f, 2, 2)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID == WatchedAction)
...
sealed class HeartBreakerTower(ModuleBase module) : Components.GenericTowers(module)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
...
sealed class ExplosionTowers(ModuleBase module) : Components.CastTowers(module, (uint)AID.ExplosionTower, radius: 4f, minSoakers: 2, maxSoakers: 2);
sealed class ExplosionTowerKnockback(ModuleBase module) : Components.GenericKnockback(module)
{
...
sealed class MeteorainPortals(ModuleBase module) : Components.GenericAOEs(module)
{
...
internal class TyrantFireballLines : Components.GenericBaitStack
{
    ...
    public TyrantFireballLines(ModuleBase module, uint aid, int numTargets)
        : base(module, aid)
    {
...
sealed class TwoWayFireball(ModuleBase module)
    : TyrantFireballLines(module, (uint)AID.TwoWayFireballStart, 2);

sealed class FourWayFireball(ModuleBase module)
    : TyrantFireballLines(module, (uint)AID.FourWayFireballStart, 4);

sealed class MeteorMechanicHints(ModuleBase module) : ModuleComponent(module)
{
...
sealed class ExplosionTowerHints(ModuleBase module) : ModuleComponent(module)
{
    private readonly ExplosionTowers? _towers = module.FindComponent<ExplosionTowers>();
    ...
    public override void DrawArenaForeground(int slot, Actor pc)
    {
        ...
        var member = Module.World.Party.Members[slot];
