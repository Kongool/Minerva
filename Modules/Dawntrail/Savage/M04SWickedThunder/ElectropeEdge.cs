...
sealed class ElectropeEdgeWitchgleam(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.ElectropeEdgeWitchgleamAOE)
{
...
sealed class ElectropeEdgeSpark1(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ElectropeEdgeSpark1, new AOEShapeRect(10f, 5f));
sealed class ElectropeEdgeSpark2(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ElectropeEdgeSpark2, new AOEShapeRect(30f, 15f));

sealed class ElectropeEdgeSidewiseSpark(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.ElectropeEdgeSidewiseSparkR, (uint)AID.ElectropeEdgeSidewiseSparkL], new AOEShapeCone(60f, 90f.Degrees()));

sealed class ElectropeEdgeStar(ModuleBase module) : Components.UniformStackSpread(module, 6f, 6f)
{
...
sealed class LightningCage(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.LightningCageAOE)
{
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        if (status.ID == (uint)SID.ElectricalCondenser && Raid.FindSlot(actor.InstanceID) is var slot && slot >= 0)
            Order[slot] = (status.ExpireAt - World.CurrentTime).TotalSeconds < 30d ? 1 : 2;
    }
...
sealed class LightningCageWitchgleam(ModuleBase module) : Components.GenericBaitAway(module, (uint)AID.LightningCageWitchgleamAOE)
{
