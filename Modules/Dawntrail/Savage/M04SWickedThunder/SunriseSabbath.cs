...
sealed class SunriseSabbath(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        if (status.ID is (uint)SID.Positron or (uint)SID.Negatron && Raid.FindSlot(actor.InstanceID) is var slot && slot >= 0)
        {
            ...
            BaitOrder[slot] = (status.ExpireAt - World.CurrentTime).TotalSeconds < 30d ? 1 : 2;
        }
    }
}

sealed class SunriseSabbathSoaringSoulpress(ModuleBase module) : Components.GenericTowers(module)
{
...
sealed class SunriseSabbathElectronStream(ModuleBase module) : Components.GenericBaitAway(module)
{
