...
sealed class StampedingThunder(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        switch (spell.Action.ID)
        {
            case (uint)AID.IonClusterVisualR:
                AOE = [new(_shape, caster.Position - new WDir(5f, default), caster.Rotation, World.FutureTime(2.4d))];
                break;
            ...
            case (uint)AID.StampedingThunderFinish:
                ++NumCasts;
                AOE = [];
                Bounds = M04SWickedThunder.IonClusterBounds;
                Center = new(M04SWickedThunder.P1DefaultCenter.X + 3f * (M04SWickedThunder.P1DefaultCenter.X - caster.PosRot.X), M04SWickedThunder.P1DefaultCenter.Z);
                SmallArena = true;
                break;
        }
    }
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        {
            Bounds = M04SWickedThunder.P1DefaultBounds;
            Center = M04SWickedThunder.P1DefaultCenter;
            SmallArena = false;
        }
    }
}

sealed class ElectronStream(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class ElectronStreamCurrent(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.AxeCurrent)
{
