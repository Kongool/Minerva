...
sealed class TwoThreeFourSnapTwistDropTheNeedle(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class FlipToABSide(ModuleBase module) : Components.GenericBaitStack(module)
{
    ...
    {
        if (!active && Source != null && (activation - World.CurrentTime).TotalSeconds < 5.1d)
        {
    ...
    public void SetActivation(double delay) => activation = World.FutureTime(delay);
