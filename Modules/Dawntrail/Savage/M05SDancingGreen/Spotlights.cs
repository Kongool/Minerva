...
class Spotlights1(ModuleBase module) : Components.GenericTowers(module)
{
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        var activation2 = World.FutureTime(33.6d);
        ...
        var activation1 = World.FutureTime(22d);
        for (var i = 0; i < 4; ++i)
        {
...
sealed class Spotlights2(ModuleBase module) : Spotlights1(module)
{
    ...
    public override void OnActorPlayActionTimelineEvent(Actor actor, ushort id)
    {
        if (spotlightSet == null && actor.OID == (uint)OID.Spotlight && id == 0x11DC)
        {
            ...
            if (spotlightSet is bool set)
            {
                ...
                var activation2 = World.FutureTime(19.5d);
                for (var i = 0; i < 4; ++i)
                {
                    AddTower(spotlights2[i], forbiddenSecond, activation2);
                }
                var activation1 = World.FutureTime(9.5d);
                for (var i = 0; i < 4; ++i)
                {
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        if (status.ID == (uint)SID.BurnBabyBurn && Raid.FindSlot(actor.InstanceID) is var slot && slot >= 0)
        {
            _orders[slot] = (status.ExpireAt - World.CurrentTime).TotalSeconds < 15d ? 1 : 2;
        }
    }
}
