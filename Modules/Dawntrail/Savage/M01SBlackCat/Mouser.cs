...
sealed class Mouser(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID is (uint)AID.MouserTelegraphFirst or (uint)AID.MouserTelegraphSecond)
            _aoes.Add(new(rect, spell.LocXZ, spell.Rotation, World.FutureTime(9.7d)));
...
sealed class GrimalkinGaleShockwave(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.GrimalkinGaleShockwaveAOE, 21f, true, stopAfterWall: true);
sealed class GrimalkinGaleSpread(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.GrimalkinGaleSpreadAOE, 5f);

sealed class SplinteringNails(ModuleBase module) : Components.CastCounter(module, (uint)AID.SplinteringNailsAOE)
{
