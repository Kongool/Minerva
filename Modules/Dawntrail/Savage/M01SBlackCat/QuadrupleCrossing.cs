...
sealed class QuadrupleCrossingProtean(ModuleBase module) : Components.GenericBaitAway(module)
{
    public WPos? Origin;
    ...
    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        ...
        else if (_clone == source)
        {
            ...
            _activation = World.FutureTime(17d);
        }
    }
}

sealed class QuadrupleCrossingAOE(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        switch (spell.Action.ID)
        {
            ...
            case (uint)AID.LeapingQuadrupleCrossingShadeProtean:
                _aoes.Add(new(_shape, caster.Position.Quantized(), caster.Rotation, World.FutureTime(5.9d), _aoes.Count < 4 ? Colors.Danger : default, risky: false));
                break;
