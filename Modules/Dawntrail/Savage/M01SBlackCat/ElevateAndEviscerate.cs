...
sealed class ElevateAndEviscerateShockwave(ModuleBase module) : Components.GenericAOEs(module, default, "GTFO from shockwave!")
{
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        ...
        if (_kb.CurrentTarget != null && _kb.CurrentTarget != actor)
        {
            ...
            else if (_kb.CurrentKnockbackDistance == 10f && InBounds(_kb.Cache))
            {
    ...
    {
        if (spell.Action.ID is (uint)AID.ElevateAndEviscerateHitAOE or (uint)AID.ElevateAndEviscerateKnockbackAOE && InBounds(_kb.Cache))
        {
            _aoe = [new(Rect, ArenaChanges.CellCenter(ArenaChanges.CellIndex(_kb.Cache)), default, World.FutureTime(_kb.CurrentKnockbackDistance == default ? 1.4d : 3.6d))];
        }
...
sealed class ElevateAndEviscerate(ModuleBase module) : Components.GenericKnockback(module, stopAfterWall: true)
{
    ...
    public override ReadOnlySpan<Knockback> ActiveKnockbacks(int slot, Actor actor)
    {
        if (CurrentTarget != null && actor == CurrentTarget && CurrentKnockbackDistance > 0)
            return new Knockback[1] { new(Center, CurrentKnockbackDistance, CurrentDeadline, direction: actor.Rotation, kind: Kind.DirForward, ignoreImmunes: true) };
        return [];
    }
    ...
    public override void AddHints(int slot, Actor actor, TextHints hints)
    {
        ...
        if (CurrentTarget == actor)
            hints.Add($"{(CurrentKnockbackDistance > 0 ? "Knockback" : "Hit")} in {Math.Max(default, (CurrentDeadline - World.CurrentTime).TotalSeconds):f1}s", false);
    }
...
sealed class ElevateAndEviscerateHint(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly ElevateAndEviscerate _kb = module.FindComponent<ElevateAndEviscerate>()!;
