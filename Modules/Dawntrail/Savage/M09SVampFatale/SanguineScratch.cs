...
sealed class BreakdownWing(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        ...
        if (status.ID == (uint)SID._Gen_2056)
        {
            ...
            if (status.Extra is 0x426 or 0x427)
            {
                for (var i = 0; i < _vamps.Count(); i++)
                {
                    ...
                    {
                        _aoes.Add(new(status.Extra == 0x426 ? new AOEShapeCircle(7f) : new AOEShapeDonut(4f, 15f), NumCasts == 0 ? _vamps[i].EndPos : _vamps[i].StartPos, activation: World.FutureTime(5.1d)));
                    }
                }
            }
        }
        ...
        if (status.ID == (uint)SID.VampetteDistance)
        {
            ...
            var end = Center - (actor.Position - Center);
...
sealed class SanguineScratch(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
