...
namespace Minerva.Shadowbringers.Hunt.RankS.Aglaope;
...
sealed class SongOfTorment(ModuleBase module) : Components.CastInterruptHint(module, (uint)AID.SongOfTorment, hintExtra: "Raidwide + Bleed");

sealed class SeductiveSonata(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class DeathlyVerse(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DeathlyVerse, 6f);
sealed class Tornado(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Tornado, 6f);
sealed class FourfoldSuffering(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FourfoldSuffering, new AOEShapeDonut(5f, 50f));
sealed class AncientAero(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AncientAero, new AOEShapeRect(42.4f, 3f));
sealed class AncientAeroIII(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.AncientAeroIII);
sealed class AncientAeroIIIKB(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.AncientAeroIII, 10f, shape: new AOEShapeCircle(30f));

sealed class AglaopeStates : StateMachineBuilder
{
    public AglaopeStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.Hunt, CFCID = 0u, NameID = 8653u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
