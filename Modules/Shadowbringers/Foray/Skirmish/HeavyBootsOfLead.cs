...
namespace Minerva.Shadowbringers.Foray.Skirmish.HeavyBootsOfLead;
...
sealed class MagitekLaser(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MagitekLaser, new AOEShapeRect(75f, 2f));
sealed class Pulverize(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Pulverize, 15f);
sealed class Ventilate(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Ventilate, 8f);
sealed class Compress(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Compress, new AOEShapeCross(50f, 3.5f));
sealed class Accelerate(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.Accelerate, 6f, 8);
sealed class Dispose(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Dispose, new AOEShapeCone(40f, 30f.Degrees()));

sealed class Shatter(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Shatter, 6f);
sealed class GarleanFire(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GarleanFire, 5f);

sealed class HeavyBootsOfLeadStates : StateMachineBuilder
{
    public HeavyBootsOfLeadStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.ForayFATE, GroupID = 735u, CFCID = 735u, NameID = 1612u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
