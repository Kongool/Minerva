...
namespace Minerva.Shadowbringers.Foray.Skirmish.ConflictingWithTheFirstLaw;
...
sealed class CannonShot(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.CannonShot, 12f);
sealed class EmbalmingEarth(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.EmbalmingEarth, 10f);
sealed class AccursedPox(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AccursedPox, 8f);
sealed class Darkness(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Darkness, 5f);
...
{
    public ConflictingWithTheFirstLawStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.ForayFATE, GroupID = 735u, CFCID = 735u, NameID = 1605u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
