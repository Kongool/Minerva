...
namespace Minerva.Shadowbringers.Foray.Skirmish.PyromancerSupreme;
...
sealed class Firestarter(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Firestarter, new AOEShapeDonut(6f, 40f));
sealed class GarleanFireSnowsOfBozja(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.GarleanFire, (uint)AID.SnowsOfBozja], 5f);
sealed class ThermalShock(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ThermalShock, 20f);

sealed class PyromancerSupremeStates : StateMachineBuilder
{
    public PyromancerSupremeStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.ForayFATE, GroupID = 735u, CFCID = 735u, NameID = 1616u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
