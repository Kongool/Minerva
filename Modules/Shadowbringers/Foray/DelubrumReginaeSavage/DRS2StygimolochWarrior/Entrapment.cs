...
sealed class EntrapmentAttract(ModuleBase module) : Components.GenericKnockback(module, (uint)AID.EntrapmentAttract)
{
    private DateTime _activation;

    public override ReadOnlySpan<Knockback> ActiveKnockbacks(int slot, Actor actor)
    {
        return new Knockback[1] { new(new(Center.X, Center.Z + Bounds.Radius), 60f, _activation, kind: Kind.TowardsOrigin, ignoreImmunes: true) };
    }
...
class Entrapment : Components.CastCounter
{
    ...
    public Entrapment(ModuleBase module, Pattern[] allowedPatterns) : base(module, (uint)AID.MassiveExplosion)
    {
    ...
    public override void Update()
    {
        ...
        if (player != null)
            for (var z = 0; z < 7; ++z)
                for (var x = 0; x < 7; ++x)
                    if (player.Position.InCircle(Center + CellOffset(x, z), 10f))
    ...
    {
        var index = IndexFromOffset(position - Center);
    ...
    private void DrawTraps(BitMask mask, bool safe, bool background)
    {
        ...
        {
            var pos = Center + CellOffset(index);
            if (background)
...
sealed class EntrapmentNormal(ModuleBase module) : Entrapment(module, _allowedPatterns)
{
...
sealed class EntrapmentInescapable(ModuleBase module) : Entrapment(module, _allowedPatterns)
{
    private static readonly Pattern[] _allowedPatterns = [
...
sealed class LethalBlow(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.LethalBlow, new AOEShapeRect(44f, 24f));
sealed class LeapingSpark(ModuleBase module) : Components.CastCounter(module, (uint)AID.LeapingSparkAOE);
sealed class Devour(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Devour, new AOEShapeCone(6f, 60f.Degrees()));
