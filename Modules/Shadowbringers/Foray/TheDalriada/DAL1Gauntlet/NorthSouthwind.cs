...
sealed class NorthSouthwind(ModuleBase module) : Components.GenericKnockback(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (_kb.Length != 0)
        {
            ...
            if (!IsImmune(slot, act))
            {
                ...
                if (count != 0)
                {
                    hints.AddForbiddenZone(new SDKnockbackInAABBSquareFixedDirectionPlusMixedAOEs(Center, dir, 22f, [.. _aoe2.AOEs], count), act);
                }
                else
                {
    ...
    public override bool DestinationUnsafe(int slot, Actor actor, WPos pos)
    {
        ...
        return aoes2.Count != 0 && aoes2.Ref(0).Check(pos) || !InBounds(pos);
    }
}
