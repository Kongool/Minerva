...
sealed class InfraredBlast(ModuleBase module) : Components.InterceptTether(module, (uint)AID.InfraredBlast, (uint)TetherID.InfraredBlast)
{
    ...
    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        ...
        if (tether.ID == (uint)TetherID.InfraredBlast)
        {
            if (players.Count == 0)
            {
                foreach (var a in World.Actors.Actors.Values)
                {
    ...
    public override void AddHints(int slot, Actor actor, TextHints hints)
    {
        ...
        if (!TetheredPlayers[slot])
        {
            ...
            for (var i = 0; i < count; ++i)
            {
                if (Tethers[i].Player.OID != default)
                {
                    untakenTethers = true;
                    break;
                }
