...
sealed class ArenaChange(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        if (spell.Action.ID == (uint)AID.HolyIV && Bounds != CLL3Adrammelech.DefaultArena)
        {
            _aoe = [new(donut, Center, default, Module.CastFinishAt(spell, 1.2d))];
        }
    }
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        {
            Bounds = CLL3Adrammelech.DefaultArena;
            Center = CLL3Adrammelech.DefaultArena.Center;
        }
    }
}
