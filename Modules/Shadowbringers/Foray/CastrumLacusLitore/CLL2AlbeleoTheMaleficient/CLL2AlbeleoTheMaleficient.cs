...
namespace Minerva.Shadowbringers.Foray.CastrumLacusLitore.CLL2AlbeleoTheMaleficient;
...
sealed class DiffractiveLaser(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DiffractiveLaser, 5f);
sealed class MagitekCannon(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MagitekCannon, 6f);
sealed class MagitekRay(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MagitekRay, new AOEShapeRect(40f, 3f));
sealed class GrandSword(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GrandSword, new AOEShapeCone(27f, 60f.Degrees()));
sealed class BalefulGaze(ModuleBase module) : Components.CastGaze(module, (uint)AID.BalefulGaze, range: 35f);
sealed class AbyssalCry(ModuleBase module) : Components.CastInterruptHint(module, (uint)AID.AbyssalCry);

sealed class CLL2AlbeleoTheMaleficentStates : StateMachineBuilder
{
    public CLL2AlbeleoTheMaleficentStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CastrumLacusLitore, GroupID = 735u, CFCID = 735u, NameID = 9433u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class CLL2AlbeleoTheMaleficent(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
