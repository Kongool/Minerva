...
namespace Minerva.Shadowbringers.Foray.CriticalEngagement.CE63WornToShadow;
...
sealed class Stormcall(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.Explosion)
{
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        ...
        {
            _aoes.Add(new(circle, Center + 30f * (actor.Position - Center).Normalized(), default, World.FutureTime(status.Extra == 0x1E ? 9.7f : 19.9f), actorID: actor.InstanceID));
            if (_aoes.Count == 3)
...
sealed class BladedBeak(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.BladedBeak);
sealed class NihilitysSong(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.NihilitysSong);
sealed class Fantod(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FantodAOE, 3f);

sealed class Foreshadowing(ModuleBase module) : Components.GenericAOEs(module)
{
...
{
    public CE63WornToShadowStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, CFCID = 778u, NameID = 28u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")] // bnpcname=9973
public sealed class CE63WornToShadow(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
