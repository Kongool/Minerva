...
namespace Minerva.Shadowbringers.Foray.CriticalEngagement.CE44FamiliarFace;
...
class TectonicEruption(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TectonicEruption, 6f);
class RockCutter(ModuleBase module) : Components.SingleTargetDelayableCast(module, (uint)AID.RockCutter);
class AncientQuake(ModuleBase module) : Components.RaidwideCastDelay(module, (uint)AID.AncientQuake, (uint)AID.AncientQuakeAOE, 0.8f);
class Roxxor(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.Roxxor, 6f);

class ControlTowerAppear(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
    private static readonly AOEShapeCircle circle = new(6f);
    private readonly List<Polygon> activeTowers = [];
    ...
    public override void OnCastFinished(Actor caster, ActorCastInfo spell)
    {
        ...
        if (id == (uint)AID.ControlTowerAppear)
        {
            ...
            Bounds = new ArenaBoundsCustom(CE44FamiliarFace.ArenaPolygon, [.. activeTowers]);
        }
        else if (id == (uint)AID.Towerfall)
        {
            ...
            for (var i = 0; i < count; ++i)
            {
                if (activeTowers[i].Center == pos)
                {
                    ...
                    {
                        Bounds = CE44FamiliarFace.DefaultArena;
                    }
                    return;
                }
            }
...
class Towerfall(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Towerfall, new AOEShapeRect(40f, 5f));

class ExtremeEdge(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.ExtremeEdgeL, (uint)AID.ExtremeEdgeR], new AOEShapeRect(60f, 18f));

class IntractableLand(ModuleBase module) : Components.Exaflare(module, 8f)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
...
class Hammerfall(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        {
            _aoes.Add(new(circle, actor.Position.Quantized(), default, World.FutureTime(12.6d)));
        }
    }
...
{
    public CE44FamiliarFaceStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, CFCID = 778u, NameID = 29u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")] // bnpcname=9693
public class CE44FamiliarFace(WorldState ws, Actor primary) : ModuleBase(ws, primary, DefaultArena.Center, DefaultArena)
{
