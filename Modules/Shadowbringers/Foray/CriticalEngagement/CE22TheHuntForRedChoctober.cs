...
namespace Minerva.Shadowbringers.Foray.CriticalEngagement.CE22TheHuntForRedChoctober;
...
sealed class ChocoMeteorStrike(ModuleBase module) : Components.Exaflare(module, 8f)
{
...
sealed class ChocoBeak1(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ChocoBeak1, new AOEShapeRect(40f, 10f));
sealed class ChocoBeak2(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ChocoBeak2, new AOEShapeRect(40f, 8f));
sealed class ChocoSlash(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.ChocoSlash);
sealed class ChocoMeteoruption(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ChocoMeteoruption, 6f);
sealed class ChocoComet(ModuleBase module) : Components.CastTowersOpenWorld(module, (uint)AID.ChocoComet, 5f);
sealed class ChocoMeteorain(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ChocoMeteorain, 21f, 2);
sealed class ChocoMeteorImpact(ModuleBase module) : Components.StackWithIcon(module, (uint)IconID.ChocoMeteorImpact, (uint)AID.ChocoMeteorImpact, 5f, 5.1f, 8);

sealed class ChocoMeteorStream(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
    private static readonly AOEShapeCircle circle = new(8f);
    private readonly List<WPos> initialPositions = [];
...
{
    public CE22TheHuntForRedChoctoberStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, GroupID = 735u, CFCID = 735u, NameID = 7u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class CE22TheHuntForRedChoctober(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
