...
namespace Minerva.Shadowbringers.Foray.CriticalEngagement.CE62LooksToDieFor;
...
sealed class Thundercall(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Thundercall, "Raidwide + summon lighting orbs");

sealed class LightningBoltDistantClap(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class TwistingWinds(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TwistingWinds, new AOEShapeRect(80f, 5f));

sealed class CloudToGround(ModuleBase module) : Components.Exaflare(module, 5f)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
...
sealed class Flame(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Flame, "Raidwide + summon flame orbs");

sealed class Burn(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
    ...
    public override void OnActorModelStateChange(Actor actor, byte modelState, byte animState1, byte animState2)
    {
        if (actor.OID == (uint)OID.BallOfFire && animState1 == 1)
            _aoes.Add(new(_shape, actor.Position.Quantized(), default, World.FutureTime(5d)));
    }
...
sealed class Lash(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.Forelash, (uint)AID.Backlash], new AOEShapeCone(40f, 90f.Degrees()));
sealed class Charybdis(ModuleBase module) : Components.CastHint(module, (uint)AID.Charybdis, "Set hp to 1");
sealed class Roar(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Roar);
sealed class Levinbolt(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.LevinboltAOE, 6f);
sealed class SerpentsEdge(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.SerpentsEdge);

sealed class CE62LooksToDieForStates : StateMachineBuilder
{
    public CE62LooksToDieForStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, CFCID = 778u, NameID = 30u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")] // bnpcname=9925
public sealed class CE62LooksToDieFor(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-200f, -580f), new ArenaBoundsCircle(20f))
{
