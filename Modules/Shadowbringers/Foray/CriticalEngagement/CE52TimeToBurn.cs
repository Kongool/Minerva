...
namespace Minerva.Shadowbringers.Foray.CriticalEngagement.CE52TimeToBurn;
...
sealed class TimeEruptionBombReproduce(ModuleBase module) : Components.GenericAOEs(module)
{
        ...
        switch (spell.Action.ID)
        {
            case (uint)AID.TimeBomb:
                if (caster == Module.PrimaryActor)
                    _bombsActivation = World.FutureTime(23.2d);
                break;
            ...
            case (uint)AID.StartTime:
                _eruptionStart = World.CurrentTime;
                break;
        }
    }
...
sealed class FireTankbuster(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.FireTankbuster);
sealed class FireRaidwide(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.FireRaidwide);

sealed class CE52TimeToBurnStates : StateMachineBuilder
{
    public CE52TimeToBurnStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 778u, NameID = 26u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")] // bnpcname=9930
public sealed class CE52TimeToBurn(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-550f, default), new ArenaBoundsSquare(29.5f), onlyLoadIfTargetable: true)
{
