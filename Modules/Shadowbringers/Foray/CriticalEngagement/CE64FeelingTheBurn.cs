...
namespace Minerva.Shadowbringers.Foray.CriticalEngagement.CE64FeelingTheBurn;
...
sealed class DiveFormation(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DiveFormation, new AOEShapeRect(60f, 3f));

sealed class AntiPersonnelMissile(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.BallisticImpact)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        ...
        if (iconID == (uint)IconID.BallisticImpact)
            _aoes.Add(new(_shape, actor.Position, default, World.FutureTime(11d)));
    }
...
sealed class ChainCannonEscort(ModuleBase module) : Components.GenericAOEs(module)
{
            ...
            else
            {
                ...
                if (countP == 0)
                {
                    foreach (var a in Module.World.Actors.Actors.Values)
                        if (a.OID == default && a.Position.InSquare(Center, 24f))
                            participants.Add(a);
                }
...
sealed class ChainCannonBoss(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        if (spell.Action.ID == (uint)AID.ChainCannonBossAOE)
        {
            ...
            {
                _aoe = [new(ChainCannonEscort.Rect, caster.Position.Quantized(), caster.Rotation, World.FutureTime(1d))];
            }
        }
    }
}

sealed class SurfaceMissile(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SurfaceMissileAOE, 6f);
sealed class SuppressiveMagitekRays(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.SuppressiveMagitekRays);
sealed class Analysis(ModuleBase module) : Components.CastHint(module, (uint)AID.Analysis, "Face open weakpoint to charging adds");
sealed class PreciseStrike(ModuleBase module) : Components.CastWeakpoint(module, (uint)AID.PreciseStrike, new AOEShapeRect(60f, 3f), (uint)SID.FrontUnseen, (uint)SID.BackUnseen, default, default);

sealed class CE64FeelingTheBurnStates : StateMachineBuilder
{
    public CE64FeelingTheBurnStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, CFCID = 778u, NameID = 18u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")] // bnpcname=9945
public sealed class CE64FeelingTheBurn(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-240f, -230f), new ArenaBoundsSquare(24f))
{
