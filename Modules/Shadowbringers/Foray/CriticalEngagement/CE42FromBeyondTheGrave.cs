...
namespace Minerva.Shadowbringers.Foray.CriticalEngagement.CE42FromBeyondTheGrave;
...
class ArenaChange(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        if (spell.Action.ID == (uint)AID.GallowsMarch && Bounds.Radius > 25f)
        {
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        {
            Bounds = new ArenaBoundsCircle(25f); // default arena got no extra collision, just a donut aoe
...
class DevourSoul(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.DevourSoul);
class Blight(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Blight);

class GallowsMarch(ModuleBase module) : Components.StatusDrivenForcedMarch(module, 3f, (uint)SID.ForwardMarch, (uint)SID.AboutFace, (uint)SID.LeftFace, (uint)SID.RightFace)
{
...
class ShockSphere(ModuleBase module) : Components.Voidzone(module, 7f, GetSpheres)
{
    private static List<Actor> GetSpheres(ModuleBase module) => module.Enemies((uint)OID.ShockSphere);
}

class SoulPurge(ModuleBase module) : Components.GenericAOEs(module)
{
...
class CrimsonBlade(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.CrimsonBlade, new AOEShapeCone(50f, 90f.Degrees()));
class BloodCyclone(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BloodCyclone, 5f);
class Aethertide(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.AethertideAOE, 8f);
class MarchingBreath(ModuleBase module) : Components.CastInterruptHint(module, (uint)AID.MarchingBreath, showNameInHint: true); // heals all allies by 20% of max health (raidwide)
class TacticalAero(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TacticalAero, new AOEShapeRect(40f, 4f));
class EntropicFlame(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.EntropicFlame, new AOEShapeRect(60f, 4f));
class DarkFlare(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DarkFlare, 8f);
class SoulSacrifice(ModuleBase module) : Components.CastInterruptHint(module, (uint)AID.SoulSacrifice, showNameInHint: true); // WarWraith sacrifices itself to give boss a damage buff

class PurifyingLight : Components.SimpleAOEs
{
    public PurifyingLight(ModuleBase module) : base(module, (uint)AID.PurifyingLight, 12f)
    {
...
{
    public CE42FromBeyondTheGraveStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, CFCID = 778u, NameID = 20u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")] // bnpcname=9931
public class CE42FromBeyondTheGrave(WorldState ws, Actor primary) : ModuleBase(ws, primary, startingArena.Center, startingArena)
{
