...
namespace Minerva.Shadowbringers.Foray.CriticalEngagement.CE31MetalFoxChaos;
...
sealed class MagitekBitLasers(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        ...
        var index = 0;
        var time = World.CurrentTime > _times[0];
...
sealed class Rush(ModuleBase module) : Components.BaitAwayChargeCast(module, (uint)AID.Rush, 7f);
sealed class LaserShower(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.LaserShower, 10f);
sealed class DiffractiveLaser(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DiffractiveLaser, new AOEShapeCone(60f, 75f.Degrees()));
sealed class SatelliteLaser(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.SatelliteLaser, "Raidwide + all lasers fire at the same time");

sealed class CE31MetalFoxChaosStates : StateMachineBuilder
{
    public CE31MetalFoxChaosStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, CFCID = 735u, NameID = 13u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")] // bnpcname=9424
public sealed class CE31MetalFoxChaos(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-234f, 262f), new ArenaBoundsSquare(29.5f))
{
    protected override bool CheckPull() => base.CheckPull() && Raid.Player()!.Position.InSquare(Center, 30f);
}
