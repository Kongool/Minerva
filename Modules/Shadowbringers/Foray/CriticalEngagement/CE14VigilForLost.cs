...
namespace Minerva.Shadowbringers.Foray.CriticalEngagement.CE14VigilForLost;
...
sealed class LightLeapExplosiveFlare(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.LightLeap, (uint)AID.ExplosiveFlare], 10f);
sealed class ChemicalMissile : Components.SimpleAOEs
{
    public ChemicalMissile(ModuleBase module) : base(module, (uint)AID.ChemicalMissile, 12f)
    {
        MaxDangerColor = 2;
    }
}
sealed class TailMissile(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TailMissileAOE, 30f);
sealed class Shockwave(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Shockwave, 16f);
sealed class CripplingBlow(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.CripplingBlow);
sealed class PlasmaField(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.PlasmaField);
sealed class Towers(ModuleBase module) : Components.CastTowersOpenWorld(module, (uint)AID.Explosion, 6f);
sealed class MagitekRay(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MagitekRay, new AOEShapeRect(50f, 2f));

sealed class CE14VigilForLostStates : StateMachineBuilder
{
    public CE14VigilForLostStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, CFCID = 735u, NameID = 3u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")] // bnpcname=9396
public sealed class CE14VigilForLost(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
