...
namespace Minerva.Shadowbringers.Foray.CriticalEngagement.CE23FiresOfWar;
...
sealed class ScaldingScolding(ModuleBase module) : Components.Cleave(module, (uint)AID.ScaldingScolding, new AOEShapeCone(8f, 60f.Degrees()));

sealed class CantTouchThis(ModuleBase module) : Components.GenericStackSpread(module)
{
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        if (status.ID == (uint)SID.CantTouchThis)
            Stacks.Add(new(actor, 6f, 2, activation: World.FutureTime(16d)));
    }
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (countP == 0)
        {
            foreach (var a in Module.World.Actors.Actors.Values)
                if (a.OID == default && a.Position.InCircle(Center, 20f))
                    participants.Add(a);
        }
        var count = Stacks.Count;
...
sealed class Pyroplexy(ModuleBase module) : Components.CastTowersOpenWorld(module, (uint)AID.Pyroplexy, 4f);
sealed class CatchingFire(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.CatchingFire);

sealed class TooHotToHandle(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class ExplosiveCountdown(ModuleBase module) : Components.ConcentricAOEs(module, _shapes)
{
    ...
    public override void OnCastFinished(Actor caster, ActorCastInfo spell)
    {
        if (Sequences.Count != 0)
        {
            ...
            AdvanceSequence(order, spell.LocXZ, World.FutureTime(4d));
        }
    }
...
sealed class HotTemper(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HotTemper, 24f);
sealed class SlowDeflagration(ModuleBase module) : Components.CastHint(module, (uint)AID.SlowDeflagration, "Enrage!", true);
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, GroupID = 735u, CFCID = 735u, NameID = 9u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class CE23FiresOfWar(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
