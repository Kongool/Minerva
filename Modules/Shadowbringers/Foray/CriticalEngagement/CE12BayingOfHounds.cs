...
namespace Minerva.Shadowbringers.Foray.CriticalEngagement.CE12BayingOfHounds;
...
sealed class Hellclaw(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.Hellclaw);
sealed class TailBlow(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TailBlow, new AOEShapeCone(19f, 45f.Degrees()));
sealed class LavaSpit(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.LavaSpitAOE, 5f);
sealed class ScorchingLash(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ScorchingLash, new AOEShapeRect(50f, 5f));

sealed class Hellpounce(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnCastFinished(Actor caster, ActorCastInfo spell)
    {
        switch (spell.Action.ID)
        {
            case (uint)AID.Hellpounce:
                ...
                Activate(loc, center - offset, World.FutureTime(3.7d));
                break;
...
sealed class DragonsLionsBreath(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.DragonsBreathAOER, (uint)AID.DragonsBreathAOEL, (uint)AID.LionsBreathAOE], new AOEShapeCone(60f, 30f.Degrees()));

sealed class VoidTornado(ModuleBase module) : Components.CastHint(module, (uint)AID.VoidTornado, "Set hp to 1");

sealed class VoidQuake(ModuleBase module) : Components.GenericAOEs(module) // this concentric AOE can happen forwards or backwards in order with the same AID as the starter
{
...
{
    public CE12BayingOfHoundsStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, CFCID = 735u, NameID = 2u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")] // bnpcname=9394
public sealed class CE12BayingOfHounds(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
