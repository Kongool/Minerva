...
namespace Minerva.Shadowbringers.Foray.CriticalEngagement.CE54NeverCryWolf;
...
sealed class IcePillar(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.IcePillarAOE, 4f);
sealed class PillarPierce(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PillarPierce, new AOEShapeRect(80f, 2f));
sealed class Shatter(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Shatter, 8f);

sealed class BracingWind(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.BracingWind, 40f, false, 1, new AOEShapeRect(60f, 6f), Kind.DirForward)
{
...
sealed class LunarCry(ModuleBase module) : Components.CastLineOfSightAOE(module, (uint)AID.LunarCry, 80f)
{
...
sealed class ThermalGust(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        if (actor.OID == (uint)OID.Imaginifer)
            _aoes.Add(new(_shape, actor.Position.Quantized(), actor.Rotation, World.FutureTime(6.5d)));
...
sealed class AgeOfEndlessFrost(ModuleBase module) : Components.GenericRotatingAOE(module)
{
    ...
    public override void OnCastFinished(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID is (uint)AID.AgeOfEndlessFrostFirstAOE or (uint)AID.AgeOfEndlessFrostRestAOE)
            AdvanceSequence(spell.LocXZ, spell.Rotation, World.CurrentTime);
    }
}

sealed class StormWithout(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.StormWithout, new AOEShapeDonut(10f, 40f));
sealed class StormWithin(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.StormWithin, 10f);
sealed class AncientGlacier(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AncientGlacierAOE, 6f);
sealed class Glaciation(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Glaciation);

sealed class CE54NeverCryWolfStates : StateMachineBuilder
{
    public CE54NeverCryWolfStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, CFCID = 778u, NameID = 25u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")] // bnpcname=9941
public sealed class CE54NeverCryWolf(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-830f, 190f), new ArenaBoundsSquare(24f))
{
    ...
    protected override bool CheckPull() => base.CheckPull() && Raid.Player()!.Position.InSquare(Center, 24f);
}
