...
namespace Minerva.Shadowbringers.Foray.CriticalEngagement.CE41WithDiremiteAndMain;
...
class CrystallineFracture(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.CrystallineFracture, 4f);
class ResonantFrequencyDim(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ResonantFrequencyDim, 6f);
class ResonantFrequencyCorrupted(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ResonantFrequencyCorrupted, 6f);

class CrystallineStingers(ModuleBase module) : Components.CastLineOfSightAOE(module, (uint)AID.CrystallineStingers, 60f)
{
...
class AetherialStingers(ModuleBase module) : Components.CastLineOfSightAOE(module, (uint)AID.AetherialStingers, 60f)
{
    public override ReadOnlySpan<Actor> BlockerActors()
    {
...
class Subduction(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Subduction, 8f);

class Earthbreaker(ModuleBase module) : Components.ConcentricAOEs(module, _shapes)
{
...
class CrystalNeedle(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.CrystalNeedle);
class Shardstrike(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.ShardstrikeAOE, 5f)
{
...
class Hailfire(ModuleBase module) : Components.GenericBaitAway(module)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        ...
        {
            CurrentBaits.Add(new(Module.PrimaryActor, actor, rect, World.FutureTime(8.2d + (iconID - (uint)IconID.Hailfire1) * 2.1d)));
...
{
    public CE41WithDiremiteAndMainStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, CFCID = 778u, NameID = 21u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")] // bnpcname=9969
public class CE41WithDiremiteAndMain(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
