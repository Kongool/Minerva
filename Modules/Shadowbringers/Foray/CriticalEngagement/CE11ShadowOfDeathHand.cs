...
namespace Minerva.Shadowbringers.Foray.CriticalEngagement.CE11ShadowOfDeathHand;
...
sealed class BestialLoyalty(ModuleBase module) : Components.CastHint(module, (uint)AID.BestialLoyalty, "Summon crows");
sealed class RunWild(ModuleBase module) : Components.CastInterruptHint(module, (uint)AID.RunWild, showNameInHint: true);
sealed class HardBeak(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.HardBeak);
sealed class Helldive(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.Helldive, 6f, 8);
...
sealed class BlindsideBarrage(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.BlindsideBarrage, "Raidwide + deathwall appears");
sealed class RollingBarrage(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.RollingBarrageAOE, 8f);
sealed class Whirlwind(ModuleBase module) : Components.Voidzone(module, 4f, GetWhirlwind, 3f)
{
    private static List<Actor> GetWhirlwind(ModuleBase module) => module.Enemies((uint)OID.Whirlwind);
}

sealed class Wind(ModuleBase module) : Components.GenericKnockback(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (_kb.Length != 0)
        {
            ...
            if (!IsImmune(slot, act))
            {
                ...
                hints.AddForbiddenZone(new SDCone(Center - 10f * dir.ToDirection(), 30f, dir, 135f.Degrees()), act);
            }
        }
    }
}

sealed class PiercingBarrage(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.PiercingBarrageBoss, (uint)AID.PiercingBarrageCrow], new AOEShapeRect(40f, 4f));

sealed class CE11ShadowOfDeathHandStates : StateMachineBuilder
{
    public CE11ShadowOfDeathHandStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, CFCID = 735u, NameID = 5u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")] // bnpcname=9400
public sealed class CE11ShadowOfDeathHand(WorldState ws, Actor primary) : ModuleBase(ws, primary, startingArena.Center, startingArena)
{
