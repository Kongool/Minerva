...
namespace Minerva.Shadowbringers.Foray.CriticalEngagement.CE24PatriotGames;
...
sealed class PlasmaGun(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.PlasmaGun);
sealed class Neutralization(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Neutralization, new AOEShapeCone(30f, 60f.Degrees()));
sealed class OrderedFire(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.OrderedFire, 10f);
sealed class SearingConduction(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SearingConduction, 15f);
sealed class ElectrifyingConduction(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.ElectrifyingConduction);

sealed class LightningRod(ModuleBase module) : Components.GenericTowers(module)
{
...
sealed class ElectrochemicalReaction(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
    private static readonly AOEShapeRect rect = new(50f, 12.5f);
    private readonly List<(WPos position, int charges)> bartizans = [];
        ...
        skip:
            ...
            for (var i = 0; i < len; ++i)
            {
                ...
                if (laser.position.AlmostEqual(pos, 1f))
                {
                    ...
                    if (laser.charges == 2)
                    {
                        _aoes.Add(new(rect, laser.position, source.Rotation, World.FutureTime(10.2d)));
...
{
    public CE24PatriotGamesStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CriticalEngagement, GroupID = 735u, CFCID = 735u, NameID = 10u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class CE24PatriotGames(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-239.999f, 413.999f), new ArenaBoundsSquare(24.5f))
{
    protected override bool CheckPull() => base.CheckPull() && Raid.Player()!.Position.InSquare(Center, 25f);
}
