...
sealed class LighterNoteBait(ModuleBase module) : ModuleComponent(module)
{
    private readonly List<(Actor caster, bool isNorthSouth)> casters = [];
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (targets[slot])
        {
            hints.AddForbiddenZone(new SDCircle(Center, 24.5f), activation);
        }
    }
...
sealed class LighterNoteExaflare(ModuleBase module) : Components.Exaflare(module, 6f)
{
    private readonly List<(Actor caster, bool isNorthSouth)> casters = [];
    private DateTime activation;
