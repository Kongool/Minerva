...
sealed class UpperLaser(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class LowerLaser(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorEAnim(Actor actor, uint state)
    {
        if (state == 0x00100020u && actor.OID == (uint)OID.ArenaFeatures)
        {
            var center = Center;
            // the lasers are placed poorly for some reason and can be off by 1-2°, so we need to round to nearest 60°
            AOEs.Add(new(cone, center.Quantized(), Angle.FromDirection(actor.Position - center).Round(60f), World.FutureTime(9.7d)));
        }
    }
