...
namespace Minerva.Shadowbringers.Alliance.A20HeavyArtilleryUnit;
...
sealed class ArenaChanges(ModuleBase module) : ModuleComponent(module)
{
    public override void OnMapEffect(byte index, uint state)
    {
        if (index == 0x0D)
        {
            Bounds = A20HeavyArtilleryUnit.Arena3b;
        }
        else if (index == 0x0E)
        {
            Bounds = A20HeavyArtilleryUnit.Arena3c;
        }
    }
}
...
sealed class ManeuverHighPoweredLaser(ModuleBase module) : Components.SimpleAOEGroupsByTimewindow(module, [(uint)AID.ManeuverHighPoweredLaser], new AOEShapeRect(90f, 4f));
...
{
    public A20HeavyArtilleryUnitStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 736u, CFCID = 736u, NameID = 9656u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class A20HeavyArtilleryUnit(WorldState ws, Actor primary) : ModuleBase(ws, primary, IsArena1(primary) ? new(200f, -216.25f) : IsArena2(primary) ? new(200f, -316.532f) : arena3a.Center,
