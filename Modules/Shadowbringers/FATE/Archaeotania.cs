...
namespace Minerva.Shadowbringers.FATE.Archaeotania;
...
class BlizzardBreath(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BlizzardBreath, new AOEShapeCone(60f, 22.5f.Degrees()));
class FlamesOfTheApocalypse(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FlamesOfTheApocalypse, new AOEShapeDonut(15f, 30f));
class MindBlast(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MindBlast, 21f);

// note: helpers target their corresponding target for the duration of their life
class Megaflare(ModuleBase module) : Components.UniformStackSpread(module, 6f, default, 8)
{
...
class TidalWave(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.TidalWaveVisual, 48f, kind: Kind.DirForward);
class WindSlash(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WindSlashAOE, 8f);
class Windwinder(ModuleBase module) : Components.Voidzone(module, 5f, m => m.Enemies((uint)OID.Twister).Where(a => !a.IsDead));

abstract class CivilizationBuster(ModuleBase module, uint aid) : Components.SimpleAOEs(module, aid, new AOEShapeRect(62f, 7.5f));
class CivilizationBuster1(ModuleBase module) : CivilizationBuster(module, (uint)AID.CivilizationBuster1);
class CivilizationBuster2(ModuleBase module) : CivilizationBuster(module, (uint)AID.CivilizationBuster2);

class Touchdown(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Touchdown, 30f); // TODO: verify falloff
class PillarImpact(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PillarImpact, 7f);
class PillarPierce(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PillarPierce, new AOEShapeRect(65f, 4f));
class AncientAevis(ModuleBase module) : Components.Adds(module, (uint)OID.AncientAevis);
class HeadlongRush(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HeadlongRush, new AOEShapeRect(13.2f, 5f));
class Aether(ModuleBase module) : Components.Adds(module, (uint)OID.Aether);

class ArchaeotaniaStates : StateMachineBuilder
{
    public ArchaeotaniaStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.Fate, CFCID = 1432u, NameID = 8234u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")]
public class Archaeotania(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(279, 249), new ArenaBoundsSquare(29))
{
