...
namespace Minerva.Shadowbringers.FATE.Formidable;
...
class Spincrush(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Spincrush, new AOEShapeCone(15f, 60f.Degrees()));
class FireShot(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FireShot, 7f);

class FiresOfMtGulg(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        if (spell.Action.ID is (uint)AID.FiresOfMtGulg or (uint)AID.FiresOfMtGulgRepeat)
        {
            if (_aoe.Length != 0)
            {
                ...
                aoe.Activation = World.FutureTime(3.1d);
            }
            if (++NumCasts >= 7)
            {
                _aoe = [];
...
class BarrageFire(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.BarrageFire, "Raidwide + 7 repeats after");
...
class DrillShot(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.DrillShot, 6)
{
...
class ExplosionMissile(ModuleBase module) : ModuleComponent(module)
{
...
class ExplosionGrenade(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ExplosionGrenade, 12f);

class DwarvenDischarge(ModuleBase module, AOEShape shape, uint oid, uint aid, double delay) : Components.GenericAOEs(module)
{
    ...
    {
        if (actor.OID == oid)
            _aoes.Add(new(shape, actor.Position.Quantized(), default, World.FutureTime(delay), actorID: actor.InstanceID));
    }
...
class DwarvenDischargeDonut(ModuleBase module) : DwarvenDischarge(module, new AOEShapeDonut(9f, 60f), (uint)OID.DwarvenChargeDonut, (uint)AID.DwarvenDischargeDonut, 9.3d);
class DwarvenDischargeCircle(ModuleBase module) : DwarvenDischarge(module, new AOEShapeCircle(8f), (uint)OID.DwarvenChargeCircle, (uint)AID.DwarvenDischargeCircle, 8.1d);

class AutomatonEscort(ModuleBase module) : Components.Adds(module, (uint)OID.AutomatonEscort);
class SteamDome(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.SteamDome, 15f);

class DynamicSensoryJammer(ModuleBase module) : Components.StayMove(module, 3f)
{
...
{
    public FormidableStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.Fate, CFCID = 1464u, NameID = 8822u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
