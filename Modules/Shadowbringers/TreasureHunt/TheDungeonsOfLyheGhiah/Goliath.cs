...
namespace Minerva.Shadowbringers.TreasureHunt.DungeonsOfLyheGhiah.Goliath;
...
sealed class Wellbore(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Wellbore, 15f);
sealed class WellboreVoidzone(ModuleBase module) : Components.Voidzone(module, 8f, GetVoidzones) // not sure if size is correct, visual of the voidzone is somewhere between 7.5 and 8
{
...
sealed class Compress1(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Compress1, new AOEShapeCross(100f, 3.5f));
sealed class Compress2(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Compress2, new AOEShapeRect(102.1f, 3.5f));
sealed class Accelerate(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.Accelerate, 6f, 8, 8);
sealed class Incinerate(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Incinerate);
sealed class Fount(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Fount, 4f);
sealed class MechanicalBlow(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.MechanicalBlow);

sealed class Spin(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Spin, 11f);
sealed class Mash(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Mash, new AOEShapeRect(15.23f, 2f));
...
{
    public GoliathStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 688u, CFCID = 688u, NameID = 8953u, PrimaryActorOID = (uint)OID.Goliath, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
