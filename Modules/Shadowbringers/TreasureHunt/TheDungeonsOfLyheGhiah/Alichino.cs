...
namespace Minerva.Shadowbringers.TreasureHunt.DungeonsOfLyheGhiah.Alichino;
...
sealed class DiscordantEcho(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DiscordantEcho, 6f);
sealed class HeatGazeDonut(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.HeatGazeDonut1, (uint)AID.HeatGazeDonut2], new AOEShapeDonut(5f, 10f));
sealed class HeatGazeConeBig(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HeatGazeCone1, new AOEShapeCone(43.3f, 60f.Degrees()));
sealed class HeatGazeConeSmall(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HeatGazeCone2, new AOEShapeCone(19.9f, 30f.Degrees()));
sealed class Slapstick(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Slapstick);
sealed class Knockout(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.Knockout);
...
{
    public AlichinoStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 688u, CFCID = 688u, NameID = 8955u, PrimaryActorOID = (uint)OID.Alichino, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
