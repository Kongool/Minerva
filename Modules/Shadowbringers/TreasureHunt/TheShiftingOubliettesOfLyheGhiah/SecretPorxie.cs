...
namespace Minerva.Shadowbringers.TreasureHunt.ShiftingOubliettesOfLyheGhiah.SecretPorxie;
...
sealed class BrewingStorm(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BrewingStorm, new AOEShapeCone(5f, 30f.Degrees()));
sealed class HarrowingDream(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HarrowingDream, 6f);
sealed class BecloudingDust(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BecloudingDust, 6f);

sealed class Sweep(ModuleBase module) : Components.Exaflare(module, 6f)
{
...
{
    public SecretPorxieStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 745u, CFCID = 745u, NameID = 9795u, PrimaryActorOID = (uint)OID.SecretPorxie, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
