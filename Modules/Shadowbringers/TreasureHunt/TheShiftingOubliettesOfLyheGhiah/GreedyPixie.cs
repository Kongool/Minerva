...
namespace Minerva.Shadowbringers.TreasureHunt.ShiftingOubliettesOfLyheGhiah.GreedyPixie;
...
sealed class Windrune(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WindRune, new AOEShapeRect(40f, 4f));
sealed class SongRune(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SongRune, 6f);
sealed class StormRune(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.StormRune);

sealed class BushBash(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.BushBash1, (uint)AID.BushBash2], 12f);
sealed class NatureCall(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.NatureCall1, (uint)AID.NatureCall2], new AOEShapeCone(30f, 60f.Degrees()));
...
{
    public GreedyPixieStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 745u, CFCID = 745u, NameID = 9797u, PrimaryActorOID = (uint)OID.GreedyPixie, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
