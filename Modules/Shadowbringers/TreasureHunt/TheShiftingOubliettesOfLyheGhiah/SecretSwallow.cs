...
namespace Minerva.Shadowbringers.TreasureHunt.ShiftingOubliettesOfLyheGhiah.SecretSwallow;
...
sealed class ElectricWhorl(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SeventhWave, 11f);
sealed class PrevailingCurrent(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PrevailingCurrent, new AOEShapeRect(24f, 3f));
sealed class SeventhWave(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ElectricWhorl, new AOEShapeDonut(8f, 60f));
sealed class Hydrocannon(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Hydrocannon2, 8f);
sealed class Ceras(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.Ceras);
sealed class BodySlam(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BodySlam, 10f);
...
{
    public SecretSwallowStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 745u, CFCID = 745u, NameID = 9782u, PrimaryActorOID = (uint)OID.SecretSwallow, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
