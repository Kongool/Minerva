...
namespace Minerva.Shadowbringers.TreasureHunt.ShiftingOubliettesOfLyheGhiah.DaenOseTheAvariciousEchidna;
...
sealed class AbyssalPillar(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AbyssalPillar, 3f);

sealed class Petriburst(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.Petriburst, 6f, PartyState.MaxPartySize, PartyState.MaxPartySize);
sealed class PetrifactionBoss(ModuleBase module) : Components.CastGaze(module, (uint)AID.PetrifactionBoss);

sealed class PetrifactionPlayer(ModuleBase module) : Components.GenericGaze(module)
{
...
sealed class VoidStream(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.VoidStream1, (uint)AID.VoidStream2,
...
{
    public DaenOseTheAvariciousEchidnaStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 745u, CFCID = 745u, NameID = 9808u, PrimaryActorOID = (uint)OID.DaenOseTheAvariciousEchidna, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
