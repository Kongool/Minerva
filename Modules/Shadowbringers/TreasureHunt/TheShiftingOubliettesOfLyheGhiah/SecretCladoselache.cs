...
namespace Minerva.Shadowbringers.TreasureHunt.ShiftingOubliettesOfLyheGhiah.SecretCladoselache;
...
sealed class PelagicCleaverRotation(ModuleBase module) : Components.GenericRotatingAOE(module)
{
...
sealed class PelagicCleaver(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PelagicCleaver, new AOEShapeCone(40f, 30f.Degrees()));
sealed class TidalGuillotine(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TidalGuillotine, 13f);
sealed class ProtolithicPuncture(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.ProtolithicPuncture);
sealed class BiteAndRun(ModuleBase module) : Components.BaitAwayChargeCast(module, (uint)AID.BiteAndRun, 2.5f);
...
{
    public SecretCladoselacheStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 745u, CFCID = 745u, NameID = 9778u, PrimaryActorOID = (uint)OID.SecretCladoselache, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
