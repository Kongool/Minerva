...
namespace Minerva.Shadowbringers.TreasureHunt.ShiftingOubliettesOfLyheGhiah.DaenOseTheAvariciousUltros;
...
sealed class AquaBreath(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AquaBreath, new AOEShapeCone(13f, 45f.Degrees()));
sealed class Tentacle(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Tentacle, 8f);
sealed class Wallop(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Wallop, new AOEShapeRect(20f, 5f));
sealed class Megavolt(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Megavolt, 11f);
sealed class Waterspout(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Waterspout, 4f);
sealed class SoakingSplatter(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SoakingSplatter, 10f);
sealed class FallingWater(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.FallingWater, 8f);
sealed class ThunderIII(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.ThunderIII);

sealed class WaveOfTurmoil(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.WaveOfTurmoil, 20f, stopAtWall: true)
{
...
{
    public DaenOseTheAvariciousUltrosStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 745u, CFCID = 745u, NameID = 9808u, PrimaryActorOID = (uint)OID.DaenOseTheAvariciousUltros, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
