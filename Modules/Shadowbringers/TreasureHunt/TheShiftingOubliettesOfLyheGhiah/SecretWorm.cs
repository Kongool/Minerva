...
namespace Minerva.Shadowbringers.TreasureHunt.ShiftingOubliettesOfLyheGhiah.SecretWorm;
...
sealed class Hydrocannon(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Hydrocannon, 8f);
sealed class FreshwaterCannon(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FreshwaterCannon, new AOEShapeRect(46f, 2f));
sealed class BrineBreath(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.BrineBreath);

sealed class AquaBurstHydroburst(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        if (actor.OID == (uint)OID.Bubble)
        {
            ...
            _aoes.Add(new(circle, pos, default, World.FutureTime(5.7d), shapeDistance: circle.Distance(pos, default)));
        }
    }
...
sealed class Bubble(ModuleBase module) : Components.GenericBaitAway(module, centerAtTarget: true)
{
    private readonly AOEShapeCircle circle = new(10f);
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        ...
        {
            CurrentBaits.Add(new(Module.PrimaryActor, actor, circle, World.FutureTime(5d)));
        }
    }
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (CurrentBaits.Count != 0)
        {
            ...
            if (b.Target == actor)
            {
                hints.AddForbiddenZone(sdcircle ??= new SDCircle(Center, 17.5f), b.Activation);
            }
        }
    }
...
{
    public SecretWormStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 745u, CFCID = 745u, NameID = 9780u, PrimaryActorOID = (uint)OID.SecretWorm, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
