...
namespace Minerva.Shadowbringers.TreasureHunt.ShiftingOubliettesOfLyheGhiah.SecretSerpent;
...
sealed class DouseVoidzone(ModuleBase module) : Components.Voidzone(module, 7.5f, GetVoidzones)
{
...
sealed class DouseScaleRipple(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.Douse, (uint)AID.ScaleRipple], 8f);
sealed class FangsEnd(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.FangsEnd);
sealed class Drench1(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Drench1, new AOEShapeCone(15.29f, 45f.Degrees()));
sealed class Drench2(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Drench2, new AOEShapeCone(13.45f, 45f.Degrees()));
...
{
    public SecretSerpentStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 745u, CFCID = 745u, NameID = 9776u, PrimaryActorOID = (uint)OID.SecretSerpent, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
