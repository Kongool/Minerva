...
namespace Minerva.Shadowbringers.TreasureHunt.ShiftingOubliettesOfLyheGhiah.SecretPegasus;
...
sealed class BurningBright(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BurningBright, new AOEShapeRect(47f, 3f));
sealed class Nicker(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Nicker, 12f);
sealed class CloudCall(ModuleBase module) : Components.CastHint(module, (uint)AID.CloudCall, "Calls thunderclouds");
sealed class LightningBolt(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.LightningBolt, 8f);
...
{
    public SecretPegasusStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 745u, CFCID = 745u, NameID = 9793u, PrimaryActorOID = (uint)OID.SecretPegasus, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
