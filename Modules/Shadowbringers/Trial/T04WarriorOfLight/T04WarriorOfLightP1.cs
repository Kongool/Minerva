...
namespace Minerva.Shadowbringers.Trial.T04WarriorOfLightP1;
...
sealed class TerrorUnleashed(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.TerrorUnleashed);

// range 6 circle
sealed class SolemnConfiteor(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SolemnConfiteor1, 6f);

// range 5-60 donut
sealed class CoruscantSaberDonut(ModuleBase module)
    : Components.SimpleAOEs(module, (uint)AID.CoruscantSaberDonut, new AOEShapeDonut(5f, 60f));

// range 10 circle
sealed class CoruscantSaberCircle(ModuleBase module)
 ...
 */
class ElementStayMove(ModuleBase module) : Components.StayMove(module)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        if (iconID is (uint)IconID.Move)
        {
            ...
            if (p >= 0)
            {
                ...
                var state = new PlayerState(r, activation: World.FutureTime(4d), finish: World.FutureTime(6));
                SetState(p, in state);
            }
        }
    }
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        else if (id is (uint)AID.ImbuedCoruscance or (uint)AID.ImbuedCoruscance1 && _imbuedBlizzard)
        {
            ...
            var _activation = World.CurrentTime.AddSeconds(5.5d);
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        ...
        {
            PlayerStates[slot] = new(Requirement.Stay, activation: World.CurrentTime, finish: status.ExpireAt);
        }
    }
    ...
    public override void AddHints(int slot, Actor actor, TextHints hints)
    {
        ...
        switch (state.Requirement)
        {
            case Requirement.Stay:
                if (state.Activation <= World.CurrentTime && World.CurrentTime <= state.Finish)
                {
    ...
    public override void Update()
    {
        ...
        if (PlayerStates[slot].Finish != default && PlayerStates[slot].Finish <= World.CurrentTime)
        {
...
// circle aoe
sealed class ImbuedCoruscance(ModuleBase module)
...
sealed class ImbuedCoruscanceDonut(ModuleBase module)
...
sealed class UltimateCrossover(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.UltimateCrossover, hint: "TankLB3 when 'Transcend your limits...'");

sealed class Ascendance(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Ascendance);
...
sealed class Twincast(ModuleBase module) : Components.CastTowers(module, (uint)AID.MeteoricBurst, 4f, 2, 5)
{
     ...
     * We can use the position of 0 (100f, 89.5f) and Center (100, 100) to
     ...
     * We use a method to run the calculation : WPos.RotateAroundOrigin(rotAngFloat, Center, startPos)
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        DateTime _activation = World.FutureTime(10f);
        WPos pos;
        ...
        if (index <= 07)
            pos = WPos.RotateAroundOrigin(rotAngFloat, Center, startPos);
...
sealed class ElddragonDive(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.ElddragonDive);

// Dragon swoops across arena.
sealed class Cauterize(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Cauterize, new AOEShapeRect(40f, 10f));

// Large proximity bait. Target should run it away from party.
sealed class DelugeOfDeath(ModuleBase module)
...
// Circle cast stack.
sealed class AbsoluteHoly(ModuleBase module)
...
sealed class TheBitterEnd(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.TheBitterEnd, 5f, true, true, true);
...
sealed class RadiantBraver(ModuleBase module) : Components.BaitAwayIcon(module, new AOEShapeCone(60f, 46f.Degrees()),
    (uint)IconID.RadiantBraverIcon, (uint)AID.RadiantBraver, 6D)
{
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        ...
        {
            CurrentBaits.Add(new(Center, World.Actors.Find(targetID) ?? actor, Shape, World.FutureTime(ActivationDelay)));
        }
    }
...
sealed class RadiantBraverCleave(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.RadiantBraver, new AOEShapeCone(60F, 46F.Degrees()));
 ...
 */
sealed class RadiantDesperado(ModuleBase module) : Components.LineStack(module, aidMarker: (uint)AID.RadDespMarker, aidResolve: (uint)AID.RadiantDesperado1, activationDelay: 6.0f, maxStackSize: 4, markerIsFinalTarget: false)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        if (id == AidMarker && World.Actors.Find(spell.MainTargetID) is Actor target)
        {
            CurrentBaits.Add(new(caster, target, rect, World.FutureTime(ActionDelay), maxCasts: MaxCasts));
        }
...
sealed class RadiantMeteor(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.RadiantMeteor1, 20f)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        var _activation = World.FutureTime(5.0f);
...
sealed class BrimstoneEarth(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BrimstoneEarth1, new AOEShapeCircle(6f));

// Brimstone AOE circle that grows with each instant cast.
sealed class BrimstoneEarthGrow(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BrimstoneEarth1, new AOEShapeCircle(0.5f), maxCasts: 4)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
    ...
    public void BespokeAOE(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID is (uint)AID.BrimstoneEarth1 or (uint)AID.BrimstoneEarth2)
        {
            ...
            for (var i = 0; i < _genAOE; i++)
            {
                ...
                DateTime _activation = World.FutureTime(5f + (1.0f * i));
    ...
    public override void OnCastFinished(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID is (uint)AID.BrimstoneEarth1 or (uint)AID.BrimstoneEarth2 && Casters.Count != 0)
            Casters.RemoveAll(cast => cast.Activation <= World.CurrentTime);
    }
...
sealed class SwordOfLight(ModuleBase module)
    : Components.SimpleAOEs(module, (uint)AID.SwordOfLight, new AOEShapeCone(50f, 26.5f.Degrees()))
{
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        if (state == 0x00020001u)
        {
            ...
            if (pos != default)
            {
                ...
                var _activation = World.FutureTime(2.7d);
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        {
            Casters.RemoveAll(cast => cast.Activation <= World.CurrentTime && cast.Shape is AOEShapeCone);
        }
    }
}

sealed class SuitonSan(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.SuitonSan, 30f, kind: Kind.DirForward)
{
...
// stack cast
sealed class KatonSan(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.KatonSan1, 6f, 6, 8);

// Bait away tethers
sealed class FlareBreath(ModuleBase module) : Components.BaitAwayTethers(module, new AOEShapeCone(60f, 45f.Degrees()),
...
sealed class FlareBreathAOE(ModuleBase module)
...
sealed class PerfectDecimation(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PerfectDecimation1, new AOEShapeCone(60f, 22.5f.Degrees()), maxCasts: 4);
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 738u, CFCID = 738u, NameID = 9462u, PrimaryActorOID = (uint)OID.WarriorOfLight, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "wen (ported from BMR)")]

[SkipLocalsInit]
public sealed class T04WarriorOfLightP1(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(100f, 100f), new ArenaBoundsSquare(20f));
