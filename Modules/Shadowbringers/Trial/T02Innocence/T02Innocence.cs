...
namespace Minerva.Shadowbringers.Trial.T02Innocence;
...
sealed class CrownTrash(ModuleBase module) : Components.AddsMulti(module, T02Innocence.CrownMobs);

sealed class RealmRazer(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Realmrazer1);

sealed class RighteousBolt(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.RighteousBolt,
    new AOEShapeCircle(3f), centerAtTarget: true, tankbuster: true);

sealed class ScoldsBridle(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.ScoldsBridle);

sealed class DayBreak(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Daybreak, new AOEShapeCircle(6f));

// Causes confusion if you are hit by enthrall.
sealed class Enthrall(ModuleBase module) : Components.CastGaze(module, (uint)AID.Enthrall);

// create towers for the meteor; min 1 max 2 for four towers
sealed class SinSphere(ModuleBase module) : Components.GenericTowers(module)
{
...
sealed class ExaltedRaidwides(ModuleBase module) : Components.RaidwideCasts(module, [(uint)AID.ExaltedPlumes, (uint)AID.ExaltedWing]);

sealed class HolyTrinity(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HolyTrinity, new AOEShapeRect(40f, 2f));

sealed class SoulAndBodyLongCast(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.SoulAndBody, (uint)AID.SoulAndBody1], new AOEShapeDonutSector(15f, 20f, 20f.Degrees()));
 ...
 */
sealed class SoulAndBody(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SoulAndBody, new AOEShapeDonutSector(15f, 20f, 20f.Degrees()), maxCasts: 8)
{
    ...
    public void BespokeAOE(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID is (uint)AID.SoulAndBody or (uint)AID.SoulAndBody1)
        {
             ...
             * that is in the Center position dies.
             */
            var _genAOE = 26;
            ...
            for (var i = 0; i < _genAOE; i++)
            {
                ...
                WPos futureOrigin = WPos.RotateAroundOrigin(-rotAngFloat, Center, spell.LocXZ);
                ...
                DateTime _activation = World.FutureTime(2.7f + (0.6f * i));
    ...
    public override void OnCastFinished(Actor caster, ActorCastInfo spell)
    {
        if ((spell.Action.ID is (uint)AID.SoulAndBody or (uint)AID.SoulAndBody1) && Casters.Count != 0)
            Casters.RemoveAll(cast => cast.Activation <= World.CurrentTime && cast.Shape is AOEShapeDonutSector);
    }
    ...
    public override void OnActorDestroyed(Actor actor)
    {
        if (actor.OID == (uint)OID.NailOfCondemnation && actor.Position == Center)
        {
            Casters.Clear();
        }
    }
 ...
 */
sealed class Reprobation(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Reprobation1, new AOEShapeRect(21f, 2f));
sealed class ReprobationLong(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Reprobation3, new AOEShapeRect(42f, 2));

sealed class Shadowreaver(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Shadowreaver);

sealed class Manacle(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Manacle, new AOEShapeCircle(6f));

sealed class PhaseTwoRaidwides(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GuiltyVerdict, (uint)AID.FlamingSword);

sealed class GodRayCone(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GodRayCone, new AOEShapeCone(5f, 50f.Degrees()));
sealed class GodRayMedium(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GodRayMedium, new AOEShapeDonutSector(5f, 10f, 50f.Degrees()));
sealed class GodRayOuter(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GodRayOuter, new AOEShapeDonutSector(10f, 20f, 50f.Degrees()));

// stack rectangle
sealed class LightPillar(ModuleBase module) : Components.LineStack(module, aidMarker: (uint)AID.LightPillarMarker, aidResolve: (uint)AID.LightPillar1, 4.7d, 40f, 3f, 8, 8);

// Giant proximity aoe. Safe zone is an estimate.
sealed class BeatificVision(ModuleBase module)
    : Components.SimpleAOEs(module, (uint)AID.BeatificVision, new AOEShapeRect(45f, 15f));

sealed class DropOfLight(ModuleBase module) : Components.BaitAwayIcon(module, new AOEShapeCircle(10f), (uint)IconID.DropOfLightTarget, (uint)AID.DropOfLight1, centerAtTarget: true);

[SkipLocalsInit]
sealed class T02InnocenceStates : StateMachineBuilder
{
    public T02InnocenceStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 666u, NameID = 8353u, PrimaryActorOID = (uint)OID.Innocence, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "wen (ported from BMR)")]
[SkipLocalsInit]
 ...
 */
public sealed class T02Innocence(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(100f, 100f), new ArenaBoundsCircle(20f))
{
    ...
    protected override void DrawEnemies(int pcSlot, Actor pc)
    {
        ...
        Arena.Actors(World.Actors.Where(a => !a.IsAlly), Colors.Enemy);
    }
