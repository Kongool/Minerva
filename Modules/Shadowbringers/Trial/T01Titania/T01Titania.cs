...
namespace Minerva.Shadowbringers.Trial.T01Titania;
...
sealed class BrightSabbath(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.BrightSabbath);

sealed class PhantomRune(ModuleBase module)
    : Components.SimpleAOEs(module, (uint)AID.PhantomRuneDonut, new AOEShapeDonut(5f, 60f));

sealed class PhantomRune1(ModuleBase module)
    : Components.SimpleAOEs(module, (uint)AID.PhantomRuneCircle, new AOEShapeCircle(10f));
//angle is an estimate : tank cleave
sealed class DivinationRune(ModuleBase module) : Components.BaitAwayIcon(module, new AOEShapeCone(60f, 37.5f.Degrees()), (uint)IconID.DivinationIcon, (uint)AID.DivinationRune, tankbuster: true);

sealed class FrostRune(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FrostRune1, new AOEShapeCircle(10f));

sealed class Uplift(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.Uplift, 8f);

sealed class GentleBreeze(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GentleBreeze, new AOEShapeRect(60f, 4f));

sealed class LeafStorm(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.Leafstorm1, (uint)AID.Leafstorm3], new AOEShapeCone(50f, 10f.Degrees()));

sealed class WarAndPease(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.WarAndPease, new AOEShapeCircle(10f), centerAtTarget: true);

sealed class PucksBreath(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.PucksBreath, 6f);

sealed class PucksCaprice(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.PucksCaprice);

sealed class BeingMortal1(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.BeingMortal1);

// Knockback distance 10 : away from origin
sealed class PucksRebuke1(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.PucksRebuke1, 10f)
{
...
sealed class PucksRebuke2(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PucksRebuke2, new AOEShapeCircle(5f));
...
sealed class WoodsEmbrace(ModuleBase module) : Components.GenericBaitAway(module, centerAtTarget: true)
{
...
sealed class TitaniaAdds(ModuleBase module) : Components.AddsMulti(module,
[
...
sealed class Pease(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Pease, new AOEShapeCircle(6f));

sealed class PeasebombSpread(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.Peasebomb, 6f);
...
sealed class WaterPuddles(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        {
            _fireCasting = true;
            _activation = World.FutureTime(2.7f);
        }
    }
...
sealed class WaterTowers(ModuleBase module) : Components.GenericTowers(module)
{
...
sealed class FlameRune(ModuleBase module) : Components.StackWithIcon(module, (uint)IconID.FireStackIcon, (uint)AID.FlameHammer, 5f, 3d, 8, 8);


[SkipLocalsInit]
sealed class TitaniaStates : StateMachineBuilder
{
    public TitaniaStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 657u, CFCID = 657u, NameID = 8361u, PrimaryActorOID = (uint)OID.Titania, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "wen (ported from BMR)")]

[SkipLocalsInit]
public sealed class Titania(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(100f, 100f), new ArenaBoundsSquare(20f));
