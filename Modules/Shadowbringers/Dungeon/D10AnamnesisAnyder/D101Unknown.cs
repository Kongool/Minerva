...
namespace Minerva.Shadowbringers.Dungeon.D10AnamnesisAnyder.D101Unknown;
...
sealed class Inscrutability(ModuleBase module) : Components.RaidwideCasts(module, [(uint)AID.Inscrutability1, (uint)AID.Inscrutability2]);
sealed class FetidFang(ModuleBase module) : Components.SingleTargetDelayableCasts(module, [(uint)AID.FetidFang1, (uint)AID.FetidFang2]);

sealed class SetbackClearout(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.Setback1, (uint)AID.Setback2, (uint)AID.Clearout1, (uint)AID.Clearout2],
new AOEShapeCone(9f, 60f.Degrees()));

sealed class Explosion(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Explosion, 8f);

sealed class LuminousRay(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.LuminousRay1, (uint)AID.LuminousRay2], new AOEShapeRect(50f, 4f));

sealed class Reflection(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorEAnim(Actor actor, uint state)
    {
        ...
        if (angle != default)
        {
            _aoe = [new(cone, actor.Position.Quantized(), angle, World.FutureTime(14.4d))];
        }
    }
...
abstract class EctoplasmicRay(ModuleBase module, uint aid1, uint aid2) : Components.LineStack(module, aidMarker: aid1, aid2, 5.2d, markerIsFinalTarget: false);
sealed class EctoplasmicRay1(ModuleBase module) : EctoplasmicRay(module, (uint)AID.EctoplasmicRayMarker1, (uint)AID.EctoplasmicRay1);
sealed class EctoplasmicRay2(ModuleBase module) : EctoplasmicRay(module, (uint)AID.EctoplasmicRayMarker2, (uint)AID.EctoplasmicRay2);
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 714u, CFCID = 714u, NameID = 9260u, PrimaryActorOID = (uint)OID.Unknown1, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D101Unknown(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
