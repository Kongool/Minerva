...
namespace Minerva.Shadowbringers.Dungeon.D10AnamnesisAnyder.D102Kyklops;
...
class TheFinalVerse(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.TheFinalVerse);
class WanderersPyre(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.WanderersPyre, 5f);
class OpenHearth(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.OpenHearth, 6f);
class EyeOfTheCyclone(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.EyeOfTheCyclone, new AOEShapeDonut(8f, 25f));
class RagingGlower(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.RagingGlower, new AOEShapeRect(45f, 3f));
class MinaSwipe2000(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MinaSwipe2000, new AOEShapeCone(12f, 60f.Degrees()));
class MinaSwing2000(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MinaSwing2000, 12f);

class TerribleBladeHammer(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        void SetupPattern(WPos[] primaryPattern, WPos[] secondaryPattern, double primaryTime, double secondaryTime)
        {
            ...
            void AddAOEs(WPos[] pattern, double activationTime)
            {
                ...
                var act = World.FutureTime(activationTime);
                for (var i = 0; i < len; ++i)
                {
...
{
    public D102KyklopsStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 714u, NameID = 9263u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D102Kyklops(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(20, -80.1f), new ArenaBoundsRect(14.5f, 14.4f));
