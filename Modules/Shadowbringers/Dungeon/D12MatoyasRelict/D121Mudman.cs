...
namespace Minerva.Shadowbringers.Dungeon.D12MatoyasRelict.D121Mudman;
...
class StoneAge(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.StoneAge);
class HardRock(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.HardRock);
class MudVoidzone(ModuleBase module) : Components.Voidzone(module, 5f, GetVoidzone)
{
    private static List<Actor> GetVoidzone(ModuleBase module) => module.Enemies((uint)OID.MudVoidzone);
}
class Quagmire(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Quagmire, 6f);
class FallingRock(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.FallingRock, 6f, 4, 4);

class BrittleBreccia(ModuleBase module) : Components.ConcentricAOEs(module, _shapes)
{
    ...
    public override void OnCastFinished(Actor caster, ActorCastInfo spell)
    {
        if (Sequences.Count != 0)
        {
            ...
            AdvanceSequence(order, spell.LocXZ, World.FutureTime(1.5d), spell.Rotation);
        }
    }
}

class RockyRoll(ModuleBase module) : Components.GenericBaitAway(module)
{
    ...
    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        ...
        {
            CurrentBaits.Add(new(source, World.Actors.Find(tether.Target)!, rect1, World.FutureTime(8.2d)));
        }
    }
    ...
    public override void Update()
    {
        ...
        var activation = World.FutureTime(9.7d);
...
{
    public D121MudmanStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 746u, NameID = 9735u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D121Mudman(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-180f, -140f), new ArenaBoundsCircle(19.5f));
