...
namespace Minerva.Shadowbringers.Dungeon.D06Amaurot.D062Bellwether;
...
sealed class ShrillShriek(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.ShrillShriek);
sealed class Aetherspike(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Aetherspike, new AOEShapeRect(40f, 4f));
sealed class Comet(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Comet, 4f);
sealed class SicklyInferno(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SicklyInferno, 5f);
sealed class Burst(ModuleBase module) : Components.CastHint(module, (uint)AID.BurstEnrage, "Enrage!", true);

sealed class D062BellwetherStates : StateMachineBuilder
{
    public D062BellwetherStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 652u, CFCID = 652u, NameID = 8202u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D062Bellwether(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
