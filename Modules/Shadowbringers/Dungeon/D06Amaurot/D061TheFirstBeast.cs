...
namespace Minerva.Shadowbringers.Dungeon.D06Amaurot.D061FirstBeast;
...
sealed class VenomousBreath(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.VenomousBreath, new AOEShapeCone(9f, 60f.Degrees()));
sealed class MeteorRainBurningSky(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.MeteorRain, (uint)AID.TheBurningSkyAOE], 6f);
sealed class TheFallingSkyCosmicKissEarthquake(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.TheFallingSky, (uint)AID.CosmicKiss, (uint)AID.Earthquake], 10f);
sealed class Towerfall(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Towerfall, new AOEShapeRect(35f, 20f));
sealed class TheBurningSkySpread(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.TheBurningSkySpread, 6f);

sealed class Meteors(ModuleBase module) : Components.GenericBaitAway(module, centerAtTarget: true)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        if (iconID == (uint)IconID.Meteor)
            CurrentBaits.Add(new(Module.PrimaryActor, actor, circle, World.FutureTime(8.1d)));
    }
...
sealed class TheFinalSky(ModuleBase module) : Components.CastLineOfSightAOE(module, (uint)AID.TheFinalSky, 70f, safeInsideHitbox: false)
{
...
{
    public D061FirstBeastStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 652u, CFCID = 652u, NameID = 8201u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D061FirstBeast(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-80f, 82f), new ArenaBoundsSquare(19.5f));
