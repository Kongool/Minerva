...
namespace Minerva.Shadowbringers.Dungeon.D06Amaurot.D063Therion;
...
sealed class ShadowWreck(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.ShadowWreck);
sealed class Misfortune(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Misfortune, 6f);
sealed class ThereionCharge(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TherionCharge, 20f);

sealed class Border(ModuleBase module) : Components.GenericAOEs(module, warningText: "Platform will be removed during next Apokalypsis!")
{
...
sealed class Apokalypsis(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class DeathlyRayThereion(ModuleBase module) : Components.GenericAOEs(module)
{
    private AOEInstance[] _aoe = [];
...
sealed class DeathlyRayFaces(ModuleBase module) : Components.GenericAOEs(module)
{
        ...
        if (spell.Action.ID is (uint)AID.DeathlyRayFacesFirst or (uint)AID.DeathlyRayFacesRest)
        {
            if (_aoes.Count == 0)
            {
                ...
                for (var i = 0; i < countF; ++i)
                {
                    ...
                    _aoes.Add(new(_rect, f.Position.Quantized(), f.Rotation, !isFirstSet ? World.FutureTime(8.5d) : default, isFirstSet ? Colors.Danger : default, isFirstSet));
...
{
    public D063TherionStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 652u, CFCID = 652u, NameID = 8210u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D063Therion(WorldState ws, Actor primary) : ModuleBase(ws, primary, Border.DefaultArena.Center, Border.DefaultArena);
