...
namespace Minerva.Shadowbringers.Dungeon.D13Paglthan.D131Amhuluk;
...
sealed class ElectricBurst(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.ElectricBurst);
sealed class CriticalRip(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.CriticalRip);

sealed class LightningBolt(ModuleBase module) : Components.GenericBaitAway(module, (uint)AID.LightningBolt, centerAtTarget: true)
{
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        if (status.ID == (uint)SID.LightningRod)
        {
            ...
            {
                activation = World.FutureTime(10.8d);
            }
...
sealed class Shock(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        if (shape != null)
        {
            _aoes.Add(new(shape, actor.Position.Quantized(), default, World.FutureTime(3.7d)));
        }
    }
...
sealed class WideBlasterSpikeFlail(ModuleBase module) : Components.GenericAOEs(module)
{
...
{
    public D131AmhulukStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 777u, CFCID = 777u, NameID = 10075u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D131Amhuluk(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
