...
namespace Minerva.Shadowbringers.Dungeon.D13Paglthan.D133LunarBahamut;
...
sealed class Upburst(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Upburst, 2f);
sealed class BigBurst(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BigBurst, 9f);
sealed class PerigeanBreath(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PerigeanBreath, new AOEShapeCone(30f, 45f.Degrees()));
sealed class MegaflareAOE(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MegaflareAOE, 6f);
sealed class MegaflareSpread(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.MegaflareSpread, 5f);
sealed class MegaflareDive(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MegaflareDive, new AOEShapeRect(41f, 6f));
sealed class LunarFlareBig(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.LunarFlareBig, 11f);
sealed class LunarFlareSmall(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.LunarFlareSmall, 6f);
sealed class Gigaflare(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Gigaflare);
sealed class Flatten(ModuleBase module) : Components.SingleTargetDelayableCast(module, (uint)AID.Flatten);

sealed class AkhMorn(ModuleBase module) : Components.UniformStackSpread(module, 4f, default, 4, 4)
{
...
sealed class KanRhaiBait(ModuleBase module) : Components.GenericBaitAway(module, centerAtTarget: true)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        ...
        {
            CurrentBaits.Add(new(Module.PrimaryActor, actor, Cross, World.FutureTime(5.6d), customRotation: Angle.AnglesCardinals[1]));
        }
    }
...
sealed class KanRhaiAOE(ModuleBase module) : Components.GenericAOEs(module)
{
...
{
    public D133LunarBahamutStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 777u, CFCID = 777u, NameID = 10077u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D133LunarBahamut(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
