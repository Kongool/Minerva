...
namespace Minerva.Shadowbringers.Dungeon.D09GrandCosmos.D092LeananSith;
...
sealed class OdeToLostLove(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.OdeToLostLove);
sealed class StormOfColor(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.StormOfColor);
sealed class FarWindSpread(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.FarWindSpread, 5f);
sealed class FarWind(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FarWind, 8f);
sealed class OdeToFallenPetals(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.OdeToFallenPetals, new AOEShapeDonut(5f, 60f));
sealed class IrefulWind(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.IrefulWind, 10f, kind: Kind.DirForward, stopAtWall: true);

sealed class GreenTiles(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        else if (!shape.Check(actor.Position)) // BMR gated this on its AI being on; Minerva states the hint
        // and lets the consumer decide whether to act on it
        {
    ...
    private void HandleTransportingActor(int slot, AIHints hints)
    {
        ...
        if (time == default)
        {
            transportingCheckStartTimes[slot] = World.CurrentTime;
        }
        else if (World.CurrentTime >= time.AddSeconds(0.25d))  // we need to delay the drop or server lag will cause the seed to drop at an old position
        {
...
{
    public D092LeananSithStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 692u, CFCID = 692u, NameID = 9044u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D092LeananSith(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(0f, -60f), new ArenaBoundsSquare(19.5f))
{
