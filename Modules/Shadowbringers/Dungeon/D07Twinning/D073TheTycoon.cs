...
namespace Minerva.Shadowbringers.Dungeon.D07Twinning.D073TheTycoon;
...
sealed class HighTensionDischarger(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.HighTensionDischarger);
sealed class RailCannon(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.RailCannon);
sealed class HighGravity(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HighGravity, 8f);
sealed class ShatteredCrystal(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.ShatteredCrystal, 5f);

sealed class TemporalParadoxMagitekRay(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        void AddAOE(AOEShapeRect shape, double delay) => _aoes.Add(new(shape, actor.Position.Quantized(), actor.Rotation, World.FutureTime(delay)));
...
{
    public D073TheTycoonStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 655u, CFCID = 655u, NameID = 8167u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D073TheTycoon(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
