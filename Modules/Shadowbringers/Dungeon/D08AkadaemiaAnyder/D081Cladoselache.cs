...
namespace Minerva.Shadowbringers.Dungeon.D08AkadaemiaAnyder.D081Cladoselache;
...
sealed class MarineMayhem(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.MarineMayhem);
sealed class ProtolithicPuncture(ModuleBase module) : Components.SingleTargetDelayableCast(module, (uint)AID.ProtolithicPuncture);
sealed class PelagicCleaver1(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PelagicCleaver1, new AOEShapeCone(40f, 30f.Degrees()));
sealed class PelagicCleaver2(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PelagicCleaver2, new AOEShapeCone(50f, 30f.Degrees()));
sealed class TidalGuillotine(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.TidalGuillotine1, (uint)AID.TidalGuillotine2], 13f);
sealed class AquaticLance(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.AquaticLance, 8f);
sealed class AquaticLanceVoidzone(ModuleBase module) : Components.Voidzone(module, 8f, GetVoidzones)
{
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 661u, CFCID = 661u, NameID = 8235u, PrimaryActorOID = (uint)OID.Cladoselache, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D081Cladoselache(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
