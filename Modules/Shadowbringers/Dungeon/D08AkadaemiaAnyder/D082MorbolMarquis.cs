...
namespace Minerva.Shadowbringers.Dungeon.D08AkadaemiaAnyder.D082MorbolMarquis;
...
sealed class ArborStorm(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.ArborStorm);

abstract class Leash(ModuleBase module, uint aid) : Components.SingleTargetEventDelay(module, aid, (uint)AID.Lash, 3.4d)  // actual delay can be higher since boss needs to run into melee range for it
{
...
sealed class LeashSapShower(ModuleBase module) : Leash(module, (uint)AID.SapShowerVisual);
sealed class LeashPutridBreath(ModuleBase module) : Leash(module, (uint)AID.PutridBreath);

sealed class SapShower(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.SapShower, 8f);

sealed class ExtensibleTendrilsPutridBreath(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        ...
        if (remainingCasts > 0)
        {
            ...
            if ((delay1 - World.CurrentTime).TotalSeconds <= 2.5d)
            {
        ...
        if (activation != default && (delay2 - World.CurrentTime).TotalSeconds <= 4.9d)
        {
...
sealed class BlossomArenaChanges(ModuleBase module) : ModuleComponent(module)
{
...
{
    public D082MorbolMarquisStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 661u, CFCID = 661u, NameID = 8272u, PrimaryActorOID = (uint)OID.MorbolMarquis, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D082MorbolMarquis(WorldState ws, Actor primary) : ModuleBase(ws, primary, DefaultBounds.Center, DefaultBounds)
{
