...
namespace Minerva.Shadowbringers.Dungeon.D08AkadaemiaAnyder.D083Quetzalcoatl;
...
sealed class Thunderbolt(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Thunderbolt);
sealed class Shockbolt(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.Shockbolt);
sealed class ShockingPlumage(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ShockingPlumage, new AOEShapeCone(40f, 30f.Degrees()));
sealed class WindingCurrent(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WindingCurrent, new AOEShapeDonut(5f, 40f));
sealed class ThunderstormAOE(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ThunderstormAOE, 5f);
sealed class ThunderstormSpread(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.ThunderstormSpread, 5f);
...
sealed class OrbCollecting(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        var activation = aoes.Count != 0 ? aoes.Ref(0).Activation.AddSeconds(1.1d) : World.FutureTime(2d);
...
sealed class ExpandingOrb(ModuleBase module) : Components.GenericAOEs(module)
{
...
{
    public D083QuetzalcoatlStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 661u, CFCID = 661u, NameID = 8273u, PrimaryActorOID = (uint)OID.Quetzalcoatl, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D083Quetzalcoatl(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
