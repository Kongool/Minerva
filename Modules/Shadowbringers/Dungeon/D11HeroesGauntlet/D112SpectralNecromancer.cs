...
namespace Minerva.Shadowbringers.Dungeon.D11HeroesGauntlet.D112SpectralNecromancer;
...
class AbsoluteDarkII(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AbsoluteDarkII, new AOEShapeCone(40f, 60f.Degrees()));
class PainMire(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PainMire, 9f)
{
...
class BleedVoidzone(ModuleBase module) : Components.Voidzone(module, 8f, m => m.Enemies((uint)OID.BleedVoidzone).Where(x => x.EventState != 7));
class TwistedTouch(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.TwistedTouch);
class ChaosStorm(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.ChaosStorm);
class DarkDeluge(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DarkDeluge, 5f);
class NecrobombBaitAway(ModuleBase module) : Components.BaitAwayIcon(module, 9.25f, (uint)IconID.Baitaway, (uint)AID.DeathThroes); // note: explosion is not always exactly the position of player, if zombie teleports to player it is player + zombie hitboxradius = 1.25 away

class Necrobombs(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        foreach (var e in World.Actors.Where(x => !x.IsAlly && x.Tether.ID == (uint)TetherID.CrawlingNecrobombs))
...
class Burst(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorModelStateChange(Actor actor, byte modelState, byte animState1, byte animState2)
    {
        if (modelState == 54)
        {
            _aoes.Add(new(circle, actor.Position.Quantized(), default, World.FutureTime(6d))); // activation time can be vastly different, even twice as high so we take a conservative delay
        }
    }
...
{
    public D112SpectralNecromancerStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 737u, NameID = 9508u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D112SpectralNecromancer(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
