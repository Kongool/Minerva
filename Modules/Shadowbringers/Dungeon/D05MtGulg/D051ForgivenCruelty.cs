...
namespace Minerva.Shadowbringers.Dungeon.D05MtGulg.D051ForgivenCruelty;
...
class Rake(ModuleBase module) : Components.SingleTargetDelayableCast(module, (uint)AID.Rake);
class CycloneWing(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.CycloneWing);
class LumenInfinitum(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.LumenInfinitum, new AOEShapeRect(40f, 2.5f));
class HurricaneWing(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HurricaneWing, 10f);

class TyphoonWing(ModuleBase module, uint aid) : Components.SimpleAOEs(module, aid, new AOEShapeCone(25f, 30f.Degrees()));
class TyphoonWing1(ModuleBase module) : TyphoonWing(module, (uint)AID.TyphoonWing1);
class TyphoonWing2(ModuleBase module) : TyphoonWing(module, (uint)AID.TyphoonWing2);

class D051ForgivenCrueltyStates : StateMachineBuilder
{
    public D051ForgivenCrueltyStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 659u, NameID = 8260u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
public class D051ForgivenCruelty(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
