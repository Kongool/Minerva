...
namespace Minerva.Shadowbringers.Dungeon.D05MtGulg.D053ForgivenWhimsy;
...
class PerfectContrition(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        if (actor.OID == (uint)OID.Brightsphere)
            _aoes.Add(new(donut, actor.Position.Quantized(), default, World.FutureTime(10.6d)));
    }
...
class Catechism(ModuleBase module) : Components.SingleTargetCastDelay(module, (uint)AID.Catechism, (uint)AID.Catechism2, 0.5f);
class SacramentOfPenance(ModuleBase module) : Components.RaidwideCastDelay(module, (uint)AID.SacramentOfPenanceVisual, (uint)AID.SacramentOfPenance, 0.5f);

class JudgmentDay(ModuleBase module) : Components.GenericTowers(module)
{
    public override void OnActorCreated(Actor actor)
    {
        if (actor.OID == (uint)OID.Towers)
            Towers.Add(new(actor.Position.Quantized(), 5f, activation: World.FutureTime(7.6d)));
    }
...
class Exegesis(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
...
{
    public D053ForgivenWhimsyStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 659u, NameID = 8261u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
public class D053ForgivenWhimsy(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-240f, -50f), new ArenaBoundsSquare(14.5f));
