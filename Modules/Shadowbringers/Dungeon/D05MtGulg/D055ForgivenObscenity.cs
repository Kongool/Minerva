...
namespace Minerva.Shadowbringers.Dungeon.D05MtGulg.D055ForgivenObscenity;
...
sealed class Orbs(ModuleBase module) : Components.GenericAOEs(module, default, "GTFO from voidzone!")
{
...
sealed class GoldChaser(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class SacramentSforzando(ModuleBase module) : Components.SingleTargetCastDelay(module, (uint)AID.SacramentSforzando, (uint)AID.SacramentSforzando2, 0.8d);
sealed class OrisonFortissimo(ModuleBase module) : Components.RaidwideCastDelay(module, (uint)AID.OrisonFortissimo, (uint)AID.OrisonFortissimo2, 0.8d);

sealed class DivineDiminuendoCircle(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.DivineDiminuendoCircle1, (uint)AID.DivineDiminuendoCircle2,
(uint)AID.DivineDiminuendoCircle3], 8f);

sealed class DivineDiminuendoDonut1(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DivineDiminuendoDonut1, new AOEShapeDonut(10f, 16f));
sealed class DivineDiminuendoDonut2(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DivineDiminuendoDonut2, new AOEShapeDonut(18f, 32f));

sealed class ConvictionMarcato(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.ConvictionMarcato1, (uint)AID.ConvictionMarcato2,
(uint)AID.ConvictionMarcato3], new AOEShapeRect(40f, 2.5f));

sealed class PenancePianissimo(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorEAnim(Actor actor, uint state)
    {
        ...
        {
            case 0x00040008u:
                Bounds = new ArenaBoundsRect(14.5f, 19.5f);
                break;
            case 0x00010002u:
                _aoe = [];
                var center = Center;
                Bounds = new ArenaBoundsCustom([new Polygon(center, 15f, 64)], [new Rectangle(center + new WDir(15f, default), 0.5f, 20f),
...
{
    public D055ForgivenObscenityStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 659u, CFCID = 659u, NameID = 8262u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
public sealed class D055ForgivenObscenity(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-240f, 237f), new ArenaBoundsRect(14.5f, 19.5f))
{
