...
namespace Minerva.Shadowbringers.Quest.MSQ.FadedMemories.Ardbert;
...
class Overcome(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Overcome, new AOEShapeCone(8f, 60f.Degrees()), 2);
class Skydrive(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Skydrive, 5f);
class LandsOfOld(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.LandsOfOld);

class SkyHighDrive(ModuleBase module) : Components.GenericRotatingAOE(module)
{
...
class AvalancheAxe1(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AvalanceAxe1, 10f);
class AvalancheAxe2(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AvalanceAxe2, 10f);
class AvalancheAxe3(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AvalanceAxe3, 10f);
class OvercomeAllOdds(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.OvercomeAllOdds, new AOEShapeCone(60f, 15f.Degrees()), 1)
{
...
class Soulflash(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Soulflash1, 4f);
class EtesianAxe(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.EtesianAxe, 15f, kind: Kind.DirForward);
class Soulflash2(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Soulflash2, 8f);

class GroundbreakerExaflares(ModuleBase module) : Components.Exaflare(module, 6f)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
...
class GroundbreakerCone(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GroundbreakerCone, new AOEShapeCone(40f, 45f.Degrees()));
class GroundbreakerDonut(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GroundbreakerDonut, new AOEShapeDonut(5f, 20f));
class GroundbreakerCircle(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GroundbreakerCircle, 15f);

class ArdbertStates : StateMachineBuilder
{
    public ArdbertStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.Quest, CFCID = 69311u, NameID = 8258u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")]
public class Ardbert(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-392f, 780f), new ArenaBoundsCircle(20f));
