...
[SkipLocalsInit]
sealed class P2IntermissionLimitCut(ModuleBase module) : LimitCut(module, 3.2d);

[SkipLocalsInit]
sealed class P2IntermissionHawkBlaster(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.HawkBlasterIntermission)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        if (spell.Action.ID == WatchedAction)
        {
            if (NumCasts == 0)
            {
                var offset = spell.TargetXZ - Center;
    ...
    private WPos? SafeSpotHint(int slot)
    {
        ...
        return Center + (invert ? -offset : offset);
    }
}
