...
[SkipLocalsInit]
sealed class P4FateCalibrationAlphaStillnessMotion(ModuleBase module) : Components.StayMove(module)
{
    ...
    public void ApplyNextRequirement()
    {
        ...
        Array.Fill(PlayerStates, new(req, World.FutureTime(4.1d)));
    }
}

[SkipLocalsInit]
sealed class P4FateCalibrationAlphaDebuffs(ModuleBase module) : Components.UniformStackSpread(module, 4f, 30f, 3)
{
            ...
            case (uint)AID.FateCalibrationAlphaDefamation:
                ...
                if (defamationSlot >= 0)
                {
                    ...
                    if (defamationTarget != null)
                        AddSpread(defamationTarget, World.FutureTime(20.1d));
                }
                break;
            case (uint)AID.FateCalibrationAlphaSharedSentence:
                ...
                if (sharedSlot >= 0)
                {
                    ...
                    if (sharedTarget != null)
                        AddStack(sharedTarget, World.FutureTime(20.1d), _avoidMask); // note: avoid mask is typically empty here, since aggravated assaults happen later
                }
                break;
...
[SkipLocalsInit]
sealed class P4FateCalibrationAlphaSacrament(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        switch (spell.Action.ID)
        {
            case (uint)AID.FateCalibrationAlphaSacrament:
                _casters.Add((caster, World.FutureTime(18.1d)));
                if (_casters.Count == 3)
                    InitSafeSpots();
                break;
