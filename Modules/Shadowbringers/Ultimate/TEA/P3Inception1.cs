...
[SkipLocalsInit]
sealed class P3Inception1(ModuleBase module) : Components.CastCounter(module, (uint)AID.JudgmentCrystalAOE)
{
    ...
    private void InitAssignments()
    {
        ...
        var alexNorth = ((TEA)Module).AlexPrime()?.PosRot.Z < Center.Z;
        var boxPos = Center + new WDir(default, alexNorth ? 13f : -13f);
        for (var slot = 0; slot < _tetherSources.Length; ++slot)
        {
            ...
            if (sphere != null)
            {
                ...
                var sphereWest = pos.X < Center.X;
                var sameSideSphere = _plasmaspheres.Find(o => o != sphere && (o.PosRot.X < Center.X) == sphereWest);
                ...
                if (!sphereWest)
                    spotDir = -spotDir;
                _assignedPositions[slot] = Center + 18f * spotDir.ToDirection();
            }
            else
            {
