...
[SkipLocalsInit]
sealed class P3WormholeLimitCut(ModuleBase module) : LimitCut(module, 2.7d);
[SkipLocalsInit]
sealed class P3WormholeSacrament(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SacramentWormhole, new AOEShapeCross(100f, 8f));

[SkipLocalsInit]
sealed class P3WormholeRepentance(ModuleBase module) : ModuleComponent(module)
{
    public int NumSoaks;
    ...
    public override void AddHints(int slot, Actor actor, TextHints hints)
    {
        ...
        var dirToAlex = (alex.Position - Center).Normalized();
    ...
    public override void AddMovementHints(int slot, Actor actor, MovementHints movementHints)
    {
        ...
        if (!ShouldSoakWormhole(order) || !_chakramsDone)
        {
            var dirToAlex = (alex.Position - Center).Normalized();
            var dirToSide = SelectSide(order, dirToAlex);
            movementHints.Add(actor.Position, Center + SafeSpotOffset(order, dirToAlex, dirToSide), Colors.Safe);
        }
    }
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        ...
        foreach (var w in _wormholes)
            Arena.ZoneCircleOutline(w, _radiuses[NumSoaks], shouldSoak && dirToSide.Dot(w - Center) > 0f ? Colors.Safe : default);

        if (!shouldSoak || !_chakramsDone)
            Arena.ZoneCircleOutline(Center + SafeSpotOffset(pcOrder, dirToAlex, dirToSide), 1f, Colors.Safe);
    }
...
[SkipLocalsInit]
sealed class P3WormholeIncineratingHeat(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.IncineratingHeat, 5f, 8, 8);

[SkipLocalsInit]
sealed class P3WormholeEnumeration(ModuleBase module) : Components.UniformStackSpread(module, 5f, default, 3, 3, raidwideOnResolve: false) // TODO: verify enumeration radius
{
