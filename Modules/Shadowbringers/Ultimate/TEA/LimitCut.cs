...
[SkipLocalsInit]
abstract class LimitCut(ModuleBase module, double alphaDelay) : Components.GenericBaitAway(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        {
            var hitIn = Math.Max(default, (float)(_nextHit - World.CurrentTime).TotalSeconds);
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        if (iconID is >= 79u and <= 86u)
        {
            ...
            if (_chaser == null)
            {
                ...
                _nextHit = World.FutureTime(9.5d);
            }
        }
    }
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        switch (spell.Action.ID)
        {
            case (uint)AID.AlphaSwordP2:
                ...
                _nextHit = World.FutureTime(1.5d);
                break;
            ...
            case (uint)AID.SuperBlasstyChargeP3:
                ...
                CurrentBaits.Clear();
                _nextHit = World.FutureTime(_alphaDelay);
                break;
