...
[SkipLocalsInit]
sealed class P4TemporalPrison(ModuleBase module) : Components.GenericTowers(module)
{
        ...
        if (actor.OID == (uint)OID.ArenaFeatures)
        {
            switch (state)
            {
                case 0x04000840u:
                    ...
                    if (position != -1)
                    {
                        Towers.Add(new(Center + 12f * (position * -45f.Degrees() - 180f.Degrees()).ToDirection(), 4f, 1, 1, default, World.FutureTime(8d)));
                    }
                    break;
                case 0x10002000u:
