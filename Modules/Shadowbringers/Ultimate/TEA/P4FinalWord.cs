...
[SkipLocalsInit]
sealed class P4FinalWordDebuffs(ModuleBase module) : P4ForcedMarchDebuffs(module)
{
...
[SkipLocalsInit]
sealed class P4FinalWordStillnessMotion(ModuleBase module) : Components.StayMove(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        if (spell.Action.ID is (uint)AID.OrdainedMotionSuccess or (uint)AID.OrdainedMotionFail or (uint)AID.OrdainedStillnessSuccess or (uint)AID.OrdainedStillnessFail)
        {
            ...
            if (slot >= 0)
            {
                PlayerStates[slot] = PlayerStates[slot].Requirement != _first ? default : new(_first == Requirement.Move ? Requirement.Stay : Requirement.Move, World.FutureTime(11d));
            }
        }
    }
}
