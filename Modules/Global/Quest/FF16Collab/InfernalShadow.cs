...
class VulcanBurst(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.VulcanBurstReal, "Time your dodge correctly");
class Hellfire(ModuleBase module) : Components.RaidwideCastDelay(module, (uint)AID.HellfireVisual, (uint)AID.HellfireRaidwide, 0.6f);
class Incinerate(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.IncinerateReal, 5f);

class SpreadingFire(ModuleBase module) : Components.ConcentricAOEs(module, _shapes)
{
...
abstract class Cone75(ModuleBase module, uint aid) : Components.SimpleAOEs(module, aid, new AOEShapeCone(40f, 75f.Degrees()));
class SmolderingClaw(ModuleBase module) : Cone75(module, (uint)AID.SmolderingClawReal);
class TailStrike(ModuleBase module) : Cone75(module, (uint)AID.TailStrikeReal);

class FireRampageCleave(ModuleBase module) : Components.GenericAOEs(module)
{
...
class FieryRampageCircle(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FieryRampageCircleReal, 16f);
class FieryRampageRaidwide(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.FieryRampageRaidwideReal, "Time your dodge correctly");
class Pyrosault(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PyrosaultReal, 10f);
class Fireball(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FireballReal, 6f);
class CrimsonRush(ModuleBase module) : Components.ChargeAOEs(module, (uint)AID.CrimsonRushReal, 10f);

class CrimsonStreak(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
...
class Eruption(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.EruptionReal, 8f);

class Eruption2(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
...
class BurningStrike(ModuleBase module) : ModuleComponent(module)
{
...
class SearingStomp(ModuleBase module) : ModuleComponent(module)
{
    private bool casting;
...
[ModuleInfo(Group = ModuleGroup.Quest, CFCID = 70334u, NameID = 12564u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")] // also: CFC 959
public class InfernalShadow(WorldState ws, Actor primary) : ModuleBase(ws, primary, default, new ArenaBoundsCircle(20f))
{
