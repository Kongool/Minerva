...
namespace Minerva.Global.Quest.FF15Collab.Iseultalon;
...
class Thunderbolt(ModuleBase module) : Components.UniformStackSpread(module, default, 5f)
{
...
class Electrocution(ModuleBase module) : Components.GenericTowers(module)
{ // Noctis always goes to soak this tower, except on first cast as a tutorial
    ...
    private WPos DeterminePosition(Actor caster, ActorCastInfo spell) => spell.TargetID == caster.InstanceID ? caster.Position : World.Actors.Find(spell.TargetID)?.Position ?? spell.LocXZ;
}

class Electrocution2(ModuleBase module) : Components.CastTowers(module, (uint)AID.Electrocution2, 3f)
{
...
class Stomp(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Stomp);

class DeathRay : Components.SimpleAOEs
{
    public DeathRay(ModuleBase module) : base(module, (uint)AID.DeathRay2, new AOEShapeRect(40f, 1f))
    {
        Color = Colors.Danger;
    }
}

class TailWhip(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TailWhip, new AOEShapeCone(12f, 135f.Degrees()));

class DeathRay2(ModuleBase module) : Components.GenericAOEs(module)
{
...
class NeedleShot(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.NeedleShot, 5f);

class IseultalonStates : StateMachineBuilder
{
    public IseultalonStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.Quest, CFCID = 68695u, NameID = 7895u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
public class Iseultalon(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-289, -30), new ArenaBoundsCircle(25)) // note the arena is actually a 6 sided polygon
{
