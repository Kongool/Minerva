...
namespace Minerva.Global.Quest.FF15Collab.Garuda;
...
class GustFront(ModuleBase module) : Components.UniformStackSpread(module, 1.2f, default)
{
...
class Microburst(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Microburst, 18f)
{
...
class MistralShriek(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MistralShriek, 30f)
{
    private bool casting;
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (World.CurrentTime > done && World.CurrentTime < done.AddSeconds(2d))
            hints.ActionsToExecute.Push(ActionID.MakeSpell(AID.Warpstrike), Module.PrimaryActor, ActionQueue.Priority.High);
    }
}

class MistralSong(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MistralSong, new AOEShapeCone(20f, 75f.Degrees()));
class WickedTornado(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WickedTornado, new AOEShapeDonut(8f, 20f));
class MiniSupercell(ModuleBase module) : Components.LineStack(module, aidMarker: (uint)AID.MiniSupercell, (uint)AID.MiniSupercell2, 5d, 45f, 3f, 2);
class MiniSupercellKB(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.MiniSupercell2, 50f, shape: new AOEShapeRect(45f, 3f), stopAtWall: true);

class GravitationalForce(ModuleBase module) : Components.VoidzoneAtCastTarget(module, 5f, (uint)AID.GravitationalForce2, GetVoidzones, 0f)
{
    private static List<Actor> GetVoidzones(ModuleBase module) => module.Enemies((uint)OID.GravityVoidzone);
}
class MistralGaol(ModuleBase module) : Components.CastHint(module, (uint)AID.MistralGaol, "Prepare for Quick Time Event (spam buttons when it starts)");

class GarudaStates : StateMachineBuilder
{
    public GarudaStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.Quest, CFCID = 68696u, NameID = 7893u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")] // also: CFC 646
public class Garuda(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(0, 0), new ArenaBoundsCircle(22))
{
