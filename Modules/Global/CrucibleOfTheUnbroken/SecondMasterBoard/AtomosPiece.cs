...
namespace Minerva.Global.CrucibleOfTheUnbroken.SecondMasterBoard.AtomosPiece;
...
sealed class ManaManifestation(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ManaManifestation, new AOEShapeCircle(8.0f));
sealed class TerrorEye(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TerrorEye, 6.0f);
sealed class FireIII(ModuleBase module) : Components.StackWithIcon(module, (uint)IconID.FireIIIStack, (uint)AID.FireIII, 6.0f, 8.1f, minStackSize: 1);
sealed class ThunderIII(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ThunderIII, 5.0f);
sealed class ThunderIIISpread(ModuleBase module) : Components.SpreadFromIcon(module, (uint)IconID.ThunderIIISpread, (uint)AID.ThunderIIISpread, 5.0f, 8.1f);
sealed class DarkII(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DarkII, new AOEShapeCone(60.0f, 60.0f.Degrees()));
sealed class ForsakenFount(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.ForsakenFount);
sealed class TailDrive(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TailDrive, new AOEShapeCone(30.0f, 45.0f.Degrees()));
sealed class RottenBreath(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.RottenBreath, new AOEShapeCone(28.0f, 60.0f.Degrees()));

sealed class Shockwave(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.Shockwave, 16.0f) {
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints) {
        ...
        hints.AddForbiddenZone(new SDKnockbackInCircleAwayFromOrigin(Module.Center, knockback.Origin, 16.0f, 19.0f));
    }
}

sealed class WaterIII(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.WaterIII, 20.0f) {
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints) {
        if (Casters.Count == 0) {
            return;
        }
        ...
        hints.AddForbiddenZone(new SDKnockbackInCircleAwayFromOrigin(Module.Center, knockback.Origin, 20.0f, 20.0f));
    }
}

sealed class WaterPuddles(ModuleBase module) : Components.PersistentInvertibleVoidzone(module, 5.0f, GetVoidzones) {
    ...
    public override void Update() {
        ...
        InvertResolveAt = World.CurrentTime;
    }
...
sealed class AethericChargeBig(ModuleBase module) : Components.Voidzone(module, 12.0f, GetVoidzones) {
    private static Actor[] GetVoidzones(ModuleBase module) {
...
sealed class AethericChargeSmall(ModuleBase module) : Components.Voidzone(module, 6.0f, GetVoidzones) {
    private static Actor[] GetVoidzones(ModuleBase module) {
...
sealed class AtomosPieceStates : StateMachineBuilder {
    public AtomosPieceStates(ModuleBase module) : base(module) {
        TrivialPhase()
...
[ModuleInfo(CFCID = 1092u, NameID = 14642u, PrimaryActorOID = (uint)OID.AtomosPiece, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Equilius (ported from BMR)")]
public sealed class AtomosPiece(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(120f, -420f), new ArenaBoundsCircle(20f)) {
