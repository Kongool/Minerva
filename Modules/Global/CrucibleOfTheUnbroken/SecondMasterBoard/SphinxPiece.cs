...
namespace Minerva.Global.CrucibleOfTheUnbroken.SecondMasterBoard.SphinxPiece;
...
sealed class BanishDonut(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BanishDonut, new AOEShapeDonut(10.0f, 60.0f));
sealed class BanishCone(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.BanishCone, (uint)AID.BanishCone2],
    new AOEShapeCone(60.0f, 90.0f.Degrees()));
sealed class BanishCircle(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BanishCircle, 18.0f);
sealed class NumericRiddle(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.NumericRiddle);
...
sealed class Assignment : Components.GenericAOEs {
    ...
    public Assignment(ModuleBase module) : base(module) {
...
sealed class MnemonicRiddle(ModuleBase module) : ModuleComponent(module) {
    private OID? animal;
...
sealed class SphinxPieceStates : StateMachineBuilder {
    public SphinxPieceStates(ModuleBase module) : base(module) {
        TrivialPhase()
...
[ModuleInfo(CFCID = 1092u, NameID = 14665u, PrimaryActorOID = (uint)OID.SphinxPiece, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Equilius (ported from BMR)")]
public sealed class SphinxPiece(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(120f, 0f), new ArenaBoundsRect(20f, 20f)) {
