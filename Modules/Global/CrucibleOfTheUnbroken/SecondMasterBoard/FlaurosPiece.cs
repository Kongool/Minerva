...
namespace Minerva.Global.CrucibleOfTheUnbroken.SecondMasterBoard.FlaurosPiece;
...
sealed class HeatLightning(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HeatLightning, 6.0f);
sealed class LineVoltage(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.LineVoltage, new AOEShapeRect(100.0f, 1.0f));
sealed class LineVoltageBig(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.LineVoltageBig, new AOEShapeRect(100.0f, 3.0f));
sealed class ChargedLightning(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ChargedLightning, new AOEShapeRect(100.0f, 20.0f));
sealed class ElectricShock(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ElectricShock, 16.0f);
sealed class ErraticBlaster(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.ErraticBlaster, "TankBuster + applies Paralysis");

sealed class AddMovement(ModuleBase module) : Components.Adds(module, (uint)OID.LightningSprite) {
...
sealed class FlaurosPieceStates : StateMachineBuilder {
    public FlaurosPieceStates(ModuleBase module) : base(module) {
        TrivialPhase()
...
[ModuleInfo(CFCID = 1092u, NameID = 14631u, PrimaryActorOID = (uint)OID.FlaurosPiece, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Equilius (ported from BMR)")]
public sealed class FlaurosPiece(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(120f, -420f), new ArenaBoundsCircle(20f)) {
