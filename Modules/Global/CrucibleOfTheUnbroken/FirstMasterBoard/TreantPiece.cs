...
namespace Minerva.Global.CrucibleOfTheUnbroken.FirstMasterBoard.TreantPiece;
...
sealed class RustlingBreezeLeftRight(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.RustlingBreezeLeftRight,
        (uint)AID.RustlingBreezeLeftRight1], new AOEShapeCone(60.0f, 75.0f.Degrees()));
sealed class RustlingBreezeMiddle(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.RustlingBreezeMiddle, new AOEShapeCone(60.0f, 45.0f.Degrees()));
sealed class AqueousDischarge(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AqueousDischarge, 5.0f);
sealed class Silkscreen(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Silkscreen, new AOEShapeRect(40.0f, 2.0f));
sealed class AcornBomb(ModuleBase module) : Components.SpreadFromIcon(module, (uint)IconID.AcornBombIcon, (uint)AID.AcornBomb, 3.0f, 5.1f);

sealed class BossPuddle(ModuleBase module) : Components.Voidzone(module, 10.0f, GetVoidzones) {
...
sealed class ArborealStorm(ModuleBase module) : Components.GenericAOEs(module) {
...
sealed class TreantPieceStates : StateMachineBuilder {
    public TreantPieceStates(ModuleBase module) : base(module) {
        TrivialPhase()
...
[ModuleInfo(CFCID = 1091u, NameID = 14618u, PrimaryActorOID = (uint)OID.TreantPiece, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Equilius (ported from BMR)")]
public sealed class TreantPiece(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(120f, -420f), new ArenaBoundsCircle(20f)) {
