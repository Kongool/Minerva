...
namespace Minerva.Global.CrucibleOfTheUnbroken.FirstMasterBoard.ProgenitrixPiece;
...
sealed class ScaldingScolding(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ScaldingScolding, new AOEShapeCone(40.0f, 90.0f.Degrees()));
sealed class MassiveExplosion(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.MassiveExplosion);
sealed class ToxicFumes(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ToxicFumes, new AOEShapeCone(40.0f, 10.0f.Degrees()));
sealed class FireII(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FireII, 6.0f);

sealed class Meltdown(ModuleBase module) : Components.BaitAwayIcon(module, new AOEShapeRect(40.0f, 6.0f), (uint)IconID.MeltdownTankBuster, (uint)AID.Meltdown,
    activationDelay: 7.9d, tankbuster: true);
sealed class MeltdownKnockback(ModuleBase module) : Components.SimpleKnockbacks(module, default, 15.0f, kind: Kind.AwayFromOrigin) {
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID) {
        if (iconID == (uint)IconID.MeltdownTankBuster) {
            var targetPlayer = World.Actors.Find(targetID);
            if (targetPlayer == null) {
                return;
            }

            target = targetPlayer;
            activation = World.FutureTime(7.9f);
        }
    }
...
sealed class FirePuddles(ModuleBase module) : Components.Voidzone(module, 6.0f, GetVoidzones) {
...
sealed class FieryFuryKnockback(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.FieryFuryKnockback, 15.0f, maxCasts: 2);
sealed class FieryFuryAOE : Components.SimpleAOEs {
    public FieryFuryAOE(ModuleBase module) : base(module, (uint)AID.FieryFuryAOE, 6.0f, maxCasts: 2) {
        MaxDangerColor = 1;
    }
}

sealed class ProgenitrixPieceStates : StateMachineBuilder {
    public ProgenitrixPieceStates(ModuleBase module) : base(module) {
        TrivialPhase()
...
[ModuleInfo(CFCID = 1091u, NameID = 14623u, PrimaryActorOID = (uint)OID.ProgenitrixPiece, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Equilius (ported from BMR)")]
public sealed class ProgenitrixPiece(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(120f, -420f), new ArenaBoundsCircle(20f)) {
