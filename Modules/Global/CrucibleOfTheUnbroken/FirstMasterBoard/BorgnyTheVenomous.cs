...
namespace Minerva.Global.CrucibleOfTheUnbroken.FirstMasterBoard.BorgnyTheVenomous;
...
sealed class Cauterize(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Cauterize, new AOEShapeRect(40.0f, 10.0f));
sealed class TouchdownKnockback(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.TouchdownKnockback, 30.0f, stopAtWall: true);
sealed class TouchdownAOE(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TouchdownAOE, 6.0f);
sealed class FumingVomit(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FumingVomit, 6.0f);

sealed class ToxicBreathBoss(ModuleBase module) : Components.GenericAOEs(module) {
...
sealed class ToxicVomit(ModuleBase module) : Components.BaitAwayIcon(module, 6.0f, (uint)IconID.ToxicVomitIcon) {
    public override void OnEventCast(Actor caster, ActorCastEvent spell) {
...
sealed class MagitekArmorPuddles(ModuleBase module) : Components.Voidzone(module, 6.0f, GetVoidzones) {
...
sealed class PoisonClouds(ModuleBase module) : Components.GenericAOEs(module) {
...
sealed class WrigglingPhlegm(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.WrigglingPhlegm, 6.0f);
sealed class WrigglingPhlegmBait(ModuleBase module) : Components.BaitAwayIcon(module, 6.0f, (uint)IconID.WrigglingPhlegmLockOn) {
...
sealed class BorgnyTheVenomousStates : StateMachineBuilder {
    public BorgnyTheVenomousStates(ModuleBase module) : base(module) {
        TrivialPhase()
...
[ModuleInfo(CFCID = 1091u, NameID = 14628u, PrimaryActorOID = (uint)OID.BorgnyTheVenomous, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Equilius (ported from BMR)")]
public sealed class BorgnyTheVenomous(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(920f, -420f), new ArenaBoundsCircle(20f)) {
