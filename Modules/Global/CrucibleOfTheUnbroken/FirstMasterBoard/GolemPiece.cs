...
namespace Minerva.Global.CrucibleOfTheUnbroken.FirstMasterBoard.GolemPiece;
...
sealed class EarthenRing(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.EarthenRing, new AOEShapeDonut(5.0f, 50.0f));
sealed class Plaincracker(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Plaincracker, 20.0f);
sealed class Obliterate(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.Obliterate, 17.0f);
sealed class Outcrop(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Outcrop, new AOEShapeCone(40.0f, 30.0f.Degrees()));
...
sealed class Rockslide(ModuleBase module) : Components.GenericAOEs(module) {
...
sealed class PlaincrackerSwap(ModuleBase module) : Components.GenericAOEs(module) {
    private readonly List<AOEInstance> aoes = [];
...
sealed class SkyRock(ModuleBase module) : Components.GenericAOEs(module) {
    private readonly List<AOEInstance> aoes = [];
...
sealed class GolemPieceStates : StateMachineBuilder {
    public GolemPieceStates(ModuleBase module) : base(module) {
        TrivialPhase()
...
[ModuleInfo(CFCID = 1091u, NameID = 14617u, PrimaryActorOID = (uint)OID.GolemPiece, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Equilius (ported from BMR)")]
public sealed class GolemPiece(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(520f, 0f), new ArenaBoundsRect(20.0f, 15.0f)) {
