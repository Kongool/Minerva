...
namespace Minerva.Global.CrucibleOfTheUnbroken.FirstMasterBoard.MorbolPiece;
...
sealed class AcidMist(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AcidMist, 6.0f);
sealed class Tremblor(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.Tremblor, 10.0f);
sealed class VineProbe(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.VineProbe, new AOEShapeRect(13.0f, 4.0f));

sealed class GreenPuddle(ModuleBase module) : Components.Voidzone(module, 6.0f, GetVoidzones)
{
...
sealed class ExtremelyBadBreathBoss(ModuleBase module) : Components.GenericAOEs(module)
{
...
{
    public MorbolPieceStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 1091u, NameID = 14599u, PrimaryActorOID = (uint)OID.MorbolPiece, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Equilius (ported from BMR)")]
public sealed class MorbolPiece(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(120f, -420f), new ArenaBoundsCircle(20f))
{
