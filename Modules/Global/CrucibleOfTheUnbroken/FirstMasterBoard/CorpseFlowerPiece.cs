...
namespace Minerva.Global.CrucibleOfTheUnbroken.FirstMasterBoard.CorpseFlowerPiece;
...
public class FloralTrap(ModuleBase module) : Components.CastCounter(module, (uint)AID.FloralTrap) {
    ...
    private readonly Func<ModuleBase, IEnumerable<Actor>> Sources = GetVoidzones;
    private bool Inverted => InvertResolveAt != default;

    public override void OnActorEAnim(Actor actor, uint state) {
        if (actor.OID == (uint)OID.ThornPuddle && state == 0x00100020) {
            growPuddlesTimers[actor] = World.CurrentTime;
        }
    }
    ...
    private AOEShapeCircle ShapeSize(Actor source) {
        ...
        var duration = (World.CurrentTime - growPuddle).TotalSeconds;
...
sealed class RottenStench(ModuleBase module) : Components.GenericWildCharge(module, 6.0f, (uint)AID.RottenStenchVisual) {
...
sealed class AcidRainBait(ModuleBase module) : Components.BaitAwayIcon(module, 6.0f, (uint)IconID.AcidRainLockOn) {
    public override void OnCastStarted(Actor caster, ActorCastInfo spell) {
...
sealed class AcidRain(ModuleBase module) : Components.StandardChasingAOEs(module, 6.0f, (uint)AID.AcidRainStart, (uint)AID.AcidRainRest, 5.0f, 1.0d, 8,
    icon: (uint)IconID.AcidRainLockOn);

sealed class Devour(ModuleBase module) : Components.GenericBaitProximity(module) {
...
sealed class CorpseFlowerPieceStates : StateMachineBuilder {
    public CorpseFlowerPieceStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 1091u, NameID = 14603u, PrimaryActorOID = (uint)OID.CorpseFlowerPiece, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Equilius (ported from BMR)")]
public sealed class CorpseFlowerPiece(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(120f, -420f), new ArenaBoundsCircle(20f)) {
