...
namespace Minerva.Modules.Global.CrucibleOfTheUnbroken.FirstMasterBoard.GargoylePiece;
...
sealed class Desolation(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Desolation, new AOEShapeRect(60.0f, 3.5f));
sealed class GrimFate(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.GrimFate);
sealed class SeaOfPitch(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SeaOfPitch, 6.0f);

sealed class RipplingEvisceration(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class SweepingEviscerationTether(ModuleBase module) : Components.StretchTetherDuo(module, 21.0f, 8.1f);
sealed class SweepingEvisceration(ModuleBase module) : Components.GenericBaitAway(module)
{
    ...
    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        ...
        tetherSource = source;
        tetherTarget = target;
        activation = World.FutureTime(11.0f);
    }
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        if (spell.Action.ID == (uint)AID.SweepingEviscerationTeleport)
        {
            ...
            CurrentBaits.Add(new(newPosition, tetherSource, shape, World.FutureTime(2.8f), customRotation: spell.Rotation));
            CurrentBaits.Add(new(newPosition, tetherSource, shape, World.FutureTime(4.8f), customRotation: spell.Rotation - 180.0f.Degrees()));
        }
    }
...
sealed class Malady(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Malady, 6.0f);
sealed class MaladyOrbs : Components.PersistentInvertibleVoidzone
{
    private static List<Actor> orbs = [];

    public MaladyOrbs(ModuleBase module) : base(module, 2.0f, GetVoidzones)
    {
    ...
    private static List<Actor> GetVoidzones(ModuleBase module) => orbs;
}

sealed class PurpleVoidZone(ModuleBase module) : Components.Voidzone(module, 6.0f, GetVoidzones)
{
...
sealed class FivefoldFallout(ModuleBase module) : Components.GenericKnockback(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        {
            knockbacks.Add(new(spell.LocXZ, 20.0f, World.FutureTime(11.8d), kind: Kind.AwayFromOrigin));
        }
    }
    ...
    public override ReadOnlySpan<Knockback> ActiveKnockbacks(int slot, Actor actor)
    {
        ...
        if ((knockback.Activation - World.CurrentTime).TotalSeconds < 6.0d)
        {
...
{
    public GargoylePieceStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 1091u, NameID = 14608u, PrimaryActorOID = (uint)OID.GargoylePiece, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Equilius (ported from BMR)")]
public sealed class GargoylePiece(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(120f, 0f), new ArenaBoundsSquare(20f));
