...
namespace Minerva.Global.CrucibleOfTheUnbroken.FirstMasterBoard.AdministratorPiece;
...
sealed class AutoCannons(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AutoCannons, new AOEShapeRect(40.0f, 2.5f));
sealed class Object199(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Object199);
sealed class Headspin(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Headspin, 5.0f);
sealed class FlameThrower(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FlameThrower, new AOEShapeCone(40.0f, 60.0f.Degrees()));
sealed class NoxiousNova(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.NoxiousNova, "Applies poison!");
sealed class Confound(ModuleBase module) : Components.CastInterruptHint(module, (uint)AID.Confound, showNameInHint: true);

sealed class PiercingLaser(ModuleBase module) : Components.GenericWildCharge(module, 2.5f, (uint)AID.PiercingLaserBoss, 40.0f) {
...
sealed class AdministratorPieceStates : StateMachineBuilder {
    public AdministratorPieceStates(ModuleBase module) : base(module) {
        TrivialPhase()
...
[ModuleInfo(CFCID = 1091u, NameID = 14610u, PrimaryActorOID = (uint)OID.AdministratorPiece, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Equilius (ported from BMR)")]
public sealed class AdministratorPiece(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(120f, -420f), new ArenaBoundsCircle(20f)) {
