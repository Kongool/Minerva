...
namespace Minerva.Global.CrucibleOfTheUnbroken.SecondBoard.MantiCorePiece;
...
sealed class ArmAndHammer(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.ArmAndHammerLeft, (uint)AID.ArmAndHammerRight],
    new AOEShapeCone(30.0f, 90.0f.Degrees()));
sealed class DeadlyHold(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.DeadlyHold);
sealed class Hammerleap(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Hammerleap, 30.0f);
sealed class HeadsAndTails(ModuleBase module) : Components.SimpleAOEGroups(module,
...
sealed class WildCharge(ModuleBase module) : Components.GenericAOEs(module)
{
...
{
    public ManticorePieceStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 1089u, NameID = 14545u, PrimaryActorOID = (uint)OID.ManticorePiece, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Equilius (ported from BMR)")]
public sealed class ManticorePiece(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(120f, -420f), new ArenaBoundsCircle(20f));
