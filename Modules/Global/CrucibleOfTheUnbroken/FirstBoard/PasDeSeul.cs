...
namespace Minerva.Global.CrucibleOfTheUnbroken.FirstBoard.PasDeSeul;
...
sealed class BloodRainCircle(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BloodRainCircle, 8f);
sealed class BloodRainDonut(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BloodRainDonut, new AOEShapeDonut(8f, 40f));

sealed class VoidAeroIIRect(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.VoidAeroII, new AOEShapeRect(60f, 4f));

sealed class VoidAeroIICone(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.VoidAeroII1, new AOEShapeCone(60f, 10f.Degrees()));

//Prioritizing mage add higher than the knight. Mage Gives Damage up buffs to boss.
sealed class SuccubusMageAdd(ModuleBase module) : Components.Adds(module, (uint)OID.SuccubusMage, 2);
sealed class SuccubusKnightAdd(ModuleBase module) : Components.Adds(module, (uint)OID.SuccubusKnight, 1);

sealed class VoidFireII(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.VoidFireII, 10f);

//HeartShatter1 = 46938, // 233C->self, 1.0s cast, range 24 circle
sealed class HeartShatter(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        if (actor.OID == (uint)OID.Pheromone)
        {
            ...
            if (_orbs.Count == 2)
            {
                ...
                var activation = World.FutureTime(13.5d);
...
//SweetSteel = 46930, // SuccubusKnight->self, 6.0s cast, range 10 120.000-degree cone
sealed class SweetSteel(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SweetSteel, new AOEShapeCone(10f, 60f.Degrees()));

sealed class PasDeSeulStates : StateMachineBuilder
{
    public PasDeSeulStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 1088u, NameID = 14541u, PrimaryActorOID = (uint)OID.PasDeSeul, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "wen (ported from BMR)")]
public sealed class PasDeSeul : ModuleBase
{
