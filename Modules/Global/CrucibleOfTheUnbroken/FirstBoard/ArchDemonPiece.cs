...
namespace Minerva.Global.CrucibleOfTheUnbroken.FirstBoard.ArchDemonPiece;
...
sealed class AbyssalLanceAdds(ModuleBase module) : Components.AddsPointless(module, (uint)OID.AbyssalLance);

//AbyssalLance shoots narrow laser rectangles towards pc.
sealed class AbyssalCharge(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AbyssalCharge, new AOEShapeRect(40f, 2f));
...
sealed class Dismember(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Dismember, new AOEShapeRect(35f, 4f), 1);

// Laser swords that appear above a r6 circle and step down into the ground.
sealed class AbyssalTransfixion(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AbyssalTransfixion1, 6f);

// Laser swords that appear above the aoe circle and stab back down where pc was standing.
sealed class AbyssalTransfixion2(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AbyssalTransfixion2, 3f);

// Laser axe swing that covers half the arena.
sealed class AbyssalSwing(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AbyssalSwing1, new AOEShapeCone(40f, 90f.Degrees()));

sealed class ArchDemonPieceStates : StateMachineBuilder
{
    public ArchDemonPieceStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 1088u, NameID = 14533u, PrimaryActorOID = (uint)OID.ArchDemonPiece, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "wen (ported from BMR)")]

public sealed class ArchDemonPiece(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(520f, 0f), new ArenaBoundsRect(20f, 14.8f));
