...
namespace Minerva.Global.CrucibleOfTheUnbroken.FirstBoard.BanemitePiece;
...
sealed class BedrockUplift(ModuleBase module) : Components.ConcentricAOEs(module, [new AOEShapeCircle(6f), new AOEShapeDonut(6f, 12f),
...
sealed class DeadlyThrust(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.DeadlyThrust, 2f, tankbuster: true);

// Aoe circles that drop on center then around the outside and explode one after another
sealed class VenomWeb(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.VenomWeb1, 9f, 6);

sealed class MitelingAdds(ModuleBase module) : Components.Adds(module, (uint)OID.MitelingPiece);

//Silkscreen = 46909, // MitelingPiece->self, 5.0s cast, range 40 width 4 rect
sealed class Silkscreen(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Silkscreen, new AOEShapeRect(40f, 2f));

sealed class BanemitePieceStates : StateMachineBuilder
{
    public BanemitePieceStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 1088u, NameID = 14536u, PrimaryActorOID = (uint)OID.BanemitePiece, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "wen (ported from BMR)")]

public sealed class BanemitePiece(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(120f, -420f), new ArenaBoundsCircle(20f));
