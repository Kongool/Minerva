...
namespace Minerva.Global.CrucibleOfTheUnbroken.ThirdBoard.CampeadorPiece;
...
sealed class HealingWater(ModuleBase module) : Components.Adds(module, (uint)OID.FlowertenderPiece, 4)
{
...
sealed class Needles(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Needles, 15.0f, maxCasts: 3);
sealed class SeedingNeedlesRaidwide(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.SeedingNeedlesRaidwide);
sealed class SeedingNeedlesCircle(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SeedingNeedlesCircle, 9.0f);
sealed class NeedleGnarl(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.NeedleGnarl, 8.0f);
sealed class SeedingNeedlesDonut(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SeedingNeedlesDonut, new AOEShapeDonut(5.0f, 40.0f));

sealed class CampeadorPieceStates : StateMachineBuilder
{
    public CampeadorPieceStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 1090u, NameID = 14587u, PrimaryActorOID = (uint)OID.CampeadorPiece, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Equilius (ported from BMR)")]
public sealed class CampeadorPiece(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(120f, -420f), new ArenaBoundsCircle(20f))
{
