...
namespace Minerva.Global.CrucibleOfTheUnbroken.ThirdBoard.ZuPiece;
...
sealed class Crossbreeze(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Crossbreeze, new AOEShapeCross(50.0f, 4.0f));
sealed class AiryPursuit(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AiryPursuitAOE, 6.0f);
sealed class CarveTether(ModuleBase module) : Components.StretchTetherDuo(module, 16.0f, 5.0f);

sealed class CrossbreezeBait(ModuleBase module) : Components.BaitAwayIcon(module, new AOEShapeCross(50.0f, 4.0f), (uint)IconID.Crossbreeze,
    (uint)AID.CrossbreezeIcon, 8.1f, centerAtTarget: true)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        ...
        {
            CurrentBaits.Add(new(source, World.Actors.Find(targetID) ?? actor, new AOEShapeCross(50.0f, 4.0f), World.FutureTime(ActivationDelay), customRotation: 0.0f.Degrees(), arenaProjectionLayer: ArenaProjectionLayer, restrictToArenaProjectionLayer: RestrictToArenaProjectionLayer));
        }
    }
...
sealed class FlyingFrenzy(ModuleBase module) : Components.BaitAwayIcon(module, 6.0f, (uint)IconID.TankBuster, (uint)AID.FlyingFrenzyAOE, activationDelay: 8.1f,
    tankbuster: true)
{
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
...
sealed class AiryPursuitPuddles(ModuleBase module) : Components.StandardChasingAOEs(module, 6.0f, (uint)AID.AiryPursuitAOE, (uint)AID.AiryPursuitTeleport, 3.5f,
...
sealed class AiryPursuitBait(ModuleBase module) : Components.BaitAwayIcon(module, 6.0f, (uint)IconID.AiryPursuit)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
...
sealed class Carve(ModuleBase module) : Components.GenericAOEs(module)
{
...
{
    public ZuPieceStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 1090u, NameID = 14572u, PrimaryActorOID = (uint)OID.ZuPiece, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Equilius (ported from BMR)")]
public sealed class ZuPiece(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(120f, -420f), new ArenaBoundsCircle(20f))
{
