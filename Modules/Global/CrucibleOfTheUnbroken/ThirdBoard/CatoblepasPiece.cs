...
namespace Minerva.Global.CrucibleOfTheUnbroken.ThirdBoard.CatoblepasPiece;
...
sealed class BestialRoar(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.BestialRoar);
sealed class SinisterGleam(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SinisterGleam, new AOEShapeCone(60f, 90f.Degrees()));
sealed class DemonicEye(ModuleBase module) : Components.Voidzone(module, 2f, GetEyes, 2f)
{
    private static Actor[] GetEyes(ModuleBase module)
    {
...
sealed class DemonicEyeCircle(ModuleBase module) : Components.Voidzone(module, 2f, module => module.Enemies((uint)OID.DemonicEyeCircle).Where(z => z.Renderflags == 0), 2f);
sealed class DemonicEyeDonut(ModuleBase module) : Components.Voidzone(module, 2f, module => module.Enemies((uint)OID.DemonicEyeDonut).Where(z => z.Renderflags == 0), 2f);
sealed class NearFarburst(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void Update()
    {
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            var activation = World.CurrentTime.AddSeconds(19d);
            var startrot = (start - Module.Center).ToAngle();
            var currot = (cur - Module.Center).ToAngle();
            ...
            for (var j = 0; j < 8; j++)
            {
                ...
                {
                    var finalPos = Module.Center + (angle + 135f.Degrees() * (ccw ? 1f : -1f)).ToDirection() * 20f;
...
sealed class FalseDemonEye(ModuleBase module) : Components.GenericGaze(module, (uint)AID.FalseDemonEye)
{
...
{
    public CatoblepasPieceStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 1090u, NameID = 14577u, PrimaryActorOID = (uint)OID.CatoblepasPiece, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "gynorhino (ported from BMR)")]
public sealed class CatoblepasPiece(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(120f, -420f), new ArenaBoundsCircle(20f));
