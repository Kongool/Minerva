...
namespace Minerva.Global.CrucibleOfTheUnbroken.ThirdBoard.SirenPiece;
...
sealed class SongOfTorment(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.SongOfTorment);
sealed class FeralLunge(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FeralLunge, new AOEShapeRect(50.0f, 8.0f));
sealed class DeadMansDirgeOuter(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DeadMansDirgeOuter, 12.0f);
sealed class DeadMansDirgeInner(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DeadMansDirgeInner, new AOEShapeDonut(3.0f, 43.0f));
sealed class Wallop(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Wallop, new AOEShapeRect(5.0f, 1.5f));

sealed class UnmooringMelody(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        {
            aoes.Add(new(shape, spell.TargetXZ, Angle.FromDirection(Module.Center - spell.TargetXZ)));
        }
...
sealed class Burst(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        {
            aoes.Add(new(shape, actor.Position, actor.Rotation, World.FutureTime(5.7f)));
        }
    }
...
sealed class InvitingVerse(ModuleBase module) : Components.StatusDrivenForcedMarch(module, 3.0f, (uint)SID.ForwardMarch, (uint)SID.AboutFace, (uint)SID.LeftFace,
    (uint)SID.RightFace)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        var outwards = Angle.FromDirection(actor.Position - Module.Center) - move0.dir;
...
{
    public SirenPieceStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 1090u, NameID = 14583u, PrimaryActorOID = (uint)OID.SirenPiece, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Equilius (ported from BMR)")]
public sealed class SirenPiece(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(120f, -420f), new ArenaBoundsCircle(20f))
{
