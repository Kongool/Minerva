...
namespace Minerva.Global.CrucibleOfTheUnbroken.ThirdBoard.GuttlerTheGutter;
...
sealed class BeastlyFlare(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BeastlyFlare, 35.0f);
...
sealed class DeadlyDemesne(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        {
            aoes.Add(new(shape, actor.Position, actor.Rotation, World.FutureTime(12.7f)));
...
sealed class MoltenMetalBait(ModuleBase module) : Components.BaitAwayIcon(module, new AOEShapeCircle(6.0f), (uint)IconID.MoltenMetal, centerAtTarget: true)
{
...
sealed class OverpoweringPoint(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> aoes = [];
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        {
            aoes.Add(new(cone, Module.Center, Angle.AnglesCardinals[0]));
            aoes.Add(new(cone, Module.Center, Angle.AnglesCardinals[3]));
...
{
    public GuttlerTheGutterStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 1090u, NameID = 14592u, PrimaryActorOID = (uint)OID.GuttlerTheGutter, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Equilius (ported from BMR)")]
public sealed class GuttlerTheGutter : ModuleBase
{
