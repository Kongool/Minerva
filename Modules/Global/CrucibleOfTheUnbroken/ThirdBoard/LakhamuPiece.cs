...
namespace Minerva.Global.CrucibleOfTheUnbroken.ThirdBoard.LakhamuPiece;
...
sealed class Earthrender(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Earthrender, 6.0f);
sealed class SandTempest(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.SandTempest, "Applies blind debuff");

// Eventcast is triggered but the bait actually resolves a little bit later, so a timer of 1.0 second is added
sealed class EarthShaker(ModuleBase module) : Components.BaitAwayIcon(module, new AOEShapeCone(60.0f, 75.0f.Degrees()), (uint)IconID.EarthShake,
    (uint)AID.EarthShaker, 3.3f)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        {
            eventCastActivation = World.CurrentTime;
        }
    }
    ...
    public override void Update()
    {
        ...
        if (eventCastActivation.Value.AddSeconds(additionalActivationTime) > World.CurrentTime)
        {
            return;
        }
...
sealed class Burst(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        {
            aoes.Add(new(shape, actor.Position, actor.Rotation, World.FutureTime(5.9f)));
        }
    }
...
sealed class Rockslide(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.Rockslide)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        {
            aoes.Add(new(shape, actor.Position, actor.Rotation, World.FutureTime(9.7f)));
        }
    }
...
sealed class Landslip(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.Landslip, 20.0f, shape: new AOEShapeRect(45.0f, 5.0f), maxCasts: 1,
    kind: Kind.DirForward)
{
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        ...
        for (var y = 0; y < tiles; y++)
        {
            ...
            {
                // Checking each tile
                var tile = Module.Center + new WDir((x - tilesFromCenter + 0.5f) * tileSize, (y - tilesFromCenter + 0.5f) * tileSize); // center of tile
                ...
                foreach (var knockback in Casters)
                {
                    ...
                    if (!Module.InBounds(destination))
                    {
                        continue;
                    }
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        for (var i = 0; i < knockbacks.Length; i++)
        {
            ...
            hints.AddForbiddenZone(new SDIntersection([
                    ...
                    new SDKnockbackInAABBSquareFixedDirectionPlusMixedAOEs(Module.Center, direction,  19.0f, [.. rockslide.aoes], rockslide.aoes.Count)]),
...
{
    public LakhamuPieceStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 1090u, NameID = 14580u, PrimaryActorOID = (uint)OID.LakhamuPiece, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Equilius (ported from BMR)")]
public sealed class LakhamuPiece(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(120f, 0f), new ArenaBoundsRect(20f, 20f))
{
