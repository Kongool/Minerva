...
namespace Minerva.Global.GoldSaucer.SliceIsRight;
...
sealed class BambooSplits(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        {
            _aoes.Add(new(bamboospawn, actor.Position.Quantized(), default, World.FutureTime(2.7d)));
        }
    }
    ...
    public override void OnActorEAnim(Actor actor, uint state)
    {
        if (state == 0x00010002)
        {
            void AddAOE(AOEShape shape, Angle offset) => _aoes.Add(new(shape, actor.Position.Quantized(), actor.Rotation + offset, World.FutureTime(7d)));
            switch (actor.OID)
            {
...
sealed class DaigoroGilJump(ModuleBase module) : Components.SimpleChargeAOEGroups(module, [(uint)AID.FirstGilJump, (uint)AID.NextGilJump], 3.5f, extraLengthFront: 5f);

sealed class TheSliceIsRightStates : StateMachineBuilder
{
    public TheSliceIsRightStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.GoldSaucer, GroupID = 181u, CFCID = 181u, NameID = 9066u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
public sealed class TheSliceIsRight(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
