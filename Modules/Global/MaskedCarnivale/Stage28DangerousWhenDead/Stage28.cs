...
namespace Minerva.Global.MaskedCarnivale.Stage28;
...
sealed class DoomImpending(ModuleBase module) : Components.CastHint(module, (uint)AID.DoomImpending, "Heal to full before cast ends!");
sealed class MarchOfTheDraugar(ModuleBase module) : Components.CastHint(module, (uint)AID.MarchOfTheDraugar, "Summons adds! (Kill with fire!)");
sealed class NecrobaneVoidzone(ModuleBase module) : Components.PersistentInvertibleVoidzoneByCast(module, 6f, GetVoidzones, (uint)AID.MegaDeath)
{
...
sealed class NecrobaneCatapultVengefulSoul(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.Necrobane, (uint)AID.Catapult, (uint)AID.VengefulSoul], 6f);
sealed class HelblarShriekFuneralPyre(ModuleBase module) : Components.RaidwideCasts(module, [(uint)AID.HelblarShriek, (uint)AID.FuneralPyre]);
sealed class BilrostSquall(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BilrostSquall, 10f);
sealed class Cackle(ModuleBase module) : Components.CastInterruptHint(module, (uint)AID.Cackle);

sealed class Brainstorm(ModuleBase module) : Components.StatusDrivenForcedMarch(module, 2f, (uint)SID.ForwardMarch, (uint)SID.AboutFace, (uint)SID.LeftFace, (uint)SID.RightFace)
{
...
{
    public Stage28States(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.MaskedCarnivale, GroupID = 697u, CFCID = 697u, NameID = 9233u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
public sealed class Stage28 : ModuleBase
{
