...
namespace Minerva.Global.MaskedCarnivale.Stage27;
...
sealed class Fireball(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Fireball, 8f);
sealed class Snort(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.Snort, 15f, stopAtWall: true);

sealed class Fungah(ModuleBase module) : Components.GenericKnockback(module, stopAtWall: true)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        if (actor.OID == (uint)OID.Bomb && Module.Enemies((uint)OID.Bomb).Count == 8)
        {
            ...
            {
                _activation = World.FutureTime(5.3d);
                otherpatterns = true;
            }
        }
    }
...
sealed class Explosion(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        ...
        else if (_snortingeffectends > World.CurrentTime)
        {
            ...
            for (var i = 0; i < count; ++i)
            {
                ...
                aoes[i] = new(circle, pos + Math.Min(15f, Module.IntersectRayBounds(pos, raydir) - 0.4f) * raydir, default, _activation); // 0.4f = half of hitbox radius is used for some reason
            }
            return aoes;
        }
        return [];
    ...
    {
        if (_snortingeffectends != default && _snortingeffectends < World.CurrentTime)
        {
...
sealed class Hints2(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void AddHints(int slot, Actor actor, TextHints hints)
    {
        ...
        {
            hints.Add($"Explosion in ca.: {Math.Max(35f - (World.CurrentTime - _activation).TotalSeconds, 0.0f):f1}s");
        }
    }
...
{
    public Stage27States(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.MaskedCarnivale, GroupID = 696u, CFCID = 696u, NameID = 3046u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
public sealed class Stage27 : ModuleBase
{
