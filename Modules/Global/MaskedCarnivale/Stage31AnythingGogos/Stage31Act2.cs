...
namespace Minerva.Global.MaskedCarnivale.Stage31.Act2;
...
sealed class GogoFlareHoly(ModuleBase module) : Components.RaidwideCasts(module, [(uint)AID.GogoFlare, (uint)AID.GogoHoly]);
sealed class GogoMeteorSmall(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GogoMeteor1, 5f);
sealed class GogoMeteorMedCharybdis(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.GogoMeteor3, (uint)AID.GogoMeteor4, (uint)AID.Charybdis], 8f);
sealed class GogoMeteorLarge(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.GogoMeteor2, (uint)AID.GogoMeteor5], 16f);
sealed class GogoMeteorBig(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.GogoMeteor6, "Use Diamondback!");
sealed class Icestorm(ModuleBase module) : Components.RaidwideCastDelay(module, (uint)AID.Icestorm, (uint)AID.Icestorm2, 0.9d, "Raidwide + Frostbite + Heavy");
sealed class ThunderIII(ModuleBase module) : Components.VoidzoneAtCastTarget(module, 6f, (uint)AID.GogoThunderIII, GetVoidzones, 0.8d)
{
...
sealed class GogoBlizzardIII(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class GogoFireIIIHint(ModuleBase module) : Components.CastHint(module, (uint)AID.GogoFireIII, "Pyretic, dodge AOE then stop everything!");

sealed class Pyretic(ModuleBase module) : Components.StayMove(module)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID == (uint)AID.GogoFireIII)
...
{
    public Stage31Act2States(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.MaskedCarnivale, GroupID = 754u, CFCID = 754u, NameID = 9908u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
public sealed class Stage31Act2(WorldState ws, Actor primary) : ModuleBase(ws, primary, Layouts.ArenaCenter, Layouts.CircleSmall);
