...
namespace Minerva.Global.MaskedCarnivale.Stage20.Act3;
...
sealed class TentacleFireball(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.Tentacle, (uint)AID.Fireball], 8f);
sealed class Wallop(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Wallop, Rect)
{
    public static readonly AOEShapeRect Rect = new(57.2f, 5f);
}
sealed class WallopKB(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.Wallop, 20f, shape: Wallop.Rect);
sealed class ImpSong(ModuleBase module) : Components.CastInterruptHint(module, (uint)AID.ImpSong, showNameInHint: true);
sealed class Snort(ModuleBase module) : Components.CastHint(module, (uint)AID.Snort, "Use Diamondback!");
sealed class SnortKB(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.Snort, 30f);
...
{
    public Stage20Act3States(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.MaskedCarnivale, GroupID = 630u, CFCID = 630u, NameID = 3046u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
public sealed class Stage20Act3 : ModuleBase
{
