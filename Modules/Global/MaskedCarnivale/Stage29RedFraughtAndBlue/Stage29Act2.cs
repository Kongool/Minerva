...
namespace Minerva.Global.MaskedCarnivale.Stage29.Act2;
...
sealed class BigSplash(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.BigSplashFirst, "Diamondback! (Multiple raidwides + knockbacks)");
sealed class BigSplashKB(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.BigSplashFirst, 25f);
sealed class Cascade(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Cascade, "Raidwide + Tornados spawn");
sealed class WateryGrasp(ModuleBase module) : Components.CastHint(module, (uint)AID.WateryGrasp, "Spawns hands. Focus left hand first.");
sealed class Throttle(ModuleBase module) : Components.CastHint(module, (uint)AID.Throttle, "Prepare to use Excuviation to remove debuff");
sealed class FluidSwing(ModuleBase module) : Components.CastInterruptHint(module, (uint)AID.FluidSwing);
sealed class FluidSwingKnockback(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.FluidSwing, 50f, kind: Kind.DirForward);

sealed class ProteanWave(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.ProteanWave1, (uint)AID.ProteanWave3], new AOEShapeCone(39f, 15f.Degrees()));

sealed class KnockbackPull(ModuleBase module) : Components.GenericKnockback(module)
{
...
sealed class Unwind(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Unwind, 10f);
sealed class FluidBall(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FluidBall, 5f);

sealed class FluidConvectionDynamic(ModuleBase module) : Components.GenericAOEs(module)
{
...
{
    public Stage29Act2States(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.MaskedCarnivale, GroupID = 698u, CFCID = 698u, NameID = 9241u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
public sealed class Stage29Act2 : ModuleBase
{
