...
namespace Minerva.Global.MaskedCarnivale.Stage29.Act1;
...
sealed class SeaOfFlames(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SeaOfFlames, 6f);
sealed class FireII(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FireII, 5f);
sealed class PillarOfFlame(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.PillarOfFlame1, (uint)AID.PillarOfFlame2], 8f);
sealed class Rush(ModuleBase module) : Components.CastHint(module, (uint)AID.Rush, "GTFO from boss! (Distance based charge)");
sealed class FlareStar(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FlareStar, 10f);
sealed class FireBlast(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FireBlast, new AOEShapeRect(74f, 2f));
sealed class PyreticHint(ModuleBase module) : Components.CastHint(module, (uint)AID.Pyretic, "Pyretic, stop everything! Dodge the AOE after it runs out.");
...
{
    public Stage29Act1States(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.MaskedCarnivale, GroupID = 698u, CFCID = 698u, NameID = 9239u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
public sealed class Stage29Act1 : ModuleBase
{
