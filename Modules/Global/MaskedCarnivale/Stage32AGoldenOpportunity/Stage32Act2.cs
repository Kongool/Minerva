...
namespace Minerva.Global.MaskedCarnivale.Stage32.Act2;
...
sealed class GoldorFireIII(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GoldorFireIII, 8f);
sealed class GoldorBlast(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GoldorBlast, new AOEShapeRect(60f, 5f));
sealed class GoldenCross(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GoldenCross, new AOEShapeCross(100f, 3.5f));

sealed class GoldenBeam(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class TwentyFourCaratSwing(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TwentyFourCaratSwing, 12f);

sealed class GoldorQuake(ModuleBase module) : Components.ConcentricAOEs(module, [new AOEShapeCircle(10f), new AOEShapeDonut(10f, 20f), new AOEShapeDonut(20f, 30f)])
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
    ...
    public override void OnCastFinished(Actor caster, ActorCastInfo spell)
    {
        if (Sequences.Count != 0)
        {
            ...
            AdvanceSequence(order, spell.LocXZ, World.FutureTime(1.5d));
        }
    }
}

sealed class GoldorAeroIII(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.GoldorAeroIII, 10f)
{
...
sealed class GoldorAeroIIIGoldorRushRaidwide(ModuleBase module) : Components.RaidwideCasts(module, [(uint)AID.GoldorAeroIII, (uint)AID.GoldorRush]);
sealed class GoldorRush(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.GoldorRush, 10f);
sealed class TwentyFourCaratInhale(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.TwentyFourCaratInhale, 30f, kind: Kind.TowardsOrigin);
sealed class GoldorGravity(ModuleBase module) : Components.RaidwideCastDelay(module, (uint)AID.GoldorGravity, (uint)AID.GoldorGravity2, 0.8d, "Dmg + Heavy debuff");
sealed class GoldorThunderIII(ModuleBase module) : Components.RaidwideCastDelay(module, (uint)AID.GoldorThunderIIIVisual, (uint)AID.GoldorThunderIII1, 0.8d, "Prepare to cleanse Electrocution");
sealed class GoldorThunderIII2(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GoldorThunderIII2, 6f);
sealed class GoldorBlizzardIII(ModuleBase module) : Components.CastInterruptHint(module, (uint)AID.GoldorBlizzardIIIVisual);
...
{
    public Stage32Act2States(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.MaskedCarnivale, GroupID = 948u, CFCID = 948u, NameID = 12471u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
public sealed class Stage32Act2(WorldState ws, Actor primary) : ModuleBase(ws, primary, Layouts.ArenaCenter, Layouts.CircleSmall);
