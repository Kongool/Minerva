...
namespace Minerva.Global.MaskedCarnivale.Stage24.Act2;
...
sealed class Starstorm(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Starstorm, 5f);
sealed class Mechanogravity(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Mechanogravity, 6f);
sealed class RagingAxe(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.RagingAxe, new AOEShapeCone(5f, 45f.Degrees()));
sealed class CondensedLibraTripleHit(ModuleBase module) : Components.SingleTargetCasts(module, [(uint)AID.CondensedLibra, (uint)AID.TripleHit], "Use Diamondback!");
sealed class Silence(ModuleBase module) : Components.CastInterruptHint(module, (uint)AID.Silence);
...
{
    public Stage24Act2States(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.MaskedCarnivale, GroupID = 634u, CFCID = 634u, NameID = 8128u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
public sealed class Stage24Act2 : ModuleBase
{
