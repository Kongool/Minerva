...
namespace Minerva.Global.MaskedCarnivale.Stage30.Act3;
...
sealed class MagicDrain(ModuleBase module) : Components.CastHint(module, (uint)AID.MagicDrain, "Reflect magic damage for 30s");
sealed class Hyperdrive(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.HyperdriveFirst, (uint)AID.HyperdriveRest], 5f);
sealed class AnkleGraze(ModuleBase module) : Components.CastHint(module, (uint)AID.AnkleGraze, "Applies bind, prepare to use Excuviation!");

sealed class LawOfTheTorch(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.LawOfTheTorch1, (uint)AID.LawOfTheTorch2], new AOEShapeCone(34f, 10f.Degrees()));

sealed class SwiftsteelKB(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.SwiftsteelKB, 10f)
{
...
sealed class Swiftsteel1(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Swiftsteel1, 4f);
sealed class Swiftsteel2(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Swiftsteel2, new AOEShapeDonut(8f, 20f));

sealed class SparksteelVoidzone(ModuleBase module) : Components.VoidzoneAtCastTarget(module, 6f, (uint)AID.Sparksteel1, GetVoidzones, 0.8d)
{
...
{
    public SparksteelAOE(ModuleBase module) : base(module, [(uint)AID.Sparksteel2, (uint)AID.Sparksteel3], 8f, expectedNumCasters: 8)
    {
        MaxDangerColor = 4;
    }
}

sealed class Shattersteel(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Shattersteel, 5f);
sealed class SphereShatter(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        if (actor.OID == (uint)OID.IceBoulder)
        {
            ...
            _aoes.Add(new(circle, pos, default, World.FutureTime(8.4d), shapeDistance: circle.Distance(pos, default)));
        }
    }
...
sealed class RubberBullet(ModuleBase module) : Components.GenericKnockback(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        {
            _kb = [new(Module.PrimaryActor.Position, 20f, World.FutureTime(6.3d))];
        }
    }
...
sealed class Explosion(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        {
            var pos = actor.Position.Quantized();
            AOEs.Add(new(circle, pos, default, World.FutureTime(8.4d), shapeDistance: circle.Distance(pos, default)));
        }
    }
...
{
    public Stage30Act3States(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.MaskedCarnivale, GroupID = 699u, CFCID = 699u, NameID = 9245u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
public sealed class Stage30Act3(WorldState ws, Actor primary) : ModuleBase(ws, primary, Layouts.ArenaCenter, Layouts.CircleSmall)
{
