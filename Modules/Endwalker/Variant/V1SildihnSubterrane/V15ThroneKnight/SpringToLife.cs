...
sealed class SpringToLife(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        switch (spell.Action.ID)
        {
            case (uint)AID.ActivatePuppetVisual:
                _aoes.Add(new(rect, caster.Position.Quantized(), spell.Rotation, World.FutureTime(2.9d)));
                break;
