...
namespace Minerva.Endwalker.Hunt.RankS.Ker;
...
sealed class MinaxGlare(ModuleBase module) : Components.TemporaryMisdirection(module, (uint)AID.MinaxGlare);
sealed class Heliovoid(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Heliovoid, 12f);

sealed class AncientBlizzard(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.AncientBlizzard1, (uint)AID.AncientBlizzard2, (uint)AID.WhispersManifest3], new AOEShapeDonut(8f, 40f));
sealed class AncientHoly(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.AncientHoly1, (uint)AID.AncientHoly2, (uint)AID.WhispersManifest2], 20f);

sealed class Interment(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.ForeInterment, (uint)AID.RearInterment, (uint)AID.RightInterment, (uint)AID.LeftInterment,
...
sealed class EternalDamnation(ModuleBase module) : Components.CastGazes(module, [(uint)AID.EternalDamnation1, (uint)AID.EternalDamnation2, (uint)AID.WhispersManifest4]);

sealed class WhisperedIncantation(ModuleBase module) : Components.CastHint(module, (uint)AID.WhisperedIncantation, "Remembers the next skill and uses it again when casting Whispers Manifest");

sealed class MirroredIncantation(ModuleBase module) : ModuleComponent(module)
{
...
sealed class AncientFlare(ModuleBase module) : Components.StayMove(module, 2.5d)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        if (spell.Action.ID is (uint)AID.AncientFlare or (uint)AID.AncientFlare2 or (uint)AID.WhispersManifest1)
        {
            ...
            PlayerState state = new(Requirement.Stay, World.CurrentTime);
            for (var i = 0; i < len; ++i)
            {
...
{
    public KerStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.Hunt, CFCID = 0u, NameID = 10615u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
