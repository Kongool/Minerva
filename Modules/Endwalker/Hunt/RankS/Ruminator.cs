...
namespace Minerva.Endwalker.Hunt.RankS.Ruminator;
...
class ChitinousTrace(ModuleBase module) : Components.GenericAOEs(module)
{
...
class StygianVapor(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.StygianVapor);

class RuminatorStates : StateMachineBuilder
{
    public RuminatorStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.Hunt, CFCID = 0u, NameID = 10620u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")]
