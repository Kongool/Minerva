...
namespace Minerva.Endwalker.Hunt.RankS.NarrowRift;
...
class EmptyPromise(ModuleBase module) : Components.GenericAOEs(module)
{
...
class VanishingRay(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        switch (spell.Action.ID)
        {
            case (uint)AID.VanishingRayStart:
                _activation = World.FutureTime(4d);
                break;
...
class ContinualMeddling(ModuleBase module) : Components.StatusDrivenForcedMarch(module, 2f, (uint)SID.ForwardMarch, (uint)SID.AboutFace, (uint)SID.LeftFace, (uint)SID.RightFace)
{
...
{
    public NarrowRiftStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 0u, NameID = 10622u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")]
