...
namespace Minerva.Endwalker.FATE.Daivadipa;
...
class LitPath(ModuleBase module) : Components.GenericAOEs(module)
{
...
class Burn(ModuleBase module) : Components.GenericAOEs(module)
{
...
class Drumbeat(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.Drumbeat);

class LeftwardTrisulaRightwardParasu(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.LeftwardTrisula, (uint)AID.RightwardParasu], new AOEShapeCone(65f, 90f.Degrees()));
class ErrantAkasa(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ErrantAkasa, new AOEShapeCone(60f, 45f.Degrees()));
class CosmicWeave(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.CosmicWeave, 18f);
class KarmicFlames(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.KarmicFlames, 20f);
class YawningHells(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.YawningHells, 8f);
class InfernalRedemption(ModuleBase module) : Components.RaidwideCastDelay(module, (uint)AID.InfernalRedemptionVisual, (uint)AID.InfernalRedemption, 1f);

class DivineCall(ModuleBase module) : Components.StatusDrivenForcedMarch(module, 2f, (uint)SID.ForwardMarch, (uint)SID.AboutFace, (uint)SID.LeftFace, (uint)SID.RightFace)
{
...
{
    public DaivadipaStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.Fate, CFCID = 1763u, NameID = 10269u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
public class Daivadipa(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-608f, 811f), new ArenaBoundsSquare(24.5f))
{
