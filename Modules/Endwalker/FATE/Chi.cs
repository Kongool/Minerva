...
namespace Minerva.Endwalker.FATE.Chi;
...
sealed class Bunkerbuster(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        {
            _aoes.Add(new(Square, actor.Position, Angle.AnglesCardinals[1], World.FutureTime(activation)));
        }
    }
...
sealed class BouncingBomb(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        if (activation != default)
        {
            _aoes.Add(new(Bunkerbuster.Square, actor.Position, Angle.AnglesCardinals[1], World.FutureTime(activation)));
        }
    }
...
sealed class Combos(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
...
sealed class Hellburner(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.Hellburner, 5f, tankbuster: true);

sealed class MissileShower(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.MissileShowerVisual, "Raidwide x2");
sealed class ThermobaricExplosive(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ThermobaricExplosive2, 25f);

sealed class AssaultCarapace(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.AssaultCarapace1, (uint)AID.AssaultCarapace2], new AOEShapeRect(120f, 16f));
sealed class AssaultCarapace3(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AssaultCarapace3, new AOEShapeDonut(16f, 60f));

sealed class ForeArmsRearGuns(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.ForeArms1, (uint)AID.ForeArms2,
(uint)AID.RearGuns1, (uint)AID.RearGuns2], new AOEShapeCone(45f, 90f.Degrees()));

sealed class FreeFallBombs(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FreeFallBombs, 6f);

sealed class ChiStates : StateMachineBuilder
{
    public ChiStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.Fate, GroupID = 1855u, CFCID = 1855u, NameID = 10400u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
public sealed class Chi(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(650f, default), new ArenaBoundsSquare(29.5f))
{
