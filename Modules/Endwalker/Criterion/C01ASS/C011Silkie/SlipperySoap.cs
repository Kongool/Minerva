...
sealed class SlipperySoapCharge(ModuleBase module) : Components.GenericKnockback(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        switch (spell.Action.ID)
        {
            case (uint)AID.SlipperySoapTargetSelection:
                _chargeTarget = World.Actors.Find(spell.MainTargetID);
                _chargeResolve = World.FutureTime(5.5d);
                break;
...
sealed class SlipperySoapAOE(ModuleBase module) : Components.GenericAOEs(module)
{
    private SlipperySoap.Color _color;
...
sealed class SoapsudStatic(ModuleBase module) : Components.UniformStackSpread(module, default, 5f)
{
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
