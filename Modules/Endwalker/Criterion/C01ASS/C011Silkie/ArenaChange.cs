...
sealed class ArenaChange(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        if (spell.Action.ID is (uint)AID.NFizzlingSuds or (uint)AID.SFizzlingSuds && Bounds.Radius > 20f)
        {
            var center = Center;
            var shape = new AOEShapeCustom(center, [new Square(center, 29.5f)], [new Square(center, 20f)]);
            _aoe = [new(shape, center, default, World.FutureTime(3.8d), shapeDistance: shape.Distance(center, default))];
        }
    }
