...
abstract class SpringCrystalsRect(ModuleBase module, bool moveCasters, bool risky, double delay) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        if (actor.OID is (uint)OID.NSpringCrystalRect or (uint)OID.SSpringCrystalRect)
        {
            ...
            _aoes.Add(new(_shape, pos, actor.Rotation, World.FutureTime(delay), risky: risky));
...
class SpringCrystalsRectMove(ModuleBase module) : SpringCrystalsRect(module, true, false, 40.3d);
class SpringCrystalsRectStay(ModuleBase module) : SpringCrystalsRect(module, false, true, 24.2d);

class SpringCrystalsSphere(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        {
            _aoes.Add(new(_shape, actor.Position.Quantized(), default, World.FutureTime(23.9d)));
        }
    }
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        if (actor.OID is (uint)OID.NSpringCrystalSphere or (uint)OID.SSpringCrystalSphere && status.ID == (uint)SID.Bubble)
        {
            ...
            if (index >= 0)
            {
                ...
                aoe.Origin += new WDir(aoe.Origin.X < Center.X ? 20f : -20f, default);
            }
            else
            {
