...
class Analysis(ModuleBase module) : ModuleComponent(module)
{
...
class AnalysisRadiance(ModuleBase module) : Components.GenericGaze(module)
{
...
class TargetedLight(ModuleBase module) : Components.GenericGaze(module)
{
    public bool Active;
    private readonly Analysis? _analysis = module.FindComponent<Analysis>();
    ...
    {
        if (Active)
            return new Eye[1] { new(Center, _activation, _safeDir[slot], inverted: true) };
        return [];
    }
