...
abstract class RousingReincarnation(ModuleBase module, uint aid) : Components.CastCounter(module, aid);
sealed class NRousingReincarnation(ModuleBase module) : RousingReincarnation(module, (uint)AID.NRousingReincarnationAOE);
sealed class SRousingReincarnation(ModuleBase module) : RousingReincarnation(module, (uint)AID.SRousingReincarnationAOE);

// note on towers: indices are 0-7 CW from N, even (cardinal) are blue, odd (intercardinal) are orange
sealed class MalformedPrayer1(ModuleBase module) : Components.GenericTowers(module)
{
    ...
    private void UpdateTowers()
    {
        ...
        foreach (var index in _towerOrder.Skip(NumCasts).Take(2))
        {
            ...
            Towers.Add(new(Center + 11f * (180f.Degrees() - index * 45f.Degrees()).ToDirection(), 4, forbiddenSoakers: forbidden));
        }
    }
}
...
sealed class PointedPurgation : Components.BaitAwayTethers
{
    ...
    public PointedPurgation(ModuleBase module) : base(module, new AOEShapeCone(60f, 22.5f.Degrees()), (uint)TetherID.PointedPurgation)
    {
