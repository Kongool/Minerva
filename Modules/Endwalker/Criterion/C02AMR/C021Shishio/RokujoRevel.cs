...
sealed class RokujoRevel(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        ...
        if (countLines > 1)
        {
            aoes.Add(new(_shapeLine, Center, _pendingLines[1].dir, _pendingLines[1].activation, risky: false));
        }
        ...
        if (countLines != 0)
        {
            aoes.Add(new(_shapeLine, Center, _pendingLines[0].dir, _pendingLines[0].activation, color));
        }
