...
sealed class Palladion(ModuleBase module) : ModuleComponent(module)
{
...
sealed class PalladionArena(ModuleBase module) : ModuleComponent(module)
{
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        for (var i = 0; i < 8; ++i)
            Arena.PathLineTo(Center + 14f * (i * 45f).Degrees().ToDirection());
...
sealed class PalladionShockwave(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        if (spell.Action.ID == (uint)AID.PalladionAOE)
        {
            ...
            _activation = World.FutureTime(3d);
            ++NumCasts;
        }
    }
}
...
sealed class PalladionStack : Components.UniformStackSpread
{
    ...
    public PalladionStack(ModuleBase module) : base(module, 6f, default, raidwideOnResolve: false)
    {
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        {
            ++_numCasts;
            UpdateStack(World.FutureTime(3d));
        }
    }
...
sealed class PalladionVoidzone(ModuleBase module) : Components.VoidzoneAtCastTarget(module, 6f, (uint)AID.PalladionAOE, GetVoidzones, 0.9d)
{
...
sealed class PalladionClearCut(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly Palladion? _palladion = module.FindComponent<Palladion>();
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        if (_palladion != null && _palladion.NumBaitsDone < _palladion.NumBaitsAssigned && !_palladion.BaitOrder[_palladion.NumBaitsDone])
            return new AOEInstance[1] { new(_shape, Center) };
        return [];
    }
}
...
sealed class PalladionWhiteFlame : Components.GenericBaitAway
{
    ...
    public PalladionWhiteFlame(ModuleBase module) : base(module)
    {
        _palladion = module.FindComponent<Palladion>();
        UpdateBaiters();
    }
    ...
    public override void Update()
    {
        CurrentBaits.Clear();
        if (_palladion != null && _palladion.NumBaitsDone < _palladion.NumBaitsAssigned && _palladion.BaitOrder[_palladion.NumBaitsDone])
            foreach (var t in Raid.WithoutSlot(false, true, true).SortedByRange(Center).Take(2))
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        if (CurrentBaits.Count != 0)
            Arena.Actor(Center, default, Colors.Object);
...
sealed class PalladionDestroyPlatforms(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.PalladionDestroyPlatforms, "Go to safe platform!")
{
