...
class Gorgospit(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.Gorgospit)
{
    ...
    public override void OnActorPlayActionTimelineEvent(Actor actor, ushort id)
    {
        if (actor.OID == (uint)OID.IllusoryHephaistosSnakes && id == 0x11D2)
            Casters.Add((actor, World.FutureTime(8d)));
    }
...
class Snake2(ModuleBase module) : PetrifactionCommon(module)
{
    struct PlayerState
    {
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        switch (status.ID)
        {
            case (uint)SID.EyeOfTheGorgon:
                SetPlayerLongPetrify(Raid.FindSlot(actor.InstanceID), (status.ExpireAt - World.CurrentTime).TotalSeconds > 25d);
                break;
