...
class QuadrupedalImpact(ModuleBase module) : Components.GenericKnockback(module, (uint)AID.QuadrupedalImpactAOE)
{
...
class QuadrupedalCrush(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.QuadrupedalCrushAOE)
{
    private WPos? _source;
...
class CentaurTetraflare(ModuleBase module) : TetraOctaFlareCommon(module)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
...
class CentaurDiflare(ModuleBase module) : Components.UniformStackSpread(module, 6f, default, 4, 4)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
...
// TODO: hints
class BlazingFootfalls(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void DrawArenaBackground(int pcSlot, Actor pc)
    {
        if (NumMechanicsDone == 0)
        {
            ...
            Arena.ZoneRect(Center, new WDir(0, 1), Bounds.Radius, Bounds.Radius, _trailblazeHalfWidth, Colors.AOE);
        }
        if (NumMechanicsDone == 2)
        {
            // draw second trailblaze
            Arena.ZoneRect(Center, new WDir(1, 0), Bounds.Radius, Bounds.Radius, _trailblazeHalfWidth, Colors.AOE);
        }

        if (_firstCrush && NumMechanicsDone < 2)
        {
            // draw first crush
            Arena.ZoneCircle(Center + Bounds.Radius * new WDir(_firstSafeLeft ? 1 : -1, 0), _crushRadius, Colors.AOE);
        }
        if (!_firstCrush && NumMechanicsDone is >= 2 and < 4)
        {
            // draw second crush
            Arena.ZoneCircle(Center + Bounds.Radius * new WDir(0, _secondSafeTop ? 1 : -1), _crushRadius, Colors.AOE);
        }
    }
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        ...
        {
            // draw first safespot
            Arena.ZoneCircleOutline(Center + _safespotOffset * new WDir(_firstSafeLeft ? -1 : 1, 0), _safespotRadius, Colors.Safe, 2);
        }
        if (NumMechanicsDone < 4 && _seenVisuals > 1)
        {
            // draw second safespot
            Arena.ZoneCircleOutline(Center + _safespotOffset * new WDir(0, _secondSafeTop ? -1 : 1), _safespotRadius, Colors.Safe, 2);
        }

        if (NumMechanicsDone == 0)
        {
            // draw knockback from first trailblaze
            var adjPos = pc.Position + _trailblazeKnockbackDistance * new WDir(pc.PosRot.X < Center.X ? -1 : 1, 0);
            Components.GenericKnockback.DrawKnockback(pc, adjPos, Arena);
        }
        if (NumMechanicsDone == 2)
        {
            // draw knockback from second trailblaze
            var adjPos = pc.Position + _trailblazeKnockbackDistance * new WDir(0, pc.PosRot.Z < Center.Z ? -1 : 1);
            Components.GenericKnockback.DrawKnockback(pc, adjPos, Arena);
        }
        ...
        if (!_firstCrush && NumMechanicsDone == 1)
        {
            ...
            var adjPos = Components.GenericKnockback.AwayFromSource(pc.Position, Center + Bounds.Radius * new WDir(_firstSafeLeft ? -1 : 1, 0), _impactKnockbackRadius);
            Components.GenericKnockback.DrawKnockback(pc, adjPos, Arena);
        }
        if (_firstCrush && NumMechanicsDone == 3)
        {
            // draw knockback from second impact
            var adjPos = Components.GenericKnockback.AwayFromSource(pc.Position, Center + Bounds.Radius * new WDir(0, _secondSafeTop ? -1 : 1), _impactKnockbackRadius);
            Components.GenericKnockback.DrawKnockback(pc, adjPos, Arena);
        }
    }
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        switch (spell.Action.ID)
        {
            case (uint)AID.BlazingFootfallsImpactVisual:
                if (_seenVisuals > 0)
                {
                    _secondSafeTop = spell.LocXZ.Z < Center.Z;
                }
                else
                {
                    _firstSafeLeft = spell.LocXZ.X < Center.X;
                }
                ++_seenVisuals;
                break;
