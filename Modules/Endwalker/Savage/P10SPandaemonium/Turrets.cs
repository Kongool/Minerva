...
class Turrets(ModuleBase module) : Components.GenericKnockback(module, (uint)AID.PealOfCondemnation, 1, stopAfterWall: true)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        ...
        _activation = World.FutureTime(8.1d);
    ...
    private List<(Actor? source, Actor? target)> ImminentTurretsWithTargets()
    {
        ...
        for (var i = NumCasts; i < len && count < 2; ++i)
        {
            ...
            var target = World.Actors.Find(turret?.TargetID ?? 0);
