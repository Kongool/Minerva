...
class DarkCurrent(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        if (index is 1 or 2 && state is 0x00020001 or 0x00200010)
        {
            ...
            void AddAOE(WDir offset, DateTime act) => _aoes.Add(new(_shape, (Center + offset).Quantized(), default, act));
            for (var i = 0; i < 8; ++i)
            {
                var act = World.FutureTime(7.1d + i * 1.1d);
...
class BlindingLight(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.BlindingLightAOE, 6f);
