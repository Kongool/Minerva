...
class Gaiaochos(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GaiaochosTransition, new AOEShapeDonut(7f, 30f));

// TODO: we could show it earlier, casters do PATE 11D2 ~4s before starting cast
class UltimaRay(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.UltimaRay, new AOEShapeRect(20f, 3f));

class MissingLink(ModuleBase module) : Components.Chains(module, (uint)TetherID.MissingLink, (uint)AID.MissingLink);

class DemiParhelion(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DemiParhelionAOE, new AOEShapeCircle(2f));

class Geocentrism(ModuleBase module) : Components.GenericAOEs(module)
{
...
class DivineExcoriation(ModuleBase module) : Components.UniformStackSpread(module, default, 1f)
{
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        if (iconID == (uint)IconID.DivineExcoriation)
            AddSpread(actor, World.FutureTime(3.1d));
    }
...
class GaiaochosEnd(ModuleBase module) : ModuleComponent(module)
{
...
class UltimaBlow(ModuleBase module) : Components.CastCounter(module, (uint)AID.UltimaBlow)
{
    ...
    {
        if (tether.ID == (uint)TetherID.ClassicalConceptsShapes && World.Actors.Find(tether.Target) is var target && target != null)
        {
