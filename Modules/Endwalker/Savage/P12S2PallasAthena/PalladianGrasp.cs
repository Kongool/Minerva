...
class PalladianGrasp(ModuleBase module) : Components.CastCounter(module, default)
{
    ...
    private Actor? Target() => World.Actors.Find(NumCasts == 0 ? _firstPrimaryTarget : Module.PrimaryActor.TargetID);
    private WPos Origin(Actor target) => Center + new WDir(target.PosRot.X < Center.X ? -_shape.HalfWidth : +_shape.HalfWidth, 0);
}
