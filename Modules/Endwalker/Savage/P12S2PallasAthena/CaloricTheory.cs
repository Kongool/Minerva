...
class CaloricTheory1Part1(ModuleBase module) : Components.UniformStackSpread(module, 7f, 7f, 1)
{
    private BitMask _initialMarkers; // these shouldn't spread

    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID == (uint)AID.CaloricTheory1InitialFire && World.Actors.Find(spell.TargetID) is var target && target != null)
        {
...
class CaloricTheory1Part2(ModuleBase module) : Components.UniformStackSpread(module, 7f, default, 2)
{
...
class CaloricTheory1Part3(ModuleBase module) : Components.UniformStackSpread(module, 7f, 7f, 2)
{
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        switch (status.ID)
        {
            case (uint)SID.Pyrefaction:
                if (status.ExpireAt > World.FutureTime(1)) // don't pick up debuffs from previous part
...
class CaloricTheory2Part1(ModuleBase module) : Components.UniformStackSpread(module, 7, 7, 1, 1)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        switch (spell.Action.ID)
        {
            case (uint)AID.CaloricTheory2InitialFire:
                if (World.Actors.Find(spell.TargetID) is var fireTarget && fireTarget != null)
                    AddStack(fireTarget, Module.CastFinishAt(spell)); // fake stack
                break;
            case (uint)AID.CaloricTheory2InitialWind:
                if (World.Actors.Find(spell.TargetID) is var windTarget && windTarget != null)
...
class CaloricTheory2Part2(ModuleBase module) : Components.UniformStackSpread(module, 7f, 7f)
{
    public bool Done;
...
class EntropicExcess(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.EntropicExcess, 7);
