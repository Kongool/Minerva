...
class SunshadowTether(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void Update()
    {
        ...
        foreach (var bird in ActiveBirds())
        {
            ...
            var target = targetID != 0 ? World.Actors.Find(targetID) : null;
    ...
    public override void DrawArenaBackground(int pcSlot, Actor pc)
    {
        ...
        {
            var targetID = BirdTarget(bird);
            var target = (targetID != 0 && targetID != pc.InstanceID) ? World.Actors.Find(targetID) : null;
            if (target != null && target.Position != bird.Position)
            {
                var dir = (target.Position - bird.Position).Normalized();
