...
class ArchaicRockbreakerCenter(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ArchaicRockbreakerCenter, 6f);

class ArchaicRockbreakerShockwave(ModuleBase module) : Components.GenericKnockback(module, (uint)AID.ArchaicRockbreakerShockwave)
{
    private readonly DateTime _activation = module.World.FutureTime(6.5d);
    ...
    {
        if (Bounds == P9SKokytos.arenaUplift0)
            return new Knockback[1] { new(Center, 21f, _activation, safeWalls: Walls0, ignoreImmunes: true) };
        else if (Bounds == P9SKokytos.arenaUplift45)
            return new Knockback[1] { new(Center, 21f, _activation, safeWalls: Walls45, ignoreImmunes: true) };
        return [];
    }
...
{
    public ArchaicRockbreakerPairs(ModuleBase module) : base(module, 6f, default, 2)
    {
        foreach (var p in Raid.WithoutSlot(true, true, true).Where(p => p.Class.IsSupport()))
            AddStack(p, World.FutureTime(7.8f));
    }
...
class ArchaicRockbreakerLine(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ArchaicRockbreakerLine, 8f, maxCasts: 8);

class ArchaicRockbreakerCombination(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        if (inOutShape != null)
        {
            _aoes.Add(new(inOutShape, Module.PrimaryActor.Position, default, World.FutureTime(6.9d)));
            _aoes.Add(new(_shapeCleave, Module.PrimaryActor.Position, Module.PrimaryActor.Rotation + offset, World.FutureTime(10d)));
        }
    }
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        switch (spell.Action.ID)
        {
            case (uint)AID.InsideRoundhouseAOE:
                PopAOE();
                _aoes.Add(new(_shapeIn, Module.PrimaryActor.Position, default, World.FutureTime(6)));
                break;
            case (uint)AID.OutsideRoundhouseAOE:
                PopAOE();
                _aoes.Add(new(_shapeOut, Module.PrimaryActor.Position, default, World.FutureTime(6)));
                break;
...
class ArchaicDemolish(ModuleBase module) : Components.UniformStackSpread(module, 6f, default, 4)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
