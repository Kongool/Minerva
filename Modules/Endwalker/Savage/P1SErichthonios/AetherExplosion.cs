...
class AetherExplosion(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void Update()
    {
        if (_memberWithSOT != null)
            _explodingCells = CellFromOffset(_memberWithSOT.Position - Center);
    }

    public override void AddHints(int slot, Actor actor, TextHints hints)
    {
        if (actor != _memberWithSOT && _explodingCells != Cell.None && _explodingCells == CellFromOffset(actor.Position - Center))
        {
    ...
    public override void DrawArenaBackground(int pcSlot, Actor pc)
    {
        ...
        if (Bounds is not ArenaBoundsCircle)
        {
        ...
        for (var i = 0; i < 4; ++i)
        {
            Arena.ZoneCone(Center, 0, P1S.InnerCircleRadius, start + 22.5f.Degrees(), 22.5f.Degrees(), Colors.AOE);
            Arena.ZoneCone(Center, P1S.InnerCircleRadius, Bounds.Radius, start + 67.5f.Degrees(), 22.5f.Degrees(), Colors.AOE);
            start += 90.Degrees();
        }
    }
