...
class Knockback : ModuleComponent
{
    ...
    public Knockback(ModuleBase module) : base(module)
    {
        _isFlare = Module.PrimaryActor.CastInfo?.IsSpell(AID.KnockbackPurge) ?? false;
        _knockbackTarget = World.Actors.Find(Module.PrimaryActor.CastInfo?.TargetID ?? 0);
    ...
    {
        if (Module.PrimaryActor.CastInfo != null && actor == _knockbackTarget && !InBounds(_knockbackPos))
        {
