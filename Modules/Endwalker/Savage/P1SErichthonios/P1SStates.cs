...
{
    public P1SStates(ModuleBase module) : base(module)
    {
    ...
    private void ShiningCells(uint id, float delay)
    {
        Cast(id, (uint)AID.ShiningCells, delay, 7, "Cells")
            .OnExit(() => Module.Bounds = new ArenaBoundsCircle(Module.Bounds.Radius))
            .SetHint(StateMachine.StateHint.Raidwide);
    }
    ...
    private void SlamShut(uint id, float delay)
    {
        Cast(id, (uint)AID.SlamShut, delay, 7, "SlamShut")
            .OnExit(() => Module.Bounds = new ArenaBoundsSquare(Module.Bounds.Radius))
            .SetHint(StateMachine.StateHint.Raidwide);
    }
