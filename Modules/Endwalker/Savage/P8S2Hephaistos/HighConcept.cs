...
class HighConceptCommon(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        ...
        {
            SID.ImperfectionAlpha => (status.ExpireAt - World.CurrentTime).TotalSeconds > 15 ? PlayerRole.LongAlpha : PlayerRole.ShortAlpha,
            SID.ImperfectionBeta => (status.ExpireAt - World.CurrentTime).TotalSeconds > 15 ? PlayerRole.LongBeta : PlayerRole.ShortBeta,
            SID.ImperfectionGamma => (status.ExpireAt - World.CurrentTime).TotalSeconds > 15 ? PlayerRole.LongGamma : PlayerRole.ShortGamma,
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        if (secondColorHC2 != TowerColor.Unknown)
            SecondTowersHC2.Add((Center + secondOffsetHC2, secondColorHC2));
    }
    ...
    protected void DrawTower(float offsetZ, bool assigned) => DrawTower(Center + new WDir(0, offsetZ), assigned);
...
class HighConcept1(ModuleBase module) : HighConceptCommon(module)
{
...
class HighConcept2(ModuleBase module) : HighConceptCommon(module)
{
    public override void AddHints(int slot, Actor actor, TextHints hints)
    {
        if (NumAssignedRoles < 8)
