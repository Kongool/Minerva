...
class HammersCells(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.DestroySideTiles, "GTFO from dangerous tile!")
{
    public bool Active;
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        ...
        for (var z = -2; z <= 2; ++z)
        {
            for (var x = -2; x <= 2; ++x)
            {
                ...
                if (InBounds(center) && CellDangerous(x, z))
                {
    ...
    public override void OnCastFinished(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID == WatchedAction)
        {
            Bounds = new ArenaBoundsRect(15f, 25f);
        }
    }
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        if (index is >= 0x07 and <= 0x0B)
        {
            ...
            activation = World.FutureTime(16d);
        }
        else if (index == 0x1A)
        {
...
abstract class Rect(ModuleBase module, uint aid) : Components.SimpleAOEs(module, aid, new AOEShapeRect(50f, 5f));
class DestroySideTiles(ModuleBase module) : Rect(module, (uint)AID.DestroySideTiles);
class HammersLevinforge(ModuleBase module) : Rect(module, (uint)AID.Levinforge);

class HammersSpire(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ByregotSpire, new AOEShapeRect(50f, 15f))
{
