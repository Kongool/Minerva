...
class FortuneFluxOrder(ModuleBase module) : ModuleComponent(module)
{
    public enum Mechanic { None, AOE, Knockback }

    public List<(WPos source, Mechanic mechanic, DateTime activation, Angle rotation)> Mechanics = [];
    ...
    private void TryAdd()
    {
        if (_currentTethered != null && _currentMechanic != Mechanic.None)
        {
            if (activation == default)
                activation = World.FutureTime(100d);
...
class FortuneFluxAOE(ModuleBase module) : Components.GenericAOEs(module)
{
...
class FortuneFluxKnockback(ModuleBase module) : Components.GenericKnockback(module)
{
    private readonly FortuneFluxOrder? _order = module.FindComponent<FortuneFluxOrder>();
