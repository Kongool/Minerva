...
sealed class TetraktysBorder(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        if (index == 0x04)
        {
            switch (state)
            {
                case 0x00200010u:
                    var center = Center;
                    _aoe = [new(new AOEShapeCustom(center, [new Square(center, 24f)], GetTriangle()), center, default, World.FutureTime(6.5d))];
                    break;
                case 0x00020001u:
                    _aoe = [];
...
sealed class Tetraktys(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        void AddAOE(AOEShapeTriCone shape, WPos pos, Angle rot) => AOEs.Add(new(shape, pos.Quantized(), rot, World.FutureTime(3.8d)));
...
sealed class TetraktuosKosmosCounter(ModuleBase module) : Components.CastCounter(module, (uint)AID.TetraktuosKosmosAOETri); // to handle tutorial of TetraktuosKosmos

sealed class TetraktuosKosmos(ModuleBase module) : Components.GenericAOEs(module)
{
    public readonly List<AOEInstance> AOEs = [];
    ...
    private void AddAOEs(int[] indices)
    {
        for (var i = 0; i < indices.Length; ++i)
        {
            ...
            AOEs.Add(new(shape, pos.Quantized(), Angles[angle], World.FutureTime(7.9d)));
        }
    }
