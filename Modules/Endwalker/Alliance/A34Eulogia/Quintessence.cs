...
sealed class Quintessence(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        _index = index;
        position = (Center + offset).Quantized();
        InitIfReady();
    }
    ...
    private void InitIfReady()
    {
        ...
        {
            var count = _aoes.Count;
            _aoes.Add(new(shape ?? cone, position, rotation, count == 0 ? World.FutureTime(26.4d) : _aoes.Ref(0).Activation.AddSeconds(_aoes.Count * 3.5d)));
            _index = default;
            cast = default;
        }
    }
