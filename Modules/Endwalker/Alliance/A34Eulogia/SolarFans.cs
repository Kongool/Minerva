...
sealed class SolarFans(ModuleBase module) : Components.ChargeAOEs(module, (uint)AID.SolarFansAOE, 5f);

sealed class RadiantRhythm(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (_aoes.Count == 0 && spell.Action.ID == (uint)AID.SolarFansAOE)
        {
            ...
            void AddAOE(Angle rotation, DateTime activation) => _aoes.Add(new(_shape, Center.Quantized(), rotation, activation));
        }
    }
...
sealed class RadiantFlourish(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly List<AOEInstance> _aoes = [];
