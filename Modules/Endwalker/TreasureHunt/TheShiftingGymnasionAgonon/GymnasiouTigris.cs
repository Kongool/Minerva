...
namespace Minerva.Endwalker.TreasureHunt.ShiftingGymnasionAgonon.GymnasiouTigris;
...
class AbsoluteZero(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AbsoluteZero, new AOEShapeCone(45f, 45f.Degrees()));
class FrumiousJaws(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.FrumiousJaws);
class BlizzardIII(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.BlizzardIII, 6f);
class Eyeshine(ModuleBase module) : Components.CastGaze(module, (uint)AID.Eyeshine);
class CatchingClaws(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.CatchingClaws, new AOEShapeCone(12f, 45f.Degrees()));
...
{
    public GymnasiouTigrisStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 909u, NameID = 11999u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
