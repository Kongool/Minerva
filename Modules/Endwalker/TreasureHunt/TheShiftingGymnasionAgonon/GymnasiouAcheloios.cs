...
namespace Minerva.Endwalker.TreasureHunt.ShiftingGymnasionAgonon.GymnasiouAcheloios;
...
class DoubleHammer(ModuleBase module) : Components.GenericAOEs(module)
{
...
class QuadrupleHammer(ModuleBase module) : Components.GenericRotatingAOE(module)
{
...
class VolcanicHowl(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.VolcanicHowl);
class Earthbreak(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Earthbreak, 5f);
class DeadlyHold(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.DeadlyHold);
class TailSwing(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TailSwing, 13f);
class CriticalBite(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.CriticalBite, new AOEShapeCone(10f, 60f.Degrees()));
...
{
    public GymnasiouAcheloiosStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 909u, NameID = 12019u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
