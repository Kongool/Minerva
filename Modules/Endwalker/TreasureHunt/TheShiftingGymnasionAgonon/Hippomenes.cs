...
namespace Minerva.Endwalker.TreasureHunt.ShiftingGymnasionAgonon.Hippomenes;
...
class DibridTetrabridBlaster(ModuleBase module) : Components.GenericAOEs(module)
{
...
class Thunderbolt(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.Thunderbolt, 6f);
class Gouge(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.Gouge);
class ElectricBurst(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.ElectricBurst);
class Shock(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Shock, 10f);
class RumblingThunder(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.RumblingThunder, 6f);
class ElectricWhisker(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ElectricWhisker, new AOEShapeCone(40f, 30f.Degrees()));
...
{
    public HippomenesStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 909u, NameID = 12030u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
