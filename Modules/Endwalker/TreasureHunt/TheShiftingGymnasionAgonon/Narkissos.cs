...
namespace Minerva.Endwalker.TreasureHunt.ShiftingGymnasionAgonon.Narkissos;
...
class Brainstorm(ModuleBase module) : Components.StatusDrivenForcedMarch(module, 2f, (uint)SID.ForwardMarch, (uint)SID.AboutFace, (uint)SID.LeftFace, (uint)SID.RightFace)
{
...
class FetchingFulgence(ModuleBase module) : Components.CastGaze(module, (uint)AID.FetchingFulgence);
class Lash(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.Lash);
class PotentPerfume(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PotentPerfume, 8f);

class SapShowerTendrilsHint(ModuleBase module) : ModuleComponent(module)
{
    private bool active;
...
{
    public SapShower(ModuleBase module) : base(module, (uint)AID.SapShower, 8f)
    {
        Color = Colors.Danger;
    }
}

class ExtensibleTendrils(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ExtensibleTendrils, new AOEShapeCross(25f, 3f));
class PutridBreath(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PutridBreath, new AOEShapeCone(25f, 45f.Degrees()));
class RockHard(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.RockHard, 6f);
class BeguilingGasTM(ModuleBase module) : Components.TemporaryMisdirection(module, (uint)AID.BeguilingGas);
class BeguilingGas(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.BeguilingGas);
...
{
    public NarkissosStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 909u, NameID = 12029u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
