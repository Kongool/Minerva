...
namespace Minerva.Endwalker.TreasureHunt.ShiftingGymnasionAgonon.LyssaChrysine;
...
class HeavySmash2(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HeavySmash2, 6f);
class FrigidStone(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FrigidStone, 5f);

class FrigidNeedle(ModuleBase module) : Components.ConcentricAOEs(module, _shapes)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID == (uint)AID.FrigidNeedleVisual)
            AddSequence(Center.Quantized(), Module.CastFinishAt(spell, 0.5f));
    }
...
class CircleOfIce(ModuleBase module) : Components.ConcentricAOEs(module, _shapes)
{
...
class PillarPierce(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PillarPierce, new AOEShapeRect(80f, 2f));
class SkullDasher(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.SkullDasher);
class HeavySmash(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.HeavySmash, 6f, 8, 8);

class IcePillar(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        if (actor.OID == (uint)OID.IcePillar)
            _aoes.Add(new(circle, actor.Position.Quantized(), default, World.FutureTime(3.7d)));
    }
    public override void OnCastFinished(Actor caster, ActorCastInfo spell)
    {
...
class Howl(ModuleBase module) : Components.CastHint(module, (uint)AID.Howl, "Calls adds");

class LyssaChrysineStates : StateMachineBuilder
{
    public LyssaChrysineStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 909u, NameID = 12024u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
