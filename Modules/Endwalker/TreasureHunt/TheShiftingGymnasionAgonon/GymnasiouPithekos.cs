...
namespace Minerva.Endwalker.TreasureHunt.ShiftingGymnasionAgonon.GymnasiouPithekos;
...
class Spark(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Spark, new AOEShapeDonut(14f, 30f));
class SweepingGouge(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.SweepingGouge);
class Thundercall(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Thundercall, 3f);

class Thundercall2(ModuleBase module) : Components.GenericBaitAway(module, centerAtTarget: true)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        ...
        {
            CurrentBaits.Add(new(Module.PrimaryActor, actor, circle, World.FutureTime(6d)));
        }
    }
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (CurrentBaits.Count != 0 && CurrentBaits.Ref(0).Target == actor)
        {
            ...
            if (b.Target == actor)
            {
                hints.AddForbiddenZone(new SDCircle(Center, 17.5f), b.Activation);
            }
        }
    }
...
class LightningBolt(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.LightningBolt, 6f);
class ThunderIV(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ThunderIV, 18f);
...
{
    public GymnasiouPithekosStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 909u, NameID = 12001u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
