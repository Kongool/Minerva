...
namespace Minerva.Endwalker.TreasureHunt.ShiftingGymnasionAgonon.GymnasiouSatyros;
...
class StormWing(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.StormWing, new AOEShapeCone(40f, 45f.Degrees()));
class FlashGale(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FlashGale, 6);
class WindCutter(ModuleBase module) : Components.Voidzone(module, 4f, GetVoidzones)
{
    private static List<Actor> GetVoidzones(ModuleBase module) => module.Enemies((uint)OID.StormsGrip);
}
class Wingblow(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Wingblow, 15f);
class DreadDive(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.DreadDive);
...
{
    public GymnasiouSatyrosStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 909u, NameID = 12003u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
