...
namespace Minerva.Endwalker.TreasureHunt.ShiftingGymnasionAgonon.GymnasiouLeon;
...
class InfernoBlast(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.InfernoBlast, new AOEShapeRect(46f, 10f));

sealed class RoarFlareStar(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.Roar, (uint)AID.FlareStar], 12f);

class MarkOfTheBeast(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MarkOfTheBeast, new AOEShapeCone(8f, 60f.Degrees()));
class Pounce(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.Pounce);
class MagmaChamber(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MagmaChamber, 8f);
...
{
    public GymnasiouLeonStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 909u, NameID = 11997u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
