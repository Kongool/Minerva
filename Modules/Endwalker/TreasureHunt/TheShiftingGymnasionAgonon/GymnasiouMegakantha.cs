...
namespace Minerva.Endwalker.TreasureHunt.ShiftingGymnasionAgonon.GymnasiouMegakantha;
...
class SludgeBomb(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SludgeBomb2, 8f);
class RustlingWind(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.RustlingWind, new AOEShapeRect(15f, 2f));
class AcidMist(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AcidMist, 6f);
class OdiousAir(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.OdiousAir, new AOEShapeCone(12f, 60f.Degrees()));
class VineWhip(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.VineWhip);

class OdiousAtmosphere(ModuleBase module) : Components.GenericAOEs(module)
{
...
{
    public GymnasiouMegakanthaStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 909u, NameID = 12009u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
