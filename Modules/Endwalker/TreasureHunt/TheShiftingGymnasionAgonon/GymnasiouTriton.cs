...
namespace Minerva.Endwalker.TreasureHunt.ShiftingGymnasionAgonon.GymnasiouTriton;
...
class PelagicCleaver(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PelagicCleaver, new AOEShapeCone(40f, 30f.Degrees()));
class FoulWaters(ModuleBase module) : Components.VoidzoneAtCastTarget(module, 5f, (uint)AID.FoulWaters, GetVoidzones, 1.4f)
{
    private static List<Actor> GetVoidzones(ModuleBase module) => module.Enemies((uint)OID.Bubble);
}
class AquaticLance(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AquaticLance, 13f);
class ProtolithicPuncture(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.ProtolithicPuncture);
...
{
    public GymnasiouTritonStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 909u, NameID = 12006u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
