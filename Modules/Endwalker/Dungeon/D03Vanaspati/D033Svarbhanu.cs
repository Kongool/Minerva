...
namespace Minerva.Endwalker.Dungeon.D03Vanaspati.D033Svarbhanu;
...
sealed class ArenaChange(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        if (spell.Action.ID == (uint)AID.FlamesOfDecay && Bounds.Radius > 21f)
        {
            var center = Center;
...
sealed class ChaoticUndercurrent(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    private void AddAOEsForPattern(bool isBlue)
    {
        ...
        void AddAOEs((int, int) indices)
        {
            ...
            {
                var activation = World.FutureTime(7.7d);
...
sealed class CosmicKissSpread(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.CosmicKissSpread, 6f);
sealed class CosmicKissCircle(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.CosmicKissCircle, 6f);

sealed class CosmicKissRect(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        if (state == 0x00020001u && _aoes.Count == 0)
        {
            ...
            void AddAOEs(int[] indices, double delay)
            {
                ...
                for (var i = 0; i < 3; ++i)
                {
                    _aoes.Add(new(rect, coords[indices[i]].Quantized(), Angle.AnglesCardinals[0], World.FutureTime(delay)));
                }
            }
        }
    }
...
sealed class CosmicKissRaidwide(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.CosmicKiss);

sealed class CosmicKissKnockback(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.CosmicKiss, 13f)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (count != 0 && Casters.Count != 0)
        {
            ...
            if (!IsImmune(slot, act))
            {
                ...
                hints.AddForbiddenZone(new SDKnockbackInAABBSquareAwayFromOriginPlusAOERects(Center, c.Origin, 13f, 19f, rects, 40f, 5f, len), act);
            }
        }
    }
}

sealed class FlamesOfDecay(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.FlamesOfDecay);
sealed class GnashingOfTeeth(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.GnashingOfTeeth);

sealed class D033SvarbhanuStates : StateMachineBuilder
{
    public D033SvarbhanuStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 789u, CFCID = 789u, NameID = 10719u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS) (ported from BMR)")]
public sealed class D033Svarbhanu(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(300f, -157f), new ArenaBoundsSquare(24.5f));