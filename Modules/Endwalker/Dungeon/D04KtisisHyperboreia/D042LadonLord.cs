...
namespace Minerva.Endwalker.Dungeon.D04KtisisHyperboreia.D042LadonLord;
...
class PyricBreath(ModuleBase module) : Components.GenericAOEs(module)
{
...
class PyricSphere(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PyricSphere, new AOEShapeCross(50f, 2f)); // we could draw this almost 5s earlier, but why bother with 10s cast time
class PyricBlast(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.PyricBlast, 6f, 4, 4);
class Scratch(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.Scratch);
class Intimidation(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Intimidation);

class D042LadonLordStates : StateMachineBuilder
{
    public D042LadonLordStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 787u, NameID = 10398u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS) (ported from BMR)")]
public class D042LadonLord(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(default, 48f), new ArenaBoundsSquare(19.5f));
