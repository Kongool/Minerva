...
namespace Minerva.Endwalker.Dungeon.D07Smileton.D071Face;
...
class OffMyLawnKnockback(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.OffMyLawnKnockback, 8, kind: Kind.DirForward, stopAtWall: true);
class TemperTemperSpread(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.TemperTemperSpread, 5);
...
{
    public D071FaceStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 794u, NameID = 10331u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (ported from BMR)")]
public class D071Face(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-45, -20), new ArenaBoundsSquare(20));
