...
namespace Minerva.Endwalker.Dungeon.D11LapisManalis.D112GalateaMagna;
...
sealed class ScarecrowChase(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        ...
        {
            _aoes.Add(new(cross, actor.Position.Quantized(), Angle.AnglesIntercardinals[1], World.FutureTime(9.9d + (-(uint)IconID.Icon1 + iconID) * 3d), risky: false));
...
sealed class WaxingCycle(ModuleBase module) : Components.ConcentricAOEs(module, [new AOEShapeCircle(10f), new AOEShapeDonut(10f, 40f)])
{
    ...
    public override void OnCastFinished(Actor caster, ActorCastInfo spell)
    {
        if (Sequences.Count != 0)
        {
            ...
            AdvanceSequence(order, spell.LocXZ, World.FutureTime(2.7d));
        }
    }
}

sealed class WaningCycle(ModuleBase module) : Components.ConcentricAOEs(module, [new AOEShapeDonut(10f, 40f), new AOEShapeCircle(10f)])
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
...
sealed class GlassyEyed(ModuleBase module) : Components.GenericGaze(module)
{
...
sealed class TenebrismTowers(ModuleBase module) : Components.GenericTowers(module)
{
    public override void OnMapEffect(byte index, uint state)
    {
        if (state == 0x00010008u)
        {
            ...
            if (position != default)
            {
                Towers.Add(new(position.Quantized(), 5f, activation: World.FutureTime(6d)));
            }
        }
    }
...
sealed class SoulScythe(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SoulScythe, 18f);

sealed class D112GalateaMagnaStates : StateMachineBuilder
{
    public D112GalateaMagnaStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 896u, CFCID = 896u, NameID = 10308u, PrimaryActorOID = (uint)OID.GalateaMagna, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
public sealed class D112GalateaMagna(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena, true)
{
