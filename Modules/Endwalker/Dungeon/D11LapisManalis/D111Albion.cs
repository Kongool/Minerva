...
namespace Minerva.Endwalker.Dungeon.D11LapisManalis.D111Albion;
...
sealed class WildlifeCrossing(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    private readonly List<Actor> animals = [];
    ...
    public override void Update()
    {
        ...
        for (var i = 0; i < 2; ++i)
        {
            ...
            var reset = s.Reset;
            if (reset != default && World.CurrentTime > reset)
            {
...
sealed class Icebreaker(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Icebreaker, 17f);

sealed class IcyThroes(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.IcyThroes1, (uint)AID.IcyThroes2], 6f);
sealed class IcyThroesSpread(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.IcyThroesSpread, 6f);
sealed class KnockOnIce(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.KnockOnIce, 5f);

sealed class Slam(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.RightSlam, (uint)AID.LeftSlam], new AOEShapeRect(80f, 10f));
sealed class AlbionsEmbrace(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.AlbionsEmbrace);

sealed class RoarOfAlbion(ModuleBase module) : Components.CastLineOfSightAOE(module, (uint)AID.RoarOfAlbion, 60f)
{
...
{
    public D111AlbionStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 896u, CFCID = 896u, NameID = 11992u, PrimaryActorOID = (uint)OID.Albion, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
public sealed class D111Albion(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(24f, -744f), new ArenaBoundsSquare(19.5f));
