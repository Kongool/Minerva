...
namespace Minerva.Endwalker.Dungeon.D09AlzadaalsLegacy.D091Ambujam;
...
sealed class ToxinShowerCorrosiveVenom(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        if (pos is WPos p)
        {
            _aoes.Add(new(circle, p.Quantized(), default, World.FutureTime(10.5d)));
        }
    }
...
sealed class ToxicCorrosiveFountain(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.ToxicFountain, (uint)AID.CorrosiveFountain], 8f, 10, 13);

sealed class BigWave(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.BigWave);

sealed class D091AmbujamStates : StateMachineBuilder
{
    public D091AmbujamStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 844u, NameID = 11241u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS) (ported from BMR)")]
public sealed class D091Ambujam(WorldState ws, Actor primary) : ModuleBase(ws, primary, DefaultBounds.Center, DefaultBounds)
{
