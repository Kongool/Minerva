...
namespace Minerva.Endwalker.Dungeon.D05Aitiascope.D052Rhitahtyn;
...
class ArenaChanges(ModuleBase module) : ModuleComponent(module)
{
    ...
    private readonly List<Rectangle> union = [];
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        else if (state == 0x00200010)
        {
            ...
            Safespots = true;
            activation = World.FutureTime(7.3d);
            UpdateArena();
        }
    ...
    {
        Bounds = new ArenaBoundsCustom([.. rect, .. union]);
    }
...
class ShieldSkewer(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ShieldSkewer, new AOEShapeRect(40f, 7f))
{
...
class Shrapnel(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        for (var i = 0; i < 18; ++i)
            _aoes.Add(new(circle, coords[i], default, World.FutureTime(8d)));
    }
...
class Impact(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Impact, new AOEShapeRect(14f, 20f));
class TartareanSpark(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TartareanSpark, new AOEShapeRect(40f, 3f));
class AnvilOfTartarus(ModuleBase module) : Components.SingleTargetDelayableCast(module, (uint)AID.AnvilOfTartarus);
class TartareanImpact(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.TartareanImpact);

class D052RhitahtynStates : StateMachineBuilder
{
    public D052RhitahtynStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 786u, NameID = 10292u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS) (ported from BMR)")]
public class D052Rhitahtyn(WorldState ws, Actor primary) : ModuleBase(ws, primary, ArenaChanges.ArenaCenter, ArenaChanges.DefaultBounds);
