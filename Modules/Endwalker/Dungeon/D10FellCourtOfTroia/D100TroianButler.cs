...
namespace Minerva.Endwalker.Dungeon.D10FellCourtOfTroia.D100TroianButler;
...
sealed class Karma(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Karma, new AOEShapeCone(30f, 45f.Degrees()));
sealed class Dark(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Dark, new AOEShapeCone(6f, 60f.Degrees()));
sealed class ArachneWeb(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ArachneWeb, 6f);
sealed class Swoop(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Swoop, 6f);
sealed class UnholyDarkness(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.UnholyDarkness, 8f);

sealed class D100TroianButlerStates : StateMachineBuilder
{
    public D100TroianButlerStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 869u, NameID = 11367u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D100TroianButler(WorldState ws, Actor primary) : ModuleBase(ws, primary, IsArena1(primary) ? arena1.Center : arena2.Center, IsArena1(primary) ? arena1 : arena2)
{
