...
namespace Minerva.Endwalker.Dungeon.D02TowerOfBabil.D021Barnabas;
...
sealed class ArenaChange(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        {
            Bounds = D021Barnabas.SmallerBounds;
            _aoe = [];
        }
    }
    ...
    {
        if (spell.Action.ID is (uint)AID.GroundAndPound1 or (uint)AID.GroundAndPound2 && Bounds.Radius > 15f)
        {
            _aoe = [new(donut, Center, default, Module.CastFinishAt(spell, 6.1d))];
        }
    }
}

sealed class Magnetism(ModuleBase module) : Components.GenericKnockback(module)
{
    ...
    public override ReadOnlySpan<Knockback> ActiveKnockbacks(int slot, Actor actor)
    {
        if (activation != default)
        {
            ...
            if (shape)
            {
                ...
                knockback[0] = new(Center, 9f, activation, _shape, a90, kind);
                knockback[1] = new(Center, 9f, activation, _shape, -a90, kind);
                return knockback;
            }
            else
                return new Knockback[1] { new(Center, 5f, activation, kind: isPull ? Kind.TowardsOrigin : Kind.AwayFromOrigin, ignoreImmunes: true) };
        }
        return [];
    }
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        switch (iconID)
        {
            case (uint)IconID.Plus:
                ...
                activation = World.FutureTime(8.1d);
                break;
...
sealed class Cleaves(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.ElectromagneticRelease1, (uint)AID.GroundAndPound1, (uint)AID.GroundAndPound2,
(uint)AID.DynamicPoundMinus, (uint)AID.DynamicPoundPlus], new AOEShapeRect(40f, 3f));
sealed class Circles(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.ElectromagneticRelease2, (uint)AID.DynamicScraplineMinus,
(uint)AID.DynamicScraplinePlus, (uint)AID.RollingScrapline, (uint)AID.Shock], 8f);

sealed class ShockingForce(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.ShockingForce, 6f, 4, 4);

sealed class D021BarnabasStates : StateMachineBuilder
{
    public D021BarnabasStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 785u, CFCID = 785u, NameID = 10279u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS) (ported from BMR)")]
public sealed class D021Barnabas(WorldState ws, Actor primary) : ModuleBase(ws, primary, arenaCenter, StartingBounds)
{
