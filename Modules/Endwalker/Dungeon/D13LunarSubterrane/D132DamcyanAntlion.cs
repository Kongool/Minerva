...
namespace Minerva.Endwalker.Dungeon.D13LunarSubterrane.D132DamcyanAntlion;
...
sealed class ArenaChange(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        if (spell.Action.ID == (uint)AID.Sandblast && Bounds.Radius > 21f)
        {
            var center = Center;
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        {
            Bounds = new ArenaBoundsRect(19.5f, 20f);
            _aoe = [];
        }
    }
}

sealed class Sandblast(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Sandblast);

sealed class Landslip(ModuleBase module) : Components.GenericKnockback(module)
{
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        var length = Bounds.Radius * 2f; // casters are at the border, orthogonal to borders
...
sealed class EarthenGeyser(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.EarthenGeyser, 10f, 4, 4);
sealed class QuicksandVoidzone(ModuleBase module) : Components.Voidzone(module, 10f, GetVoidzone)
{
...
sealed class PoundSand(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PoundSand, 12f);

sealed class AntlionMarch(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class Towerfall(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        if (index > 0x00 && state == 0x00020001u)
        {
            ...
            _aoes.Add(new(rect, new WPos(posX, posZ).Quantized(), rot, World.FutureTime(13d - _aoes.Count)));
        }
    }
...
{
    public D132DamcyanAntlionStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 823u, CFCID = 823u, NameID = 12484u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
public sealed class D132DamcyanAntlion(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(0f, 60f), new ArenaBoundsRect(19.5f, 25f));
