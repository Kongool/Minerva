...
namespace Minerva.Endwalker.Dungeon.D12Aetherfont.D123Octomammoth;
...
sealed class Wallop(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Wallop, new AOEShapeRect(22f, 4f));
sealed class VividEyes(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.VividEyes, new AOEShapeDonut(20f, 26f));
sealed class Clearout(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Clearout, new AOEShapeCone(16f, 60f.Degrees()));
sealed class TidalBreathBreathstroke(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.TidalBreath, (uint)AID.Breathstroke], new AOEShapeCone(35f, 90f.Degrees()));
sealed class TidalRoar(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.TidalRoar);
sealed class WaterDrop(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.WaterDrop, 6f);
sealed class SalineSpit(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SalineSpit2, 8f);
sealed class Telekinesis(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Telekinesis2, 12f);

sealed class D123OctomammothStates : StateMachineBuilder
{
    public D123OctomammothStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 822u, CFCID = 822u, NameID = 12334u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "dhoggpt, Malediktus (ported from BMR)")]
public sealed class D123Octomammoth(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
