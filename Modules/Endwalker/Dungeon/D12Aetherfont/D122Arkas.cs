...
namespace Minerva.Endwalker.Dungeon.D12Aetherfont.D122Arkas;
...
sealed class ArenaChange(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        if (index == 0x00)
        {
            ...
            {
                Bounds = D122Arkas.SmallerBounds;
                Center = D122Arkas.SmallerBounds.Center;
                _aoe = [];
            }
            else if (state == 0x00080004u)
            {
                Bounds = D122Arkas.DefaultBounds;
                Center = D122Arkas.DefaultBounds.Center;
            }
        }
    }
...
sealed class SpunLightning(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SpunLightning, new AOEShapeRect(30f, 4f));
sealed class LightningClaw(ModuleBase module) : Components.StackWithIcon(module, (uint)IconID.Stackmarker, (uint)AID.LightningClaw2, 6, 5.2f, 4, 4);

sealed class ForkedFissures(ModuleBase module) : Components.GenericAOEs(module)
{
        ...
        if (state is 0x00200010u or 0x00020001u)
        {
            ...
            if (pattern != default)
            {
                ...
                for (var i = 15; i >= 0; --i)
                {
                    var start = starts[i];
                    var end = ends[i];
                    _aoes.Add(new(new AOEShapeRect((start - end).Length(), 2f), start, Angle.FromDirection(end - start), World.FutureTime(6d)));
                }
            }
        }
    }
...
sealed class ElectrifyLightningLeapRampageSpinningClaw(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.Electrify, (uint)AID.LightningLeap1,
(uint)AID.LightningLeap2, (uint)AID.LightningRampage1, (uint)AID.LightningRampage2, (uint)AID.SpinningClaw], 10f);

sealed class RipperClaw(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.RipperClaw);
sealed class Shock(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Shock, 6f);
sealed class BattleCryElectricEruption(ModuleBase module) : Components.RaidwideCasts(module, [(uint)AID.BattleCry1, (uint)AID.BattleCry2, (uint)AID.ElectricEruption]);

sealed class D122ArkasStates : StateMachineBuilder
{
    public D122ArkasStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 822u, CFCID = 822u, NameID = 12337u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "dhoggpt, Malediktus (ported from BMR)")]
public sealed class D122Arkas(WorldState ws, Actor primary) : ModuleBase(ws, primary, DefaultBounds.Center, DefaultBounds)
{
