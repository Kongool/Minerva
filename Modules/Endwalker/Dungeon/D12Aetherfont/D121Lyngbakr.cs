...
namespace Minerva.Endwalker.Dungeon.D12Aetherfont.D121Lyngbakr;
...
sealed class ExplosiveResonantFrequency(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class SonicBloop(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.SonicBloop);
sealed class Waterspout(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.Waterspout, 5f);
...
sealed class Tidalspout(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.Tidalspout, 6f, 4, 4);
sealed class UpsweepBodySlam(ModuleBase module) : Components.RaidwideCasts(module, [(uint)AID.Upsweep, (uint)AID.BodySlam]);

sealed class D121LyngbakrStates : StateMachineBuilder
{
    public D121LyngbakrStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 822u, CFCID = 822u, NameID = 12336u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "dhoggpt, Malediktus (ported from BMR)")]
public sealed class D121Lyngbakr(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
