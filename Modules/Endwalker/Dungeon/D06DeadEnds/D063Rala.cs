...
namespace Minerva.Endwalker.Dungeon.D06DeadEnds.D063Rala;
...
class Doom(ModuleBase module) : ModuleComponent(module)
{
...
class LamellarLightCircle(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.LamellarLight1, 15f, 3);
class Lifesbreath(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Lifesbreath, new AOEShapeRect(50f, 5f));
class LamellarLightRect(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.LamellarLight3, new AOEShapeRect(40f, 2f));
class StillEmbrace(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.StillEmbrace, 6f);
class Benevolence(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.Benevolence, 6f, 4, 4);

class LovingEmbrace(ModuleBase module, uint aid) : Components.SimpleAOEs(module, aid, new AOEShapeCone(45f, 90f.Degrees()));
class LovingEmbraceLeft(ModuleBase module) : LovingEmbrace(module, (uint)AID.LovingEmbraceLeft);
class LovingEmbraceRight(ModuleBase module) : LovingEmbrace(module, (uint)AID.LovingEmbraceRight);

class Pity(ModuleBase module) : Components.SingleTargetDelayableCast(module, (uint)AID.Pity);
class WarmGlow(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.WarmGlow);

class D063RalaStates : StateMachineBuilder
{
    public D063RalaStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 792u, NameID = 10316u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS) (ported from BMR)")]
public class D063Rala(WorldState ws, Actor primary) : ModuleBase(ws, primary, defaultBounds.Center, defaultBounds)
{
