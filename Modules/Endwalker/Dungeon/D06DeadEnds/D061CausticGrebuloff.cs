...
namespace Minerva.Endwalker.Dungeon.D06DeadEnds.D061CausticGrebuloff;
...
sealed class CertainSolitude(ModuleBase module) : Components.GenericStackSpread(module)
{
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        if (status.ID == (uint)SID.CravenCompanionship)
        {
            ...
            if (Stacks.Count == 0)
            {
                Stacks.Add(new(actor, 1.5f, 4, 4, activation: World.FutureTime(5d)));
            }
        }
    }
...
sealed class Necrosis(ModuleBase module) : Components.CleansableDebuff(module, (uint)SID.Necrosis);

sealed class NecroticFluidMist(ModuleBase module) : Components.Exaflare(module, 6f)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID == (uint)AID.NecroticFluid)
        {
            ...
            {
                Pattern.Southward => GetSouthwardExplosions(caster.Position, Center),
                Pattern.Northward => GetNorthwardExplosions(caster.Position, Center),
                _ => 0
            };
...
sealed class Befoulment(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.Befoulment, 6f);
sealed class BlightedWater(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.BlightedWater, 6f, 4, 4);
sealed class CoughUpAOE(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.CoughUpAOE, 6f);

sealed class WaveOfNausea(ModuleBase module) : Components.GenericAOEs(module)
{
...
sealed class PoxFlail(ModuleBase module) : Components.SingleTargetDelayableCast(module, (uint)AID.PoxFlail);
sealed class Miasmata(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Miasmata);

sealed class D061CausticGrebuloffStates : StateMachineBuilder
{
    public D061CausticGrebuloffStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 792u, CFCID = 792u, NameID = 10313u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS) (ported from BMR)")]
public sealed class D061CausticGrebuloff(WorldState ws, Actor primary) : ModuleBase(ws, primary, defaultBounds.Center, defaultBounds)
{
