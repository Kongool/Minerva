...
namespace Minerva.Endwalker.Quest.Job.Reaper.TheKillingArt;
...
class VoidMortar(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.VoidMortar1, 13);
class FocusInferi(ModuleBase module) : Components.VoidzoneAtCastTarget(module, 6, (uint)AID.FocusInferi, m => m.Enemies((uint)OID.Voidzone).Where(x => x.EventState != 7), 0);
class CarnemLevareCross(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.CarnemLevare1, new AOEShapeCross(40, 4));

class CarnemLevareDonut(ModuleBase module) : Components.GenericAOEs(module)
{
...
class MeatySlice(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MeatySlice, new AOEShapeRect(50, 6));

abstract class Cleave(ModuleBase module, uint aid) : Components.SimpleAOEs(module, aid, new AOEShapeCone(40, 60.Degrees()));
class Cleaver(ModuleBase module) : Cleave(module, (uint)AID.Cleaver);
class FlankCleaver(ModuleBase module) : Cleave(module, (uint)AID.FlankCleaver);

class Adds(ModuleBase module) : Components.AddsMulti(module, [(uint)OID.VoidHecteyes, (uint)OID.VoidPersona], 1);

class OrcusStates : StateMachineBuilder
{
    public OrcusStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.Quest, CFCID = 69614u, NameID = 10581u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")]
public class Orcus(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
