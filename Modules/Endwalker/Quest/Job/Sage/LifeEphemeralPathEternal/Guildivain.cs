...
namespace Minerva.Endwalker.Quest.Job.Sage.LifeEphemeralPathEternal.Guildivain;
...
class AetherstreamTether(ModuleBase module) : Components.BaitAwayTethers(module, new AOEShapeRect(50f, 2f), (uint)TetherID.Noulith)
{
...
{
    public Tracheostomy(ModuleBase module) : base(module, (uint)AID.Tracheostomy, new AOEShapeDonut(10f, 20f))
    {
        World.Actors.EventStateChanged.Subscribe((act, state) =>
        {
            if (act.OID == 0x1EA1A1 && act.EventState == 7)
                Bounds = AncelAndMahaud.AncelAndMahaud.ArenaBounds;
        });
    }
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        if (spell.Action.ID == WatchedAction)
            Bounds = Guildivain.SmallBounds;
    }
}

class Scalpel(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.RightScalpel, (uint)AID.LeftScalpel,
...
class Laparotomy(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Laparotomy, new AOEShapeCone(15f, 60f.Degrees()));
class Amputation(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Amputation, new AOEShapeCone(20f, 60f.Degrees()));

class Hypothermia(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Hypothermia);
class Cryonics(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.Cryonics, 6f);
class Craniotomy(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Craniotomy);

class Frigotherapy(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.Frigotherapy, 5f);

class GuildivainStates : StateMachineBuilder
{
    public GuildivainStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 69608u, NameID = 10733u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")]
public class Guildivain(WorldState ws, Actor primary) : ModuleBase(ws, primary, SmallBounds.Center, AncelAndMahaud.AncelAndMahaud.ArenaBounds)
{
