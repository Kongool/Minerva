...
namespace Minerva.Endwalker.Quest.MSQ.WhereEverythingBegins.P1;
...
class Nox(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Nox, 10, 5);
class VoidGravity(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.VoidGravity, 6);
class VoidVortex(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.VoidVortex, 6);

class ScarmiglioneP1States : StateMachineBuilder
{
    public ScarmiglioneP1States(ModuleBase module) : base(module)
    {
        TrivialPhase()
            .ActivateOnEnter<Nox>()
...
[ModuleInfo(Group = ModuleGroup.Quest, CFCID = 70130u, NameID = 11407u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "ported from BossmodReborn")]
public class ScarmiglioneP1(WorldState ws, Actor primary) : ModuleBase(ws, primary, ArenaBounds.Center, ArenaBounds)
{
