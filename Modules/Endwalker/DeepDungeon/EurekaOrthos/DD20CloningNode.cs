...
namespace Minerva.Endwalker.DeepDungeon.EurekaOrthos.DD20CloningNode;
...
sealed class FlameBreath(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        {
            var activation = World.FutureTime(9.6d);

            if ((caster.Position - Center).LengthSq() > 625f)
            {
...
sealed class PiercingLaser(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PiercingLaser, new AOEShapeRect(40f, 2.5f));

sealed class DD20CloningNodeStates : StateMachineBuilder
{
    public DD20CloningNodeStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 898u, CFCID = 898u, NameID = 12261u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class DD20CloningNode(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-300f, -300f), new ArenaBoundsCircle(19.5f));
