...
namespace Minerva.Endwalker.DeepDungeon.EurekaOrthos.DD50ServomechanicalChimera14X;
...
class RamsDragonVoice(ModuleBase module) : Components.GenericAOEs(module)
{
    private static readonly AOEShapeDonut donut = new(8f, 40f);
    private static readonly AOEShapeCircle circle = new(9f);
    private readonly List<AOEInstance> _aoes = [];
    private readonly List<(AOEShape, DateTime)> _shapes = [];
    ...
    public override void OnActorModelStateChange(Actor actor, byte modelState, byte animState1, byte animState2)
    {
        ...
        void AddAOEs(AOEShape first, AOEShape second)
        {
            _shapes.Add((first, World.FutureTime(2.4d)));
            _shapes.Add((second, World.FutureTime(5.5d)));
        }
    }
...
class TC(ModuleBase module, uint aid) : Components.BaitAwayChargeCast(module, aid, 4f);
class ThunderousCold(ModuleBase module) : TC(module, (uint)AID.ThunderousCold);
class ColdThunder(ModuleBase module) : TC(module, (uint)AID.ColdThunder);

class Breath(ModuleBase module, uint aid) : Components.SimpleAOEs(module, aid, new AOEShapeCone(40f, 90f.Degrees()));
class RightbreathedCold(ModuleBase module) : Breath(module, (uint)AID.RightbreathedCold);
class LeftbreathedThunder(ModuleBase module) : Breath(module, (uint)AID.LeftbreathedThunder);

class ChargeTether(ModuleBase module) : Components.StretchTetherDuo(module, 15f, 5.1f);

class Cacophony(ModuleBase module) : Components.GenericAOEs(module)
{
...
class CacophonyTether(ModuleBase module) : Components.StretchTetherSingle(module, (uint)TetherID.OrbTether, 15f, needToKite: true);

class DD50ServomechanicalChimera14XStates : StateMachineBuilder
{
    public DD50ServomechanicalChimera14XStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 901u, NameID = 12265u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class DD50ServomechanicalChimera14X(WorldState ws, Actor primary) : ModuleBase(ws, primary, new(-300f, -300f), new ArenaBoundsCircle(19.5f));
