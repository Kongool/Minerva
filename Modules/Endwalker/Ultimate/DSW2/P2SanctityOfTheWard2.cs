...
sealed class P2SanctityOfTheWard2HeavensStakeCircles(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HeavensStakeAOE, 7f);
sealed class P2SanctityOfTheWard2HeavensStakeDonut(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HeavensStakeDonut, new AOEShapeDonut(15f, 30f));
sealed class P2SanctityOfTheWard2VoidzoneFire(ModuleBase module) : Components.Voidzone(module, 7f, m => m.Enemies((uint)OID.VoidzoneFire).Where(z => z.EventState != 7));
sealed class P2SanctityOfTheWard2VoidzoneIce(ModuleBase module) : Components.Voidzone(module, 7f, m => m.Enemies((uint)OID.VoidzoneIce).Where(z => z.EventState != 7));

sealed class P2SanctityOfTheWard2Knockback(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.FaithUnmoving, 16f)
{
    private readonly DSW2Config _config = Service.Config.Get<DSW2Config>();

    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (_config.P2Sanctity2AutomaticAntiKB && Casters.Count > 0 && !actor.Position.InCircle(Center, 12f))
        {
            ...
            hints.ActionsToExecute.Push(action, actor, ActionQueue.Priority.High, World.Actors.Find(Casters.Ref(0).ActorID)?.CastInfo?.NPCRemainingTime ?? default);
        }
    }
}

// note: technically it's a 2-man stack, but that is not really helpful here...
sealed class P2SanctityOfTheWard2HiemalStorm(ModuleBase module) : Components.CastCounter(module, (uint)AID.HiemalStormAOE)
{
...
sealed class P2SanctityOfTheWard2Towers1(ModuleBase module) : Components.CastTowers(module, (uint)AID.Conviction2AOE, 3f)
{
    struct PlayerData
    {
    ...
    private WPos StormPlacementPosition(int quadrant)
    {
        ...
        return Center + _stormPlacementOffset * dir.ToDirection();
    }
...
sealed class P2SanctityOfTheWard2Towers2(ModuleBase module) : Components.CastTowers(module, (uint)AID.Conviction3AOE, 3)
{
    private bool _preyOnTH;
    private BitMask _preyTargets;
    ...
    {
        base.OnEventCast(caster, spell);
        if (spell.Action.ID == (uint)AID.Conviction2AOE && !spell.TargetXZ.InCircle(Center, 7f))
        {
            ...
            foreach (var t in spell.Targets)
            {
                ...
                {
                    var towerOffset = spell.TargetXZ - Center;
