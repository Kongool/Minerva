...
abstract class P6Wyrmsbreath(ModuleBase module, bool allowIntersect) : Components.GenericBaitAway(module, (uint)AID.FlameBreath) // note: cast is arbitrary
{
    ...
    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        if (tether.ID is (uint)TetherID.FlameBreath or (uint)TetherID.IceBreath or (uint)TetherID.FlameIceBreathNear)
        {
            ...
            var boss = World.Actors.Find(tether.Target);
            if (slot >= 0 && boss != null)
            {
...
sealed class P6Wyrmsbreath1(ModuleBase module) : P6Wyrmsbreath(module, true);
sealed class P6Wyrmsbreath2(ModuleBase module) : P6Wyrmsbreath(module, false);
...
sealed class P6WyrmsbreathTankbusterShared(ModuleBase module) : Components.GenericSharedTankbuster(module, (uint)AID.DarkOrb, 6f)
{
    ...
    public override void Update()
    {
        ...
        if (_main?.Glows.Raw == 3)
        {
            ...
            Target = World.Actors.Find(Source?.TargetID ?? 0);
            Activation = Module.CastFinishAt(Source?.CastInfo);
        }
    }
}

sealed class P6WyrmsbreathTankbusterSolo(ModuleBase module) : Components.GenericBaitAway(module, centerAtTarget: true)
{
    private readonly P6Wyrmsbreath? _main = module.FindComponent<P6Wyrmsbreath>();
    ...
    public override void Update()
    {
        ...
        if (_main?.Glows.Raw is 1 or 2)
        {
            ...
            var target = World.Actors.Find(source?.TargetID ?? default);
...
sealed class P6WyrmsbreathCone(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly P6Wyrmsbreath? _main = module.FindComponent<P6Wyrmsbreath>();
