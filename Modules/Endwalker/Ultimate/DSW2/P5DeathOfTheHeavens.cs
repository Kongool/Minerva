...
sealed class P5DeathOfTheHeavensHeavyImpact(ModuleBase module) : HeavyImpact(module, 10.5d);

sealed class P5DeathOfTheHeavensGaze(ModuleBase module) : DragonsGaze(module, (uint)OID.BossP5, 23.5d);

// TODO: make more meaningful somehow
sealed class P5DeathOfTheHeavensDooms(ModuleBase module) : ModuleComponent(module)
{
...
{
    public P5DeathOfTheHeavensLightningStorm(ModuleBase module) : base(module, default, 5f)
    {
...
sealed class P5DeathOfTheHeavensHeavensflame(ModuleBase module) : Components.GenericKnockback(module, (uint)AID.HeavensflameAOE)
{
    ...
    {
        return new Knockback[1] { new(Center, _knockbackDistance) };
    }

    public override void Update()
    {
        foreach (var (slot, player) in Raid.WithSlot(false, true, true))
            _playerAdjustedPositions[slot] = !KnockbackDone ? AwayFromSource(player.Position, Center, _knockbackDistance) : player.Position;
    }
...
sealed class P5DeathOfTheHeavensMeteorCircle(ModuleBase module) : Components.Adds(module, (uint)OID.MeteorCircle);
