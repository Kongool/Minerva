...
{
    public P2SanctityOfTheWard1Gaze(ModuleBase module) : base(module, (uint)OID.BossP2, 9.4d)
    {
        EnableHints = true;
    }
}
...
sealed class P2SanctityOfTheWard1Sever(ModuleBase module) : Components.UniformStackSpread(module, 6f, default, 4)
{
...
sealed class P2SanctityOfTheWard1Flares(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.BrightFlare, "GTFO from charges and spheres!")
{
    ...
    private Angle BuildChargeInfo(uint oid)
    {
        ...
        if (!Utils.AlmostEqual(actor.PosRot.Z, Center.Z, 1f))
            return default;
        if (!Utils.AlmostEqual(Math.Abs(actor.PosRot.X - Center.X), 5f, 1f))
            return default;

        var right = actor.PosRot.X > Center.X;
        ...
        WPos posAt(Angle dir) => Center + 21f * dir.ToDirection();
...
// hints & assignments
sealed class P2SanctityOfTheWard1Hints(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void Update()
    {
        ...
        {
            _inited = true;
            _severStartDir = Angle.FromDirection(_sever.Source.Position - Center);
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        foreach (var safespot in MovementHintOffsets(pcSlot).Take(1))
        {
            ...
            if (_groupEast.None())
                Arena.ZoneCircleOutline(Center - safespot, 1f, Colors.Safe); // if there are no valid assignments, draw spots for both groups
        }
    }
