...
sealed class P6WrothFlames : Components.GenericAOEs
{
    ...
    public P6WrothFlames(ModuleBase module) : base(module)
    {
        bossmodule = (DSW2)module;
        if (bossmodule._HraesvelgrP6 is Actor hraesvelgr)
        {
            _aoes.Add(new(_shapeCauterize, hraesvelgr.Position.Quantized(), hraesvelgr.Rotation, World.FutureTime(8.1d)));
    ...
    public override void OnActorCreated(Actor actor)
    {
        if (actor.OID == (uint)OID.ScarletPrice)
        {
            if (_aoes.Count == 4)
            {
                _startingSpot = new(_startingSpot.X, actor.PosRot.Z < Center.Z ? 120f : 80f);
            }
            ...
            _aoes.Add(new(_shapeBlast, actor.Position.Quantized(), default, World.FutureTime(delay)));
        }
    }
...
sealed class P6AkhMorn(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.AkhMornFirst, 6f, 8, 8)
{
...
sealed class P6AkhMornVoidzone(ModuleBase module) : Components.Voidzone(module, 6f, GetVoidzones)
{
...
sealed class P6SpreadingEntangledFlames(ModuleBase module) : Components.UniformStackSpread(module, 4f, 5f, 2)
{
    ...
    private WPos[] SafeSpots(Actor actor)
    {
        ...
        var x = Center.X;
        var z = Center.Z + (_wingTail.NumAOEs != 1 ? default : _voidzonesNorth ? 10f : -10f);
