...
sealed class P5WrathOfTheHeavensSkywardLeap(ModuleBase module) : Components.UniformStackSpread(module, default, 24f, raidwideOnResolve: false)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        if (iconID == (uint)IconID.SkywardLeapP5)
            AddSpread(actor, World.FutureTime(6.4d));
    }
    ...
    private WPos SafeSpot()
    {
        ...
        if (relNorth == null)
            return default;
        var dirToNorth = Angle.FromDirection(relNorth.Position - Center);
        return Center + 20f * (dirToNorth + 60f.Degrees()).ToDirection();
    }
}

sealed class P5WrathOfTheHeavensSpiralPierce(ModuleBase module) : Components.BaitAwayTethers(module, new AOEShapeRect(50f, 8f), (uint)TetherID.SpiralPierce, (uint)AID.SpiralPierce)
{
    public override void AddMovementHints(int slot, Actor actor, MovementHints movementHints)
    {
    ...
    private WPos SafeSpot(Actor actor)
    {
        ...
        foreach (var b in CurrentBaits)
            toMidpoint += b.Source.Position - Center;
        var relSouthDir = Angle.FromDirection(-toMidpoint);
        var offset = toMidpoint.OrthoL().Dot(bait.Source.Position - Center) > 0f ? 20f.Degrees() : -20f.Degrees();
        return Center + 20f * (relSouthDir + offset).ToDirection();
    }
}

sealed class P5WrathOfTheHeavensChainLightning(ModuleBase module) : Components.UniformStackSpread(module, default, 5f)
{
    public BitMask Targets;

    public void ShowSpreads(double delay) => AddSpreads(Raid.WithSlot(true, true, true).IncludedInMask(Targets).Actors(), World.FutureTime(delay));
...
sealed class P5WrathOfTheHeavensTwister(ModuleBase module) : Components.GenericAOEs(module, default, "GTFO from twister!")
{
    ...
    private static WPos[] GetPositions(ModuleBase module)
    {
...
sealed class P5WrathOfTheHeavensCauterizeBait(ModuleBase module) : ModuleComponent(module)
{
    ...
    private WPos SafeSpot()
    {
        ...
        return Center + 20f * (charibert.Position - Center).Normalized();
    }
}

sealed class P5WrathOfTheHeavensAscalonsMercyRevealed(ModuleBase module) : Components.BaitAwayEveryone(module, module.Enemies((uint)OID.BossP5).FirstOrDefault(), new AOEShapeCone(50f, 15f.Degrees()), (uint)AID.AscalonsMercyRevealedAOE);

// TODO: detect baiter
sealed class P5WrathOfTheHeavensLiquidHeaven(ModuleBase module) : Components.VoidzoneAtCastTarget(module, 6f, (uint)AID.LiquidHeaven, m => m.Enemies((uint)OID.VoidzoneLiquidHeaven).Where(z => z.EventState != 7), 1.1f);

// TODO: detect baiter
sealed class P5WrathOfTheHeavensAltarFlare(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AltarFlareAOE, 8f);

sealed class P5WrathOfTheHeavensEmptyDimension(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.EmptyDimension, new AOEShapeDonut(6f, 70f))
{
