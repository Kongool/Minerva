...
sealed class HeavensflameAOE(ModuleBase module) : Components.CastCounter(module, (uint)AID.HeavensflameAOE);

sealed class HeavensflameKnockback(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.FaithUnmoving, 16f)
{
    ...
    public override void AddHints(int slot, Actor actor, TextHints hints)
    {
        ...
        if (Casters.Count > 0 && IsImmune(slot, World.CurrentTime))
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        ...
        foreach (var hint in PositionHints(pcSlot))
        {
            ...
            //var adjPos = ClampToBounds(_knockbackSource.Position + 50 * dir);
            //Arena.AddLine(Center, adjPos, Colors.Safe);
        }
    ...
    private List<WPos> PositionHints(int slot)
    {
        ...
        switch (Service.Config.Get<DSW1Config>().Heavensflame)
        {
            case DSW1Config.HeavensflameHints.Waymarks:
                {
                    if (World.Waymarks.GetFieldMark((int)Waymark.A + (icon - 1)) is var alt1 && alt1 != null)
                        hints.Add(new(alt1.Value.XZ()));
                    if (World.Waymarks.GetFieldMark((int)Waymark.N1 + (icon - 1)) is var alt2 && alt2 != null)
