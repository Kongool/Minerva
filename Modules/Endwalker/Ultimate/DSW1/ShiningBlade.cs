...
sealed class ShiningBladeKnockback(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.FaithUnmoving, 16f)
{
    ...
    public override void AddHints(int slot, Actor actor, TextHints hints)
    {
        ...
        for (var i = 0; i < count; ++i)
        {
            ...
            if (_dirToAdelphel.Dot(e.to - Center) > 0)
...
sealed class ShiningBladeFlares(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.BrightFlare, "GTFO from explosion!")
{
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        ...
        for (var i = 0; i < max; ++i)
            aoes[i] = new(_shape, (Center + _flares[i]).Quantized()); // TODO: activation
    ...
    public override void OnActorPlayActionTimelineEvent(Actor actor, ushort id)
    {
        ...
        var bossOffset = actor.Position - Center;
        if (!Utils.AlmostEqual(bossOffset.LengthSq(), Bounds.Radius * Bounds.Radius, 1f))
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        if (spell.Action.ID == (uint)AID.ShiningBlade && _flares.Count <= 1)
        {
            ...
            var endOffset = spell.TargetXZ - Center;
            _flares.Clear();
...
sealed class ShiningBladeExecution(ModuleBase module) : Components.CastCounter(module, (uint)AID.Execution)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        switch (spell.Action.ID)
        {
            case (uint)AID.ShiningBlade:
                _target = World.Actors.Find(Module.Enemies((uint)OID.SerAdelphel).FirstOrDefault()?.TargetID ?? 0);
                break;
