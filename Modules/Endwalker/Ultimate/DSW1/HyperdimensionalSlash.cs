...
sealed class HyperdimensionalSlash(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void Update()
    {
        ...
        var coneTarget = Raid.WithSlot(false, true, true).ExcludedFromMask(_laserTargets).Actors().Closest(Center);
        _coneDir = coneTarget != null ? Angle.FromDirection(coneTarget.Position - Center) : default;
    }
    ...
    public override void AddHints(int slot, Actor actor, TextHints hints)
    {
        ...
        if (Raid.WithSlot(false, true, true).IncludedInMask(otherLasers).WhereActor(target => _aoeLaser.Check(actor.Position, Center, Angle.FromDirection(target.Position - Center))).Any())
        ...
        var hitByCone = _aoeCone.Check(actor.Position, Center, _coneDir);
    ...
    public override void DrawArenaBackground(int pcSlot, Actor pc)
    {
        ...
        foreach (var t in Raid.WithSlot(false, true, true).IncludedInMask(_laserTargets).Actors())
            _aoeLaser.Draw(Arena, Center, Angle.FromDirection(t.Position - Center));
        _aoeCone.Draw(Arena, Center, _coneDir);
    }
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        ...
        if (_laserTargets[pcSlot])
            Arena.AddLine(Center, TearPosition(pc));
    }
    ...
    private WPos TearPosition(Actor target) => Center + Bounds.ClampToBounds(50f * (target.Position - Center).Normalized());
}
