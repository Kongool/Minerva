...
sealed class P3HelloWorld(ModuleBase module) : Components.GenericTowers(module)
{
    ...
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        switch (status.ID)
        {
            ...
            case (uint)SID.HWPrepLocalTether:
                if ((status.ExpireAt - World.CurrentTime).TotalSeconds < 30d)
    ...
    private WPos[] PositionsForTowers(int slot)
    {
        ...
        foreach (var i in _defamationTowers.SetBits())
            defamationMid += Towers[i].Position - Center;
