...
sealed class P5Sigma(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        ...
        foreach (var safeSpot in SafeSpotOffsets(pcSlot))
            Arena.ZoneCircleOutline(Center + safeSpot, 1, Colors.Safe);
    }
    ...
    public override void OnActorPlayActionTimelineEvent(Actor actor, ushort id)
    {
        switch (actor.OID)
        {
            case (uint)OID.RightArmUnit: // TODO: can it be left unit instead?..
                if (id == 0x1E43)
                    _waveCannonNorthDir -= actor.Position - Center;
                break;
            case (uint)OID.BossP5:
                if (id == 0x1E43)
                    _waveCannonNorthDir = actor.Position - Center; // just in case...
                break;
        }
    }
...
sealed class P5SigmaHyperPulse(ModuleBase module) : Components.BaitAwayTethers(module, new AOEShapeRect(100f, 3f), (uint)TetherID.SigmaHyperPulse, (uint)AID.SigmaHyperPulse)
{
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
...
sealed class P5SigmaWaveCannon(ModuleBase module) : Components.GenericBaitAway(module, (uint)AID.SigmaWaveCannonAOE)
{
...
sealed class P5SigmaTowers(ModuleBase module) : Components.GenericTowers(module)
{
    ...
    private void InitAssignments()
    {
        ...
        foreach (var t in Towers)
            relNorth -= t.Position - Center;

        foreach (ref var tower in Towers.AsSpan())
        {
            var offset = tower.Position - Center;
...
sealed class P5SigmaRearLasers(ModuleBase module) : Components.GenericRotatingAOE(module)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        ...
        Rotation = rot;
        Sequences.Add(new(_shape, actor.Position, actor.Rotation, rot, World.FutureTime(10.1d), 0.6d, 14, 9));
    }
...
sealed class P5SigmaDoubleAOEs(ModuleBase module) : Components.GenericAOEs(module)
{
    public bool Show;
    ...
    public override void OnActorPlayActionTimelineEvent(Actor actor, ushort id)
    {
        ...
        var act = World.FutureTime(15.1d);
...
sealed class P5SigmaNearDistantWorld(ModuleBase module) : P5NearDistantWorld(module)
{
