...
sealed class P4WaveCannonProtean(ModuleBase module) : Components.GenericBaitAway(module)
{
...
sealed class P4WaveCannonProteanAOE(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.P4WaveCannonProteanAOE, new AOEShapeRect(100f, 3f));

// TODO: generalize (line stack)
sealed class P4WaveCannonStack : ModuleComponent
{
    ...
    public P4WaveCannonStack(ModuleBase module) : base(module)
    {
    ...
    {
        if (Imminent && Raid.WithSlot(true, true, true).IncludedInMask(_targets).WhereActor(p => _shape.Check(actor.Position, Center, Angle.FromDirection(p.Position - Center))).Count() is var clips && clips != 1)
    ...
    public override void AddMovementHints(int slot, Actor actor, MovementHints movementHints)
    {
        if (SafeDir(slot) is var safeDir && safeDir != default)
            movementHints.Add(actor.Position, Center + 12f * safeDir.ToDirection(), Colors.Safe);
    }
    ...
    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        if (Imminent)
            foreach (var (_, p) in Raid.WithSlot(true).IncludedInMask(_targets))
                _shape.Outline(Arena, Center, Angle.FromDirection(p.Position - Center), Colors.Safe);

        var safeDir = SafeDir(pcSlot);
        if (safeDir != default)
            Arena.ZoneCircleOutline(Center + 12 * safeDir.ToDirection(), 1, Colors.Safe);
    }
