...
sealed class P3OversampledWaveCannon(ModuleBase module) : ModuleComponent(module)
{
    private Actor? _boss;
    ...
    {
        hints.AddPredictedDamage(Raid.WithSlot(false, true, true).Mask(), _resolve);
        ...
        var safeConePlayers = Raid.WithoutSlot().ClockOrder(actor, Center, !targetCW).Skip(2).Take(2).ToList();
        if (targetCW)
        ...
        var dirToUnsafeCleave = actor.DirectionTo(Center).ToAngle() - _playerAngles[slot];
    ...
    private List<(WPos pos, bool assigned)> SafeSpots(int slot)
    {
        ...
        WPos adjust(float x, float z) => Center + new WDir(_bossAngle.Rad < 0 ? -x : x, z);
...
sealed class P3OversampledWaveCannonSpread(ModuleBase module) : Components.UniformStackSpread(module, default, 7f)
{
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
