...
sealed class P3SniperCannon(ModuleBase module) : Components.UniformStackSpread(module, 6f, 6f)
{
    ...
    private WPos SafeSpotAt(Angle dirIfStacksNorth) => Center + 19f * (_config.P3IntermissionStacksNorth ? dirIfStacksNorth : 180f.Degrees() - dirIfStacksNorth).ToDirection();
}

sealed class P3WaveRepeater(ModuleBase module) : Components.ConcentricAOEs(module, _shapes)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        ...
        if (!AdvanceSequence(order, caster.Position, World.FutureTime(2.1d)))
            ReportError($"Unexpected ring {order}");
    }
}

class P3IntermissionVoidzone(ModuleBase module) : Components.Voidzone(module, 6f, GetVoidzones)
{
...
sealed class P3ColossalBlow(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorPlayActionTimelineEvent(Actor actor, ushort id)
    {
        if (actor.OID is (uint)OID.LeftArmUnit or (uint)OID.RightArmUnit && id is 0x1E43 or 0x1E44)
            AOEs.Add(new(_shape, actor.Position.Quantized(), default, World.FutureTime(13.5d)));
    }
