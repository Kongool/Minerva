...
sealed class P5Delta(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void OnEventCast(Actor caster, ActorCastEvent spell)
    {
        if (spell.Action.ID is (uint)AID.BeyondDefenseAOE)
            _beyondDefenceTarget = World.Actors.Find(spell.MainTargetID);
    }
        ...
        if (id == 0x1E43)
        {
            switch (actor.OID)
            {
                case (uint)OID.BeetleHelper:
                    _eyeDir = (actor.Position - Center).Normalized().OrthoR();
                    break;
                case (uint)OID.FinalHelper:
                    _eyeDir = (actor.Position - Center).Normalized().OrthoL();
                    break;
            }
        }
    }
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        if (actor.OID is (uint)OID.LeftArmUnit or (uint)OID.RightArmUnit)
        {
            ...
            if (rotation != default)
                ArmRotations[ArmIndex(actor.Position - Center)] = rotation;
        }
    }
    ...
    private void InitAssignments()
    {
        ...
        float slotToOffsetX(int slot) => _eyeDir.OrthoR().Dot((Raid[slot]?.Position ?? Center) - Center);
...
sealed class P5DeltaOpticalLaser(ModuleBase module) : Components.GenericAOEs(module, (uint)AID.OpticalLaser)
{
    public Actor? Source;
    ...
    public override void OnActorPlayActionTimelineEvent(Actor actor, ushort id)
    {
        if (id == 0x1E43 && actor.OID is (uint)OID.BeetleHelper or (uint)OID.FinalHelper)
        {
            ...
            _activation = World.FutureTime(20d);
        }
    }
}

sealed class P5DeltaExplosion(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DeltaExplosion, 3f)
{
...
sealed class P5DeltaHyperPulse(ModuleBase module) : Components.GenericAOEs(module)
{
    private readonly P5Delta? _delta = module.FindComponent<P5Delta>();
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        ...
        else if (_delta != null)
        {
            ...
            for (var i = 0; i < len; ++i)
            {
                var pos = (Center + _delta.ArmOffset(i)).Quantized();
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        {
            var rot = _delta.ArmRotations[_delta.ArmIndex(caster.Position - Center)];
...
sealed class P5DeltaOversampledWaveCannon(ModuleBase module) : Components.UniformStackSpread(module, default, 7f)
{
    private readonly P5Delta? _delta = module.FindComponent<P5Delta>();
    private Actor? _boss;
...
sealed class P5DeltaSwivelCannon(ModuleBase module) : Components.GenericAOEs(module)
{
