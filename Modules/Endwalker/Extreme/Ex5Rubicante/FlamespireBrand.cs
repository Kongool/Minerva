...
class Welts(ModuleBase module) : Components.GenericStackSpread(module, true)
{
...
class Flamerake(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override ReadOnlySpan<AOEInstance> ActiveAOEs(int slot, Actor actor)
    {
        ...
        if (NumCasts == 0)
        {
            return new AOEInstance[1] { new(_first, Center.Quantized(), _offset, _activation) };
        }
        else
        {
            ...
            for (var i = 0; i < 4; ++i)
            {
                ...
                aoes[i] = new(_rest, (Center + offset * dir.ToDirection()).Quantized(), dir, _activation);
            }
            return aoes;
        }
    }
