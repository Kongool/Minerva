...
class OrdealOfPurgation(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        if (spell.Action.ID == (uint)AID.OrdealOfPurgation)
        {
            ...
            void AOEFromDirection(int index)
            {
                ...
                if (shape != null)
                {
                    _aoes.Add(new(shape, (Center + 20f * dir.ToDirection()).Quantized(), dir + 180f.Degrees(), activation));
                }
            }
        }
    }
