...
class Paradeigma(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void DrawArenaBackground(int pcSlot, Actor pc)
    {
        ...
        foreach (var c in _fireLine)
            Arena.ZoneTri(Center + c, RotatedPosition(c), Center, Colors.AOE);
    }

    public override void DrawArenaForeground(int pcSlot, Actor pc)
    {
        if (_fireLine.Count == 2)
            Arena.AddLine(Center + _fireLine[0], Center + _fireLine[1], Colors.Danger);
    }
    ...
    public override void OnMapEffect(byte index, uint state)
    {
        ...
        else if (index == 5)
        {
            ...
            {
                case 0x00020001:
                    _fireLine.Add(new(+Bounds.Radius, -Bounds.Radius));
                    _fireLine.Add(new(-Bounds.Radius, +Bounds.Radius));
                    break;
                case 0x00400020:
                    _fireLine.Add(new(-Bounds.Radius, -Bounds.Radius));
    ...
    private WPos RotatedPosition(WDir offset)
    {
        return _flow switch
        {
            FlowDirection.CW => Center + offset.OrthoR(),
            FlowDirection.CCW => Center + offset.OrthoL(),
            _ => Center + offset
        };
    }
    ...
    private (WPos, Angle) RotatedPosRot((WDir, Angle) posRot)
    {
        return _flow switch
        {
            FlowDirection.CW => (Center + posRot.Item1.OrthoR(), posRot.Item2 - 90.Degrees()),
            FlowDirection.CCW => (Center + posRot.Item1.OrthoL(), posRot.Item2 + 90.Degrees()),
            _ => (Center + posRot.Item1, posRot.Item2)
        };
    }
    ...
    {
        var p1 = Center + corner;
        var p2 = RotatedPosition(corner);
        var pMid = WPos.Lerp(p1, p2, 0.5f);
        var dirMid = (pMid - Center).Normalized();
        return pos.InCone(Center, dirMid, 45.Degrees());
    }
}
