...
class DragonsDescent(ModuleBase module) : Components.GenericKnockback(module, (uint)AID.DragonsDescent)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        ...
        {
            _source = actor;
            _activation = World.FutureTime(8.2f);
        }
    }
}

class DoubleMeteor(ModuleBase module) : Components.UniformStackSpread(module, default, 15f) // TODO: verify falloff
{
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        if (iconID == (uint)IconID.DoubleMeteor)
            AddSpread(actor, World.FutureTime(11.1d));
    }
...
class Explosion(ModuleBase module) : ModuleComponent(module)
{
    public bool Done;
...
class Cauterize(ModuleBase module) : Components.GenericBaitAway(module, (uint)AID.Cauterize)
{
    private static readonly AOEShapeRect rect = new(50f, 6f);

    public override void OnTethered(Actor source, in ActorTetherInfo tether)
    {
        if (tether.ID == (uint)TetherID.Cauterize && World.Actors.Find(tether.Target) is var target && target != null)
        {
