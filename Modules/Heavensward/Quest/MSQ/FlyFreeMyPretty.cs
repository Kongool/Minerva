...
namespace Minerva.Heavensward.Quest.MSQ.FlyFreeMyPretty;
...
class MagitekMissiles(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MagitekMissiles, 15f);
class ShrapnelShell(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ShrapnelShell, 6f);
class Firebomb(ModuleBase module) : Components.Voidzone(module, 4, m => m.Enemies((uint)OID.FireVoidzone).Where(e => e.EventState != 7));

class AugmentedUprising(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AugmentedUprising, new AOEShapeCone(8.5f, 60f.Degrees()));
...
class Overpower(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Overpower, new AOEShapeCone(6f, 45f.Degrees()));
class GrandSword(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GrandSword, new AOEShapeCone(21f, 60f.Degrees()));
class MagitekRay(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MagitekRay, 6f);
class GrandStrike(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GrandStrike, new AOEShapeRect(48f, 2f));

class Bounds(ModuleBase module) : ModuleComponent(module)
{
    public override void OnEventDirectorUpdate(uint updateID, uint param1, uint param2, uint param3, uint param4)
    {
        if (updateID == 0x10000002u)
        {
            Module.Bounds = Grynewaht.CircleBounds;
            Module.Center = Grynewaht.CircleBounds.Center;
        }
    }
}
...
[ModuleInfo(Group = ModuleGroup.Quest, CFCID = 67894u, NameID = 5576u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "Malediktus (ported from BMR)")]
public class Grynewaht(WorldState ws, Actor primary) : ModuleBase(ws, primary, default, hexBounds)
{
