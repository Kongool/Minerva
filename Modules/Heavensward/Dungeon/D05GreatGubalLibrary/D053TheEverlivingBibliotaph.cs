...
namespace Minerva.Heavensward.Dungeon.D05GreatGubalLibrary.D053EverlivingBibliotaph;
...
class VoidSparkBait(ModuleBase module) : Components.GenericBaitAway(module, centerAtTarget: true)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        if (iconID == (uint)IconID.VoidSpark)
            CurrentBaits.Add(new(Module.PrimaryActor, actor, Circle, World.FutureTime(5.1d)));
    }
    ...
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        ...
        if (CurrentBaits.Count != 0 && CurrentBaits.Ref(0).Target == actor)
            hints.AddForbiddenZone(new SDCircle(Center, 22.5f));
    }
...
class VoidSpark(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        if (actor.OID == (uint)OID.Voidsphere)
            _aoes.Add(new(VoidSparkBait.Circle, actor.Position.Quantized(), default, World.FutureTime(9.8d)));
    }
...
class DeepDarkness(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DeepDarkness, new AOEShapeDonut(12f, 25f));
class MagicBurst(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.MagicBurst, 15f);
class VoidBlizzardIIIAOE(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.VoidBlizzardIIIAOE, 5f);
class AbyssalSwing(ModuleBase module) : Components.Cleave(module, (uint)AID.AbyssalSwing, new AOEShapeCone(7.5f, 45f.Degrees()), [(uint)OID.Biblioklept]);
class AbyssalCharge(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AbyssalCharge, new AOEShapeRect(41f, 2f));

class VoidCall(ModuleBase module) : Components.GenericTowers(module, prioritizeInsufficient: true)
{
    public override void OnActorCreated(Actor actor)
    {
        if (actor.OID == (uint)OID.Tower)
            Towers.Add(new(actor.Position, 3, activation: World.FutureTime(17.8d)));
...
{
    public D053EverlivingBibliotaphStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 31u, NameID = 3930u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D053EverlivingBibliotaph(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
