...
namespace Minerva.Heavensward.Dungeon.D11Antitower.D113Calcabrina;
...
class TerrifyingGlanceBait(ModuleBase module) : Components.BaitAwayIcon(module, new AOEShapeCone(42.25f, 61f.Degrees()), (uint)IconID.Gaze, (uint)AID.TerrifyingGlance, 3.1f, source: module.Enemies((uint)OID.Calcabrina)[0]);
class TerrifyingGlanceGaze(ModuleBase module) : Components.GenericGaze(module)
{
...
class Brace(ModuleBase module) : Components.DirectionalParry(module, [(uint)OID.Calcabrina])
{
...
class HeatGazeBrina(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HeatGazeBrina, new AOEShapeDonut(5f, 10f));
class HeatGazeCalca(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HeatGazeCalca, new AOEShapeCone(19.9f, 30f.Degrees()));
class Knockout(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.Knockout);

class Slapstick(ModuleBase module) : ModuleComponent(module)
{
...
{
    public D113CalcabrinaStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 141u, NameID = 4813u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D113Calcabrina(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
