...
namespace Minerva.Heavensward.Dungeon.D06AetherochemicalResearchFacility.D063LahabreaIgeyorhm;
...
class GripOfNight(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.GripOfNight, new AOEShapeCone(40f, 75f.Degrees()));
class DarkFireIIAOE(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.DarkFireIIAOE, 6f);
class EndOfDays(ModuleBase module) : Components.LineStack(module, aidMarker: (uint)AID.EndOfDays, (uint)AID.EndOfDays2);
...
class Stars(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    private void ActivateAOE(AOEShape smallShape, AOEShape bigShape, WPos midpoint, Actor source, Actor target)
    {
        ...
        var activation = World.FutureTime(10.6d);
    ...
    {
        tutorialFlag = true;
        var activation = World.FutureTime(7.8d);
        var count = _stars.Count;
        for (var i = 0; i < count; ++i)
...
{
    public D063LahabreaIgeyorhmStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 38u, NameID = 2143u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS) (ported from BMR)")]
public class D063LahabreaIgeyorhm(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
