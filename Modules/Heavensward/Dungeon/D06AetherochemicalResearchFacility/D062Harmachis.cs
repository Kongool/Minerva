...
namespace Minerva.Heavensward.Dungeon.D06AetherochemicalResearchFacility.D062Harmachis;
...
class Paradox(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Paradox, 5f);
class Petrifaction(ModuleBase module) : Components.CastGaze(module, (uint)AID.Petrifaction);
class Ka(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Ka, new AOEShapeCone(45f, 30f.Degrees()));
class GaseousBomb(ModuleBase module) : Components.StackWithIcon(module, (uint)IconID.Stack, (uint)AID.GaseousBomb, 5f, 4.1d, 4, 4);
class BallisticMissile(ModuleBase module) : Components.UniformStackSpread(module, 4f, default, 2, 2)
{
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        if (status.ID == (uint)SID.Bind)
            AddStack(actor, World.FutureTime(6.2d));
    }
...
class ChthonicHush(ModuleBase module) : Components.Cleave(module, (uint)AID.ChthonicHush, new AOEShapeCone(13.3f, 60f.Degrees()))
{
...
{
    public D062HarmachisStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 38u, NameID = 3821u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS) (ported from BMR)")]
public class D062Harmachis(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
