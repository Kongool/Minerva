...
namespace Minerva.Heavensward.Dungeon.D06AetherochemicalResearchFacility.D060BiocultureNode;
...
class Hole(ModuleBase module) : ModuleComponent(module)
{
    public override void OnActorEAnim(Actor actor, uint state)
    {
        if (state == 0x00040008)
        {
            if (actor.OID == (uint)OID.Boss1Hole)
                Bounds = D060BiocultureNode.Arena1b;
            else if (actor.OID == (uint)OID.Boss2Hole)
                Bounds = D060BiocultureNode.Arena2b;
        }
    }
}

class Puncture(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Puncture, new AOEShapeRect(5.5f, 1.5f));
class TailSlap(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TailSlap, new AOEShapeCone(7f, 60f.Degrees()));
class CalcifyingMist(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.CalcifyingMist, new AOEShapeCone(6.9f, 45f.Degrees()));
class EerieSoundwave(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.EerieSoundwave, 7.5f);

abstract class MarrowDrain(ModuleBase module, uint aid) : Components.SimpleAOEs(module, aid, new AOEShapeCone(9.7f, 60f.Degrees()));
...
class MarrowDrain3(ModuleBase module) : MarrowDrain(module, (uint)AID.MarrowDrain3);

class TheRamsVoice(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TheRamsVoice, 9.7f);
class Sideswipe(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Sideswipe, new AOEShapeCone(9f, 45f.Degrees()));
class Gust(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Gust, 3f);

class D060BiocultureNodeStates : StateMachineBuilder
{
    public D060BiocultureNodeStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 38u, NameID = 3830u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D060BiocultureNode(WorldState ws, Actor primary) : ModuleBase(ws, primary, IsArena1(primary) ? Arena1.Center : arena2.Center, IsArena1(primary) ? Arena1 : arena2)
{
