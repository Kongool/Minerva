...
namespace Minerva.Heavensward.Dungeon.D06AetherochemicalResearchFacility.D064AscianPrime;
...
class AncientCircle(ModuleBase module) : Components.DonutStack(module, (uint)AID.AncientCircle, (uint)IconID.AncientCircle, 10f, 20f, 8d, 4, 4);

class DarkWhispers(ModuleBase module) : Components.UniformStackSpread(module, default, 6f)
{
    ...
    public override void OnEventIcon(Actor actor, uint iconID, ulong targetID)
    {
        ...
        {
            AddSpread(actor, World.FutureTime(5d));
        }
    }
...
class AncientFrost(ModuleBase module) : Components.StackWithIcon(module, (uint)IconID.AncientFrost, (uint)AID.AncientFrost, 6f, 5d, 4, 4);
class ShadowFlare(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.ShadowFlare1);
class ShadowFlareLBPhase(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.ShadowFlare2, "Raidwide x2");
class Annihilation(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.AnnihilationAOE);
class UniversalManipulation(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.UniversalManipulation, "Raidwide + Apply debuffs for later");

class HeightOfChaos(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.HeightOfChaos, 5f, tankbuster: true);

class AncientEruption(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AncientEruption, 5f);

class ChillingCross(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.ChillingCross1, (uint)AID.ChillingCross2], new AOEShapeCross(40f, 2.5f));

class DarkBlizzardIII(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.DarkBlizzardIII1, (uint)AID.DarkBlizzardIII2, (uint)AID.DarkBlizzardIII3,
(uint)AID.DarkBlizzardIII4, (uint)AID.DarkBlizzardIII5], new AOEShapeCone(41f, 10f.Degrees()));

class DarkFireII(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.DarkFireII, 6f);
class BurningChains(ModuleBase module) : Components.Chains(module, (uint)TetherID.BurningChains, (uint)AID.BurningChains);
class EntropicFlame(ModuleBase module) : Components.LineStack(module, aidMarker: (uint)AID.EntropicFlameMarker, (uint)AID.EntropicFlame, 5.2d);

class Stars(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        void AddAOEs(bool reverse)
        {
            ...
            if (reverse)
                _aoes.Reverse();
            void AddAOE(AOEShape shape, WPos pos, bool first) => _aoes.Add(new(shape, pos.Quantized(), default, World.FutureTime(first ? 11.8d : 14.8d)));
        }
...
{
    public D064AscianPrimeStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 38u, NameID = 3823u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS) (ported from BMR)")]
public class D064AscianPrime(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
