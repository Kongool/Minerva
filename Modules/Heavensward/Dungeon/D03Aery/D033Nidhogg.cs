...
namespace Minerva.Heavensward.Dungeon.D03Aery.D033Nidhogg;
...
class Fetters(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void OnCastStarted(Actor caster, ActorCastInfo spell)
    {
        ...
        {
            chaintarget = World.Actors.Find(spell.TargetID)!;
            casting = true;
        }
    }
...
class DeafeningBellow(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.DeafeningBellow);

class HotTail(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HotTail, new AOEShapeRect(68f, 8f));
class HotWing(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HotWing, new AOEShapeRect(30f, 34f));
class Cauterize(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Cauterize, new AOEShapeRect(80f, 11f));
class HorridRoarSpread(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.HorridRoarSpread, 6f);
class HorridRoar(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HorridRoar, 6f);
class HorridBlaze(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.HorridBlaze, 6f, 4, 4);
class Massacre(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Massacre);
class TheScarletPrice(ModuleBase module) : Components.SingleTargetDelayableCast(module, (uint)AID.TheScarletPrice);
class Roast(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Roast, new AOEShapeRect(30f, 4f));

class D033NidhoggStates : StateMachineBuilder
{
    public D033NidhoggStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 39u, NameID = 3458u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D033Nidhogg(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
