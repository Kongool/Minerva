...
namespace Minerva.Heavensward.Dungeon.D03Aery.D032Gyascutus;
...
class ArenaChange(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    {
        if (spell.Action.ID == (uint)AID.InflammableFumes && Bounds == D032Gyascutus.StartingBounds)
        {
...
class ProximityPyre(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ProximityPyre, 12f);
class Burst(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Burst, 10f);
class CripplingBlow(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.CripplingBlow);
class DeafeningBellow(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.DeafeningBellow);
class AshenOuroboros(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AshenOuroboros, new AOEShapeDonut(11f, 20f));
class BodySlam(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.BodySlam, 10f)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        if (Casters.Count != 0)
            hints.AddForbiddenZone(new SDInvertedCircle(Center, 9f), Casters.Ref(0).Activation);
    }
}

class D032GyascutusStates : StateMachineBuilder
{
    public D032GyascutusStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 39u, NameID = 3455u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D032Gyascutus(WorldState ws, Actor primary) : ModuleBase(ws, primary, StartingBounds.Center, StartingBounds)
{
