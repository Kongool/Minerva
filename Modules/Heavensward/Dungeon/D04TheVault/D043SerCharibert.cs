...
namespace Minerva.Heavensward.Dungeon.D04TheVault.D043SerCharibert;
...
sealed class KnightsTour(ModuleBase module) : Components.SimpleAOEGroups(module, [(uint)AID.WhiteKnightsTour, (uint)AID.BlackKnightsTour], new AOEShapeRect(40f, 2f));

sealed class AltarPyre(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.AltarPyre);

sealed class HeavensflameAOE(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HeavensflameAOE, 5f);
sealed class HolyChain(ModuleBase module) : Components.Chains(module, (uint)TetherID.HolyChain, (uint)AID.HolyChainPlayerTether);
sealed class TurretTour(ModuleBase module) : Components.Voidzone(module, 2f, GetVoidzones, 10f)
{
...
sealed class TurretTourHint(ModuleBase module) : Components.Voidzone(module, 2f, GetVoidzones, 3f)
{
    private static Actor[] GetVoidzones(ModuleBase module)
    {
        var enemies = module.Enemies(D043SerCharibert.Knights);
...
{
    public D043SerCharibertStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 34u, CFCID = 34u, NameID = 3642u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus, LTS), Xyzzy (ported from BMR)")]
public sealed class D043SerCharibert : ModuleBase
{
