...
namespace Minerva.Heavensward.Dungeon.D02SohmAl.D022Myath;
...
class ThirdLegForward(ModuleBase module) : Components.Cleave(module, (uint)AID.ThirdLegForward, new AOEShapeCone(10.9f, 60f.Degrees()))
{
...
class RazorScales(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.RazorScales, new AOEShapeCone(64.9f, 30f.Degrees()));
class PrimordialRoar(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.PrimordialRoar);
class MadDashSpread(ModuleBase module) : Components.SpreadFromCastTargets(module, (uint)AID.MadDash, 6f);
class MadDashStack(ModuleBase module) : Components.StackWithCastTargets(module, (uint)AID.MadDashStack, 6f, 4, 4);

class D022MyathStates : StateMachineBuilder
{
    public D022MyathStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 37u, NameID = 3793u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D022Myath(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
