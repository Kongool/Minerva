...
namespace Minerva.Heavensward.Dungeon.D02SohmAl.D023Tioman;
...
class HeavensfallBait(ModuleBase module) : Components.BaitAwayIcon(module, 5f, (uint)IconID.Comet, (uint)AID.Heavensfall1, 3.1f)
{
...
class Meteor(ModuleBase module) : Components.BaitAwayIcon(module, 20f, (uint)IconID.Meteor, (uint)AID.MeteorImpactVisual, 8.1f)
{
    public override void AddAIHints(int slot, Actor actor, PartyRolesConfig.Assignment assignment, AIHints hints)
    {
        base.AddAIHints(slot, actor, assignment, hints);
...
class MeteorImpact(ModuleBase module) : Components.GenericAOEs(module)
{
...
class ChaosBlastCircle(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ChaosBlastCircle, 2f);
class ChaosBlastRect(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.ChaosBlastRect, new AOEShapeRect(50.5f, 2f));
class AbyssicBuster(ModuleBase module) : Components.Cleave(module, (uint)AID.AbyssicBuster, new AOEShapeCone(31.84f, 45f.Degrees()));
class Comet(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Comet, 4f);
class Heavensfall(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Heavensfall2, 5f);

class D023TiomanStates : StateMachineBuilder
{
    public D023TiomanStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 37u, NameID = 3798u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D023Tioman(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
