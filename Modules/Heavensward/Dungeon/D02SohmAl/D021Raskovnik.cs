...
namespace Minerva.Heavensward.Dungeon.D02SohmAl.D021Raskovnik;
...
class Phytobeam(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Phytobeam, new AOEShapeRect(48.68f, 6f));
class AcidRain(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AcidRain, 6f);
class FloralTap(ModuleBase module) : Components.BaitAwayIcon(module, new AOEShapeCone(48.68f, 22.5f.Degrees()), (uint)IconID.FloralTap, (uint)AID.FloralTrap, 8.5f);
class FlowerDevour(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FlowerDevour, 8f);
class BloodyCaress(ModuleBase module) : Components.Cleave(module, (uint)AID.BloodyCaress, new AOEShapeCone(11.68f, 60f.Degrees()));

class D021RaskovnikStates : StateMachineBuilder
{
    public D021RaskovnikStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 37u, NameID = 3791u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D021Raskovnik(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
