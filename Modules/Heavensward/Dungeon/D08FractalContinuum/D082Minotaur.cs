...
namespace Minerva.Heavensward.Dungeon.D08FractalContinuum.D082Minotaur;
...
class OneOneTonzeSwipe(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.OneOneTonzeSwipe, new AOEShapeCone(9f, 60f.Degrees()));
class OneOneOneTonzeSwing(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.OneOneOneTonzeSwing, 12f);
class TenTonzeSlash(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TenTonzeSlash, new AOEShapeCone(44f, 30f.Degrees()));

class OneOneOneOneTonzeSwing(ModuleBase module) : ModuleComponent(module)
{
...
class DisorientingGroan(ModuleBase module) : Components.SimpleKnockbacks(module, (uint)AID.DisorientingGroan, 20f, stopAtWall: true);

class D082MinotaurStates : StateMachineBuilder
{
    public D082MinotaurStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 35u, NameID = 3429u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D082Minotaur(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
