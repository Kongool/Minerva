...
namespace Minerva.Heavensward.Dungeon.D08FractalContinuum.D083TheCurator;
...
class SeedOfTheRivers(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SeedOfTheRivers, 5f);
class Sanctification(ModuleBase module) : Components.Cleave(module, (uint)AID.Sanctification, new AOEShapeCone(16.5f, 60f.Degrees()), activeWhileCasting: false);

class Educator(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnEventDirectorUpdate(uint updateID, uint param1, uint param2, uint param3, uint param4)
    {
        if (updateID == 0x80000004 || activeCells == default)
            return;
        activeCells = default;
        Bounds = D083TheCurator.DefaultArena;
        Center = D083TheCurator.ArenaCenter;
    }
...
class AetherochemicalMine(ModuleBase module) : Components.GenericAOEs(module)
{
...
class AetherochemicalBombStatus(ModuleBase module) : Components.CleansableDebuff(module, (uint)SID.AetherochemicalBomb, "Bomb", "targeted");

class AetherochemicalBomb(ModuleBase module) : Components.GenericStackSpread(module)
{
    public override void OnStatusGain(Actor actor, ref ActorStatus status)
    {
        if (status.ID == (uint)SID.AetherochemicalBomb)
            Spreads.Add(new(actor, 8f, World.FutureTime(6d))); // status effect hits every 6 seconds unless cleansed/times out, radius is either 7 or 8
    }
...
{
    public D083TheCuratorStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 35u, NameID = 3434u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D083TheCurator(WorldState ws, Actor primary) : ModuleBase(ws, primary, ArenaCenter, DefaultArena)
{
