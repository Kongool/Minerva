...
namespace Minerva.Heavensward.Dungeon.D08FractalContinuum.D080CrimsonControlUnit;
...
class PassiveInfraredGuidanceSystem(ModuleBase module) : Components.Cleave(module, (uint)AID.PassiveInfraredGuidanceSystem, new AOEShapeCircle(6f), [(uint)OID.RetooledEnforcementDroid], originAtTarget: true);
class TheHand(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TheHand, new AOEShapeCone(7.5f, 60f.Degrees()));
class Shred(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Shred, new AOEShapeRect(5.5f, 2f));
class SpawnReservoir(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SpawnReservoir, 3f);
class AutoCannons(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.AutoCannons, new AOEShapeRect(42f, 2.5f));

class D080CrimsonControlUnitStates : StateMachineBuilder
{
    public D080CrimsonControlUnitStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 35u, NameID = 6924u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D080CrimsonControlUnit(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
    ...
    public override bool CheckReset() => !Raid.Player()!.Position.InSquare(Center, 70f);
