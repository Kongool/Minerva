...
namespace Minerva.Heavensward.Dungeon.D09SaintMociannesArboretum.D090VelveteenAnt;
...
sealed class PoisonBreathStickyThread(ModuleBase module) : Components.Cleave(module, (uint)AID.PoisonBreath, new AOEShapeCone(7.5f, 60f.Degrees()), [(uint)OID.ArboretumCrawler]);
sealed class TortoiseStomp(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TortoiseStomp, 11f);
sealed class UnfinalSting(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.UnfinalSting, new AOEShapeRect(9.08f, 1.5f));
sealed class FinalSting(ModuleBase module) : Components.SingleTargetCast(module, (uint)AID.FinalSting);

sealed class D090VelveteenAntStates : StateMachineBuilder
{
    public D090VelveteenAntStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 41u, NameID = 4641u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public sealed class D090VelveteenAnt : ModuleBase
{
