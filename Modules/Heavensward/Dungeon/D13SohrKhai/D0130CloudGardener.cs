...
namespace Minerva.Heavensward.Dungeon.D13SohrKhai.D130CloudGardener;
...
class RiseAndFall(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.RiseAndFall, new AOEShapeCone(9f, 135f.Degrees()));
class TightTornado(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.TightTornado, new AOEShapeRect(18f, 2f));
class Venom(ModuleBase module) : Components.Cleave(module, (uint)AID.Venom, new AOEShapeCone(10.9f, 60f.Degrees()), [(uint)OID.SanctuarySkipper]);
class DarkBlizzardIII(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.DarkBlizzardIII, 5f);

class D130CloudGardenerStates : StateMachineBuilder
{
    public D130CloudGardenerStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 171u, NameID = 4928u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D130CloudGardener(WorldState ws, Actor primary) : ModuleBase(ws, primary, IsArena1(primary) ? arena1.Center : arena2.Center, IsArena1(primary) ? arena1 : arena2)
{
