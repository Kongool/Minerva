...
namespace Minerva.Heavensward.Dungeon.D13SohrKhai.D131ChieftainMoglin;
...
class PomBom(ModuleBase module) : Components.GenericAOEs(module)
{
    ...
    public override void OnActorCreated(Actor actor)
    {
        ...
        {
            _aoe = [new(cross, actor.Position.Quantized(), default, World.FutureTime(6.3d))];
        }
    }
...
class ThousandKuponzeCharge(ModuleBase module) : Components.Cleave(module, (uint)AID.ThousandKuponzeCharge, new AOEShapeCone(9.8f, 60f.Degrees()))
{
...
class PomPraise(ModuleBase module) : ModuleComponent(module)
{
...
class HundredKuponzeSwipe(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HundredKuponzeSwipe, new AOEShapeCone(20.9f, 45f.Degrees()));
class PomFlare(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.PomFlare, 20.9f);
class PomHoly(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.PomHoly);
class SpinningMogshield(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.SpinningMogshield, 6.9f);

class D131ChieftainMoglinStates : StateMachineBuilder
{
    public D131ChieftainMoglinStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 171u, NameID = 4943u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D131ChieftainMoglin(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena.Center, arena)
{
    ...
    protected override void DrawEnemies(int pcSlot, Actor pc)
    {
        ...
        var count = smallmoogles.Count;
        List<Actor> balancedMoogles = [];
        List<Actor> offBalanceMoogles = [];
        for (var i = 0; i < count; ++i)
        {
            var moogle = smallmoogles[i];
            if (moogle.FindStatus((uint)SID.OffBalance) == null)
