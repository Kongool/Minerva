...
namespace Minerva.Heavensward.Dungeon.D13SohrKhai.D130BlizzardDragon;
...
class BridgeCreation(ModuleBase module) : ModuleComponent(module)
{
    public override void OnActorEAnim(Actor actor, uint state)
    {
        if (state == 0x00040008 && actor.OID == (uint)OID.Bridge)
        {
            Bounds = D130BlizzardDragon.Arena2;
            Center = D130BlizzardDragon.Arena2.Center;
        }
    }
}

class Touchdown(ModuleBase module) : Components.GenericAOEs(module)
{
...
class Cauterize(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Cauterize, new AOEShapeRect(53f, 10f));
class Fireball(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Fireball, 4f);
...
{
    public D130BlizzardDragonStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 171u, NameID = 4942u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "The Combat Reborn Team (Malediktus) (ported from BMR)")]
public class D130BlizzardDragon(WorldState ws, Actor primary) : ModuleBase(ws, primary, arena1.Center, arena1)
{
