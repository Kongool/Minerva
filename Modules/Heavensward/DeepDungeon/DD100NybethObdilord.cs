...
namespace Minerva.Heavensward.DeepDungeon.PalaceOfTheDead.DD100NybethObdilord;
...
class Abyss(ModuleBase module) : Components.BaitAwayCast(module, (uint)AID.Abyss, 6f);
class Catapult(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Catapult, 6f);
class CorseAdds(ModuleBase module) : Components.AddsMulti(module, [(uint)OID.BicephalicCorse, (uint)OID.GiantCorse, (uint)OID.IronCorse]);
class Doom(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Doom, new AOEShapeCone(47.4f, 60f.Degrees()));
class Shackle(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Shackle, new AOEShapeRect(52.4f, 4f));
class SummonDarkness(ModuleBase module) : Components.CastHint(module, (uint)AID.SummonDarkness, "Summoning the corse, incoming Adds! \nRemember to use a resolution to make them permanently disappear");
...
{
    public DD100NybethObdilordStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 208u, NameID = 5356u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "LegendofIceman (ported from BMR)")]
public class DD100NybethObdilord : ModuleBase
{
