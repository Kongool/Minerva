...
namespace Minerva.Heavensward.DeepDungeon.PalaceOfTheDead.DD160Todesritter;
...
class CleaveAuto(ModuleBase module) : Components.Cleave(module, (uint)AID.AutoAttack, new AOEShapeCone(11.92f, 45f.Degrees()), activeWhileCasting: false);
class HallOfSorrow(ModuleBase module) : Components.VoidzoneAtCastTarget(module, 9f, (uint)AID.HallOfSorrow, GetVoidzones, 1.3d)
{
...
class Infatuation(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Infatuation, 7f);
class Valfodr(ModuleBase module) : Components.BaitAwayChargeCast(module, (uint)AID.Valfodr, 3f);

class ValfodrKB(ModuleBase module) : Components.GenericKnockback(module, (uint)AID.Valfodr, stopAtWall: true)
{
...
{
    public DD160TodesritterStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 214u, NameID = 5438u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "LegendofIceman (ported from BMR)")]
public class DD160Todesritter(WorldState ws, Actor primary) : ModuleBase(ws, primary, SharedBounds.ArenaBounds160170180190.Center, SharedBounds.ArenaBounds160170180190);
