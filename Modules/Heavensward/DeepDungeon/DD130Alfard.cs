...
namespace Minerva.Heavensward.DeepDungeon.PalaceOfTheDead.DD130Alfard;
...
class Dissever(ModuleBase module) : Components.Cleave(module, (uint)AID.Dissever, new AOEShapeCone(10.8f, 45f.Degrees()), activeWhileCasting: false);

abstract class Voidzones(ModuleBase module, uint aid, uint oid) : Components.VoidzoneAtCastTarget(module, 6f, aid, m => GetVoidzones(m, oid), 2.1f)
{
...
class BallofFire(ModuleBase module) : Voidzones(module, (uint)AID.BallOfFire, (uint)OID.FireVoidzone);
class BallofIce(ModuleBase module) : Voidzones(module, (uint)AID.BallOfIce, (uint)OID.IceVoidzone);

class FearItself(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.FearItself, new AOEShapeDonut(5f, 50f));
...
class Hints(ModuleBase module) : ModuleComponent(module)
{
    ...
    {
        if (NumCasts < 4)
            hints.AddForbiddenZone(new SDCircle(FearCastSource, 11), World.FutureTime(10d));
        else
            hints.AddForbiddenZone(new SDInvertedCircle(FearCastSource, 5), World.FutureTime(10d));
    }
}

class DD130AlfardStates : StateMachineBuilder
{
    public DD130AlfardStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 211u, NameID = 5397u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "LegendofIceman (ported from BMR)")]
public class DD130Alfard(WorldState ws, Actor primary) : ModuleBase(ws, primary, SharedBounds.ArenaBounds120130.Center, SharedBounds.ArenaBounds120130);
