...
namespace Minerva.Heavensward.DeepDungeon.PalaceOfTheDead.DD70Yaquaru;
...
class Douse(ModuleBase module) : Components.VoidzoneAtCastTarget(module, 8f, (uint)AID.Douse, GetVoidzones, 0.8d)
{
...
class FangsEnd(ModuleBase module) : ModuleComponent(module)
{
    ...
    public override void Update()
    {
        for (var i = 0; i < 4; ++i)
        {
            ...
            if (player.FindStatus((uint)SID.Heavy) is ActorStatus st && (st.ExpireAt - World.CurrentTime).TotalSeconds > 8d)
                _heavy.Set(i);
        }
    }
...
{
    public DD70YaquaruStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(CFCID = 205u, NameID = 5321u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "legendoficeman, Malediktus (ported from BMR)")]
public class DD70Yaquaru(WorldState ws, Actor primary) : ModuleBase(ws, primary, SharedBounds.ArenaBounds607080.Center, SharedBounds.ArenaBounds607080);
