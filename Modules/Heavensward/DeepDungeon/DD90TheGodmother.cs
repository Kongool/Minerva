...
namespace Minerva.Heavensward.DeepDungeon.PalaceOfTheDead.DD90TheGodmother;
...
sealed class GreyBomb(ModuleBase module) : Components.Adds(module, (uint)OID.GreyBomb, 5, true);
sealed class Burst(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.Burst, "Kill the Grey Bomb! or take 80% of your Max HP");
// future thing to do: maybe add a tether between bomb/boss to show it needs to show the aoe needs to explode on them. . . 
sealed class HypothermalCombustion(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.HypothermalCombustion, 7.2f)
{
...
sealed class GiddyBomb(ModuleBase module) : ModuleComponent(module)
{
...
sealed class MassiveBurst(ModuleBase module) : Components.RaidwideCast(module, (uint)AID.MassiveBurst, "Knock the Giddy bomb into the boss and let it explode on the boss. \n or else take 99% damage!");
sealed class Sap(ModuleBase module) : Components.SimpleAOEs(module, (uint)AID.Sap, 8f);
sealed class ScaldingScolding(ModuleBase module) : Components.Cleave(module, (uint)AID.ScaldingScolding, new AOEShapeCone(11.75f, 60f.Degrees()))
{
...
sealed class SelfDestruct(ModuleBase module) : Components.GenericAOEs(module)
{
...
{
    public DD90TheGodmotherStates(ModuleBase module) : base(module)
    {
        TrivialPhase()
...
[ModuleInfo(Group = ModuleGroup.CFC, GroupID = 207u, CFCID = 207u, NameID = 5345u, PrimaryActorDeathEndsEncounter = true, Maturity = ModuleMaturity.WIP, Contributors = "LegendofIceman (ported from BMR)")]
public sealed class DD90TheGodmother(WorldState ws, Actor primary) : ModuleBase(ws, primary, SharedBounds.ArenaBounds2090110.Center, SharedBounds.ArenaBounds2090110);
